...
function PopupShopItemBuy:setCostInfo2(costType,costValue, costSize)
	...
	if Lang.checkUI("ui4") then
		...
		local resName = UIHelper.seekNodeByTag(self,2814) 
		local posX = 230
		resName:setPositionX(posX)
		self._costResInfo1:setPositionX(posX+10)
		self._costResInfo2:setPositionX(posX+127)
	end
end


-- i18n change lable
function PopupShopItemBuy:_dealByI18n()
	if not Lang.checkLang(Lang.CN)  then

		local UIHelper  = require("yoka.utils.UIHelper")
		local resName = UIHelper.seekNodeByTag(self,2814)
		logWarn("PopupShopItemBuy xds--------------  "..resName:getPositionX())
		resName:setAnchorPoint(cc.p(1,0.5))
		resName:setPositionX(self._costResInfo1:getPositionX() - 2 )	
