...
-- i18n pos lable
function PopupUserBaseInfo:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		for i = 1,3,1 do
			local node = UIHelper.seekNodeByName(self._panelInfo,"Node_"..i)
			local textName = UIHelper.seekNodeByName(node,"Text_name")
			local textValue = UIHelper.seekNodeByName(node,"Text_value")
			textValue:setPositionX(
				textName:getPositionX()+textName:getContentSize().width+3
			) 
		end

		self._commonNodeBk:setTitleFontSize(22)
		self._commonNodeBk2:setTitleFontSize(22)
	end
	if Lang.checkLang(Lang.JA) then -- i18n ja change
		self:getSubNodeByName("Node_3"):setPositionX(self:getSubNodeByName("Node_3"):getPositionX() + 10)
		self:getSubNodeByName("Node_2"):getChildByName("Text_value"):setPositionY(self:getSubNodeByName("Node_2"):getChildByName("Text_name"):getPositionY())
		self._commonNodeBk:setTitleFontSize(30)
	end
end
