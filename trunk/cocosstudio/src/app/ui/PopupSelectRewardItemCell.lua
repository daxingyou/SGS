...
-- i18n pos lable
function PopupSelectRewardItemCell:_dealPosByI18n()
    if not Lang.checkLang(Lang.CN) then
		local size = self._itemName:getContentSize()
		self._itemName:setFontSize(self._itemName:getFontSize()-2)
		self._itemName:setContentSize(cc.size(size.width+10,size.height))
        --self._itemName:getVirtualRenderer():setMaxLineWidth(150)
		self._itemName:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
	end
end
