...
local EXPORTED_METHODS = {
	...
	"hideBg",
	"showDoubleTips",
	"getSize",-- i18n pos lable
	"setTextItemCountFontSize", -- i18n ja change
}
...
-- i18n pos lable
function CommonIconBase:getSize()
	local size = cc.size(0,0)
	if self._panelItemContent then
		size = self._panelItemContent:getContentSize()
	end
	if self._labelItemName and self._labelItemName:isVisible() then
		local render = self._labelItemName:getVirtualRenderer()
		size.height = size.height + render:getContentSize().height
		size.height = size.height + math.abs(self._labelItemName:getPositionY())
	end
	return size
end
...
-- i18n ja change
function CommonIconBase:setTextItemCountFontSize(fontSize)
	if self._textItemCount then
		self._textItemCount:setFontSize(fontSize)
	end
end 
