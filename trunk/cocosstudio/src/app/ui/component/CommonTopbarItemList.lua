...
function CommonTopbarItemList:_updateUI(resList)
    ...
    -- i18n ja
    if not Lang.checkUI("ui4") and  self._backImage then
        if showCount == 0 then 
...
function CommonTopbarItemList:_onEventBuyItem(eventName, message)
    ...
    if awards then
        ...
        if shopType == ShopConst.SHOP_TYPE_ACTIVE or shopId == ShopConst.SEASOON_SHOP then
            ...
            if Lang.checkUI("ui4") then
                if shopId ~= ShopConst.VIP_EXCHANGE_SHOP  then --在VipViewNew显示
...
--i18n ja
function CommonTopbarItemList:getResNode(id)
    if self["_resNode" .. id] then
        return self["_resNode" .. id]
    end
    return nil
end
