...
local EXPORTED_METHODS = {
	...
	"setTextItemCountFontSize",--i18n ja
}
...
function CommonListViewLineItem:ctor()
	...
	self._itemTotalSize = nil--i18n ja bug
end
...
function CommonListViewLineItem:updateUI(awardList, scale, adaptWithContainerSize,showTopImage, isDouble)
	...
	--i18n ja bug
	self._itemTotalSize = cc.size(totalW,totalH)
	if adaptWithContainerSize then
		...
		--i18n BUG 
		--local size = self._listViewItem:getContentSize()
		--self._listViewItem:setInnerContainerSize(cc.size(math.max(size.width,totalW),
		--	math.max(size.height,totalH)))
	end
...
function CommonListViewLineItem:alignCenter()
	...
	--i18n ja bug
	if self._itemTotalSize then
		local size = self._listViewItem:getContentSize()
		--local innerSize = self._listViewItem:getInnerContainerSize()
		if self._itemTotalSize.width < size.width then
			print("innerSize width"..self._itemTotalSize.width)
			print("contentSiz width"..size.width)
			self._listViewItem:setContentSize(self._itemTotalSize)
		end
	end
end
...
--i18n ui4
function CommonListViewLineItem:setTextItemCountFontSize(fontSize)
	local children = self._listViewItem:getItems()
	for k,v in ipairs(children) do
		 v:getChildren()[1]:getIconTemplate():setTextItemCountFontSize(fontSize)
		 v:getChildren()[1]:getIconTemplate():setTextItemCountFontSize(fontSize)
	end
end
