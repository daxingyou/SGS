...
function CommonTabIcon:_init()
	...
	if not Lang.checkLang(Lang.CN) and not Lang.checkLang(Lang.ZH) then
		...
		self._text:setFontSize(
			self._text:getFontSize()-5
		)
		--self._text:setContentSize(cc.size(62,size.height))
		--self._text:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
	end

	if Lang.checkHorizontal() then
		self._imageBg:setRotation(-90)
		self._panelTouch:setRotation(-90)
		self._imageRedPoint:setPosition(48,30)
	end
end
