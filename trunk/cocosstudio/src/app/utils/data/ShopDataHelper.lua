...
-- local ShopInfo = require("app.config.shop")--in8
local ShopConst = require("app.const.ShopConst")
--商店通用函数
local ShopInfo = nil --in8
...
function ShopDataHelper.getShopType(shopId)
	--i18n config change
	if ShopInfo == nil then ShopInfo = require("app.config.shop") end
...
function ShopDataHelper.getFixRewardList(config)
	local rewardList = {}
	for i = 1,3,1 do
		if i == 1 then
			if config["type"] ~= 0 then
				table.insert(rewardList,{type = config["type"] , value = config["value"], size = config["size"]})
			end
		else
			if config["type"..i] ~= 0 then
				table.insert(rewardList,{ type = config["type"..i] , value = config["value"..i], size = config["size"..i]})
			end
		end
...
--i18n ja
function ShopDataHelper.getFixShopConfig(goodId)
	local ShopFix = require("app.config.shop_fix")
	local info = ShopFix.get(goodId)
	if not info then
		local ShopFixJa = require("app.config.shop_fix_ja")--i18n ja
		info = ShopFixJa.get(goodId) 
	end
	return info
end
