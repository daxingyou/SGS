...
--i18n 
function StateShow:ctor(entity, skillPlay, skillType, prePosition,skillInfo)
...
function StateShow:start()
    ...
    if self._skillType == 2 then
        ...
        if self._skillPlay and self._skillPlay.txt ~= 0 then
            dump(self._skillInfo ,"self._skillInfo 1：")
            -- i18n change skill
            if not Lang.checkLang(Lang.CN) and self._skillInfo then
                self._entity:showSkill(self._skillPlay.txt,self._skillInfo.name)
            else
