...
local __key_type = {
  ...
  color = "int",    --品质-3 
  res_id = "int",    --资源id-4 
  bag_description = "string",    --背包描述-5 
  description = "string",    --装备描述-6 
  fragment_id = "int",    --碎片id-7 
  sorting = "int",    --排序-8 
  attribute_type1 = "int",    --属性1类型-9 
  attribute_value1 = "int",    --属性1类型值-10 
  attribute_type2 = "int",    --属性2类型-11 
  attribute_value2 = "int",    --属性2类型值-12 
  attribute_type3 = "int",    --属性3类型-13 
  attribute_value3 = "int",    --属性3类型值-14 
  attribute_type4 = "int",    --属性4类型-15 
  attribute_value4 = "int",    --属性4类型值-16 
...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in gemstone")
...
--
function gemstone.isVersionValid(v)
    if gemstone.version then
        if v then
            return gemstone.version <= v
        else
            return false
        end
    else
...
function gemstone.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/gemstone.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/gemstone.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/gemstone.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "gemstone" )
                _isDataExist = gemstone.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "gemstone" )
                _isBaseExist = gemstone.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "gemstone" )
                _isExist = gemstone.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = gemstone._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "gemstone" )
