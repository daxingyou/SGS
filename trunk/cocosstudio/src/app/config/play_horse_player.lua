...
local __key_type = {
  rank = "int",    --名次-1 
  weight = "int",    --权重-2 
  number = "int",    --每天最多出现次数-3 
  bubble_time = "string",    --准备中说话时间段1min-4 
  text_1 = "string",    --文本1-5 
  text_2 = "string",    --文本2-6 
  text_3 = "string",    --文本3-7 
  text_4 = "string",    --文本4-8 
...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in play_horse_player")
...
--
function play_horse_player.isVersionValid(v)
    if play_horse_player.version then
        if v then
            return play_horse_player.version <= v
        else
            return false
        end
    else
...
function play_horse_player.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/play_horse_player.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/play_horse_player.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/play_horse_player.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "play_horse_player" )
                _isDataExist = play_horse_player.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "play_horse_player" )
                _isBaseExist = play_horse_player.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "play_horse_player" )
                _isExist = play_horse_player.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = play_horse_player._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "play_horse_player" )
...
--
function play_horse_player.set(rank, key, value)
