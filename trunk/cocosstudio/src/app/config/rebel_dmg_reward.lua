...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in rebel_dmg_reward")
...
--
function rebel_dmg_reward.isVersionValid(v)
    if rebel_dmg_reward.version then
        if v then
            return rebel_dmg_reward.version <= v
        else
            return false
        end
    else
...
function rebel_dmg_reward.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/rebel_dmg_reward.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/rebel_dmg_reward.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/rebel_dmg_reward.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "rebel_dmg_reward" )
                _isDataExist = rebel_dmg_reward.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "rebel_dmg_reward" )
                _isBaseExist = rebel_dmg_reward.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "rebel_dmg_reward" )
                _isExist = rebel_dmg_reward.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = rebel_dmg_reward._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "rebel_dmg_reward" )
