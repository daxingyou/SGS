...
local __key_type = {
  ...
  moving_name = "string",    --装饰动画资源-28 
  spine_x = "int",    --spine坐标x-29 
  spine_y = "int",    --spine坐标y-30 
  height = "int",    --点击区域高-31 
  width = "int",    --点击区域宽-32 
  click_x = "int",    --点击区域坐标x-33 
  click_y = "int",    --点击区域左边y-34 
  title_x = "int",    --ui标题坐标x-35 
  title_y = "int",    --ui标题坐标y-36 
  order = "int",    --层级排序-37 
  open_effect = "string",    --解锁特效-38 
  up_effect = "string",    --升级特效-39 
  up_loop_effect = "string",    --每级spine特效-40 
...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in tree_decorate_add")
...
--
function tree_decorate_add.isVersionValid(v)
    if tree_decorate_add.version then
        if v then
            return tree_decorate_add.version <= v
        else
            return false
        end
    else
...
function tree_decorate_add.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/tree_decorate_add.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/tree_decorate_add.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/tree_decorate_add.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "tree_decorate_add" )
                _isDataExist = tree_decorate_add.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "tree_decorate_add" )
                _isBaseExist = tree_decorate_add.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "tree_decorate_add" )
                _isExist = tree_decorate_add.isVersionValid(table.version)
            end
        end
    end
    ...
    if _lang ~= "cn" and _isDataExist then
		...
		local index_key = "__index_type_level"
    ...
    config._raw = tree_decorate_add._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "tree_decorate_add" )
