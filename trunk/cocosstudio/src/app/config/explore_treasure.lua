...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in explore_treasure")
...
--
function explore_treasure.isVersionValid(v)
    if explore_treasure.version then
        if v then
            return explore_treasure.version <= v
        else
            return false
        end
    else
...
function explore_treasure.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/explore_treasure.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/explore_treasure.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/explore_treasure.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "explore_treasure" )
                _isDataExist = explore_treasure.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "explore_treasure" )
                _isBaseExist = explore_treasure.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "explore_treasure" )
                _isExist = explore_treasure.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = explore_treasure._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "explore_treasure" )
