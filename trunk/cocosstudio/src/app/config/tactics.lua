...
local __key_type = {
  ...
  icon = "int",    --战法icon-3 
  color = "int",    --战法品质-4 
  profile = "string",    --战法短描述-5 
  description = "string",    --战法详情-6 
  mapping = "int",    --适用武将库-7 
  show_day = "int",    --图鉴显示天数-8 
  order = "int",    --图鉴排序-9 
  mutex = "string",    --互斥战法-10 
  limit = "int",    --限制人次-11 
  gm = "int",    --GM是否可以发放-12 
  fake = "int",    --假战力-13 
  button_type = "int",    --按钮类型-14 
  function_id = "int",    --跳转功能id-15 
  button_txt = "string",    --按钮文字-16 
  moving = "string",    --特效-17 
...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in tactics")
...
--
function tactics.isVersionValid(v)
    if tactics.version then
        if v then
            return tactics.version <= v
        else
            return false
        end
    else
...
function tactics.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/tactics.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/tactics.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/tactics.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "tactics" )
                _isDataExist = tactics.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "tactics" )
                _isBaseExist = tactics.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "tactics" )
                _isExist = tactics.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = tactics._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "tactics" )
...
--
function tactics.set(id, key, value)
