...
function MailData:processMail(id)
    ...
    if mailInfo then
        ...
        if Lang.checkLang(Lang.CN) then
            ...
            if self._simpleMailList[id] then
                self._simpleMailList[id].isRead = true
            end
        end
...
-- i18n ja 已拥有背景奖励替换为背景券 start---------------
function MailData:saveMainSceneData()
    local list = G_UserData:getMainScene():getSceneList()
    self._mainSceneList = {}
    for i, v in ipairs(list) do
        self._mainSceneList[v] = true
    end
end
function MailData:getMainSceneList()
    return self._mainSceneList
end
function MailData:processAwardsByI18n(mailAwards)
    ...
    if sceneToken > 0 then
        local data = {type = TypeConvertHelper.TYPE_RESOURCE,value = DataConst.RES_BACKGROUND,size = sceneToken}
        table.insert(awards,data)
    end
    return awards,skinAwards
end
function MailData:getNewMainSceneList()
    local newList = {}
    local current = G_UserData:getMainScene():getSceneList()
    local sceneList = self:getMainSceneList()
    for i, v in ipairs(current) do
        if not sceneList[v] then
            table.insert(newList,v)
        end
    end
    return newList
end
-- i18n ja 已拥有背景奖励替换为背景券 end---------------
