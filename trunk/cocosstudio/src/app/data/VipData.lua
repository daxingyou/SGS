...
function VipData:updateData(data)
	...
	-- i18n ja change 
	if self:getLastLevel() ~= self:getLevel() then
		if Lang.checkUI("ui4") then
			...
			G_SignalManager:dispatch(SignalConst.EVENT_POSTER_GIRL_VOICE_UPDATE,
				PosterGirlVoiceConst.TRIGGER_POS_VIP_UPGRADE,
				{old_level = self:getLastLevel(),level = self:getLevel() }
			)
			G_SignalManager:dispatch(SignalConst.EVENT_VIP_UPGRADE,self:getLastLevel(),self:getLevel())
		end
	end
	...
	if Lang.checkUI("ui4") then
		G_SignalManager:dispatch(SignalConst.EVENT_VIP_EXP_CHANGE,addExp,self:getLastLevel(),self:getLevel())
	else
