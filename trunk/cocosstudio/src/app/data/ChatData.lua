...
function ChatData:ctor(properties)
	...
	--i18n ja
	self._s2cChatReportListener = G_NetworkManager:add(MessageIDConst.ID_S2C_ChatReport, handler(self, self._s2cChatReport))
	self._s2cChatSliencePrivateListener = G_NetworkManager:add(MessageIDConst.ID_S2C_ChatSliencePrivate, handler(self, self._s2cChatSliencePrivate))
	self._s2cNotifySliencedUserListener = G_NetworkManager:add(MessageIDConst.ID_S2C_NotifySliencedUser, handler(self, self._s2cNotifySliencedUser))
...
function ChatData:_initData()
	...
	if not Lang.checkLang(Lang.CN) then
		self._crossServerAutoPlayVoiceList = {}
	end

	self:setChatSetting(require("app.data.ChatSetting").new())
	self._tranCacheI18n = {} --i18n
	self._sliencedUserDataList = {} --i18n ja
end
...
function ChatData:clear()
	...
	--i18n ja 
	self._s2cChatReportListener:remove()
	self._s2cChatSliencePrivateListener:remove()
	self._s2cNotifySliencedUserListener:remove()
end
...
function ChatData:_s2cChat(id,message)
	...
	if Lang.checkUI("ui4") then
		--i18n ja
		chatMsgData:getSender():setServer_id(message.sender_server_id)
	end
...
function ChatData:_onAddNewMessage(newMsg,notify)
	...
	elseif channel == ChatConst.CHANNEL_CROSS_SERVER then
		...
		--i18n 新增跨服消息控制
		if not Lang.checkLang(Lang.CN) then
			self:_onAddVoiceMsg2List(self._crossServerAutoPlayVoiceList,newMsg,channel)
		else
			self:_onAddVoiceMsg2List(self._guildAutoPlayVoiceList,newMsg,channel)	
		end
...
function ChatData:getMiniMsgList()
	...
	--i18n 新增跨服消息控制
	if not Lang.checkLang(Lang.CN) then
		local showCrossServerMsg = self:getChatSetting():isShowMiniMsgOfChannel(ChatConst.CHANNEL_CROSS_SERVER )
		if showCrossServerMsg then
...
function ChatData:getNextAutoPlayVoiceMsg()
	...
	--i18n 新增跨服消息控制
	if not Lang.checkLang(Lang.CN) then
		clearPlayedListFunc(self._crossServerAutoPlayVoiceList)	
	end
	...
	--i18n 新增跨服消息控制
	if not Lang.checkLang(Lang.CN) then
		local findCanPlayAudioMsgFunc = function(listArr)
			for k1,list in ipairs(listArr) do
				for k2,v in ipairs(list) do
		...
		local msg = findCanPlayAudioMsgFunc(
			{self._guildAutoPlayVoiceList,
			self._worldAutoPlayVoiceList,self._crossServerAutoPlayVoiceList}
		)
		return msg
	end
...
--i18n 新增跨服消息控制
function ChatData:clearAutoPlayVoiceList(channel)
	logWarn("VoiceManager....... clearAutoPlayVoiceList")
	if Lang.checkLang(Lang.CN) then
	 	return 	
	end
	if channel == ChatConst.CHANNEL_CROSS_SERVER then
		self._crossServerAutoPlayVoiceList = {}
	end
end
...
--i18n缓存翻译
function ChatData:cacheTranslateI18n(src,tran,tranLang)
	...
	if not self._tranCacheI18n[key] then
		self._tranCacheI18n[key] = {}
	end
	
	self._tranCacheI18n[key][tranLang] = tran

end

function ChatData:getTranslateI18n(src,tranLang)
	if not self._tranCacheI18n then
		return nil
	end
	local md5 = require("md5")
...
-- i18n ja
function ChatData:c2sChatReport(user_id,channel,time,report_type,content_type,content,
		msg_id,tar_user_name,tar_server_name,tar_server_id)
	G_NetworkManager:send(MessageIDConst.ID_C2S_ChatReport, 
		{user_id = user_id,channel = channel,time = time,
		report_type = report_type,content_type = content_type,content =content,
		msg_id = msg_id,tar_user_name = tar_user_name,tar_server_name = tar_server_name,
		tar_server_id = tar_server_id}
	)
end

-- i18n ja
function ChatData:_s2cChatReport(id,message)
	if message.ret ~= MessageErrorConst.RET_OK then
		return
	end
	
	G_SignalManager:dispatch(SignalConst.EVENT_CHAT_TIPOFF_SUCCESS)
end

-- i18n ja
function ChatData:c2sChatSliencePrivate(uid)
	G_NetworkManager:send(MessageIDConst.ID_C2S_ChatSliencePrivate, 
		{uid = uid}
	)
end

--i18n ja
function ChatData:_s2cChatSliencePrivate(id,message)
	if message.ret ~= MessageErrorConst.RET_OK then
		return
	end
end
...
--i18n ja
function ChatData:_s2cNotifySliencedUser(id,message)
	local sliencedUsers = rawget(message,"slienced_users") or {}
	self._sliencedUserDataList = {}
	local SliencedUserData = require("app.data.SliencedUserData")
	for k,v in ipairs(sliencedUsers) do
		local unitData = SliencedUserData.new()
		unitData:updateData(v)
		table.insert(self._sliencedUserDataList,unitData)
	end
end

--i18n ja
function ChatData:getSilenceUserDataList()
	return self._sliencedUserDataList
end
