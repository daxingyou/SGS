	...
	elseif questType == CustomActivityConst.CUSTOM_QUEST_TYPE_PAY_SELL_ITEM then
		...
		assert(payCfg,"vip_pay not find id "..tostring(payId))
   		if not Lang.checkLang(Lang.CN) then
			...
			local _,currency = UIHelper.convertCurrency(payCfg.rmb)
			local _,currency = UIHelper.convertDollar(currency)
			btnName = Lang.get("customactivity_btn_name_pay",{value = currency})
		else
