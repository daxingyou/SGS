...
schema["tShirt"] = {"table"}
--i18n late regist
schema["activityLateRegist"] = {"table"}
-- i18n ja mainscene
schema["mainScene"] = {"table"}
schema["activityFriendInvite"] = {"table"}
--i18n ja
...
function UserData:ctor()
	local properties = {
		...
		--i18n late regist
		activityLateRegist = require("app.i18n.extends.data.ActivityLateRegistData").new(),
		-- i18n ja mainscene
...
function UserData:clear()
	...
	--i18n late regist
	self:getActivityLateRegist():clear()
	self:getUniverseRace():clear()
    self:getTactics():clear()
	self:getBout():clear()
	self:getTShirt():clear()
	-- i18n ja mainscene
	self:getMainScene():clear()
	self:getActivityFriendInvite():clear()-- i18n ja friendInvite

	--i18n ja 
	self:getPosterGirl():clear()
...
function UserData:reset()
	...
	--i18n late regist
	self:getActivityLateRegist():reset()
	-- i18n ja mainscene
	self:getMainScene():reset()
...
function UserData:_recvOpObject(id, message)
	...
	--i18n ja 看板娘
	local function procPosterGirl(message)
		local posterGirl = rawget(message, "poster_girl")
		if posterGirl then
			
			local posterGirlInfo = rawget(posterGirl, "poster_girl") 
			if rawget(posterGirlInfo, "wear_skin") then
				self:getPosterGirl():setWear_skin(posterGirlInfo.wear_skin)
				G_SignalManager:dispatch(SignalConst.EVENT_POSTER_GIRL_CHANGE_SKIN_SUCCESS)
			end
			if rawget(posterGirlInfo, "skins") then
				self:getPosterGirl():updateData(posterGirlInfo.skins)
			end
		end
	end
	-- i18n ja 操作场景数据
	local function procMainScene(message)
		local main_scene = rawget(message, "main_scene")
		if main_scene then
			if rawget(main_scene, "main_scene") then
				self:getMainScene():updateData(main_scene.main_scene)
			end
		end
	end
	...
	--i18n ja
	procPosterGirl(message) --阵法
	-- i18n ja mainscene
	procMainScene(message)end
