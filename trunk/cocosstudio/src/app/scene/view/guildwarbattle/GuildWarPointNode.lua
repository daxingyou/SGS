...
-- i18n pos lable
function GuildWarPointNode:_dealPosI18n()
	if not Lang.checkLang(Lang.CN) then
		local image1 = ccui.Helper:seekNodeByName(self, "Image_1")
		local parent = image1:getParent()
		image1:setVisible(false)




		local image2 = display.newSprite(Path.getGuildWar("img_war_com1"), 0,-70)
		local image3 = display.newSprite(Path.getGuildWar("img_war_com2"), 0,-70-23, {
			capInsets =  cc.rect(34, 12, 61, 7),
			rect = cc.rect(0, 0, 121, 28),
		})
		
        image2:setAnchorPoint(cc.p(0.5,0.5))
        image3:setAnchorPoint(cc.p(0.5,0.5))
   
		image2:setLocalZOrder(1)
		image3:setLocalZOrder(0)
        image3:setContentSize(cc.size(180,28))
        local node = cc.Node:create()
		parent:addChild(image2)
		parent:addChild(image3)

      

		self._populationNode:setLocalZOrder(2)
		self._nameNode:setLocalZOrder(2)
		self._imageState:setLocalZOrder(2)
	end
end
