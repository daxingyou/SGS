...
--i18n
function ChatMsgScrollView:getItemPosInContainerForI18n(index)
    local size = self._scrollView:getInnerContainer():getContentSize()
    local bottomPos = self:_findItemPosition(index)
    return size.height-bottomPos
end
--i18n
function ChatMsgScrollView:getItemBottomToScreenBottomDisForI18n(index)
    local pos = self:getItemPosInContainerForI18n(index)
    local lastY = self._scrollView:getInnerContainer():getPositionY()
    return pos + lastY
end
--i18n
function ChatMsgScrollView:getItemIndexByChatMsgForI18n(chatMsg)
    for k,v in ipairs(self._dataList) do
        if v == chatMsg then
            return k
        end
...
--i18n
function ChatMsgScrollView:getItemTopToScreenBottomDisForI18n(index)
    local disToScreenBottom = self:getItemBottomToScreenBottomDisForI18n(index)
    local itemHeight =  self._showItems[index]:getTotalHeight() 
    return disToScreenBottom + itemHeight
end


--i18n
function ChatMsgScrollView:fixItemToScreenPosForI18n(index,dis)

    local posY = self:getItemPosInContainerForI18n(index)+self._showItems[index]:getTotalHeight() 
    local size = self._scrollView:getInnerContainer():getContentSize()
    local maxY = 0
    local minY = -(size.height - self._listSize.height)
    local y = -posY + dis
...
--i18n
function ChatMsgScrollView:onEnter()
    self._signalTranslateSuccess =
        G_SignalManager:add(SignalConst.EVENT_TRANSLATE_SUCCESS, handler(self, self._onEventTranslateSuccess))
end

--i18n
function ChatMsgScrollView:onExit()
    self._signalTranslateSuccess:remove()
    self._signalTranslateSuccess = nil
end

--i18n {"textId":"11","source":"zh","target":"en","sourceText":"中国","result":"China"}
function ChatMsgScrollView:_onEventTranslateSuccess(event, resultInfo)
    print("EventTranslateSuccess ok")
    G_UserData:getChat():cacheTranslateI18n(resultInfo.sourceText,resultInfo.result,resultInfo.target)

    local index = nil
    for k,v in ipairs(self._dataList) do
        print("EventTranslateSuccess ok  ---"..v:getId())
        if tostring(v:getId()) == resultInfo.textId then
            index = k
            break
        end
    end
    ...
    print("EventTranslateSuccess index  --------"..index)
    
    local item = self._showItems[index]
    local chatMsg = self._dataList[index]
    chatMsg:setTranslate(true)
    

    local dis = self:getItemTopToScreenBottomDisForI18n(index)


    item:_updateTranTxtForI18n(resultInfo.result)

    self:_refreshItemsPos()
    self:fixItemToScreenPosForI18n(index,dis)
