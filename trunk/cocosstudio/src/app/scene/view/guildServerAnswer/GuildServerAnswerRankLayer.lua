...
-- i18n change lable
function GuildServerAnswerRankLayer:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local image1 = UIHelper.seekNodeByName(self._imageArrow1 , "Image_2")
		UIHelper.swapWithLabel(image1,{
			 style = "guild_dungeon_1",
			 text = Lang.getImgText("txt_juntuanfuben_mingci01")
		})
	end
end
...
-- i18n change lable
function GuildServerAnswerRankLayer:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local size = self._imageArrow1:getContentSize()
		self._imageArrow1:setContentSize(cc.size(72,size.height))
	end
end
