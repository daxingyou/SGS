...
function GuildWarCityNode:onCreate()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		UIHelper.setLabelStyle(self._imageName,{
			 style = "guild_war_"..self._cfg.scene,
			 text = Lang.getImgText(self._cfg.name_pic) ,
		})
	else
...
-- i18n change lable
function GuildWarCityNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._imageName = UIHelper.swapWithLabel(self._imageName,{
			 style = "guild_war_1",
			 text = Lang.getImgText("txt_city_01") ,
		})

		local UIHelper  = require("yoka.utils.UIHelper")	
        self._imageProclaim = UIHelper.swapSignImage(self._imageProclaim,
		{ 
			 style = "signet_2", 
			 text = Lang.getImgText("img_seal_yixuanzhan01") ,
