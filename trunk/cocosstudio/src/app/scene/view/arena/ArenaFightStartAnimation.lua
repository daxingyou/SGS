        ...
        if self._battleData.firstOrder == playerIndex then
            if  not Lang.checkLang(Lang.CN) then
                ...
                local function _effectFunction(_effect)
                    if _effect == "routine_word_jjc_xianshou_zi" then
                        local TypeConst = require("app.i18n.utils.TypeConst")
                        local UIHelper = require("yoka.utils.UIHelper")
                        local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_xianshou"),style="effect_text_17",styleType=TypeConst.EFFECT})
                        return subLabel
                    end
                end	
                local effect = G_EffectGfxMgr:createPlayMovingGfx(_node, "moving_effect_jjc_xianshou", _effectFunction, nil , true)
                return _node
            else
