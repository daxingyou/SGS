...
function AvatarShopCellNode:updateUI(goodId)
	...
	if Lang.checkLang(Lang.JA) then 
		self._drawPosY = -10 
		self._fileNodeDraw:setScale(0.4)
	end
	--调整位置
...
-- i18n change lable
function AvatarShopCellNode:_dealI18n()
	if not Lang.checkLang(Lang.CN)  then 
		if Lang.checkLang(Lang.JA) then -- i18n ja change
			ccui.Helper:seekNodeByName(self._resource1, "Text"):setFontSize(18)
			ccui.Helper:seekNodeByName(self._resource2, "Text"):setFontSize(18)
			-- 调整头像avatar 
			self._fileNodeDraw:setPositionY(-10)   
			self._fileNodeDraw:setScale(0.4)
		end

		local UIHelper  = require("yoka.utils.UIHelper")
		UIHelper.seekNodeByName(self._resource2,"TextOr"):setFontSize(UIHelper.seekNodeByName(self._resource2,"TextOr"):getFontSize() - 2) -- i18n ja change
		self._resource1:setPositionX(self._resource1:getPositionX()-10)
		self._resource2:setPositionX(self._resource2:getPositionX()+4)

		local text = UIHelper.seekNodeByName(self._resource2,"Text")
		text:setPositionX(text:getPositionX()-5)	
		
		local textOr = UIHelper.seekNodeByName(self._resource2,"TextOr")
		textOr:setPositionX(textOr:getPositionX()-7)
	end
end
