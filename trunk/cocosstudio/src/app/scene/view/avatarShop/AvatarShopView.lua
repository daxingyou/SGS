...
-- i18n change lable
function AvatarShopView:_dealI18n()

	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image = UIHelper.seekNodeByName(self._buttonTransform,"Image_4")
		UIHelper.swapWithLabel(image,
		{ 
			 style = "icon_txt_3",
			 text = Lang.getImgText("avatar_shop_exchange") ,
			 offsetY = -30,
		})

		local text1 = UIHelper.seekNodeByName(self,"Node_Mid","Text_43")
		if Lang.checkLang(Lang.VN) then
			text1:setPositionY(text1:getPositionY()+6)
			self._textTime:setPositionY(self._textTime:getPositionY()+6)
		end
		self._textTime:setFontSize(20)	 -- i18n ja change
		text1:setFontSize(20)	
		if Lang.checkLang(Lang.JA) then
			self._textTime:setFontSize(18)
...
-- i18n change lable
function AvatarShopView:_updatePosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text1 = UIHelper.seekNodeByName(self,"Node_Mid","Text_43")
		if Lang.checkLang(Lang.VN) then
			UIHelper.alignCenterToFixPos(165,{text1,self._textTime})
		elseif Lang.checkLang(Lang.JA) then
			UIHelper.alignCenterToFixPos(152,{text1,self._textTime})
		else
			UIHelper.alignCenterToFixPos(155,{text1,self._textTime})
		end
	end
end
