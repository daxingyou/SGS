...
function PackageHistoryHeroCell:onCreate()
	...
	if Lang.checkUI("ui4") then
		self._resourceNode:setContentSize(cc.size(929,123))
		self._item1:setPositionY(-2)
		self._item2:setPositionY(-2)
	end
...
-- i18n
function PackageHistoryHeroCell:_dealByI18n()
    if Lang.checkUI("ui4") then		-- i18n ja change pos
		local UIHelper  = require("yoka.utils.UIHelper")
		for i = 1, 2 do
			-- self["_desc" .. i]:setPositionX(self["_desc" .. i]:getPositionX()-3)
			-- self["_desc" .. i]:setPositionY(self["_desc" .. i]:getPositionY()+8)
			local size1 = self["_desc" .. i]:getContentSize()
			self["_desc" .. i]:setContentSize(cc.size(174,size1.height + 12))
			self["_desc" .. i]:setFontSize(15)
			self["_desc" .. i]:setPosition(119,69)
		end
	end
end
