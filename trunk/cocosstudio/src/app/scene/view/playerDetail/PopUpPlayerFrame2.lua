...
function PopUpPlayerFrame:ctor()
    ...
    local resource = {
        file = Path.getCSB("PopUpPlayerFrame2", "playerDetail"),
        binding = {
        _btnEquip = {
...
function PopUpPlayerFrame:onCreate()
	self._commonNodeBk:addCloseEventListener(handler(self, self.onBtnClose))
	self._commonNodeBk:setTitle(Lang.get("change_role_frame"))
    ...
    self._btnEquip:setFontName(Path.getFontW8())
...
function PopUpPlayerFrame:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
       ...
       -- self._descStr:getVirtualRenderer():setVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_BOTTOM)
        ...
        self._descStr:getVirtualRenderer():setMaxLineWidth(210)

        self._descStr2:setAnchorPoint(cc.p(0.5,1))
        self._descStr2:getVirtualRenderer():setMaxLineWidth(210)
        -- 名称描述超出
        -- self._frameName:getVirtualRenderer():setMaxLineWidth(200)
        self._frameName:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER);
	end
end
...
function PopUpPlayerFrame:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        ...
        UIHelper.alignCenter(self._btnEquip,{text1,self._frameTime},{10,0})
...
-- i18n ja
function PopUpPlayerFrame:_updateEquipButton()
    if G_UserData:getHeadFrame():getCurrentFrame():getId() == self._currentChooseFrameInfo:getId() then
        self._btnEquip:setString(Lang.get("season_silk_weared"))
        self._btnEquip:setEnabled(false)
    else
        self._btnEquip:setString(Lang.get("role_frame_equip"))
        self._btnEquip:setEnabled(true)
    end
end
