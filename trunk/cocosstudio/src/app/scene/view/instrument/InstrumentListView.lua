...
function InstrumentListView:_updateView()
	...
	if Lang.checkUI("ui4") then
		...
		if isOpen then
			self._buttonRecycle:setVisible(self._selectTabIndex == InstrumentConst.INSTRUMENT_LIST_TYPE1)
		else
...
-- i18n change lable
function InstrumentListView:_swapImageByI18n()
	...
	-- i18n ja change 
	if Lang.checkUI("ui4") then
		local txtTip = ccui.Helper:seekNodeByName(self._fileNodeEmpty, "TextTip")
		txtTip:setPositionY(-15)  	 
	end
end

-- i18n ja 回收
function InstrumentListView:_showRecycleBtnI18n()
	if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
