...
for k,v in pairs(ChallengeRes) do
     -- i18n change text
    ChallengeRes[k]  = setmetatable( {_raw = v,_funcId = k } , { 
    __index = function(t, k) 
        logWarn("-------- ss "..k.." "..t._funcId)
        if k ~= "text" then
            return t._raw[k]
        end
        ...
        if t._funcId == FunctionConst.FUNC_ARENA then return Lang.getImgText("txt_challenge_battle") end
        if t._funcId == FunctionConst.FUNC_DAILY_STAGE then return Lang.getImgText("txt_challenge_daily") end
        if t._funcId == FunctionConst.FUNC_PVE_TOWER then return Lang.getImgText("txt_challenge_tower") end
        if t._funcId == FunctionConst.FUNC_PVE_SIEGE then return Lang.getImgText("txt_challenge_invade") end
        if t._funcId == FunctionConst.FUNC_PVE_TERRITORY then return Lang.getImgText("txt_challenge_patrol") end
        if t._funcId == FunctionConst.FUNC_WORLD_BOSS then return Lang.getImgText("txt_challenge_boss") end
        return ""
    end
})
end
