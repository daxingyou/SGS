...
function HeroGoldCostNode:_init()
    ...
    local styles = "text_limit_1"
    -- i18n ja change ui
    if Lang.checkUI("ui4") then 
        RES_CONST[LimitCostConst.LIMIT_COST_KEY_2].imageButtom = "img_niepan_di02"
        RES_CONST[LimitCostConst.LIMIT_COST_KEY_3].imageButtom = "img_niepan_di02"
        RES_CONST[LimitCostConst.LIMIT_COST_KEY_4].imageButtom = "img_niepan_di02"
        self._buttonAdd:setScale(0.6)
        styles = "text_gold_limit_1" 
    end
	...
	if not Lang.checkLang(Lang.CN) then
		...
		self._imageName = UIHelper.swapWithLabel(self._imageName,{ 
            style = styles,  
            text = Lang.getImgText(RES_CONST[self._costKey].imageName),
		})
    end
    ...
    -- i18n ja pos redPoint
    if Lang.checkUI("ui4") then 
        self._redPoint:setPosition(48, -29-5)   
    end
...
function HeroGoldCostNode:changeImageName()
    ...
    if Lang.checkLang(Lang.CN) then
        ...
        self._imageName:setString(
            Lang.getImgText(RES_CONST[self._costKey].imageName)
        )
    end
...
function HeroGoldCostNode:initImageFront()
    -- i18n ja change path
    if Lang.checkUI("ui4") then 
        self._imageButtom:loadTexture(Path.getTextTeam( RES_CONST[self._costKey].imageButtom ))  
        self._imageFront:loadTexture(Path.getTextTeam( RES_CONST[self._costKey].imageFront ))
        self._imageButtom:ignoreContentAdaptWithSize(false)
		self._imageButtom:setContentSize(cc.size(88, 88))
    else
...
function HeroGoldCostNode:_fullEffect()
    -- i18n ja change effect
    if Lang.checkUI("ui4") then 
        self._nodeEffect:setScale(0.7)
    end    
