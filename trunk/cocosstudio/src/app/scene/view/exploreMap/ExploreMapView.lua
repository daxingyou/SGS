...
function ExploreMapView:_exploreOver()
    ...
    local levelUp = UserCheck.isLevelUp(function(showLevelUp)
        local isPopScene = false --i18n ja
        ...
        if G_TutorialManager:isDoingStep() == false and G_NewLevelPkgManager:hasPop(LevelPkgConst.CONDITION_EXPLORE) then
            -- i18n ja change 
            G_SignalManager:dispatch(SignalConst.EVENT_NEW_LEVEL_PKG_OPEN_NOTICE,LevelPkgConst.CONDITION_EXPLORE,callback)
        else
            callback()
        end
        crashPrint("[Level Gift] ExploreMapView start ")
    ...
    local function createEventEffectFunc()
		...
		local function effectFunction(effect)
			...
			elseif effect == "tubiao" then
				...
				return sprite
            elseif not Lang.checkLang(Lang.CN) and not Lang.checkLang(Lang.JA) and effect == "routine_word_chufaqiyu_dazi" then
               ...
               local subLabel = UIHelper.createLabel({
                                                        text =  Lang.getEffectText("effect_chufaqiyu"),
                                                        color = Colors.OBVIOUS_YELLOW,
                                                        fontSize = 60,
