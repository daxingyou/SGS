...
-- i18n pos lable
function SeasonRankCell:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		-- self._fileNodeLook:setTitleText(Lang.getImgText("season_sport_btn_see") )
	end
end

-- i18n pos lable
function SeasonRankCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        self._imageSortBack:setPositionX( self._imageSortBack:getPositionX()-35)

        self._fileHeadFrame:setPositionX( self._fileHeadFrame:getPositionX()-60)
        self._fileNodeIcon:setPositionX( self._fileNodeIcon:getPositionX()-60)

        local imageServer = UIHelper.seekNodeByName(self._resource,"Image_server")
        imageServer:setPositionX(imageServer:getPositionX()-80)
        self._fileNodeName:setPositionX( self._fileNodeName:getPositionX()-80)
        self._imageHonorTitle:setPositionX(self._imageHonorTitle:getPositionX()-80)
             --日文特殊处理
             if Lang.checkLang(Lang.JA) then
                self._imageSortBack:setPositionX( self._imageSortBack:getPositionX()+15)
                imageServer:setPositionX( imageServer:getPositionX()+15)
                self._fileNodeIcon:setPositionX( self._fileNodeIcon:getPositionX()+15)
                self._fileHeadFrame:setPositionX( self._fileHeadFrame:getPositionX()+15)
                self._fileNodeName:setPositionX( self._fileNodeName:getPositionX()+15)
                self._imageHonorTitle:setPositionX( self._imageHonorTitle:getPositionX()+15)
