...
-- Describle：
local TypeConvertHelper = require("app.utils.TypeConvertHelper")--i18n ja
local DataConst = require("app.const.DataConst")--i18n ja
...
function LevelGiftItemCell:ctor()
	...
	if Lang.checkUI("ui4") then
		resource.file = Path.getCSB("VipViewLevelGiftPkgItemCell", "vip")
	end
...
function LevelGiftItemCell:updateAppstoreCheckUI(data)
	...
	if not Lang.checkUI("ui4") then
		...
		self._commonListViewItem:setListViewSize(218,70)
		self._commonListViewItem:updateUI(
			{
				{type = config.type, value = config.value, size = config.size}
			}, 0.6, true)
	end
...
function LevelGiftItemCell:updateUI(data)
	...
	if not Lang.checkUI("ui4") then
		...
		self._commonListViewItem:updateUI({
			{type = config.type, value = config.value, size = config.size},
			{type = TypeConvertHelper.TYPE_RESOURCE,value = DataConst.RES_DIAMOND,size = payCfg.gold}
		}, 0.8, true)
		self._commonListViewItem:alignCenter()
		self._textItemName:setString(config.name)
		self._textItemName1:setString(Lang.get("vip_level_gift_cell_title"))
		local UIHelper  = require("yoka.utils.UIHelper")
		UIHelper.alignCenter(self._resourceNode,{self._textItemName,self._textItemName1})
	end
...
function LevelGiftItemCell:_dealPosByI18n()
	if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
		UIHelper.alignCenter(self._resourceNode,{self._timeCountDown,self._timeCountDownDes},{0,0})
		return
	end
