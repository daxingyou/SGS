...
function ActivityView:_getActivityModuleUI(index)
	...
	if activityModuleUI == nil then
		...
		--i18n late regist
		elseif data.id == ActivityConst.ACT_ID_LATE_REGIST then
			--i18n后加入玩家补偿
			local LateRegistView = require("app.i18n.extends.ui.LateRegistView")
			activityModuleUI = LateRegistView.new(self, data.id)
		elseif data.id == ActivityConst.ACT_ID_FRIEND_INVITE then
			-- i18n ja 新增好友邀请码功能
			local FriendInviteView = require("app.scene.view.activity.friendInvite.FriendInviteView")
			activityModuleUI = FriendInviteView.new(self, data.id)
		elseif data.id > ActivityConst.ACT_ID_OPEN_SERVER_FUND  then
			activityModuleUI = OpenServerFundView.new(self, data.id,
