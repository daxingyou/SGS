...
function DailyActivityHint:_parseConfigData()
	...
	for k, v in pairs(indexs) do
		...
		if Lang.checkLang(Lang.CN) or (not Lang.checkLang(Lang.CN) and config.is_work == 1) then
			local singleData = {}
			singleData.weeks = {}
...
--i18n
function DailyActivityHint:_updateDescI18n(richtext)
	if Lang.checkUI("ui4") then
		self._textDescSc:removeAllChildren()
		richtext:removeSelf()
		self._textDescSc:addChild(richtext)
		local labelSize = richtext:getContentSize()
...
--i18n
function DailyActivityHint:_initDescI18n()
	if Lang.checkUI("ui4") then
		...
		sc:setContentSize(cc.size(630,50))
		local panel = ccui.Helper:seekNodeByName(self, "Panel")
		panel:addChild(sc)
		sc:setAnchorPoint(0,1)
		sc:setPosition(self._richTextNode:getPosition())
		self._richTextNode:setVisible(false)
		self._textDescSc = sc
	end
end

-- i18n ja
function DailyActivityHint:_dealByI18n()
	if Lang.checkLang(Lang.JA) then
		local label1 = ccui.Helper:seekNodeByName(self, "activityAwardLabel_0")
		label1:setPositionX(label1:getPositionX()-20)
		local label2 = ccui.Helper:seekNodeByName(self, "activityAwardLabel")
		label2:setPositionX(label2:getPositionX()-20)
	end
end
