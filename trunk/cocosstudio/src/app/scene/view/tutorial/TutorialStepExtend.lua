...
TutorialStepExtend.step603 =
{
	...
	findfunc = function(nodeName)
		...
		if not Lang.checkUI("ui4") then -- i18n ja change 
			...
			local clickWidget = root:getSubNodeByName("_pageView")
			return clickWidget:getCascadeBoundingBox()
		end
	end,
	clickfunc = function(sender, stepData)
...
TutorialStepExtend.step604 =
{
	...
	findfunc = function(nodeName)
		local root = nil
		if not Lang.checkUI("ui4") then -- i18n ja change 
			root = display.getRunningScene():getSubNodeByName("HeroDetailAttrModule")
		else
	...
	clickfunc = function(sender, stepData)
		if not Lang.checkUI("ui4") then -- i18n ja change 
			...
			root:_onButtonUpgradeClicked()
			--G_TutorialManager:clearTipLayer()  layer不能调用  会导致TipLayer清空子节点
		end	
