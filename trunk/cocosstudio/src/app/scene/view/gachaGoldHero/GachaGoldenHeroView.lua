...
function GachaGoldenHeroView:_updateJoyDraw(isCreateIn)
    ...
    if Lang.checkLang(Lang.JA) then -- i18n ja change
        self._imageIconTxt:setPositionX(self._imageIconTxt:getPositionX() + 10) 
        self._imageIconTxt:setFontSize(self._imageIconTxt:getFontSize() - 6)    
        self._gachaAwards:getTextImage():setFontSize(self._gachaAwards:getTextImage():getFontSize() - 4)   
        self._gachaShop:getTextImage():setFontSize(self._gachaShop:getTextImage():getFontSize() - 4)
    end 
end
...
function GachaGoldenHeroView:_touchAvatar(heroId)
    G_UserData:getGachaGoldenHero():setGachaDrawHeroViewData({heroId, self._currentChooseZhenyin}) -- i18n ja bug: 见龙在田 无法连续抽奖，抽奖后页面上抽奖按钮消失，需要退出界面重新选择
...
function GachaGoldenHeroView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        ...
        if Lang.checkLang(Lang.JA) then 
            self._imageIconTxt:setPositionX(self._imageIconTxt:getPositionX() - 10) 
            --self._gachaShop:setPositionX(self._gachaShop:getPositionX() + 10) 
            self._gachaAwards:setPositionX(self._gachaAwards:getPositionX() - 10) 
        end
	end
end
