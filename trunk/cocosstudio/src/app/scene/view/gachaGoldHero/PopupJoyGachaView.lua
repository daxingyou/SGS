...
function PopupJoyGachaView:_updateRewardsImg( ... )
    ...
    if Lang.checkLang(Lang.CN) then
		...
		UIHelper.setLabelStyle(self._imageRewardDesc,{
			 style = GachaGoldenHeroHelper.isLottery(self._prizeLists) and "text_gold_hero_5" or "text_gold_hero_6",
			 text = Lang.getImgText(imgStateName),
        })
    end
...
-- i18n change lable
function PopupJoyGachaView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._imageRewardDesc = UIHelper.swapWithLabel(self._imageRewardDesc,{
			 style = "text_gold_hero_5",
			 text = Lang.getImgText("txt_gold_03"),
		})
	end
end

-- i18n pos 
function PopupJoyGachaView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
       
        self._textNextCountDown:setPositionY(self._textCurCountDown:getPositionY())
        self._textCurTime:setPositionY(self._textCurCountDown:getPositionY()-3)
        self._textNextTime:setPositionY(self._textCurCountDown:getPositionY()-3)

        
        local textTitle = ccui.Helper:seekNodeByName(self._commonBack, "Text_title")
        textTitle:setPositionY(textTitle:getPositionY()+5)

        if Lang.checkLang(Lang.JA) then -- i18n ja change
            self._textCurCountDown:setPositionX(self._textCurCountDown:getPositionX() + 40)
            self._textCurTime:setPositionX(self._textCurTime:getPositionX() + 40)  
        end
    end
end
