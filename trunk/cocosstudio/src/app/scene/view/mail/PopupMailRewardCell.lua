...
function PopupMailRewardCell:updateUI(mailInfo,index,selectIndex)
	...
	if Lang.checkUI("ui4") then
		self._textRewardHint:setPositionX(self._textSendTime:getPositionX()+self._textSendTime:getContentSize().width/2)
	end
...
-- i18n pos lable
function PopupMailRewardCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textTitle:setFontSize(self._textTitle:getFontSize()-2)
	end
	if Lang.checkLang(Lang.JA) then
		self._textTitle:setString("")
		self._textTitle:setFontSize(22)
		self._textTitle:getVirtualRenderer():setWidth(270)
	end
end
