...
-- i18n pos lable
function ActivityGuildSprintView:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")

		self._textMyRank:setPositionX(self._textRankTitle:getPositionX()+self._textRankTitle:getContentSize().width)
		self._textMyRank:setPositionY(self._textRankTitle:getPositionY())
	end
	if Lang.checkUI("ui4") then
		self._textActDes:setPositionY(self._textActDes:getPositionY()+12)
		local size = self._textActDes:getContentSize()
		self._textActDes:setContentSize(cc.size(size.width,size.height+8))
		self._textMyRank:setPositionX(self._textRankTitle:getPositionX()+self._textRankTitle:getContentSize().width+10)
	end
end
