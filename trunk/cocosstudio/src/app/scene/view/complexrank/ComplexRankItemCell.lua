...
function ComplexRankItemCell:updateRankType(rankType, data)
	...
	if not Lang.checkLang(Lang.CN) then
		...
		local node = self:getSubNodeByName("Node_power")
		local imageName = UIHelper.seekNodeByName(node,"Image_name")
		local textBmpText = UIHelper.seekNodeByName(node,"Text_bmp_text")

		if rankType == ComplexRankConst.AVATAR_PHOTO_RANK  or rankType == ComplexRankConst.ACTIVE_PHOTO_RANK then
			textBmpText:setPositionX(840)
		end

		imageName:setPositionX(
			textBmpText:getPositionX()-imageName:getContentSize().width
		)
	end
...
-- i18n pos lable
function ComplexRankItemCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local nodeTower = UIHelper.seekNodeByName(self._resourceNode,"Node_tower")
		local imageName = UIHelper.seekNodeByName(nodeTower,"Image_name")
		local textBmpText = UIHelper.seekNodeByName(nodeTower,"Text_bmp_text")
		imageName:setAnchorPoint(cc.p(1,0.5))
		imageName:setPositionX(textBmpText:getPositionX()-2)



		local nodePower = UIHelper.seekNodeByName(self._resourceNode,"Node_power")
		local imageName = UIHelper.seekNodeByName(nodePower,"Image_name")
		local textBmpText = UIHelper.seekNodeByName(nodePower,"Text_bmp_text")
		textBmpText:setPositionX(textBmpText:getPositionX())
		imageName:setPositionX(
			textBmpText:getPositionX()-imageName:getContentSize().width
		)
		
		local nodeLevel = UIHelper.seekNodeByName(self._resourceNode,"Node_level")
		local imageName = UIHelper.seekNodeByName(nodeLevel,"Image_name")
		local textBmpText = UIHelper.seekNodeByName(nodeLevel,"Text_bmp_text")
		textBmpText:setPositionX(textBmpText:getPositionX()-5)
		imageName:setPositionX(
			textBmpText:getPositionX()-imageName:getContentSize().width
		)

		local nodeStar = UIHelper.seekNodeByName(self._resourceNode,"Node_star")
		local imageName = UIHelper.seekNodeByName(nodeStar,"Image_name")
		local textBmpText = UIHelper.seekNodeByName(nodeStar,"Text_bmp_text")
		imageName:setAnchorPoint(cc.p(1,0.5))
		imageName:setPositionX(textBmpText:getPositionX()-2)
		
		
		
		local nodeGroup = UIHelper.seekNodeByName(self._resourceNode,"Node_group")
		local textGuildNumDes = UIHelper.seekNodeByName(nodeGroup,"Text_guild_num_des")
		local textGuildNum = UIHelper.seekNodeByName(nodeGroup,"Text_guild_num")
		local textGuildPlayerDes = UIHelper.seekNodeByName(nodeGroup,"Text_guild_player_des")
		local textGuildPlayer = UIHelper.seekNodeByName(nodeGroup,"Text_guild_player")
		
		textGuildNum:setPositionX(textGuildNumDes:getPositionX() + textGuildNumDes:getContentSize().width)
		textGuildPlayer:setPositionX(textGuildPlayerDes:getPositionX() + textGuildPlayerDes:getContentSize().width)

		local nodeNormal = UIHelper.seekNodeByName(self._resourceNode,"Node_normal")
		local textGuildName = UIHelper.seekNodeByName(nodeNormal,"Text_guildName")
		textGuildName:setFontSize(textGuildName:getFontSize()-2)

		--i18n ja pos
		if Lang.checkLang(Lang.JA) then
			-- 父节点不同，相对坐标
			textGuildName:setPositionX(self._fileNodePlayer:getPositionX() + self._fileNodePlayer:getTextName():getPositionX())
		end
	end
end
