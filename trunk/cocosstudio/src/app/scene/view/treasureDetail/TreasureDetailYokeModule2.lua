...
function TreasureDetailYokeModule:_createWidgetCondition(data, heroId, isActivated)
	...
	local label = cc.Label:createWithTTF(totalDes, Path.getCommonFont(), 16)
	label:setMaxLineWidth(self._listView:getContentSize().width - 20) 
	label:setAnchorPoint(cc.p(0, 0))
	label:setPosition(cc.p(10, 0))
...
function TreasureDetailYokeModule:_createWidgetLine()
	local widget = ccui.Widget:create()
	local line = TeamViewHelper.createLine(self._listView:getContentSize().width)
