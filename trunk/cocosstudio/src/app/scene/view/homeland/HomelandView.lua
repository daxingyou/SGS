...
function HomelandView:onCreate()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		local isShow = LogicCheckHelper.funcIsShow(FunctionConst.FUNC_HOMELAND_HANDBOOK)
		self._commonHandbook:setVisible(isShow)
		self._prayNode:setVisible(isShow)
	end
	...
	if Lang.checkUI("ui4") then
		-- 缩小字号
		-- self._commonShop:getTextImage():setFontSize(self._commonShop:getTextImage():getFontSize() - 4)   
		-- self._commonPreview:getTextImage():setFontSize(self._commonPreview:getTextImage():getFontSize() - 4)

		--标题字体缩小
			local UIHelper  = require("yoka.utils.UIHelper")
			local text = UIHelper.seekNodeByName(self,"Text_13")
			text:setFontSize(18)
	end
end
...
-- i18n change lable
function HomelandView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		--[[
		local image = UIHelper.seekNodeByName(self,"Image_92")
		UIHelper.swapWithLabel(image,{
			style = "homeland_3",
			text = Lang.getImgText("txt_homeland_01") ,
		})
]]
		local image1 = UIHelper.seekNodeByName(self._buttonFold,"Image_2")
		local label = UIHelper.swapWithLabel(image1,{
			style = "homeland_5",
			text = Lang.getImgText("txt_homeland_03") ,
		})
...
-- i18n change lable
function HomelandView:_dealI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		if not Lang.checkLang(Lang.JA) then
			self._commonHelp:setPositionY(  self._commonHelp:getPositionY())
			self._commonPreview:setPositionX(self._commonPreview:getPositionX() -20)
			self._commonShop:setPositionX(self._commonShop:getPositionX() -18)
			self._commonHandbook:setPositionX(self._commonHandbook:getPositionX() -18)
		end
	end
end
