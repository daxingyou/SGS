...
function HomelandHelp.updateNodeTreeTitle( rootNode, treeData )
	...
	if Lang.checkLang(Lang.JA) then   -- i18n ja change line
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(Node_treeTitle,"Image_title")
		if not image1.loadTexture then
			--image1:getVirtualRenderer():setMaxLineWidth( image1:getFontSize()) 
			image1:getVirtualRenderer():setLineSpacing(-2)  
			-- 策划需求：调整字号统一
			image1:setFontSize(20)
			-- Lv10 等级>9时数字，不换行
			local UTF8 = require("app.utils.UTF8")
			local name = image1:getString()
			local num = string.match(name, "%d+")   
			if num and tonumber(num) >= 10 then
				name = string.gsub(name, tostring(num), "")
				local strContent = ""     
				...
				strContent = strContent .. num 
				image1:setString(strContent)
			else   
				local strContent = ""     
				local len = UTF8.utf8len(name)
				for i=1, len do  
		...
		-- Lv等级换行
		if ccui.Helper:seekNodeByName(Node_treeTitle, "Text_level" ) then
			local textLevel = ccui.Helper:seekNodeByName(Node_treeTitle, "Text_level" )  
			local str = textLevel:getString()
			str = string.gsub(str, "Lv", "Lv\n")
