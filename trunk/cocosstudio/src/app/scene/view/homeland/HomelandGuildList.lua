...
-- i18n change lable
function HomelandGuildList:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image = UIHelper.seekNodeByName(self,"Image_3")
		UIHelper.swapWithLabel(image,{
			style = "homeland_2",
			text = Lang.getImgText("txt_homeland_07") ,
		})
	...
	if Lang.checkLang(Lang.JA) then 
		local text_1 = self._buttonFold:getParent():getChildByName("Text_1")
		text_1:setFontSize(text_1:getFontSize() - 2)
		text_1:getVirtualRenderer():setHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
	end
end
