...
-- i18n change lable
function PopupHomelandPray:_updatePosI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text1 = UIHelper.seekNodeByName(self,"Text1")
      

        local image = UIHelper.seekNodeByName(self._nodeResource,"Image")
        local text = UIHelper.seekNodeByName(self._nodeResource,"Text")
       
        local UIHelper  = require("yoka.utils.UIHelper")
        UIHelper.alignCenter(self._nodeSignBg,{text1,image,text},{5,5,0})
	end
end


-- i18n change lable
function PopupHomelandPray:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local ImageTip = UIHelper.seekNodeByName(self,"ImageTip")
	    UIHelper.swapWithLabel(ImageTip,{
			 text = Lang.getImgText("txt_homeland_bless_01") ,
        ...
        local Image_1 = UIHelper.seekNodeByName(self._nodeSignBg,"Image_1")
	    UIHelper.swapWithLabel(Image_1,{
			 text = Lang.getImgText("txt_homeland_bless_02") ,
             color = cc.c3b(0xfe,0xe1,0x02),
             fontSize = 24,
             fontName = Path.getFontW8(),
		})
