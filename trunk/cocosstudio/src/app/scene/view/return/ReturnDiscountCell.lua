...
function ReturnDiscountCell:onCreate()
    ...
    --i18n change
    self._nodeProgressPosX = self._nodeProgress:getPositionX()
end
...
function ReturnDiscountCell:update(data)
    ...
    if configInfo.reward_type == 2 then
        ...
        --i18n change
        local maxPrice = configInfo.price
        local curRmb = rechargeNum
        local richText = Lang.get("return_progress",
            { 
                title = Lang.get("times_title1"), 
                cur = curRmb, curColor = Colors.colorToNumber(Colors.NORMAL_BG_ONE), 
                max = maxPrice, maxColor = Colors.colorToNumber(Colors.NORMAL_BG_ONE )
            }
        )
        if not Lang.checkLang(Lang.CN) then
            local UIHelper  = require("yoka.utils.UIHelper")
            local _,currency = UIHelper.convertCurrency(curRmb)
            local _,maxCurrency = UIHelper.convertCurrency(maxPrice)
            -- print("maxPrice:"..maxPrice)
            -- print("curRmb:"..curRmb)
            -- print("currency:"..currency)
            -- print("maxCurrency:"..maxCurrency)
            richText = Lang.get("return_progress",
                { 
                    title = Lang.get("times_title1"), 
                    cur = currency, curColor = Colors.colorToNumber(Colors.NORMAL_BG_ONE), 
                    max = maxCurrency, maxColor = Colors.colorToNumber(Colors.NORMAL_BG_ONE )
                }
            )
            -- print("cur:"..currency)
            -- print("max:"..maxCurrency)
        end
        -- print("richText:"..richText)
        self:_createProgressRichText(richText)
        local offsetX = 10
        if not Lang.checkLang(Lang.CN) then
            self._nodeProgress:setPositionX(self._nodeProgressPosX-offsetX)
        end
...
function ReturnDiscountCell:_initRewardInfo(configInfo)
    ...
    if configInfo.is_choose == 1 then    -- 奖励多选1
        ...
        if Lang.checkLang(Lang.CN) then
            ...
            self._commonRewardListNode:setGaps(ReturnDiscountCell.ITEM_GAP,140,140,430)
        end
...
-- i18n change lable
function ReturnDiscountCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
