...
function PopupDailyChooseCell:onCreate()
    ...
    if not Lang.checkLang(Lang.CN)  then
        --self._btnFight:switchToHightLight()
       -- self._btnSweep:switchToHightLight()
    end
...
function PopupDailyChooseCell:_refreshTitleAndSword()
    ...
    if  Lang.checkLang(Lang.CN)  then
        ...
        UIHelper.setLabelStyle( self._textTitle,{
            style =  "daily_challenge_"..(self._info.id % 10),
            text = self._info.difficulty,

        })
        if Lang.checkUI("ui4") then
            self._textTitle:setFontSize(36)
            self._textTitle:setPositionY(355)
        end
    end
...
function PopupDailyChooseCell:_refreshCell()
    ...
    if Lang.checkUI("ui4") then
        if self._textDetail:isVisible() and self._textDetail:getContentSize().height > 20 then
            self._descBg:setContentSize(cc.size(151,43))
        else
            self._descBg:setContentSize(cc.size(151,30))
        end
    end
...
-- i18n change lable
function PopupDailyChooseCell:_dealI18n()
    if Lang.checkSquareLanguage() then

	elseif not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textReward:setPositionX(self._textReward:getPositionX()-2)
        self._reward:setPositionX(self._reward:getPositionX()+15)
    end
    if Lang.checkLang(Lang.JA) then
        self._textDetail:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        self._textDetail:setFontSize(16)
        self._textReward:setFontSize(18)
        local UIHelper  = require("yoka.utils.UIHelper")
        local text = UIHelper.seekNodeByName(self._reward,"Text")
        text:setFontSize(20)
    end
    if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
        local bg = UIHelper.createImage({texture = Path.getDailyChallengeIcon("img_difficulty_ditu") })
        bg:setScale9Enabled(true)
        bg:setCapInsets(cc.rect(50,15,1,1))
        bg:setContentSize(cc.size(151,43))
        bg:setAnchorPoint(0.5,0)
        bg:setPosition(110,128)
        self._nodeBG:addChild(bg)
        self._descBg = bg
        self._textDetail:setAnchorPoint(0.5,0)
        self._textDetail:setPositionY(146)
    end
end

-- i18n pos lable
function PopupDailyChooseCell:_updateRewardPos()
    if Lang.checkUI("ui4") then
        local UIHelper  = require("yoka.utils.UIHelper")
        local text = UIHelper.seekNodeByName(self._reward,"Text")
        local image = UIHelper.seekNodeByName(self._reward,"Image")
	    UIHelper.alignCenter(self._descBg,{self._textReward,image,text},{0,0,0})
    end
end
