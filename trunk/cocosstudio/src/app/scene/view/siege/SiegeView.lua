...
-- i18n pos btn
function SiegeView:_dealPosByI18n()
    if Lang.checkLang(Lang.ZH) or Lang.checkUI("ui4") then
         self._btnReward:setPositionX(self._btnReward:getPositionX()+10)
         self._btnRank:setPositionX(self._btnRank:getPositionX()+20)
	end
	if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
		local textRank = UIHelper.seekNodeByName(self,"TextRank")
		local textCorpsRank = UIHelper.seekNodeByName(self,"TextCorpsRank")
		self._textRankCrop:setPositionX(textRank:getPositionX()+textRank:getContentSize().width+5)
		self._textRankPersonal:setPositionX(textCorpsRank:getPositionX()+textCorpsRank:getContentSize().width+5)
	end
	if Lang.checkLang(Lang.JA) then
		self._btnRank:getButton():setPositionY(-10)
	end

	if Lang.checkLang(Lang.JA) then -- i18n ja change
		self._textRankCrop:getParent():setAnchorPoint(cc.p(0, 0.5))
		self._textRankCrop:getParent():setPositionX(20)
		self._textRankCrop:getParent():setContentSize(cc.size(275, 78))
		self._textRankPersonal:getParent():setAnchorPoint(cc.p(0, 0.5))
		self._textRankPersonal:getParent():setPositionX(20)
		self._textRankPersonal:getParent():setContentSize(cc.size(275, 78))
		self._rewardCrop:setPositionX(163)
		self._rewardPersonal:setPositionX(163)
	end
end
