...
function PopupPetBreakResult:_createRoleEffect()
	...
	local function effectFunction(effect)
		...
		if  not Lang.checkLang(Lang.CN) and effect == "moving_shenshoushengxing_z" then
			local function _effectFunction(_effect)
				if _effect == "routine_word_shenshoushengxing_zi" then
			...
			local effect = G_EffectGfxMgr:createPlayMovingGfx(_node, "moving_shenshoushengxing_z", _effectFunction, nil , false)
			return _node
		end
        return cc.Node:create()
    end
