	...
	for i = 1, 2 do
		...
		if userData then
			...
			if not Lang.checkLang(Lang.CN) then
				...
				UIHelper.dealVTextWidget(self["_textCount"..i],Lang.get("camp_play_off_win_count", {count = winNums[i]}))
			else
...
function SingleRaceBattlePlayerNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		for i = 1, 2 do
			self["_nodePower"..i]:setLocalZOrder(1)
			self["_textCount"..i]:getVirtualRenderer():setLineSpacing(-3)
		end
	end
