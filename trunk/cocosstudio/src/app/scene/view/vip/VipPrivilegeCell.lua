...
function VipPrivilegeCell:ctor()
	...
	if Lang.checkUI("ui4") then
        resource.file =  Path.getCSB("VipPrivilegeCell2", "vip")
    end
	self._textDesc = nil 
...
function VipPrivilegeCell:updateUI(textTitle, textDesc, isRed)
	...
	local richElementList = nil
	if Lang.checkUI("ui4") then
		richElementList = RichTextHelper.convertSubTitleToRichMsgArr(colorParam or {
			textColor = descColor or  Colors.NORMAL_BG_ONE	,--跑马灯的默认字体颜色
			fontSize = 20,--跑马灯的默认字体大小
		},subTitles)
	else
		richElementList = RichTextHelper.convertSubTitleToRichMsgArr(colorParam or {
			textColor = descColor or  Colors.NORMAL_BG_ONE	,--跑马灯的默认字体颜色
