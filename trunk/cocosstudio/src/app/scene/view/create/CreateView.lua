...
-- i18n change lable
function CreateView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local image1 = UIHelper.seekNodeByName(self._panelDesign,"Image_1")
		local x,y = image1:getPosition()
		local label = UIHelper.createLabel(
			{ 
				style = "create_role_1", 
				text = Lang.getImgText("img_createtitle") ,
				position = cc.p(x,y),
				anchorPoint = cc.p(0.5,0.5),
			}
		)
		image1:getParent():addChild(label)


		self._imageTitleMale01:ignoreContentAdaptWithSize(true)
		self._imageTitleMale02:ignoreContentAdaptWithSize(true)
		self._imageTitleFemale01:ignoreContentAdaptWithSize(true)
		self._imageTitleFemale02:ignoreContentAdaptWithSize(true)

		local imageBg1 = UIHelper.seekNodeByTagList(self._nodeTitle,211)
		local imageBg2 = UIHelper.seekNodeByName(self._nodeTitle2,"Image_26_1")
		local imageBg3 = UIHelper.seekNodeByName(self._nodeTitle2,"Image_26_0")
		local imageBg4 = UIHelper.seekNodeByTagList(self._nodeTitle,2907)

		imageBg1:setVisible(false)
		imageBg2:setVisible(false)
		imageBg3:setVisible(false)
		imageBg4:setVisible(false)
		self._imageTitleMale02:setVisible(false)
		self._imageTitleFemale02:setVisible(false)
	end
end
