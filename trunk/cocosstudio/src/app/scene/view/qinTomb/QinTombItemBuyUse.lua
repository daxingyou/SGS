...
function QinTombItemBuyUse:updateUI(shop_fix_id)
    ...
    local shopFixConfig = nil
    if Lang.checkUI("ui4") then
        shopFixConfig = UserDataHelper.getFixShopConfig(self._shopFixId)
    else
        shopFixConfig = ShopFix.get(self._shopFixId)
    end
    ...
    if Lang.checkUI("ui4") then
        if self._shopItemData == nil or shopItemData == false then
            self._shopItemData = G_UserData:getShops():getFixShopGoodsByResId(ShopConst.VIP_EXCHANGE_SHOP, self._itemType,self._itemId)
        end
    end
...
function QinTombItemBuyUse:updateUIExt(shop_fix_id)
    ...
    if Lang.checkUI("ui4") then
        if self._shopItemData == nil then
            self._shopItemData = G_UserData:getShops():getShopGoodsById(ShopConst.VIP_EXCHANGE_SHOP, self._shopFixId)
        end
    end
	self._itemIcon:unInitUI()
	self._itemIcon:initUI(self._itemType, self._itemId)
