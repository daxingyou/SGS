	...
	if type == FriendConst.FRIEND_LIST then
		if data:isCanGivePresent() then
			...
			if not Lang.checkLang(Lang.CN) then
				...
				UIHelper.setSignStyle(self._alreadyDone,
					{ 
						style = "signet_8", 
						text = Lang.getImgText("txt_yizengsong") ,
...
-- i18n change lable
function FriendListViewCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        self._alreadyDone = UIHelper.swapSignImage(self._alreadyDone,
		{ 
			 style = "signet_8", 
			 text = Lang.getImgText("txt_yizengsong") ,
