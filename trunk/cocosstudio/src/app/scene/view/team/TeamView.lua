...
function TeamView:onCreate()
	...
	self:_initData()
	self:_initView()
	self:_initInfo() -- i18n ja
end

-- i18n ja add func
function TeamView:_initInfo()
	if Lang.checkUI("ui4") then
		self._panelDesign:setLocalZOrder(TeamView.ZORDER_1)
		self._nodeContent:setLocalZOrder(TeamView.ZORDER_2)
		self._topbarBase:setLocalZOrder(TeamView.ZORDER_3)
		self._nodeTotalEffect:setLocalZOrder(TeamView.ZORDER_4) 

		self._listViewLineup:setPosition(12, 117)
		self._listViewLineup:setContentSize(cc.size(95+7, 446))
	
		self._nodeAwake:setPositionY(-47)		-- 有2觉醒星级： 为了防止高个武将把觉醒星星挡掉，在上层加入一个同样的觉醒控件
		...
		-- 	local langInfo = FunctionLevel.get(FunctionConst.FUNC_PET_HOME)
		-- 	UIHelper.setLabelStyle(ccui.Helper:seekNodeByName(self._buttonPet, "TextImage"), {
		-- 		style = "limit_1_ja",
		-- 		text = langInfo.name,
		-- 	})       
		-- end
...
-- i18n ja hero SSR
function TeamView:_updateHeroSSR()
	...
	local heroParam = TypeConvertHelper.convert(TypeConvertHelper.TYPE_HERO, heroBaseId, nil, nil, unitData:getLimit_level())
	local _imgList = {
						"img_com_grade_cheng", 
						"img_com_grade_lv",
						"img_com_grade_lan",   
	...
	imageBg:setName("img_com_grade")
	self._imageTip:getParent():removeChildByName("img_com_grade")
...
function TeamView:_initPageView()
	if Lang.checkUI("ui4") then -- i18n ja change size
		self._pageView:setContentSize(cc.size(self._panelMid:getContentSize().width*1.4, self._panelMid:getContentSize().height))
		self._pageView:setPositionX(self._pageView:getPositionX() )
	end
...
function TeamView:getCurHeroSpine()
	...
	if Lang.checkUI("ui4") then  -- i18n ja 新手引导
		spine = self._panelMid
	end
...
function TeamView:_onClickButtonPet()
...
function TeamView:_changeHeroFormation(eventName, pos, oldHeroId)
	...
	self:_updateHeroPageView()
	self:_switchPanelView(1)
	self:_updateHeroSSR()  -- i18n ja change SSR
end
...
function TeamView:checkHeroTrainRP(curHeroData)
	...
	if Lang.checkUI("ui4") then
		local reach3 = RedPointHelper.isModuleReach(FunctionConst.FUNC_HERO_TRAIN_TYPE3, curHeroData) --可觉醒
		-- local reach4 = RedPointHelper.isModuleReach(FunctionConst.FUNC_HERO_TRAIN_TYPE4, curHeroData) --可界限
		local HeroDataHelper = require("app.utils.data.HeroDataHelper")
		local reach4 = HeroDataHelper.isPromptHeroLimitEx(curHeroData)
		local reachList = {}
		if reach1 == true then
			table.insert(reachList,Path.getTextSignet(resLevelUp))
		end
		if reach2 == true then
			table.insert(reachList,Path.getTextSignet(resRankUp))
		end
		if reach3 == true then
			table.insert(reachList,Path.getTextSignet("img_hint05b"))
		end
		if reach4 == true then
			table.insert(reachList,Path.getTextSignet("img_hint07b"))
		end
		if #reachList > 0 then
			self._imageTip:setVisible(true)
			TeamViewHelper.playSkewFloatSwitchEffectEx(self._imageTip, reachList)
		else
			self._imageTip:setVisible(false)
		end
		return
...
-- i18n change lable
function TeamView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")

        local image3 = UIHelper.seekNodeByName(self._btnEmbattle,"Image_11")
		if image3 then
            ...
            if Lang.checkLang(Lang.ZH) then
				style = "icon_txt_30"
				offsetY = -37
			elseif Lang.checkUI("ui4") then 	-- i18n ja label style
				style = "limit_1_ja"
				offsetY = -37
			end
