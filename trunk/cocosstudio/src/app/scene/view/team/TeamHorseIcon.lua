...
function TeamHorseIcon:updateIcon(pos)
	...
	if self._horseId then
		...
		self._textName:setFontSize(18)	-- i18n ja change
...
function TeamHorseIcon:_onPanelTouch()
	...
	if self._horseId then
		if Lang.checkUI("ui4") then   -- i18n ja 已存在装备
			...
			if scene:getName() == "horseDetail" then   
				local view = scene:getSceneView()
				view:_onBtnHorseClick()
			else  
...
-- i18n pos lable
function TeamHorseIcon:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textName:setFontSize(18)
		self._textName:getVirtualRenderer():setMaxLineWidth(417)
		self._textName:setContentSize(cc.size(417,28))
	end

	-- i18n ja pos
	if Lang.checkLang(Lang.JA) then
		self._imageNameBg:setPositionY(self._imageNameBg:getPositionY() - 8)
