...
-- i18n change lable
function WorldBossView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then

		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self._commonBtnLeft,"Image_18")

		UIHelper.swapWithLabel(image1,{ 
			 style = "countryboss_4", 
			 text = Lang.getImgText("txt_boss_lvduo01") ,
		})

		
		local UIHelper  = require("yoka.utils.UIHelper")
		local image2 = UIHelper.seekNodeByName(self._commonBtnRight,"Image_18_0")

		UIHelper.swapWithLabel(image2,{ 
			 style = "countryboss_3", 
			 text = Lang.getImgText("txt_boss_taofa02") ,
		})
		...
		--self._textOverTime:setPositionX(self._textOverTime:getPositionX()+21)
		if Lang.checkLang(Lang.JA) then
			self._textBossOpenTime:setPositionX(self._textBossOpenTime:getPositionX() + 60)
			self._textBossOpenTime:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
		end
	end
end

-- i18n change lable
function WorldBossView:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local size = self._textTimeDesc:getContentSize()
		self._textOverTime:setPositionX(self._textTimeDesc:getPositionX()+size.width)
	end

end

function WorldBossView:_awardPosByI18n()
