...
function GameScene:ctor(name, view)
    ...
    --i18n ui4
    self._topLayer = cc.Node:create()
    self._topLayer:setContentSize(G_ResolutionManager:getDesignCCSize())
    self._topLayer:setPosition(display.center)
    self._topLayer:setAnchorPoint(0.5,0.5)
	self:addChild(self._topLayer,GameScene.MAX_LEVEL)
end
...
function GameScene:onEnter()
    ...
    if Lang.checkUI("ui4") then
        self._signalVipUpgrade = G_SignalManager:add(SignalConst.EVENT_VIP_UPGRADE, handler(self, self._onEventVipUpgrade))
    end
...
function GameScene:onExit()
    ...
    self._signalRechargeNotice = nil
     ...
     --i18n ui4
     if Lang.checkUI("ui4") then
        self._signalVipUpgrade:remove()
        self._signalVipUpgrade = nil
    end
...
function GameScene:getTopLayer()
...
--vip 经验显示-- i18n ui4 新增参数 oldLevel、newLevel
function GameScene:_onEventVipExpChange(event, addExp ,oldLevel,newLevel)
    if self._disablePopVipChange then
        return
    end
   
    self:playAddVipTips(addExp,oldLevel,newLevel)
end

function GameScene:playAddVipTips(addExp ,oldLevel,newLevel)
    if Lang.checkUI("ui4") then
        local function effectFunction(effect)
            if effect == "smoving_qinmidu_txt" then
                local params = {
                    text = "+"..tostring(addExp),
                    fontSize = 30,
                ...
                --G_EffectGfxMgr:applySingleGfx(label, effect, nil)
                return label
            end
        ...
        local function eventFunction(event)
            if event == "1" then
                G_SignalManager:dispatch(SignalConst.EVENT_VIP_ADD_EXP_BALL,addExp,oldLevel,newLevel)
            elseif event == "finish" then
                if newLevel > oldLevel then
                    if not self._disableVipUpgradeEffect then
                        local effect = G_EffectGfxMgr:createPlayGfx(self._topLayer, "effect_qinmidu_taohuaup", nil, true , nil)
        ...
        local effect = G_EffectGfxMgr:createPlayMovingGfx(self._topLayer, "moving_qinmidu_shuzhi", 
...
--i18n ui4
function GameScene:setVipUpgradeEffectDisable(enable)
	if enable then
		self._disableVipUpgradeEffect = true
	else
		self._disableVipUpgradeEffect = nil
	end
end


--i18n ui4
function GameScene:_onEventVipUpgrade(event,lastVip,currVip)

end


--i18n ui4
function GameScene:setRechargeNoticeDisable(enable)
	if enable then
		self._disableRechargeNotice = true
	else
		self._disableRechargeNotice = nil
	end
end

function GameScene:showNoticeReward(message)
    local TypeConvertHelper = require("app.utils.TypeConvertHelper")
    local DataConst = require("app.const.DataConst")
...
function GameScene:_onEventRechargeNotice(event,id,message)
    if self._disableRechargeNotice then
        return
    end
    self:showNoticeReward(message)
end
