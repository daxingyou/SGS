...
def main(cfgFile=None):
    ...
    if args.config == "trunk":
        dirProjectRes = os.path.join(dirClient, "trunk")
    else:
        dirProjectRes = os.path.join(dirClient, "tags", args.config)

    dirOut = args.out
    if args.develop != None and args.develop.lower() == "true":
        dirProjectRes = os.path.join(dirClient, "dev", args.config)
        dirOut = os.path.join(args.out, "dev")
    ...
    dirPublish = os.path.join(dirOut, args.config)
