...
--播放闪烁动画
function UIActionHelper.playBlinkEffect(node)
	if not node then return end
	...
	local action1 = cc.FadeTo:create(2.0, 50)
	local action2 = cc.FadeTo:create(2.0, 255)
	local seq1 = cc.Sequence:create(action1, action2)
	local action3 = cc.ScaleTo:create(2.0, 0.9)
	local action4 = cc.ScaleTo:create(2.0, 1.0)
