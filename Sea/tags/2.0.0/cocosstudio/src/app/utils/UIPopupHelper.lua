...
function UIPopupHelper.popupFixShopBuyItem(shopItemData, shopFucConst)
    shopFucConst = shopFucConst or 0
	local LogicCheckHelper = require("app.utils.LogicCheckHelper")
	...
	if shopItemData:getConfigType() == "fix" then
		...
		local function callBackFunction(itemId,buyCount)
			--是否能购买检测
			if LogicCheckHelper.shopFixBuyCheck(shopItemData, buyCount, true) == false then
				return
			end
...
function UIPopupHelper.popupHelpInfo(funtionId)
	local popupHelpInfo = require("app.ui.PopupHelpInfo").new()
	popupHelpInfo:updateByFunctionId(funtionId)
...
--断线对话框
function UIPopupHelper.showOfflineDialog(content, isHideReconnect)
	isHideReconnect = isHideReconnect or true
	local okStr = Lang.get("common_relogin")
	local cancelStr = Lang.get("common_reconnect")
	local function okCallback()
