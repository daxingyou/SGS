...
addFunctions(import(".data.SmallAmountRechargeDataHelper"))--i18n ja
...
function UserDataHelper.getHeroChangeData(heroId)
	--i18n config change
	if attribute == nil then attribute = require("app.config.attribute") end
...
function UserDataHelper.getSameCardCount(type, value, filterId)
	...
	elseif type == TypeConvertHelper.TYPE_PET then
		count = #G_UserData:getPet():getSameCardCountWithBaseId(value)
...
function UserDataHelper.isEnoughBagMergeLevel()
	...
	return FunctionCheck.funcIsOpened(FunctionConst.FUNC_BAG_TAB_LEVEL_LIMIT)
end
