...
function TreasureDataHelper._createYokeBasicInfo(info)
	...
	for i = 1, len do
		...
		if Lang.checkLang(Lang.TW) and heroInfo.is_show == 0 and heroInfo.type == 2 then
		else
...
function TreasureDataHelper.isPromptTreasureLimitWithCostKey(treasureData, key)
	if require("app.utils.logic.FunctionCheck").funcIsOpened(FunctionConst.FUNC_TREASURE_TRAIN_TYPE3) == false then
		return false
	end
...
function TreasureDataHelper.getTreasureTransformTarList(filterIds, tempData)
	...
	for i = 1, len do
		...
		if Lang.checkLang(Lang.CN) or (not Lang.checkLang(Lang.CN) and info.is_work == 1) then
