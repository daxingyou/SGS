...
function HeroDataHelper.getHeroRankConfig(id, rank, limit)
	local config = require("app.config.hero_rank").get(id, rank, limit)
...
function HeroDataHelper.getHeroLimitCostConfig(limitLevel)
	local config = require("app.config.hero_limit_cost").get(limitLevel)
...
function HeroDataHelper.getBreakAttr(heroUnitData, addRank)
	...
	local result = HeroDataHelper.getBreakAttrWithBaseIdAndRank(baseId, rank, limitLevel)
	return result
end

function HeroDataHelper.getBreakAttrWithBaseIdAndRank(baseId, rank, limitLevel)
	local atk = 0
	local pdef = 0
	local mdef = 0
	local hp = 0
	...
	for i = 1, rank do
		local heroRankConfig = HeroDataHelper.getHeroRankConfig(baseId, i, limitLevel)
...
--获取界限突破对应的属性值
function HeroDataHelper.getLimitAttr(heroUnitData, tempLimitLevel)
	local result = {}
	...
	for i = 0, limitLevel do
		local attrInfo = HeroDataHelper.getLimitSingleAttr(heroBaseId, i)
...
function HeroDataHelper.getLimitSingleAttr(heroBaseId, limitLevel)
	local result = {}
	local config = HeroDataHelper.getHeroLimitSizeConfig(heroBaseId, limitLevel)
	if config == nil then
		return result
	end
	for j = 1, 4 do
...
function HeroDataHelper.getAllLimitCost(heroUnitData)
	...
	for key = HeroConst.HERO_LIMIT_COST_KEY_1, HeroConst.HERO_LIMIT_COST_KEY_4 do
		if key == HeroConst.HERO_LIMIT_COST_KEY_1 then
			exp = exp + heroUnitData:getLimitCostCountWithKey(key)
		else
			local info = HeroDataHelper.getHeroLimitCostConfig(0)
			local type = info["type_" .. key]
			local value = info["value_" .. key]
			local size = heroUnitData:getLimitCostCountWithKey(key)
			if size > 0 then
	...
	local limitLevel = heroUnitData:getLimit_level()
	for i = 1, limitLevel do
		local config = HeroDataHelper.getHeroLimitCostConfig(i - 1)
...
function HeroDataHelper.getLimitSingleCost(limitLevel)
	local level = limitLevel - 1
	local result = {}
	local config = HeroDataHelper.getHeroLimitCostConfig(level)
	for i = 1, 4 do
		local unit = {
			name = config["name_" .. i],
			type = config["type_" .. i],
			...
			size = config["size_" .. i],
			consume = config["consume_" .. i]
		}
		table.insert(result, unit)
	end

	local unitEx = {
		name = config.break_name,
		type = config.break_type,
		value = config.break_value,
		size = config.break_size,
		consume = 0
	}
	table.insert(result, unitEx)

	return result
end

--是否达到了rank要求，想升级到界限等级limitLevel
function HeroDataHelper.isReachLimitRank(limitLevel, curRank)
	local config = HeroDataHelper.getHeroLimitCostConfig(limitLevel)
...
function HeroDataHelper.getNeedLimitLevelWithRank(rank)
...
function HeroDataHelper.getSilkbagAttr(heroUnitData)
	...
	for i, silkbagId in ipairs(silkbagIds) do
		...
		if isEffective then
			local attr = SilkbagDataHelper.getAttrWithId(baseId)
...
--获取宝物属性
function HeroDataHelper.getTreasureAttr(pos)
	local result = {}
...
function HeroDataHelper.getTalentPower(heroUnitData, tempRank)
	...
	local power = 0
	for i = 0, rank do
		local info = HeroDataHelper.getHeroRankConfig(heroBaseId, i, limitLevel)
...
function HeroDataHelper.getTalentAttr(heroUnitData, rank, notTalent)
	...
	if heroUnitData:isInBattle() then
		...
		for i, id in ipairs(heroIds) do
			...
			for j = 1, rLv do
				local info = HeroDataHelper.getHeroRankConfig(baseId, j, limitLevel)
				if info then
...
function HeroDataHelper.getYokeOverviewInfo(heroDatas)
	...
	for i, data in ipairs(heroDatas) do
		if i ~= 1 then --主角的信息不显示
			...
			table.insert(
				info,
				{baseId = baseId, activatedCount = activatedCount, totalCount = totalCount, limitLevel = limitLevel}
			)
		end
	end
...
function HeroDataHelper.isPromptHeroLimitWithCostKey(heroUnitData, key)
	local isCanLimitBreak = heroUnitData:isCanLimitBreak()
	if not isCanLimitBreak then
		return false
	end

	if heroUnitData:isDidLimitToRed() then
		return false
	end

	local limitLevel = heroUnitData:getLimit_level()
	local curRank = heroUnitData:getRank_lv()
	local isReach = HeroDataHelper.isReachLimitRank(limitLevel, curRank)
	if not isReach then
		return false
	end

	local info = HeroDataHelper.getHeroLimitCostConfig(limitLevel)
	local curCount = heroUnitData:getLimitCostCountWithKey(key)
...
function HeroDataHelper.getHeroTransformTarList(filterIds, tempData)
	...
	for i = 1, len do
		...
		if type == 2 and color == filterColor and not isInFilter(unit) and checkLimitCondition(isDidLimit, unit) then --同阵营，同品质色，过滤Id，判断界限突破的条件
...
function HeroDataHelper.getSkillIdsWithHeroData(unitData)
	...
	local skillIds = HeroDataHelper.getSkillIdsWithBaseIdAndRank(baseId, rank, limitLevel)
	return skillIds
end

function HeroDataHelper.getSkillIdsWithBaseIdAndRank(baseId, rank, limitLevel)
	local result = {}
	local heroRankConfig = HeroDataHelper.getHeroRankConfig(baseId, rank, limitLevel)
	for i = 1, 3 do
