...
function PetDataHelper.getPetStarCostConfig(star, templet)
	local config = require("app.config.pet_star_cost").get(star, templet)
...
function PetDataHelper.getPetBreakMaterials(petUnitData)
	...
	local config = PetDataHelper.getPetStarCostConfig(starLevel, templet)
...
function PetDataHelper.getPetBreakLimitLevel(petUnitData)
	...
	local templet = petUnitData:getLvUpCost()
	local config = PetDataHelper.getPetStarCostConfig(rankLevel, templet)
	return config.lv
end

function PetDataHelper.getPetBreakCostWithStar(rank, templet)
	local rankCostConfig = PetDataHelper.getPetStarCostConfig(rank, templet)
...
function PetDataHelper.getPetBreakMaxLevel(petUnitData)
	...
	for i = 1, rankMax do
		local info = PetDataHelper.getPetStarCostConfig(i, templet)
...
function PetDataHelper.isPromptPetBreak(petUnitData)
	...
	for i, info in ipairs(costInfo) do
		if info.type == TypeConvertHelper.TYPE_PET then --同名卡
			local sameCardNum = #G_UserData:getPet():getSameCardCountWithBaseId(petUnitData:getBase_id())
...
function PetDataHelper.getAllPetBreakCost(unitData)
	...
	for i = 0, rank - 1 do
		local rankInfo = PetDataHelper.getPetStarCostConfig(i, rTemplet)
		local cardNum = rankInfo.card
		cardNum = cardNum + rankInfo.potential_card
		if cardNum > 0 then
...
function PetDataHelper.getAllPetLimitUpCost(unitData)
	...
	if config.color == 6 then --红色神兽
		...
		local costInfo = PetTrainHelper.getLimitCostInfo(petUnitData)
		RecoveryDataHelper.formatRecoveryCost(
			result,
			costInfo["type_" .. LimitCostConst.LIMIT_COST_KEY_2],
			costInfo["value_" .. LimitCostConst.LIMIT_COST_KEY_2],
			costInfo["size_" .. LimitCostConst.LIMIT_COST_KEY_2]
		)
		RecoveryDataHelper.formatRecoveryCost(
			result,
