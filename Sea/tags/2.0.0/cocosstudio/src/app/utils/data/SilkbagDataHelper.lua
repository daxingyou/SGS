...
function SilkbagDataHelper.getAttrWithId(id)
	...
	for i, heroId in ipairs(heroIds) do
		if heroId > 0 then
			...
			if Lang.checkLang(Lang.KR) then
				if unitData:getQuality() >= 4 or unitData:isLeader() then
...
function SilkbagDataHelper.isEffectiveSilkBagToHero(silkbagId, heroBaseId, heroRank, isInstrumentMaxLevel, heroLimit)
	...
	for i, heroId in ipairs(heroIds) do
		if heroId == heroBaseId then
			...
			local isEffective, limitRank, isEffectiveForInstrument, limitLevel = SilkbagDataHelper.isEffectiveWithHeroRankAndInstrument(info, heroRank, isInstrumentMaxLevel, heroLimit)
...
function SilkbagDataHelper.isEffectiveWithHeroRankAndInstrument(info, heroRank, isInstrumentMaxLevel, heroLimit)
	...
	local isEffective = isRankEffective or isInstrumentEffective or isLimitEffective
	return isEffective, limitRank, isEffectiveForInstrument, limitLevel
end
