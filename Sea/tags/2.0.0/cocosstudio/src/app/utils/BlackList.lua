...
function BlackList.filterBlack(text)
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local turn_character = require("app.config.turn_character")
		for i = 1, turn_character.length() do
			local data = turn_character.indexOf(i)
			text = string.gsub(text,data.before_character,data.after_character)
		end
	end
