...
function GuildHallBaseInfoNode:onCreate()
	...
	if not Lang.checkLang(Lang.CN) then
		 self._buttonModifyAnnouncement:setString(Lang.getImgText("guild_click_modify"))
	end
...
function GuildHallBaseInfoNode:_onButtonModifyAnnouncement()
	...
	if  Lang.checkLang(Lang.CN) then
		popup = PopupGuildAnnouncement.new(handler(self, self._onSaveAnnouncement))
	else
		local UIHelper  = require("yoka.utils.UIHelper")
		popup = PopupGuildAnnouncement.new(handler(self, self._onSaveAnnouncement),
			UIHelper.getInputLimitedConfig().legion_notice_length)
	end
...
-- i18n pos lable
function GuildHallBaseInfoNode:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")

		local panelLeft = UIHelper.seekNodeByName(self,"PanelLeft")

		local text11 = UIHelper.seekNodeByName(panelLeft,"Text_11")
		local text19 = UIHelper.seekNodeByName(panelLeft,"Text_19")
		local text113 = UIHelper.seekNodeByName(panelLeft,"Text_11_3")
		local text1130 = UIHelper.seekNodeByName(panelLeft,"Text_11_3_0")

		


		text11:setPositionX(text11:getPositionX()-12)
		text19:setPositionX(text11:getPositionX())
		text113:setPositionX(text11:getPositionX())
		text1130:setPositionX(text11:getPositionX())
		
		
		self._textLeaderName:setPositionX(text11:getPositionX()+text11:getContentSize().width+3)
		self._textLevel:setPositionX(text19:getPositionX()+text19:getContentSize().width+ 3)
		self._textMemberNumber:setPositionX(text113:getPositionX()+text113:getContentSize().width+3)
		self._textRank:setPositionX(text1130:getPositionX()+text1130:getContentSize().width+3)
		
		self._textName:setFontSize(self._textName:getFontSize()-8)
		self._textName:setPositionX(self._textName:getPositionX()-13)

		self._textLeaderName:setFontSize(self._textLeaderName:getFontSize()-2)

		self._textAnnouncement:setFontSize(self._textAnnouncement:getFontSize()-2)
	end
	if Lang.checkLang(Lang.TH) then
		local size = self._textAnnouncement:getContentSize()
		self._textAnnouncement:setContentSize(cc.size(size.width,size.height+20))
		self._textAnnouncement:getVirtualRenderer():setLineSpacing(5)
	end
end
