...
-- i18n pos lable
function SiegeChallengeInfo:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
    
     

        local textToken = UIHelper.seekNodeByName(self,"TextToken")
        local textTime = UIHelper.seekNodeByName(self,"TextTime")
        local textHp = UIHelper.seekNodeByName(self,"TextHp")
        textToken:setAnchorPoint(cc.p(1,0.5))
        textTime:setAnchorPoint(cc.p(1,0.5))
        textHp:setAnchorPoint(cc.p(1,0.5))


        local x = self._hpPercentBG:getPositionX()-self._hpPercentBG:getContentSize().width * 0.5-6
        textToken:setPositionX(x)
        textTime:setPositionX(x)
        textHp:setPositionX(x)

        self._textCountdown:setPositionX(
            textTime:getPositionX()+3
       )
        local textActivityTime = UIHelper.seekNodeByName(self,"TextActivityTime")
        if Lang.checkLang(Lang.EN) then
            textActivityTime:setPositionX(textActivityTime:getPositionX()-100)
        elseif Lang.checkLang(Lang.TH) then
            textActivityTime:setPositionX(textActivityTime:getPositionX()-150)
        end
        if Lang.checkChannel(Lang.CHANNEL_SEA ) then
            textToken:setPositionX(textToken:getPositionX()-50)
            self._tokenInfo:setPositionX(self._tokenInfo:getPositionX()-50)
            self._textRecoverTime:setPositionX(self._textRecoverTime:getPositionX()-50)
        end
