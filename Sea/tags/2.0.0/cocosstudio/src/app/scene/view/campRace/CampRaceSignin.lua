...
function CampRaceSignin:_updateWinner(index, winnerData)
	...
	if winnerData then
		...
		avatar:updateUI(winnerData:getCoverId(), nil, nil, winnerData:getLimitLevel())
...
-- i18n change lable
function CampRaceSignin:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local image = UIHelper.seekNodeByName(self._nodeWinner, "ImageTop","Image_1")
		UIHelper.swapWithLabel(image,{
			 style = "camp_race_5",
			 text = Lang.getImgText("txt_camp_win01")
		})
	
		
		local UIHelper  = require("yoka.utils.UIHelper")	
        self._imageSignin = UIHelper.swapSignImage(self._imageSignin,
		{ 
			 style = "signet_6", 
			 text = Lang.getImgText("img_yibaoming01") ,
			 anchorPoint = cc.p(0.5,0.5),
			 rotation = -10,
		},Path.getTextSignet("img_common_lv"))


		local imageTitle = UIHelper.seekNodeByName(self, "ImageTitle")
		imageTitle:ignoreContentAdaptWithSize(true)
		
		if  Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
			imageTitle:setPositionX(imageTitle:getPositionX()+80)
		end
	end
	
	if Lang.checkLang(Lang.TH) then
		self._textDetail:getVirtualRenderer():setLineSpacing(6)
	end
	
end

-- i18n pos lable
function CampRaceSignin:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local imageDown = UIHelper.seekNodeByName(self._nodeWinner, "ImageDown")
		local text88 = imageDown:getChildByName("Text_88")
		local size = imageDown:getContentSize()
		local size1 = text88:getContentSize()
		local size2 = self._textWinCountDown:getContentSize()
		text88:setPositionX(size.width * 0.5 - size2.width * 0.5)
		self._textWinCountDown:setPositionX(size.width * 0.5 + size1.width * 0.5)
	end
end

-- i18n pos lable
function CampRaceSignin:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		for i = 1, 4 do
			local textName = self["_nodeWinner"..i]:getChildByName("TextName")
			textName:setFontSize(textName:getFontSize()-4)
		end
	end
	if Lang.checkLang(Lang.TH) then
		self._btnBest8:setPositionX(self._btnBest8:getPositionX()-20)
	end
end
