...
function StageView:ctor(chapterId, stageId, showBoss)       --如果传入stageid则直接弹出详细面板,如果有bossid直接弹出boss详细面板, 不填chapterid则打开现在能打的最后一章
...
function StageView:onCreate()
    ...
    local chapterName = self._chapterInfo.chapter.." "..self._chapterInfo.name
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
        chapterName = self._chapterInfo.chapter
    end
	...
	if self._topBar then
        ...
        if Lang.checkLang(Lang.CN) or Lang.checkLang(Lang.ZH) then
            self._topBar:setTitle(chapterName, 40, Colors.DARK_BG_THREE, Colors.DARK_BG_OUTLINE)
        elseif Lang.checkLatinLanguage() then
            self._topBar:setTitle(chapterName, 36, Colors.DARK_BG_THREE, Colors.DARK_BG_OUTLINE)
        end
...
function StageView:_openChapterBoss()
    ...
    if bossState == 0 then
        ...
        local popupBossDetail = require("app.scene.view.stage.PopupBossDetail").new(self._chapterInfo.type,self._chapterInfo.id, bossData, showBoss)
...
function StageView:_showFirstReward()
    ...
    self:runAction(cc.Sequence:create(
        ...
        cc.CallFunc:create(function()
            local popupGetRewards = require("app.ui.PopupGetRewards").new()
            popupGetRewards:show(self._firstReward)
...
function StageView:_dealPosByI18n()
    ...
    if Lang.checkLang(Lang.EN) then
        ...
        self._textFamousStory:setTextAreaSize(cc.size(310 + 34, 0))
       ...
       --renderLabel:setMaxLineWidth(310+34)
        ...
        labelSize.height = math.max(76,labelSize.height)
        ...
        local scrollSize = cc.size(310 + 34, 76)
