...
-- i18n change lable
	self:_dealByI18n()	self:_initHeroListData()
...
function PopupHeroView:_initBanPickView()
	...
	-- Adaptto scrollview
	if self._isInBanView then
		self._listView:setPositionY(SeasonSportConst.SEASON_POPHEROVIEW_HEIGHTNAN)
		...
		self._panelContent:setContentSize(cc.size(self._panelContent:getContentSize().width, 
													SeasonSportConst.SEASON_POPHEROVIEW_HEIGHTNAN))
		self._listView:setContentSize(cc.size(self._listView:getContentSize().width, 
													SeasonSportConst.SEASON_POPHEROVIEW_HEIGHTNAN))
	end	
end
