...
function CustomActivityEquipmentView:_onClickButton1()
	...
	if self:_checkCost(CustomActivityConst.EQUIP_DRAW_TYPE_1) == false then
		return
	end
...
function CustomActivityEquipmentView:_onClickButton2()
	...
	if self:_checkCost(CustomActivityConst.EQUIP_DRAW_TYPE_2) == false then
		return
	end
...
function CustomActivityEquipmentView:_checkCost(drawType)
	...
	local max = self._configInfo.toplimit
	if totalCount >= max then
		G_Prompt:showTip(Lang.get("customactivity_equip_cost_reach_limit"))
	else
