...
function PopupFamousDetail:_createAnim()
    local function effectFunction(effect)
        ...
        elseif effect == "mingzi" then
            if Lang.checkHorizontal() then
                return self:_createNameHorizontal()
            end
    ...
    if Lang.checkHorizontal() then
        G_EffectGfxMgr:createPlayMovingGfx( self._nodeEffect, "moving_mingjiangfuben_h", effectFunction, eventFunc, false )
    else
...
function PopupFamousDetail:_createName()
	...
	elseif not Lang.checkLang(Lang.CN) then
        ...
        UIHelper.dealVTextWidget(label,self._configData.name )
        label:setMaxLineWidth(180)
        local node = cc.Node:create()
        label:setPositionY(-7)
        node:addChild(label)
        return node
	end
...
function PopupFamousDetail:_createReward(index, rewards)
    ...
    if Lang.checkLang(Lang.CN) then
		...
		local UIHelper  = require("yoka.utils.UIHelper")
		imgTitle = UIHelper.swapWithLabel(imgTitle,{
			 style = "text_essence_1",
			 text = Lang.getImgText("img_essence0"..tostring(index)),
		})
	end
...
function PopupFamousDetail:_createRewardItem(rewards)
    ...
    if Lang.checkLang(Lang.CN) then
		...
		imgTitle = UIHelper.swapWithLabel(imgTitle,{
			 style = "text_essence_1",
			 text = Lang.getImgText("img_essence03"),
		})
	end
...
function PopupFamousDetail:_createFightBtn()
    if self._data:getExecute_count() >= self._configData.challenge_num then
        ...
        if not Lang.checkLang(Lang.CN) then
            local UIHelper  = require("yoka.utils.UIHelper")	
            local image = UIHelper.createSignImage(
                { 
                    style = "signet_10", 
                    text = Lang.getImgText("img_yitiaozhan") ,
                    anchorPoint = cc.p(0.5,0.5),
                    rotation = -10,
                },Path.getTextSignet("img_common_lv2"))
            return  image
        else
        ...
        if not Lang.checkLang(Lang.CN) then
            local UIHelper  = require("yoka.utils.UIHelper")
            local text = UIHelper.seekNodeByName(fightBtn,"Text")
            imgSward:setPositionX(imgSward:getPositionX())
...
--i18n
function PopupFamousDetail:_createNameHorizontal()
    local path = Path.getCommonImage("img_famousdi_h")
    local bg = display.newSprite(path)
    local size = bg:getContentSize()
    bg:setPosition(20,30)
    local UIHelper  = require("yoka.utils.UIHelper")
    local label = UIHelper.createLabel({
		text = self._configData.name,
        size = 20,
    ...
    if Lang.checkLang(Lang.EN) then
        label:getVirtualRenderer():setMaxLineWidth(140)
        label:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        local labelSize = label:getVirtualRenderer():getContentSize()
        local newHeight = labelSize.height + 8
        if newHeight > size.height then
            bg:setContentSize(cc.size(size.width,newHeight))
            label:setPositionY(newHeight*0.5)
