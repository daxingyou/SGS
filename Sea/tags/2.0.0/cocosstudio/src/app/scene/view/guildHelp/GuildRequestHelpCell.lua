...
-- i18n change lable
function GuildRequestHelpCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._imageReach = UIHelper.swapSignImage(self._imageReach,
		{ 
			 style = "sign_1", 
			 text = Lang.getImgText("img_reach") ,
...
-- i18n pos lable
function GuildRequestHelpCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
	
        self._textHelping:setFontSize(
			self._textHelping:getFontSize()-2
		)

	
	end
	if  Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		self._textHelping:setPositionY(self._textHelping:getPositionY()+45)
		self._textCanReceiveCountTitle:setPositionY(self._textCanReceiveCountTitle:getPositionY()+45)
		self._textCanReceiveCount:setPositionY(self._textCanReceiveCount:getPositionY()+45)
	end
end
