    ...
    local function createEventIcon(blockData)
        ...
        if not Lang.checkLang(Lang.CN) then
              local label = UIHelper.createLabel({
                style = "explore_2",
                text = Lang.getImgText("txt_"..eventIconInfo.config.res_id2),
                position = cc.p(blockData.posX, blockData.posY + 5),
...
function ExploreMapLayer:_addPassBoxToBlock(isFirstPass)
    ...
    if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		endBoxIconTitle = UIHelper.createLabel(
            {
			 style = "explore_3",
			 text = Lang.getImgText(isFirstPass and "txt_stbx" or "txt_tgbx"),
             anchorPoint = cc.p(0.5,0.5)
		    }
        )
    else
...
function ExploreMapLayer:_createActor(  )
    ...
    if not Lang.checkLang(Lang.CN) then
        local function _effectFunction(_effect)
            if _effect == "routine_word_zidongyoulizhong_zi" then
                local UIHelper = require("yoka.utils.UIHelper")
                local TypeConst = require("app.i18n.utils.TypeConst")
                local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_zidongyoulizhong"),style="effect_text_38",styleType=TypeConst.EFFECT})

                if Lang.checkLang(Lang.KR) then
                    local node = cc.Node:create()
                    subLabel:setPositionX(60)
                    node:addChild(subLabel)
                    return node
                end
                if Lang.checkChannel(Lang.CHANNEL_SEA) then
                    local node = cc.Node:create()
                    subLabel:setAnchorPoint(1,0.5)
                    subLabel:setPositionX(130)
                    node:addChild(subLabel)
                    return node
                end
                return subLabel
