...
function GuildWarBattleView:_refreshCityName() 
	...
	if Lang.checkLang(Lang.TW) then
        self._topbarBase:setTitle(config.name,32)
    end
end
...
function GuildWarBattleView:_showCDFinishEffect(content)
    ...
    local function effectFunction(effect)
        ...
        if not Lang.checkLang(Lang.CN) and effect == "routine_word_kuang_9" then 
            ...
            local subLabel = UIHelper.createLabel({text=content,style="effect_text_20",styleType=TypeConst.EFFECT})
            return subLabel
        end
    end
...
-- i18n change lable
function GuildWarBattleView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        

        local image =  UIHelper.seekNodeByName(self._btnChangeCity,"Image_1")
	    local label = UIHelper.swapWithLabel(image,{
			 style = "icon_txt_3",
			 text = Lang.getImgText("txt_main_enter6_battlefield") ,
			 offsetY = -39,
		})
...
-- i18n pos lable
function GuildWarBattleView:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local nodeMiddleTop = UIHelper.seekNodeByName(self._panelDesign,"Node_MiddleTop")

        local size1 = self._textTime:getContentSize()
        local size2 = self._textTimeTitle:getContentSize()

		self._textTime:setPositionX(size2.width*0.5-size1.width*0.5)
        self._textTimeTitle:setPositionX(-size1.width * 0.5)
        
        if Lang.checkLang(Lang.EN) then
            local image123 = UIHelper.seekNodeByName(nodeMiddleTop,"Image_123")
            local size = image123:getContentSize()
            image123:setContentSize(cc.size(size1.width+size2.width+4,size.height))
        end
	end
end
