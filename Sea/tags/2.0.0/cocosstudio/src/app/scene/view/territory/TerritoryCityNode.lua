...
function TerritoryCityNode:_stateCountDown()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		UIHelper.setSignStyle( self._imageHeroState,{ 
				style = "signet_12", 
				text = Lang.getImgText("txt_xunluozhong01") ,
			},Path.getTextSignet("txt_xunluozhong01"))
	else
		--巡逻中
	...
	if riotEvent then
		...
		if eventState ~= TerritoryConst.RIOT_HELP then
			...
			if not Lang.checkLang(Lang.CN) then
				...
				UIHelper.setSignStyle( self._imageHeroState,{ 
						style = "signet_11", 
						text = Lang.getImgText("txt_yiqiuzhu01") ,
					},Path.getTextSignet("txt_yiqiuzhu01"))
			else
...
-- i18n change lable
function TerritoryCityNode:_createLabelByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._imageHeroState = UIHelper.swapSignImage(self._imageHeroState,
		{ 
			 style = "signet_11", 
			 text = Lang.getImgText("txt_xunluozhong01") ,
	...
	if Lang.checkHorizontal() then
		local img = ccui.Helper:seekNodeByName(self, "Image_216_0")
		img:setScale9Enabled(true)
		img:setCapInsets(cc.rect(40,10,55,1))
		img:loadTexture(Path.getCommonImage("img_modian_h"))
		local posX,posY = self._btnAdd:getPosition()
		img:setPosition(posX+10,posY+25)
		self._textCityName:setAnchorPoint(0.5,0.5)
		self._textCityName:setPosition(posX,posY+25)
	...
	elseif not Lang.checkLang(Lang.CN) then
		...
		self._textCityName:setContentSize(cc.size(size.width*3,size.height))
		self._textCityName:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
	end
	-- 城池名称上移动
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local img = ccui.Helper:seekNodeByName(self, "Image_216_0")
		img:setPositionY(img:getPositionY()+30)
		self._textCityName:setPositionY(self._textCityName:getPositionY()+25)
	end

end

-- i18n
function TerritoryCityNode:_dealHorizontal()
	if Lang.checkHorizontal() then
		local img = ccui.Helper:seekNodeByName(self, "Image_216_0")
		local width = self._textCityName:getContentSize().width + 80
		img:setContentSize(cc.size(width,32))
	end
end
