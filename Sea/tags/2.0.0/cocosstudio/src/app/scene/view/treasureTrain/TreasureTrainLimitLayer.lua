...
function TreasureTrainLimitLayer:_initView()
	...
	self._buttonHelp:updateUI(FunctionConst.FUNC_TREASURE_TRAIN_TYPE3)
...
function TreasureTrainLimitLayer:_updateBaseInfo()
	...
	if  Lang.checkLang(Lang.CN) then
		...
		self._textLevel1:setString(TreasureConst.getAddStrLevelByLimit())
		self._textLevel2:setString(TreasureConst.getAddRefineLevelByLimit())
	else
		local label1 = ccui.RichText:createByI18n()
		label1:setAnchorPoint(cc.p(0.5,0.5))
		label1:setRichTextWithJson( Lang.getImgText("txt_limit_06c_1",{name = nameStr}))
		label1:setPositionY(22)
		local label2 = ccui.RichText:createByI18n()
		label2:setAnchorPoint(cc.p(0.5,0.5))
		label2:setRichTextWithJson( Lang.getImgText("txt_limit_06c_2",{level = TreasureConst.getAddStrLevelByLimit()}))
	
	
		local label3 = ccui.RichText:createByI18n()
		label3:setAnchorPoint(cc.p(0.5,0.5))
		label3:setRichTextWithJson( Lang.getImgText("txt_limit_06c_3",{level = TreasureConst.getAddRefineLevelByLimit()}))
		label3:setPositionY(-22)
		...
		if Lang.checkLang(Lang.TH) then
			...
			label3:setPositionY(label3:getPositionY()-20)
		end
	end	
...
function TreasureTrainLimitLayer:_updateSingeCost(costKey)
	...
	local isShow = TreasureDataHelper.isPromptTreasureLimitWithCostKey(self._treasureUnitData, costKey)
...
function TreasureTrainLimitLayer:_updateBtnAndSilverState()
	if self._treasureUnitData:getLimit_cost() >= TreasureConst.TREASURE_LIMIT_RED_LEVEL then --满级不显示
