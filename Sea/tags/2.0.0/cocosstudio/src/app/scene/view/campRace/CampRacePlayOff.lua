...
function CampRacePlayOff:_playCountDownEffect(countDownTime)
    ...
    elseif index == 0 then
		...
		if  not Lang.checkLang(Lang.CN) then
			...
			local effect = G_EffectGfxMgr:createPlayMovingGfx(self._nodeCount, "moving_effect_jingjijishi_suoding", _effectFunction, nil , true)
        else
...
-- i18n change lable
function CampRacePlayOff:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
...
-- i18n  lable
function CampRacePlayOff:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
         self._textTip:setString(Lang.getImgText("camp_race_drag_hint"))
         self._textChampionTip:setString(Lang.getImgText("textChampionTip"))
    end
end
