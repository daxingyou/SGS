...
-- i18n pos lable
function WeeklyGiftPkgView:_updatePosI18n()
	if  Lang.checkLang(Lang.KR) then
		...
		self._textTime:setPositionX(textActTimeLabel:getPositionX()+textActTimeLabel:getContentSize().width+4)
	elseif not Lang.checkLang(Lang.CN) then
        ...
        UIHelper.alignCenter(image36,{image37,textActTimeLabel,self._textTime})
	end
end

-- i18n change lable
function WeeklyGiftPkgView:_dealPosI18n()
	if not Lang.checkLang(Lang.CN) then
		self._countDownNode:setPositionY(self._countDownNode:getPositionY()-10)
	end
	if Lang.checkLang(Lang.TH) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image36 = UIHelper.seekNodeByName(self._countDownNode,"Image_36")
		local size = image36:getContentSize()
		image36:setContentSize(cc.size(size.width+80,size.height))
	end
end
