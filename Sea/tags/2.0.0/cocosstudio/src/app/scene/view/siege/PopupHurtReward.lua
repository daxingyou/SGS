...
-- i18n pos lable
function PopupHurtReward:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local textHurt = UIHelper.seekNodeByName(self._popupBG,"TextHurt")
		local size = textHurt:getContentSize()
		self._textHurtNum:setPositionX(textHurt:getPositionX()+ size.width +3)


		local image21 = UIHelper.seekNodeByName(self._popupBG,"Image_21")
		local textReset = UIHelper.seekNodeByName(self._popupBG,"TextReset")
		local imageListBG = UIHelper.seekNodeByName(self._popupBG,"ImageListBG")
		
		textReset:setAnchorPoint(cc.p(1,0.5))
		textReset:setPositionX(imageListBG:getPositionX()+imageListBG:getContentSize().width*0.5-4)
		local size = textReset:getContentSize()
		image21:setPositionX(textReset:getPositionX() - size.width - 10)
	end
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA )  then
		...
		for i = 1, PopupHurtReward.REWARD_COUNT do
			...
			iconText:setPositionX(self["_item"..i]:getPositionX())
		end
		local textHurt = UIHelper.seekNodeByName(self._popupBG,"TextHurt")
		textHurt:setAnchorPoint(0.5,0.5)
		textHurt:setPosition(-205,232)
		self._textHurtNum:setAnchorPoint(0.5,0.5)
		self._textHurtNum:setPosition(-205,208)
	end
end
