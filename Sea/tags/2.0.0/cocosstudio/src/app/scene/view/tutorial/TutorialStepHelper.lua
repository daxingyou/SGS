...
function TutorialStepHelper.showGuideTip(info, tipLayer)
	...
	if string.len(info.comment) > 0 then
		...
		if Lang.checkLang(Lang.TH) then
			local offset = 20
			local rightBg = nodeRight:getSubNodeByName("Image_bk")
			local size = rightBg:getContentSize()
			rightBg:setCapInsets(cc.rect(100,60,1,1))
			rightBg:setContentSize(cc.size(size.width,size.height+offset))
			rightBg:setPositionY(rightBg:getPositionY()-offset/2)
			local leftBg = nodeLeft:getSubNodeByName("Image_bk")
			local size = leftBg:getContentSize()
			leftBg:setCapInsets(cc.rect(100,60,1,1))
			leftBg:setContentSize(cc.size(size.width,size.height+offset))
			leftBg:setPositionY(rightBg:getPositionY()-offset/2)
		end
	end
