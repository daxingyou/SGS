...
function PopupTerritoryRiotInfoCell:_updateName()
	...
	if stateIndex == TerritoryConst.RIOT_TAKEN  then
		...
		if not Lang.checkLang(Lang.CN) then
			...
			UIHelper.setSignStyle( self._imageText,{ 
				style = "signet_8", 
				text = Lang.getImgText("txt_yilingqu01") ,
			},Path.getTextSignet("img_common_lv"))
			self._imageText:setVisible(true)
		else
...
-- i18n change lable
function PopupTerritoryRiotInfoCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then

        local UIHelper  = require("yoka.utils.UIHelper")	
		local image = UIHelper.seekNodeByName(self,"Image_text")
        self._imageText = UIHelper.swapSignImage(image,
		{ 
			 style = "signet_2", 
			 text = Lang.getImgText("txt_yichaoshi01") ,
