...
function MailHelper.updateMailRichContent(mailInfo,mailTitleLable, richtextParent)
    ...
    if richtextParent then
        ...
        if Lang.checkLang(Lang.TH) then
            richtext = UIHelper.createMultiAutoCenterRichText(content, Colors.NORMAL_BG_ONE, 20, 10, 1, 496)
        elseif Lang.checkChannel(Lang.CHANNEL_SEA) then
            richtext = UIHelper.createMultiAutoCenterRichText(content, Colors.NORMAL_BG_ONE, 18, 8, 1, 496)
        else
            richtext = UIHelper.createMultiAutoCenterRichText(content, Colors.NORMAL_BG_ONE, 20, 8, 1, 496)
        end
