...
-- i18n pos 
function CampRaceRankCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._textName:setPositionX(self._textName:getPositionX()-33+5)
        
        if Lang.checkLang(Lang.TW) then
            self._textName:setPositionX(self._panelBase:getContentSize().width * 0.5)
