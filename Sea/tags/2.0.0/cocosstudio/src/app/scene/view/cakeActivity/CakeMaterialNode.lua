...
--档位，计时器间隔
local SHIFT_SPEED = {
	[1] = {0.5, 1, 1}, --1档位，0.5秒执行一次，循环次数达到1次后，上升到2档位，第3参数是一次消耗x个
	[2] = {0.4, 2, 1},
	[3] = {0.3, 3, 10},
	[4] = {0.3, 4, 30}
}
...
function CakeMaterialNode:ctor(target, type, onClick, onStepClick)
	self._target = target
	self._itemId = CakeActivityConst["MATERIAL_ITEM_ID_"..type]
	self._onClick = onClick
	self._onStepClick = onStepClick
	self._onStartCallback = nil --开始点住的回调
	self._onStopCallback = nil --放开点住的回调
	self._itemValue = CakeActivityConst["MATERIAL_VALUE_"..type]
	self._count = 0 --拥有数量
...
function CakeMaterialNode:_initColorAndIcon(type)
	...
	local icon = CakeActivityConst.getMaterialIconWithId(self._itemId)
