...
function GuildDungeonMonsterNode:onCreate()
	...
	if not Lang.checkLang(Lang.CN) then
        ...
        self._starPanel1:ignoreContentAdaptWithSize(true)
		self._starPanel2:ignoreContentAdaptWithSize(true)
		self._starPanel3:ignoreContentAdaptWithSize(true)

		self._starPanel1:setPositionX(self._starPanel1:getPositionX()-3)
		self._starPanel3:setPositionX(self._starPanel3:getPositionX()+3)
	end
...
function GuildDungeonMonsterNode:_createHeroSpine()
	self._panelAvatar:updateUI(self._monsterBattleUser:getPlayer_info().covertId)
...
-- i18n pos lable
function GuildDungeonMonsterNode:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local imageCombatValue = UIHelper.seekNodeByName(self._nodeInfo,"ImageCombatValue")
		imageCombatValue:setScale(0.9)
		self._textPowerValue:setPositionX(self._textPowerValue:getPositionX()-10)
		imageCombatValue:setPositionX(imageCombatValue:getPositionX()-6)
	end
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local size = self._imageNameBG:getContentSize()
		self._imageNameBG:setContentSize(cc.size(230, size.height))
	end
end
