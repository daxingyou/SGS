...
function HeroTrainView:_doLayoutTabIcons()
	...
	for i = 1, 4 do
		...
		if i == 4 then
			local canLimit = curHeroData:isCanLimitBreak()
			...
			if Lang.checkChannel(Lang.CHANNEL_SEA) then
				self._imageRope4:setVisible(false)
			end
			if isOpen then
				showCount = showCount + 1
				local funcLevelInfo = require("app.config.function_level").get(FunctionConst.FUNC_HERO_TRAIN_TYPE4)
				assert(funcLevelInfo, "Invalid function_level can not find funcId " .. FunctionConst.FUNC_HERO_TRAIN_TYPE4)
				table.insert(dynamicTabs, {tab = self._nodeTabIcon4, openLevel = funcLevelInfo.level})
			end
		end
	...
	if dynamicTabs[1] then
		if Lang.checkChannel(Lang.CHANNEL_SEA) then
			dynamicTabs[1].tab:setPositionY(self._nodeTabIcon2:getPositionY()-100)
		else
	...
	if dynamicTabs[2] then
		if Lang.checkChannel(Lang.CHANNEL_SEA) then
			dynamicTabs[2].tab:setPositionY(self._nodeTabIcon2:getPositionY()-100*2)
		else
...
-- i18n pos lable
function HeroTrainView:_dealPosByI18n()
	if Lang.checkHorizontal() then
		...
		self._nodeTabIcon4:setPositionY(self._nodeTabIcon3:getPositionY()-100)
		...
		self._nodeTabIcon4:setPositionX(self._nodeTabIcon4:getPositionX()+20)
