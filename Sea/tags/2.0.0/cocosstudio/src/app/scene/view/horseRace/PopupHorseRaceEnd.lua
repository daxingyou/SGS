...
function PopupHorseRaceEnd:_createHorseReward(text, count, isPerfect)
    ...
    if not Lang.checkLang(Lang.CN) then
         self._textCount:setPositionX(self._textTitle:getPositionX() + self._textTitle:getContentSize().width + 4)
    end
    ...
    if #awards == 0 then 
        ...
        for i, v in pairs(awards) do 
            if v.type ~= 0 then
                ...
                if not Lang.checkLang(Lang.CN) then
                     posX = self._textTitle:getContentSize().width + self._textTitle:getPositionX()  + (i-1)*90
                else
                     posX = 105 + (i-1)*90
                end
