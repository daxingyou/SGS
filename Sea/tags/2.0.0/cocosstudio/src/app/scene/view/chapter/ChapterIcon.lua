...
function ChapterIcon:onLangHandle()
	...
	if Lang.checkLang(Lang.EN) then
		self._chapterNameBg:setFontSize(24)
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local image = display.newSprite(Path.getCommonImage("img_wujiangtexingdi01_h"),
			0,0,{capInsets = cc.rect(82,14,1,1),rect = cc.rect(0,0,166,30)}
		)
		image:setPosition(47,14)
		image:setAnchorPoint(cc.p(0.5,0.5) )
		image:setLocalZOrder( -1 )
		image:setContentSize(cc.size(200,38))
		self._starBg:addChild(image)
	end
...
function ChapterIcon:_dealPosByI18n()
	...
	self._bossName:setAnchorPoint(cc.p(0.5,0.5))
	self._bossCome:setAnchorPoint(cc.p(0.5,0.5))
	self._panelBoss:setPositionY(180)
end
