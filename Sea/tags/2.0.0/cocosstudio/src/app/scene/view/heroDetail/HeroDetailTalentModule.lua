...
function HeroDetailTalentModule:ctor(heroUnitData, notCheckAvatar, limitLevel, isGoldTrain, hideActive)
...
function HeroDetailTalentModule:_createDes(rank)
	...
	local baseId, isEquipAvatar, avatarLimitLevel = AvatarDataHelper.getShowHeroBaseIdByCheck(self._heroUnitData)
	...
	local config = UserDataHelper.getHeroRankConfig(baseId, rank, limitLevel)
	if config == nil then
		return nil
	end
