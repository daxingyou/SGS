	...
	if rankType == ComplexRankConst.USER_GUILD_RANK then
		...
		node:updateLabel(
			"Text_guild_player",
			{
				...
				-- outlineColor = Colors.getOfficialColorOutline(data.leaderOfficialLv),
			}
		)
...
function ComplexRankItemCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		if Lang.checkChannel(Lang.CHANNEL_SEA) then
			local textGuildName = UIHelper.seekNodeByName(nodeGroup,"Text_guildName")
			textGuildName:setPositionX(textGuildName:getPositionX()-50)
		end
	end
end
