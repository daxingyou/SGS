...
function GuildServerAnswerNode:_updateExamiationInfo()
	...
	self._textExamination:setString(curQues:getQuestionDes())
...
function GuildServerAnswerNode:_playQuesNoFloatAction(callback)
	...
	if Lang.checkLang(Lang.CN) then
		...
		UIHelper.setLabelStyle(self._textQuesNo,{
            style = "text_answer_3",
            text = Lang.getImgText("txt_answer_n",{value = visibleNo})
	   	})
	end
...
function GuildServerAnswerNode:_set_ANSWER_STATE_READY()
	...
	if Lang.checkLang(Lang.CN) then
		...
		local UIHelper  = require("yoka.utils.UIHelper")	
		UIHelper.setLabelStyle(self._waveImage,{
            style = "text_answer_2",
            text = Lang.getImgText("txt_answer_wave_" .. wave)
	   	})
	end
...
function GuildServerAnswerNode:_updateTips(state)
	...
	if Lang.checkLang(Lang.CN) then
		...
		local UIHelper  = require("yoka.utils.UIHelper")	
		UIHelper.setLabelStyle(self._diedTips,{
            style = "text_answer_1",
            text = Lang.getImgText(resId)
	   	})
	end
end
...
-- i18n change lable
function GuildServerAnswerNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        self._imageSelected = UIHelper.swapSignImage(self._imageSelected,
		{ 
			 style = "answer_1", 
			 text = Lang.getImgText("answer_0") ,
			 anchorPoint = cc.p(0.5,0.5),
			 rotation = -10,
		},Path.getTextSignet("img_common_lv"))


        self._diedTips = UIHelper.swapWithLabel(self._diedTips,{
            style = "text_answer_1",
            text = Lang.getImgText("txt_answer_04")
	   	})
	   
		self._textQuesNo = UIHelper.swapWithLabel(self._textQuesNo,{
            style = "text_answer_3",
            text = Lang.getImgText("txt_answer_n01",{value = 1})
	   	})

		self._waveImage = UIHelper.swapWithLabel(self._waveImage,{
            style = "text_answer_2",
            text = Lang.getImgText("txt_answer_05")
	   	})
	end
end
...
-- i18n change lable
function GuildServerAnswerNode:_dealPosByI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		self._playingImage:setAnchorPoint(0.5,1)
		self._playingImage:setPositionY(self._playingImage:getPositionY()+70)
		self._playingImage:setScale9Enabled(false)
		local width = 538
		self._playingImage:setContentSize(cc.size(width,140))
		local size = self._textExamination:getContentSize()
		self._textExamination:setContentSize(cc.size(size.width+100,size.height))
		self._textCountDown:setPositionX(width/2)
	end
	if Lang.checkLang(Lang.TH) then
		self._textExamination:getVirtualRenderer():setLineSpacing(6)
	end

	if not Lang.checkLang(Lang.CN) then
		self._textExamination:setFontSize(self._textExamination:getFontSize()-2)
		self._textExamination:setPositionY(self._textExamination:getPositionY()+12)
		local size = self._textExamination:getContentSize()
		self._textExamination:setContentSize(cc.size(size.width,size.height+12))
	end
end
