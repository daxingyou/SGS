...
function TreasureTrainRefineLayer:_updateBaseInfo()
	...
	local heroBaseId = UserDataHelper.getHeroBaseIdWithTreasureId(self._unitData:getId())
...
-- i18n
function TreasureTrainRefineLayer:_dealPosI18n()
	if Lang.checkLang(Lang.EN) then
		local text2 = self._fileNodeAttr2:getSubNodeByName("TextName")
		text2:getVirtualRenderer():setMaxLineWidth(125)
		local text3 = self._fileNodeAttr3:getSubNodeByName("TextName")
        text3:getVirtualRenderer():setMaxLineWidth(125)
	end
end
-- i18n pos lable
function TreasureTrainRefineLayer:_updatePosI18n()
	if Lang.checkLang(Lang.EN) then
		local text2 = self._fileNodeAttr2:getSubNodeByName("TextName")
		local text3 = self._fileNodeAttr3:getSubNodeByName("TextName")
		self._fileNodeAttr2:setPositionY(82-text2:getContentSize().height+20)
		local posY = self._fileNodeAttr2:getPositionY()-30-text3:getContentSize().height+20
		self._fileNodeAttr3:setPositionY(posY)
	end
end

--i18n
function TreasureTrainRefineLayer:_updateTextNameI18n()
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		...
		local label2 = ccui.Helper:seekNodeByName(self._fileNodeName2, "TextName")
