...
function CampRaceView:ctor()
	local resource = {
		...
		binding = {
            _btnReturn = {
				events = {{event = "touch", method = "_onReturnClick"}}
			},
		}
	}
...
function CampRaceView:_refreshTitle()
    ...
    if state == CampRaceConst.STATE_PRE_OPEN then  
        if not Lang.checkLang(Lang.CN) then 
            self._imgTitle:setString(Lang.getImgText(TITLE_IMG[1]) )
        else
            self._imgTitle:loadTexture(Path.getTextSystemBigTab(TITLE_IMG[1]))
        end
        self._imageCamp:setVisible(false)
    elseif state == CampRaceConst.STATE_PRE_MATCH then
        if not Lang.checkLang(Lang.CN) then 
            self._imgTitle:setString(Lang.getImgText(TITLE_IMG[2]) )
        else
            self._imgTitle:loadTexture(Path.getTextSystemBigTab(TITLE_IMG[2]))
        end
    ...
    elseif state == CampRaceConst.STATE_PLAY_OFF then
        ...
        if TITLE_IMG[finalStatus + 2] then
            if not Lang.checkLang(Lang.CN) then 
                self._imgTitle:setString(Lang.getImgText(TITLE_IMG[finalStatus + 2]) )
            else
                self._imgTitle:loadTexture(Path.getTextSystemBigTab(TITLE_IMG[finalStatus + 2]))
            end
        else
...
function CampRaceView:_playPromotedAnim()
	...
	-- i18n change effect
	local function effectFunctionWin(effect)
		local function _effectFunction(_effect)
			if _effect == "routine_word_win_2" then
				...
				local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_jinjichenggong"),style="effect_text_21_1",styleType=TypeConst.EFFECT})
				return subLabel
			end
		end	
        G_EffectGfxMgr:createPlayMovingGfx(self._nodeEffect, "moving_jingji_shengli", _effectFunction, 
        function(effect) 
            self:_updateSubLayer() 
        end  , true)
	end
	local function effectFunctionLose(effect)
		local function _effectFunction(_effect)
			if _effect == "routine_word_shibai_9" then
				local TypeConst = require("app.i18n.utils.TypeConst")
				local UIHelper = require("yoka.utils.UIHelper")
				local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_jinjishibai"),style="effect_text_21_2",styleType=TypeConst.EFFECT})
				return subLabel
			end
		end	
		G_EffectGfxMgr:createPlayMovingGfx(self._nodeEffect, "moving_jingji_jinjishibai", _effectFunction, function(effect) 
            self:_updateSubLayer() 
        end  , true)
	end
	...
	if myRank <= 8 then 
        ...
        G_EffectGfxMgr:createPlayGfx(self._nodeEffect, effectName, function(effect) 
            self:_updateSubLayer() 
        end )
    else
        if myRank <= 8 then 
...
function CampRaceView:_playGetChampionEffect()
    -- i18n change effect
	local function effectFunctionWin()
		local function _effectFunction(_effect)
			if _effect == "routine_word_win_2" then
				local TypeConst = require("app.i18n.utils.TypeConst")
				local UIHelper = require("yoka.utils.UIHelper")
				local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_gongxiduoguan"),style="effect_text_21_1",styleType=TypeConst.EFFECT})
				return subLabel
end
		end	
	    G_EffectGfxMgr:createPlayMovingGfx(self._nodeEffect, "moving_jingji_shengli", _effectFunction, nil , true)
	end
    if Lang.checkLang(Lang.CN) then
        G_EffectGfxMgr:createPlayGfx(self._nodeEffect,  "effect_jingji_gongxiduoguan", nil, true)
    else
        effectFunctionWin()
    end
...
-- i18n pos lable
function CampRaceView:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
         self._imageCamp:setAnchorPoint(cc.p(0,0.5))
         self._imgTitle:setPositionX(114+15)
         local TypeConst = require("app.i18n.utils.TypeConst")
         local UIHelper  = require("yoka.utils.UIHelper")
         self._imgTitle = UIHelper.swap(self._imgTitle,
            {
                text="", 
                offsetX=0, 
                offsetY= 10, 
...
-- i18n  lable
function CampRaceView:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
         local size = self._imgTitle:getContentSize()
         self._imageCamp:setPositionX(size.width + self._imgTitle:getPositionX() + 10)
    end
end
return CampRaceView
