...
function TeamHeroNode:_initView()
	...
	--i18n
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.ZH) then
		...
		if Lang.checkLang(Lang.TH) then
			local value = ccui.Helper:seekNodeByName(self._nodeLevel, "TextValue")
			value:setFontSize(18)
			value:setPositionY(value:getPositionY()-1)
			local max = ccui.Helper:seekNodeByName(self._nodeLevel, "TextMax")
			max:setFontSize(18)
			max:setPositionY(max:getPositionY()-1)
			local value = ccui.Helper:seekNodeByName(self._nodePotential, "TextValue")
			value:setFontSize(18)
			value:setPositionY(value:getPositionY()-1)
		end
	end
	...
	self._treasures = {}
	for i = 1, 2 do --2个宝物
		local treasure = TeamTreasureIcon.new(self["_fileNodeTreasure" .. i], true)
...
function TeamHeroNode:onExit()
	...
	if self._curHeroData then
		...
		--记录觉醒等级
		self._lastHeroLimit = {self._curHeroData:getId(), self._curHeroData:getLimit_level()}
		--记录界限等级
...
function TeamHeroNode:_formatButtons()
	local buttons = {self._buttonMaster, self._buttonOneKey, self._buttonChange, self._buttonAvatar, self._buttonSilkbag}
	local btnCount2Pos = {
		[1] = {cc.p(306, 67)},
	...
	if Lang.checkLang(Lang.KR) then
		local quality = self._curHeroData:getQuality()
		if quality < 4 and (not isRole) then
			silkbagOpen = false	
		end
	end
	
	local showButtons = {true, oneKeyOpen, not isRole, isRole and avatarOpen, silkbagOpen} --各个按钮是否显示
...
function TeamHeroNode:_updateKarma()
	...
	self._imageKarmaTip:setPosition(cc.p(255, 140)) --恢复初始位置
...
function TeamHeroNode:_updateTreasure()
	...
	for i = 1, 2 do
		...
		if treasureId then
			...
			reach4 =
				reach4 or RedPointHelper.isModuleSubReach(FunctionConst.FUNC_TREASURE_TRAIN_TYPE3, "slotRP", treasureUnitData)
		end
...
function TeamHeroNode:_updateAwake()
	...
	if isOpen and isCanAwake then
		...
		local show = RedPointHelper.isModuleReach(FunctionConst.FUNC_HERO_TRAIN_TYPE3, self._curHeroData)
		self._imageAwakeRedPoint:setVisible(show)
	else
		self._imageAwakeBg:setVisible(false)
	end
end
...
function TeamHeroNode:_popupReplaceInstrument()
	...
	if #result == 0 then
		...
		popup:updateUI(PopupChooseInstrumentHelper.FROM_TYPE2, callBack, result, self._btnInstrumentShowRP)
		popup:openWithAction()
	end
end
...
function TeamHeroNode:_playChangeHeroSummary()
	...
	if isActive then
		...
		local heroRankConfig = HeroDataHelper.getHeroRankConfig(heroId1, 0, limitLevel)
		if heroRankConfig == nil then
			heroRankConfig = HeroDataHelper.getHeroRankConfig(heroId1, 0, 0)
		end
...
function TeamHeroNode:_checkHeroTrainPrompt()
	...
	if lastId == curId and nowLimit > lastLimit and lastLimit > -1 then
		return true
	end
...
function TeamHeroNode:_playHeroTrainPrompt()
	...
	do
		...
		TeamViewHelper.makeLimitDiffData(summary, self._curHeroData, lastLimit, finishCallback3)
	end
...
function TeamHeroNode:_swapImageByI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		local buttons = {
			{btn=self._buttonMaster,image = "Image_6",text = "img_btn_mastertxt01"},
			{btn=self._buttonOneKey,image = "Image_4",text = "img_btn_yijianqianghuatxt01"},
			{btn=self._buttonChange,image = "Image_4",text = "img_btn_replacetxt01"},
			{btn=self._buttonAvatar,image = "Image_4",text = "img_btn_fashiontxt01"},
			{btn=self._buttonSilkbag,image = "Image_4",text = "img_btn_silkbagtxt"},
		}
		for i, v in ipairs(buttons) do
			local UIHelper  = require("yoka.utils.UIHelper")
			local image = UIHelper.seekNodeByName(v.btn,v.image)
			local label = UIHelper.swapWithLabel(image,{ 
				style = "icon_txt_60",
				text = Lang.getImgText(v.text) ,
		   })
...
function TeamHeroNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		for i = 1, 6 do
			...
			if i % 2 ~= 0 then
				self["_textJiBanDes"..i]:setPositionX(self["_textJiBanDes"..i]:getPositionX()-18)
			else
		...
		self._nodeLevel:setFontSize(18)
		--self._nodeDetailTitleBasic:setFontSize(18)
		for i = 1, 6 do
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		...
		self._panelKarma:setPositionY(self._panelKarma:getPositionY()+20)
		self._nodePanelYoke:setPositionY(self._nodePanelYoke:getPositionY()+27)
		for i = 1, 6 do
			local panel = ccui.Helper:seekNodeByName(self._panelKarma, "Panel_"..i)
			panel:setPositionY(panel:getPositionY()+7)
		end
		local img185 = ccui.Helper:seekNodeByName(self._panelKarma, "Image_185")
		img185:setPositionY(img185:getPositionY()+7)
		for i = 1, 6 do
			local panel = ccui.Helper:seekNodeByName(self._panelYoke, "Panel_6_"..i)
			panel:setPositionY(100-math.floor((i-1)/2)*40)
			self["_textJiBanDes"..i]:setAnchorPoint(0,1)
			self["_textJiBanDes"..i]:getVirtualRenderer():setMaxLineWidth(155)
		end
		self._panelAttr:setContentSize(cc.size(363,584))
	end
	if Lang.checkLang(Lang.TH) then
		self._nodeLevel:setPositionX(self._nodeLevel:getPositionX()-25)
		self._fileNodeAttr1:setPositionX(self._fileNodeAttr1:getPositionX()-25)
		self._fileNodeAttr2:setPositionX(self._fileNodeAttr2:getPositionX()-25)
		self._nodePotential:setPositionX(self._nodePotential:getPositionX())
		self._fileNodeAttr3:setPositionX(self._fileNodeAttr3:getPositionX())
		self._fileNodeAttr4:setPositionX(self._fileNodeAttr4:getPositionX())
		for i = 1, 4 do
			self["_fileNodeAttr"..i]:setFontSize(20)
			local num = ccui.Helper:seekNodeByName(self["_fileNodeAttr"..i], "TextValue")
			num:setFontSize(18)
			num:setPositionY(num:getPositionY()-1)
		end
		for i = 1, 6 do
			self["_textYuanFenDes"..i]:setFontSize(20)
		end
		for i = 1, 6 do
			self["_textJiBanDes"..i]:setFontSize(20)
		end
	end
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.ZH) then
		for i = 1, 4 do
