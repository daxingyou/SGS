...
-- i18n pos lable
function PopupArenaRank:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
        local textTitle0 = UIHelper.seekNodeByName(self._resourceNode,"Node_1","Text_title_0")
		self._textMyRank:setPositionX(textTitle0:getPositionX() + textTitle0:getContentSize().width + 6)


		local textTitle0 = UIHelper.seekNodeByName(self._resourceNode,"Node_1","Text_title_0_0")
		textTitle0:setAnchorPoint(cc.p(1,0.5))
		textTitle0:setPositionX( self._resInfoEx1:getPositionX() - 6)
		
	end
end

-- i18n pos lable
function PopupArenaRank:_updateResPosI18n()
	if Lang.checkChannel(Lang.CHANNEL_SEA )  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local textRankAward = UIHelper.seekNodeByName(self._resourceNode,"Panel_root","Node_1","Text_title_0_0")
		local startX = textRankAward:getPositionX() + 3
		self._resInfoEx1:setPositionX(startX)
		self._resInfoEx2:setPositionX(startX + self._resInfoEx1:getWidth() + 10)
	end
end
