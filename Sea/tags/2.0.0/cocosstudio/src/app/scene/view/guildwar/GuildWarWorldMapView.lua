...
function GuildWarWorldMapView:_refreshTimeView() 
    ...
    elseif curTime >= timeData.time1 and curTime < timeData.endTime then
		...
		local txt = G_ServerTime:getLeftSecondsString(timeData.startTime, "00:00:00")
...
function GuildWarWorldMapView:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self,"Node_MiddleTop","Image_123")
		local size = image1:getContentSize()

		local size1 = self._textTimeTitle:getContentSize()
		local size2 = self._textTime:getContentSize()
	
		self._textTimeTitle:setPositionX(-(size1.width + size2.width)*0.5)
		self._textTimeTitle:setAnchorPoint(cc.p(0,0.5))
		self._textTime:setPositionX((size1.width + size2.width)*0.5)
