...
function PopupArenaReportCell:onCreate()
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		self._rankNode:getSubNodeByName("Image_arrow_up"):setPositionX(55)
		self._rankNode:getSubNodeByName("Image_arrow_down"):setPositionX(55)
...
function PopupArenaReportCell:_updateReportInfo(reportInfo)
	...
	self:updateLabel("_textPlayerName",
	{
		...
		-- outlineColor = Colors.getOfficialColorOutline(reportInfo.officer_level)
	})
	...
	if reqBatNum == 1 then
		...
		if Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.EN) then
			...
			local labelWin = UIHelper.seekNodeByName(self._nodeWinLose,"Label_win")
			local labelLose = UIHelper.seekNodeByName(self._nodeWinLose,"Label_lose")
			local image_winEx = UIHelper.seekNodeByName(self._nodeWinLose,"Image_winEx")
			local image_loseEx = UIHelper.seekNodeByName(self._nodeWinLose,"Image_loseEx")
			
			local Image_win = UIHelper.seekNodeByName(self._nodeWinLose,"Image_win")
			
			UIHelper.alignCenter(Image_win,{labelWin,image_winEx},{6,0})
			UIHelper.alignCenter(Image_win,{labelLose,image_loseEx},{6,0})
...
function PopupArenaReportCell:_dealPosByI18n()
	...
	if Lang.checkLang(Lang.EN) then
		self._textRankDesc:setPositionX(self._textRankDesc:getPositionX()-15)
	end

	if Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.EN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local labelWin = UIHelper.seekNodeByName(self._nodeWinLose,"Label_win")
		labelWin:setPositionY(labelWin:getPositionY() + 23 )
		
		local labelLose = UIHelper.seekNodeByName(self._nodeWinLose,"Label_lose")
		labelLose:setPositionY(labelLose:getPositionY() - 35 )

		local Image_win = UIHelper.seekNodeByName(self._nodeWinLose,"Image_win")
		local Image_lose = UIHelper.seekNodeByName(self._nodeWinLose,"Image_lose")
		Image_win:setPositionX(Image_win:getPositionX() - 20 )
		Image_lose:setPositionX(Image_lose:getPositionX() - 20 )
	end
