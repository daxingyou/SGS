...
function PopupTerritoryRiotInfo:_updateRiotProcess()
	...
	local richWidget = nil
	-- i18n richtext 
	if Lang.checkLang(Lang.CN) then
		richWidget = ccui.RichText:create()
	else
		richWidget = ccui.RichText:createByI18n()
	end
...
-- i18n pos lable
function PopupTerritoryRiotInfo:_updatePosByI18n()
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local parent = self._textTitleDesc:getParent()
		local image2 = parent:getChildByName("Image_2")
		self._textTitleDesc:setAnchorPoint(cc.p(1,0.5))
		self._textTitleDesc:setPositionX(631)
		image2:setPositionX(self._textTitleDesc:getPositionX()-self._textTitleDesc:getContentSize().width-14)
	end
end
