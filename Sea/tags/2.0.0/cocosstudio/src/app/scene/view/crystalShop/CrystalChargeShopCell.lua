...
function CrystalChargeShopCell:updateUI(cellData)
	...
	local function updateItem(index, data)
		...
		if not Lang.checkLang(Lang.CN) then
			...
			self["_itemName"..index]:setPositionY(90+7)
			if Lang.checkLang(Lang.TH) then
				self["_buyDesc"..index]:setPosition(cc.p(118, 68+2))
			else
				self["_buyDesc"..index]:setPosition(cc.p(118, 68+7))
			end
			self["_buyDesc"..index]:setAnchorPoint(cc.p(0.0, 0.5))
		end
...
--i18n
function CrystalChargeShopCell:_dealByI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local img1 = UIHelper.seekNodeByName(self._item1,"Image_45")
		img1:setVisible(false)
		local img2 = UIHelper.seekNodeByName(self._item2,"Image_45")
