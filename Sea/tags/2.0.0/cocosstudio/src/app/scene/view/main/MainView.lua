...
function MainView:onCreate()
	...
	if self._topBarList then
		self._topBarList:updateUI(TopBarStyleConst.STYLE_MAIN, true)
	end
...
function MainView:_initMusic()
	local system_sound = require("app.config.system_sound")
	local sound_info1 = system_sound.get(AudioConst.MUSIC_CITY)
	local sound_info2 = system_sound.get(AudioConst.MUSIC_BGM_NEW_CITY)
	if sound_info1 and sound_info2 then
		local delay1 = cc.DelayTime:create(sound_info1.time + MainView.DELAY_MUSIC_TIME) -- 间隔时间
		local delay2 = cc.DelayTime:create(sound_info2.time + MainView.DELAY_MUSIC_TIME)
		local sequence =
			cc.Sequence:create(
			cc.CallFunc:create(
				function()
					-- logWarn("MainView:MUSIC_CITY")
			...
			cc.CallFunc:create(
				function()
					-- logWarn("MainView:MUSIC_BGM_NEW_CITY")
