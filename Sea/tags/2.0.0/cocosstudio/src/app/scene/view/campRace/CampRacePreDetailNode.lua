...
function CampRacePreDetailNode:updatePlayer(player)
	...
	for i, id in pairs(formation) do 
		...
		self._heroIcons[i]:updateIcon(baseId, rank, limitLevel)
	end

	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
		UIHelper.dealVTextWidget(self._textRank,Lang.get("camp_per_rank", {count = player:getPer_rank()}))
	else
...
-- i18n change lable
function CampRacePreDetailNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local imagePower = ccui.Helper:seekNodeByName(self, "ImagePower")
		UIHelper.swapWithLabel(imagePower,{
			 style = "camp_race_4",
			 text = Lang.getImgText("img_camp_com02")
		})
		

		self._textRank:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
		self._textRank:getVirtualRenderer():setLineSpacing(-3)
	end
end
