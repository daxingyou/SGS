...
function ArenaView:onCreate()
	...
	self._btnTeam:updateUI(FunctionConst.FUNC_TEAM)
	-----------------------------------------------------------------
...
--i18n
function ArenaView:dealByI18n()
	if Lang.checkLang(Lang.TH) then
		local text = ccui.Helper:seekNodeByName(self._btnShop, "TextImage")
		text:setFontSize(text:getFontSize()+2)
	end
	if Lang.checkLang(Lang.ENID) then
		local text = ccui.Helper:seekNodeByName(self._panelRtMid, "Text_title_award")
		local wid = text:getVirtualRendererSize().width
		local wid2 = self._textTimes:getVirtualRendererSize().width
		self._panelRtMid:setContentSize(230,64)
		self._panelRtMid:ignoreContentAdaptWithSize(false)
		self._textTimes:setPositionX(text:getPositionX()+wid)
		self._btnAddTimes:setPositionX(self._btnAddTimes:getPositionX()+30)
	end
end
