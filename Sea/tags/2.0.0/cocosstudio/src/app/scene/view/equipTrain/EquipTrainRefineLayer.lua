...
function EquipTrainRefineLayer:_updateBaseInfo()
	...
	local heroBaseId = UserDataHelper.getHeroBaseIdWithEquipId(self._equipData:getId())
...
function EquipTrainRefineLayer:_dealPosI18n()
	...
	if Lang.checkLang(Lang.EN) then
		local text = self._fileNodeAttr2:getSubNodeByName("TextName")
        text:getVirtualRenderer():setMaxLineWidth(125)
	end
end
-- i18n
function EquipTrainRefineLayer:_updatePosI18n()
	if Lang.checkLang(Lang.EN) then
		local text = self._fileNodeAttr2:getSubNodeByName("TextName")
		self._fileNodeAttr2:setPositionY(86-text:getContentSize().height+20)
	end
end
