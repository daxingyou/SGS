...
function TeamHeroBustIcon:updateIcon(type, value, funcId, limitLevel)
	self:_hideAllWidget()
	...
	if not isOpen then
		...
		if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
			...
			UIHelper.dealVTextWidget(self._textOpenLevel,level)
			self._textOpenLevel:setPositionY(-10)
		end
		return
	end

	if type > 0 and value > 0 then
		self:_createCommonIcon(type, value, limitLevel)
	else
...
function TeamHeroBustIcon:_createCommonIcon(type, value, limitLevel)
	...
	-- end
	local itemParams = TypeConvertHelper.convert(TypeConvertHelper.TYPE_HERO, value, nil, nil, limitLevel)
	--加载背景框
...
function TeamHeroBustIcon:onlyShow(type, value, limitLevel)
	self:_hideAllWidget()
	if type > 0 and value > 0 then
		self._isOpen = true
		self:_createCommonIcon(type, value, limitLevel)
