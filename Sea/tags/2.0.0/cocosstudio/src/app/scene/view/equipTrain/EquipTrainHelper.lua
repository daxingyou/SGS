...
-- i18n通过道具id获取消耗的装备唯一id列表
function EquipTrainHelper.getCostEquipIdList(value,num)
    local cards = {}
    local ids = G_UserData:getEquipment():getEquipmentIdsWithBaseId(value)
    local count = #ids
    for i = 1, count do
...
function EquipTrainHelper.isNeedRedPointByUnitData(unitData)
    ...
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
        -- if unitData:getConfig().suit_id == LimitCostConst.MAX_SUIT_ID then
        --     return isRed
        -- end
    end
    local LogicCheckHelper = require("app.utils.LogicCheckHelper")
...
function EquipTrainHelper.haveBetterAndCanEquipJade(equipId, jadeId, slot)
    ...
    local better = false
    local list = jade:getJadeListByEquip(params)
    local UserDataHelper = require("app.utils.UserDataHelper")
