...
function GoldHeroShow:_playZhaomu( ... )
    local function effectFunction(effect)
        if effect == "mingzi" then
            ...
            mingzi:loadTexture(Path.getGoldHeroTxt(GachaGoldenHeroConst.DRAW_HERO_MINGZI[self._campPos]))
            return mingzi
        elseif effect == "shuoming" then
            if Lang.checkLang(Lang.VN)  then
                ...
                shuoming:loadTexture(Path.getGoldHeroTxt(GachaGoldenHeroConst.DRAW_HERO_DESC[self._campPos]))
                return shuoming
            end
...
function GoldHeroShow:_getCampIndex()
    ...
    self._campPos = table.keyof(heroIds, self._heroId)
end
