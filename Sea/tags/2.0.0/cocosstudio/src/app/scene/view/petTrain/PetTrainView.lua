...
local TAIL_POS_NORMAL = 305.79
local TAIL_POS_LIMIT = 155.79
...
function PetTrainView:_onClickTabIcon(index)
	...
	self:_setArrowVisible(index ~= PetConst.PET_TRAIN_LIMIT)
	self:_updateTabIcons()
	self:_updateView()
end
...
function PetTrainView:_updateTabVisible()
	local PetTrainHelper = require("app.scene.view.petTrain.PetTrainHelper")
	local tabSize = PetTrainHelper.getCurTabSize()
...
-- i18n pos lable
function PetTrainView:_dealPosByI18n()
	if Lang.checkHorizontal() then
		...
		local img9 = UIHelper.seekNodeByName(self,"Image_9")
		local img90 = UIHelper.seekNodeByName(self,"Image_9_0")
		img9:setVisible(false)
		img90:setVisible(false)
		self._image3:setVisible(false)
