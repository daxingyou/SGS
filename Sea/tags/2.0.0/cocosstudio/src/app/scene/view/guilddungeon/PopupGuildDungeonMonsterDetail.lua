...
function PopupGuildDungeonMonsterDetail:_createHeroSpine()
   self._ImageHero:updateUI(self._monsterBattleUser:getPlayer_info().covertId)
end
...
function PopupGuildDungeonMonsterDetail:_refreshEnemyFormation()
    ...
    for i=1, 6 do
        ...
        if hero and pos and pos ~= 0 then
            ...
            if i == 1 then
                self["_heroAvater"..pos]:updateUI(self._monsterBattleUser:getPlayer_info().covertId)
            else
                 self["_heroAvater"..pos]:updateUI(hero:getBase_id())    
            end
        ...
        if record then
            ...
            if Lang.checkChannel(Lang.CHANNEL_SEA) then
                v:updateView(
                    record:isIs_win(),
                    attackName,
                    Colors.getOfficialColor(record:getPlayer_officer()),true
                )
            else
                v:updateView(
                    record:isIs_win(),
...
-- i18n change lable
function PopupGuildDungeonMonsterDetail:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
...
-- i18n pos lable
function PopupGuildDungeonMonsterDetail:_adjustPosByI18n()
    if not Lang.checkLang(Lang.CN) then
         local size = self._textName:getContentSize()
         self._fileNodePower:setPositionX(size.width*0.5+self._textName:getPositionX()+10)
    end
end


-- i18n change lable
function PopupGuildDungeonMonsterDetail:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        
		local image = UIHelper.seekNodeByName(self,"Image_2")
        if image then
