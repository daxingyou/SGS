...
function PopupStageDetail:_setStageName()
	...
	self._textName:setColor(Colors.getColor(self._stageInfo.color))
...
function PopupStageDetail:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        ...
        self._costStamina:setPositionX(self._costStamina:getPositionX()+27)
        self._drop1:setPositionX(self._drop1:getPositionX()+27)
        self._drop2:setPositionX(self._drop2:getPositionX()+27)
    ...
    if Lang.checkLang(Lang.EN) then
        local textStamina = UIHelper.seekNodeByName(self,"TextStamina")
        local width = textStamina:getContentSize().width
        local posX = textStamina:getPositionX()
        self._costStamina:setPositionX(posX+width)
        local textGet = UIHelper.seekNodeByName(self,"TextGet")
        width = textGet:getContentSize().width
        posX = textGet:getPositionX()
        self._drop1:setPositionX(posX+width)
        self._drop2:setPositionX(posX+width+105)
        local imageSword = UIHelper.seekNodeByName(self._btnFight,"Image_Sword")
        imageSword:setPositionX(imageSword:getPositionX()-10)
        local textDrop = UIHelper.seekNodeByName(self,"TextDrop")
        textDrop:setAnchorPoint(0.5,0.5)
        textDrop:setPositionX(128)
    end
    if Lang.checkLang(Lang.TH) then
        local textStamina = UIHelper.seekNodeByName(self,"TextStamina")
        textStamina:setPositionX(textStamina:getPositionX()-5)
        local textGet = UIHelper.seekNodeByName(self,"TextGet")
        textGet:setPositionX(textGet:getPositionX()-5)
    end
end
