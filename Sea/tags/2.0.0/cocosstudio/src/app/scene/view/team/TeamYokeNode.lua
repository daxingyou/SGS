...
function TeamYokeNode:_updateTotalCount()
	...
	if Lang.checkLang(Lang.TH) then
		...
		self._panelTotalCountBg:setBackGroundImageScale9Enabled(true)
		self._panelTotalCountBg:setBackGroundImageCapInsets(cc.rect(166,18,1,1))
		self._panelTotalCountBg:setContentSize(cc.size(virtualContentSize.width+18,size.height))
		size = self._panelTotalCountBg:getContentSize()
	end
...
function TeamYokeNode:_updateAvatar()
	...
	for i = 1, 5 do
		...
		if info then
			...
			self["_avatar"..i]:updateView(info.baseId, info.activatedCount, info.totalCount, info.limitLevel)
		else
