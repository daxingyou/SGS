...
function PopupStageReward:_createActionNode(effect)
	if effect == "txt" then
		...
		if not Lang.checkLang(Lang.CN) then
			...
			local subLabel = UIHelper.createLabel({text=Lang.getImgText("txt_system_lingqubaoxiang"),style="system_1",styleType=TypeConst.TEXT})
			return subLabel
		else
...
function PopupStageReward:_initBoxData()
	...
	if self._chapterData:isLastStagePass() and self._chapterData:getPreward() == 0 then
		...
		if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
			local UIHelper  = require("yoka.utils.UIHelper")
			data.name = UIHelper.dealVText(data.name)
		end
