    ...
    local function updateItem(index, data, bSunday)
        ...
        if data.isActived and data.canSignedDay then
            ...
            if Lang.checkLang(Lang.TW) then
                if index == CustomActivityConst.FUNDS_WEEKITEMNUM_MAX and data.canGet == 1 then
                    self["_fileNodeIcon"..index]:setLocalZOrder(self["_imageShade"..index]:getLocalZOrder()+1)
                    self["_fileNodeIcon"..index]:getIconTemplate():setCascadeColorEnabled(true)
                    self["_fileNodeIcon"..index]:getIconTemplate():setColor(cc.c3b(100,100,100))
                end
            end
        else
...
function CustomActivityWeekFundsCell:_dealI18n()
    if not Lang.checkLang(Lang.CN) then
        for index = 1, CustomActivityConst.FUNDS_WEEKITEMNUM_MAX do
            self["_textDay"..index]:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER  )
            self["_textDay"..index]:setTextVerticalAlignment(cc.TEXT_ALIGNMENT_CENTER  )
            self["_textDay"..index]:setPositionY(self["_textDay"..index]:getPositionY()-26)
        end
    end
end
