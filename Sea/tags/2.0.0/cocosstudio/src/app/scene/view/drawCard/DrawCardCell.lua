...
function DrawCardCell:_init()
    ...
    if not Lang.checkLang(Lang.CN) then
        self._imageBookText = ccui.Helper:seekNodeByName(self._target, "ImageBookText")
        self._imageBookText:ignoreContentAdaptWithSize(true)

        if Lang.checkLang(Lang.EN) then
            self._textCountDown:setFontSize(self._textCountDown:getFontSize()-2)
            local textNotice = ccui.Helper:seekNodeByName(self._target, "TextNotice")
            if textNotice then
                textNotice:setFontSize(textNotice:getFontSize()-2)
            end
