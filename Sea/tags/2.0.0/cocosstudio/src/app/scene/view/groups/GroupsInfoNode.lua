...
function GroupsInfoNode:_initView()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		self._imageArrow:setPositionX(
			self._textAwardTips:getPositionX() +self._textAwardTips:getContentSize().width + self._imageArrow:getContentSize().width*0.5 
		)
	end
...
-- i18n pos lable
function GroupsInfoNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) and not Lang.checkChannel(Lang.CHANNEL_SEA) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._ckbAuto:setScale(1.1)
	end
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		local textNoShow = UIHelper.seekNodeByName(self._ckbAuto,"TextNoShow")
		-- textNoShow:enableOutline(cc.c3b(0xa7, 0x38, 0x01), 2)
		textNoShow:setColor(cc.c3b(0xa7, 0x38, 0x01))

		local imageAutoSize = self._imageAuto:getContentSize()
		local textNoShow = UIHelper.seekNodeByName(self._ckbAuto,"TextNoShow")
		local checkBox = UIHelper.seekNodeByName(self._ckbAuto,"CheckBox")
		
		--textNoShow:setFontSize(textNoShow:getFontSize())
		local bgWidth = textNoShow:getContentSize().width  + 23 + 6
		self._imageAuto:setContentSize(cc.size(bgWidth,imageAutoSize.height))
		self._ckbAuto:setPositionX(bgWidth+23)
		UIHelper.alignCenter(self._imageAuto,{textNoShow})
		textNoShow:setPositionX(textNoShow:getPositionX()-23 * 0.5)

		self._imageAuto:setPositionX(self._imageAuto:getPositionX()+(imageAutoSize.width-bgWidth) * 0.5 + 14)
		if Lang.checkLang(Lang.EN) then
			self._btnAdd:setPositionX(self._btnAdd:getPositionX()-38)
			self._btnLock:setPositionX(self._btnLock:getPositionX()-38)
		end
	end
end
...
-- i18n pos lable
function GroupsInfoNode:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		local label = self._btnLock:getTitleLabel()
		local image357 = UIHelper.seekNodeByName(self._btnLock,"Image_357")
		local size = self._btnLock:getContentSize()
		local size1 = label:getContentSize()
		local size2 = image357:getContentSize()
		label:setPositionX(size.width*0.5-size2.width*0.5)
		image357:setPositionX(size.width*0.5+size1.width*0.5)
		if Lang.checkLang(Lang.EN) then
			label:setPositionX(label:getPositionX()+12)
			image357:setPositionX(image357:getPositionX()-12)
		end

		local label2 = self._btnAdd:getTitleLabel()
		local image356 = UIHelper.seekNodeByName(self._btnAdd,"Image_356")
		
		label2:setHorizontalAlignment( cc.TEXT_ALIGNMENT_CENTER )
		label2:setLineSpacing(-7 )

		self:_alignNodeByI18n(self._btnAdd,label2,image356)
...
-- i18n pos lable
function GroupsInfoNode:_alignNodeByI18n(node,child1,child2)
	...
	if Lang.checkLang(Lang.EN) then
		child1:setPositionX(child1:getPositionX()+10)
		child2:setPositionX(child2:getPositionX()-10)
	end
end
