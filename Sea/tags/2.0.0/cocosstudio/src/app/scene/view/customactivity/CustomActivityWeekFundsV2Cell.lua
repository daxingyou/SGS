...
function CustomActivityWeekFundsV2Cell:_swapImageByI18n()
    ...
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
        self._imageDay:setFontSize(self._imageDay:getFontSize()-2)
    end

    
    if Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.EN) then
        local size = self._imageDay:getContentSize()
        self._imageDay:setPositionY(self._imageDay:getPositionY()-7)
        self._imageDay:setAnchorPoint(cc.p(0.5,0.5))
    end
