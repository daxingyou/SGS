...
--[[
	...
	if newState == GuildWarRunAvatorNode.INIT_STATE then
		local isBorn = self._userData:isInBorn() and self._userData:isSelf()
		if isBorn then
...
function GuildWarRunAvatorNode:_didEnterInit()
	local function getAvatarLimit(avatarBaseId)
		...
		if avatarBaseId > 0 then
			...
			limit = tonumber(require("app.config.avatar").get(avatarBaseId).limit) > 0 and HeroConst.HERO_LIMIT_MAX_LEVEL or 0
		end
		return limit
	end
