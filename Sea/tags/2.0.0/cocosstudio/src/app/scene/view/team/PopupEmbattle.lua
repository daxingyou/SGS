...
function PopupEmbattle:_createHeroAvatar(lineupPos)
	...
	local heroBaseId, isEquipAvatar, avatarLimitLevel = AvatarDataHelper.getShowHeroBaseIdByCheck(heroUnitData)
...
function PopupEmbattle:_checkIsSelectedKnight(sender)
	...
	for k, data in pairs(self._embattlePos2Data) do
		local location = data.spine:getSpineHero():convertToNodeSpace(pos)
		local rect = data.spine:getSpineHero():getBoundingBox()
