...
function FightView:ctor( report, battleData, useInitData )
	self._report = report
...
function FightView:_checkHeroShow(waveId)
	...
	if not self._battleData.alreadyPass and self._battleData.showBossId ~= 0 and waveId == self._engine:getWaveCount() then	
		local heroShow = require("app.scene.view.heroShow.HeroShow")
		heroShow = HeroShow.create(self._battleData.showBossId, function() self:_heroShowEnd(waveId) end, true, true)
	else 
...
-- i18n 新增参数
--神兽释放技能
function FightView:_SIGNAL_PLAY_PET_SKILL(camp, anim, petId, color,petSkillName)
	if anim ~= "" then 
		-- i18n
		if Lang.checkLang(Lang.CN) then
			self._fightUI:playSkillAnim(camp, anim, petId, color)
		else
			self._fightUI:playSkillAnim(camp, anim, petId, color,petSkillName)
		end
	end
end
...
function FightView:_SIGNAL_HISTORY_SHOW(hisCamp, hisId, skillShowId)
	self._fightUI:playHistoryAnim(hisCamp, hisId, skillShowId)
end
