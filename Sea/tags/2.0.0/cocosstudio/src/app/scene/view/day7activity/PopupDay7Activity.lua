...
function PopupDay7Activity:_refreshActTime()
	...
	local text = ""
	if G_UserData:getDay7Activity():isInActRunTime() then
		local timeStr = G_ServerTime:getLeftDHMSFormat(G_UserData:getDay7Activity():getActEndTime())
		text = Lang.get("days7activity_act_end_time",{time = timeStr})
		self._imageActivityEnd:setVisible(false)
	else
		local timeStr = G_ServerTime:getLeftDHMSFormat(G_UserData:getDay7Activity():getActRewardEndTime())
...
--i18n
function PopupDay7Activity:_dealByI18n()
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		self._commonTabGroupHorizon:setPositionY(self._commonTabGroupHorizon:getPositionY()+8)
	end
end
