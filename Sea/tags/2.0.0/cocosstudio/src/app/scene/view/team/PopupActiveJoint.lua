...
function PopupActiveJoint:_updateInfo()
	...
	local heroRankConfig = HeroDataHelper.getHeroRankConfig(self._heroId1, 0, limitLevel)
    if heroRankConfig == nil then
        heroRankConfig = HeroDataHelper.getHeroRankConfig(self._heroId1, 0, 0) --有的武将没配界限，就先传0
    end
	self._iconRes = nil
...
function PopupActiveJoint:_playEffect()
	local function effectFunction(effect)
		...
		if not Lang.checkLang(Lang.CN) and effect == "routine_word_hejijihuo_dazi_zi" then
			print("i18n change effect  font node： routine_word_hejijihuo_dazi_zi" )
			local UIHelper = require("yoka.utils.UIHelper")
			local subLabel = UIHelper.createBMFLabel({text=Lang.getEffectText("effect_fnt_hejijihuo"),fontName = Path.getImgFont("effect")})
			return subLabel
		end
		-- i18n change effect
		if not Lang.checkLang(Lang.CN) and effect == "routine_word_hejijihuo_xiamian_xiaozi" then
			print("i18n change effect  font node： routine_word_hejijihuo_xiamian_xiaozi" )
			local TypeConst = require("app.i18n.utils.TypeConst")
			local UIHelper = require("yoka.utils.UIHelper")
			local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_jihuohejidazhao"),style="effect_text_23",styleType=TypeConst.EFFECT})
			return subLabel
    	end
        ...
        if effect == "shuoming" then
            ...
            if self._contentDes then
				...
				if Lang.checkLang(Lang.CN) then
					...
					richText:setContentSize(cc.size(570, 0))
				end
