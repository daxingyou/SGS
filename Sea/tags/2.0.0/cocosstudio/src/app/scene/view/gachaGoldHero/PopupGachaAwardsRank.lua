...
function PopupGachaAwardsRank:_updateOwnView()
    ...
    if curRank then
        self._ownRank:setString(curRank.rank == 0 and Lang.get("common_text_no_rank") or tostring(curRank.rank))
...
function PopupGachaAwardsRank:_dealPosByI18n()
	if Lang.checkLang(Lang.EN) or  Lang.checkLang(Lang.TH) then
        ...
        self._scrollView:setContentSize(size.width,size.height-17)
        local render = self._textCountDesc:getVirtualRenderer()
        render:setMaxLineWidth(900)

        self._textCountDesc:setPositionY(self._textCountDesc:getPositionY()+10)
        self._textCountDesc:setAnchorPoint(cc.p(0,1))

        --render:setVerticalAlignment( cc.VERTICAL_TEXT_ALIGNMENT_TOP )
	end
end
