...
function CakeActivityView:_initView()
	...
	self._nodeHelp:updateUI(FunctionConst.FUNC_CAKE_ACTIVITY)
	self._buttonShop:updateUI(FunctionConst.FUNC_CAKE_ACTIVITY_SHOP)
	self._buttonGetMaterial:updateUI(FunctionConst.FUNC_CAKE_ACTIVITY_GET_MATERIAL)
...
function CakeActivityView:_updateCountDown()
	...
	if countDown >= 0 then
		...
		if not Lang.checkLang(Lang.CN) then
			self:_updateDownCountPosByI18n()
		end
...
function CakeActivityView:_updateCakeData()
	...
	self._curCakeConfigInfo = CakeActivityDataHelper.getCakeLevelConfig(self._curCakeData:getCake_level())
...
function CakeActivityView:_updateActivityStage()
	...
	self._nodeHelp:setPosition(cc.p(helpPosX, -26))
	self._rank:updateStage()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image = UIHelper.seekNodeByName(self._topbarBase,"ImageTitle")	
		local button = UIHelper.seekNodeByName(self._nodeHelp,"Button")	
		UIHelper.alignRight({image,button})
    end
	if not Lang.checkLang(Lang.CN) then
		self:_updateDownCountPosByI18n()
	end
...
function CakeActivityView:_updateCakeView()
	...
	self._textCakeName:setString(Lang.get("cake_activity_cake_name", {name = self._curCakeData:getGuild_name()}))
	...
	if totalExp == 0 then --最高级时，分母显示上一等级的值
		local info = CakeActivityDataHelper.getCakeLevelConfig(self._curCakeData:getCake_level()-1)
		totalExp = info.exp
	end
...
function CakeActivityView:_fakeUpdateCakeView(itemId)
	-- local configInfo = CakeActivityDataHelper.getCakeLevelConfig(self._fakeLevel)
	...
	-- if totalExp == 0 then --最高级时，分母显示上一等级的值
	-- 	local info = CakeActivityDataHelper.getCakeLevelConfig(self._fakeLevel-1)
	-- 	totalExp = info.exp
	-- end
	...
	local type = CakeActivityConst.getMaterialTypeWithId(itemId)
...
function CakeActivityView:_updateAwardBox()
	...
	if notReceieveId > 0 then
		...
		if cakeLevel < CakeActivityConst.MAX_LEVEL then
			local info = CakeActivityDataHelper.getCakeLevelConfig(cakeLevel+1)
...
function CakeActivityView:_updateMaterial()
	for i = 1, 3 do
		...
		local itemId = CakeActivityConst["MATERIAL_ITEM_ID_"..i]
...
function CakeActivityView:_playMaterialSound(itemId)
	local soundId = CakeActivityConst.getMaterialSoundIdWithId(itemId)
	if soundId > 0 then
...
function CakeActivityView:_onClickAwardBox(state, awardId)
	...
	if state == CakeActivityConst.AWARD_STATE_1 then
		local rewards = {}
		local info = CakeActivityDataHelper.getCakeLevelConfig(awardId)
		local item =
		{
			type = info.award_type,
			value = info.award_value,
			size = info.award_size,
		}
		table.insert(rewards, item)
		local popup = require("app.ui.PopupReward").new(Lang.get("daily_task_box"))
		popup:updateUI(rewards)
		popup:setDetailText(Lang.get("cake_activity_award_preview_tip", {level = self._curCakeData:getCake_level()+1}))
		popup:openWithAction()
	elseif state == CakeActivityConst.AWARD_STATE_2 then
		G_UserData:getCakeActivity():c2sGetGuildCakeUpLvReward(awardId)
	end
end
...
function CakeActivityView:_onEventAddCakeExp(eventName, itemId, itemNum, awards, noticeDatas, addEggLimit)
	...
	self:_updateCakeView()
	if addEggLimit then
		G_Prompt:showTip(Lang.get("cake_activity_add_egg_limit_tip"))
	end
...
-- i18n change lable
function CakeActivityView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self._panelDesign,"Image_1")
	    image1 = UIHelper.swapWithLabel(image1,{
			style = "text_anniversary_2",
			text = Lang.getImgText("txt_anniversary06") ,
			offsetX = 5
		})
		image1:setFontSize(image1:getFontSize()-2)
	end
end

function CakeActivityView:_updateDownCountPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image = UIHelper.seekNodeByName(self._panelDesign,"Image_106")
