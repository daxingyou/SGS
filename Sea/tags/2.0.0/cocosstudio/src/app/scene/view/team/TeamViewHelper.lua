...
function TeamViewHelper.makeLimitDiffData(summary,unitData,lastLimit, finishCallback)

	local nowLimit = unitData:getLimit_level()
	if nowLimit > lastLimit then
		local content = Lang.get("summary_hero_limit_add", {level = nowLimit - lastLimit})
		local param = {
			content = content,
			startPosition = {x = UIConst.SUMMARY_OFFSET_X_TEAM},
