    ...
    local function effectFunction(effect)
        ...
        if stc then
            ...
            if heroData then
                ...
                icon:updateUI(coverId, nil, limitLevel)
            end
            return icon
        ...
        elseif effect == "tianxiawus_spine" then
            ...
            local  covertId, limitLevel = self._userData:getCovertIdAndLimitLevel()
            spine:updateUI(covertId, limitLevel)
            return spine
        end
...
-- i18n pos lable
function SingleRaceChampionLayer:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        local text = UIHelper.seekNodeByName(self._buttonGoMap,"Text_1")
        text:getVirtualRenderer():setLineSpacing(-3)
	end
end
