...
function PopupStarRank:onCreate()
    ...
    if not Lang.checkLang(Lang.CN) then
        ...
        self._listRank:setBounceEnabled(false)
    end
...
function PopupStarRank:_onRankUpdate(item, index)
    ...
    -- i18n bug 
    if not Lang.checkLang(Lang.CN)  then
        if #self._rankData > 0 and self._rankData[index+1] then
            item:refreshInfo(self._rankData[index+1])
        end
    else
    if #rankData > 0 then
...
function PopupStarRank:_setMyInfo()
    ...
    -- self._textMyName:enableOutline(Color.getOfficialColorOutline(officerLevel), 2) 
