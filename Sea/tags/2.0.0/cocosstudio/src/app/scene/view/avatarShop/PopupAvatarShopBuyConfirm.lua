...
-- i18n change lable
function PopupAvatarShopBuyConfirm:_dealI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._imageHeroName:ignoreContentAdaptWithSize(true)

		self._imageNameColor:setVisible(false)
	end
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		local size = self._textDes:getContentSize()
		self._textDes:setContentSize(cc.size(size.width+50,size.height))
		if Lang.checkLang(Lang.TH) then
			self._textDes:getVirtualRenderer():setLineSpacing(8)
		end
	end
end
