...
function GuildHelpView:_updateContent()
	...
	if Lang.checkLang(Lang.TH) then
		local txt = string.gsub( Lang.get("guild_help_tab_title_"..self._selectTabIndex), "\n","")
		self._fullScreen:setTitle(txt)
	end
end
...
-- i18n pos lable
function GuildHelpView:_dealPosByI18n()
    if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local textDesc = UIHelper.seekNodeByName(self._tabGroup,"Text_desc")
        textDesc:setPositionX(textDesc:getPositionX())
	end
end
