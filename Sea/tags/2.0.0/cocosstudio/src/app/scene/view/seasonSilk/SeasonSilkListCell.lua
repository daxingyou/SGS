...
function SeasonSilkListCell:updateUI(data)
    ...
    if data.isWeared then
        ...
        self._wearButton:setString(Lang.get("season_silk_wear"))
        self._wearButton:switchToNormal()
    end
end
...
-- i18n pos lable
function SeasonSilkListCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textName:setFontSize(self._textName:getFontSize()-3)
        self._textName:setPositionX(self._textName:getPositionX())
        --self._textName:setPositionY(self._textName:getPositionY()-2)

        --self._textName:getVirtualRenderer():setMaxLineWidth(140)
       -- self._textName:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_LEFT)
        --self._textName:getVirtualRenderer():setLineSpacing(-4 )

        if Lang.checkLang(Lang.EN) then
            local UIHelper  = require("yoka.utils.UIHelper")
            self._textName:getVirtualRenderer():setMaxLineWidth(185)
            self._textName:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_LEFT )
        end
