...
-- i18n change lable
function AvatarTrainView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		
		local image = UIHelper.seekNodeByName(self._buttonRecovery,"Image_22")
	    UIHelper.swapWithLabel(image,{
			 style = "icon_txt_3",
			 text = Lang.getImgText("txt_main_enter6_recover") ,
			 offsetY = -30,
		})
	end
end
