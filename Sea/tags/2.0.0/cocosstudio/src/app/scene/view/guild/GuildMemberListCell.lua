...
function GuildMemberListCell:_dealPosByI18n()
	...
	if Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.EN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._official:setPositionX(self._official:getPositionX()-8)
		self._name:setPositionX(self._name:getPositionX()+6)
	end
	if Lang.checkLang(Lang.ENID) then
		self._duties:setTextHorizontalAlignment( cc.TEXT_ALIGNMENT_CENTER )
		self._duties:getVirtualRenderer():setMaxLineWidth(100)
	end
