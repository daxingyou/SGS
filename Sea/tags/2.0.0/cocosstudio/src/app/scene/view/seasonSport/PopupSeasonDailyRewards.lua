...
-- i18n change lable
function PopupSeasonDailyRewards:_dealByI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
         local panelCenter  = UIHelper.seekNodeByName(self,"Panel_center")
         local text  = UIHelper.seekNodeByName(self,"Text_15")
         local image  = UIHelper.seekNodeByName(self,"Image_16")
 
         UIHelper.alignCenter(panelCenter,{image,text},{5,0})
         if Lang.checkChannel(Lang.CHANNEL_SEA) then
            local image1  = UIHelper.seekNodeByName(self,"Image_1")
            image1:removeSelf()
            panelCenter:addChild(image1)
            image1:setPosition(293,27)
            text:removeSelf()
            panelCenter:addChild(text)
            text:setAnchorPoint(0.5,0.5)
            text:setPosition(293,27)
        end
    end
end
