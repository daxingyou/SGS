...
function RealNameService:_openAvoidAlert()
    ...
    if not Lang.checkLang(Lang.CN) then
        ...
        -- handler(self, self._reLogin), handler(self, self._reLogin), false)
        -- popupSystemAlert:setCloseVisible(false)
        -- popupSystemAlert:setCheckBoxVisible(false)
        -- popupSystemAlert:showGoButton(Lang.get("avoid_btn_cancel"))
...
function RealNameService:openRealNameDlg()
    ...
    if Lang.checkLang(Lang.VN) then
        self:openRealPhoneDlg(false)
    end
    
    self._showDlg = true
end

function RealNameService:openRealPhoneDlg(isAvoid)
    -- isAvoid = isAvoid or true
    local PopupRealPhone = require("app.scene.view.system.PopupRealPhone").new(isAvoid)
    PopupRealPhone:openWithAction()
end
