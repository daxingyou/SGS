...
function EventHeroNode:_refreshHeroInfo()
    ...
    if not Lang.checkLang(Lang.CN) and not Lang.checkHorizontal() then
        local UIHelper  = require("yoka.utils.UIHelper")
        UIHelper.dealVTextWidget(self._textHeroName,self._heroData.name)
    end 


    if  Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
        local parent = self._textHeroName:getParent()
        local nameBG = parent:getChildByName("NameBG")
        local size = self._textHeroName:getVirtualRenderer():getContentSize()
        local bgSize = nameBG:getContentSize()
        nameBG:setContentSize(bgSize.width,size.width+60)
...
-- i18n change lable
function EventHeroNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		UIHelper.swapWithLabel(image1,{
			 style = "explore_1",
			 text = Lang.getImgText("txt_mumingerlai01") ,
		})
...
-- i18n pos lable
function EventHeroNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local textActTimeLabel = UIHelper.seekNodeByName(self._countDownNode,"TextActTimeLabel")
    
        self._leftTimeLabel:setPositionX(
            textActTimeLabel:getPositionX()+textActTimeLabel:getContentSize().width
        )

        for i = 1, 2 do
            local textYuanContent = self["_yuanPanel"..i]:getSubNodeByName("_textYuanContent")
            textYuanContent:setFontSize(textYuanContent:getFontSize()-4)

            local textYuanTitle = self["_yuanPanel"..i]:getSubNodeByName("_textYuanTitle")
            textYuanTitle:setFontSize(textYuanTitle:getFontSize()-4)
            
            local textName = self["_yuanPanel"..i]:getSubNodeByName("_textName")
            textName:setFontSize(textName:getFontSize()-4)
            textName:setPositionX(textName:getPositionX()-10)

            
        end
        self._btnBuy:setPositionX(self._btnBuy:getPositionX()+6)
        self._priceInfo:setPositionX(self._priceInfo:getPositionX()+6)
        self._iconHero:setPositionX(self._iconHero:getPositionX()-4)
    end
    
    if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
        

        self._textYuanTitle:setPositionX(self._textYuanTitle:getPositionX()-120)
        self._textYuanTitle:setFontSize(18)
        local size = self._textYuanTitle:getContentSize()
        self._textYuanTitle:setContentSize(cc.size(size.width+130,size.height))
        
        
        self._yuanPanel1:setPositionX(self._yuanPanel1:getPositionX()-100)
        self._yuanPanel2:setPositionX(self._yuanPanel2:getPositionX()-100)
        self._iconHero:setPositionX(self._iconHero:getPositionX()-85)
        self._heroDesScrollView:setPositionX(self._heroDesScrollView:getPositionX()-100)
        local size = self._heroDesScrollView:getContentSize()
        self._heroDesScrollView:setContentSize(size.width+100,size.height)
    end

    if  Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
        local parent = self._country:getParent()
        parent:setPositionX(parent:getPositionX()-510)
        
        local nameBG = parent:getChildByName("NameBG")
        nameBG:setRotation(270)
        nameBG:setAnchorPoint(cc.p(0.5,1))
        nameBG:setScale9Enabled(true)
		nameBG:setCapInsets(cc.rect(15,50,6,1))
        nameBG:setPositionX(nameBG:getPositionX()+20)
        nameBG:setPositionY(nameBG:getPositionY()+72)

        

        self._textHeroName:setFontSize(22)
        self._textHeroName:setAnchorPoint(cc.p(0,1))
        self._textHeroName:setPositionX(self._textHeroName:getPositionX()+50)
        self._textHeroName:setPositionY(self._textHeroName:getPositionY()+80)
        
        self._textHeroName:ignoreContentAdaptWithSize(true)
        self._textHeroName:setTextAreaSize(cc.size(0,0))
