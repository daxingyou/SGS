...
function PetActiveShopCell:_showFragmentNum(type, value, index)
	...
	local textFragment = nil
	-- i18n richtext 
	if Lang.checkLang(Lang.CN) then
		textFragment = ccui.RichText:create()
	else
		textFragment = ccui.RichText:createByI18n()
	end
	...
	if Lang.checkLang(Lang.EN) then
		self["_nodeFragment"..index]:setPosition(txtX , txtY - 25)--中文括号空隙比较大
	else
