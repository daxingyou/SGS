...
-- i18n pos lable
function PopupGuildOpenRedPacket:_adjustPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image33 = self._imageGoldIcon --UIHelper.seekNodeByName(self._resourceNode,"Image_33")
        local size = self._resourceNode:getContentSize()
        self._richNode:getChildren()[1]:formatText()
        local size1 = self._richNode:getChildren()[1]:getVirtualRendererSize()
        local size2 = image33:getContentSize()
		self._richNode:setPositionX(size.width*0.5-size2.width*0.5-size1.width*0.5)	
        image33:setPositionX(size.width*0.5+size1.width*0.5)		
	end
end

--i18n
function PopupGuildOpenRedPacket:_dealByI18n()
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
        self._textRedPacketName:setFontSize(self._textRedPacketName:getFontSize()+4)
    end
    if Lang.checkLang(Lang.EN) then
        local render = self._textSnatchFinishHint:getVirtualRenderer()
	    render:setMaxLineWidth(349)
    end
