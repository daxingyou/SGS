...
function HeroListCell:update(heroId1, heroId2)
	local function updateCell(index, heroId)
		if heroId then
			...
			local heroBaseId, isEquipAvatar, avatarLimitLevel = AvatarDataHelper.getShowHeroBaseIdByCheck(data)
			local limitLevel = avatarLimitLevel or data:getLimit_level()
			self["_item" .. index]:updateUI(TypeConvertHelper.TYPE_HERO, heroBaseId)
			self["_item" .. index]:getCommonIcon():getIconTemplate():updateUI(heroBaseId, nil, limitLevel)
			...
			if rank > 0 then
				if params.color == 7 and limitLevel == 0 then -- 金将
