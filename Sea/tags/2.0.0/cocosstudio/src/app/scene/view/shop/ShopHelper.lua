...
-- 是否有需要上新提示的商品
function ShopHelper.isHaveNewRemindShop()
	local ShopConst = require("app.const.ShopConst")
	local shopMgr = G_UserData:getShops()
	local shopId = ShopConst.NORMAL_SHOP
	local subIndex = ShopHelper.convertSubIndexToSubId(shopId, ShopHelper.SHOP_COMMODITY_TAB_INDEX) -- 第二个
	local itemList = shopMgr:getShopGoodsList(shopId, subIndex)
	for i, shopItem in pairs(itemList) do
		if shopItem:getConfig().new_remind == 1 and ShopHelper.isNeedNewRemind(shopItem:getGoodId()) then
			return true
		end
	end
	return false
...
-- 需要上新显示
function ShopHelper.isNeedNewRemind(id)
	local UserSettingData = G_UserData:getUserSetting()
	local data = UserSettingData:getSettingValue("shopnNewRemind" .. G_UserData:getBase():getId()) or {}
	for i = 1, #data do
		if data[i] == id then
			return false
		end
...
function ShopHelper.saveNewRemindShop(itemList)
	local UserSettingData = G_UserData:getUserSetting()
	local data = {}
	...
	UserSettingData:setSettingValue("shopnNewRemind" .. G_UserData:getBase():getId(), data)
