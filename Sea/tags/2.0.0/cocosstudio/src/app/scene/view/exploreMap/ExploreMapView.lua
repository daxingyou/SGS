...
function ExploreMapView:_onDiceFinish()
    self._diceEffect:setVisible(false)
    self._exploreGainEft:startEffect(self._baseAwards, self._crit, handler(self, self._finishTextSummary))
...
function ExploreMapView:_setCheckBoxState()
    ...
    if not isOpen then
        ...
        if Lang.checkLang(Lang.TW) then
            ...
            self._checkBox:setPosition(cc.p(-307,85))
        end
    end
    ...
    if oneKeyOpen then
        -- self._checkBox:setPosition(-306,82.38)
    ...
    local function createEventEffectFunc()
		...
		local function effectFunction(effect)
			...
			elseif effect == "tubiao" then
				...
				return sprite
            elseif not Lang.checkLang(Lang.CN) and effect == "routine_word_chufaqiyu_dazi" then
                -- i18n change effect  font
                print("i18n change effect  font node： routine_word_chufaqiyu_dazi" )
                local UIHelper = require("yoka.utils.UIHelper")
                local subLabel = UIHelper.createBMFLabel({text=Lang.getEffectText("effect_chufaqiyu"),fontName = Path.getImgFont("effect")})
                return subLabel
			elseif effect == "mingzi" then
                -- i18n change punc
                local labelName = nil
                if not Lang.checkLang(Lang.CN) then
                     labelName = cc.Label:createWithTTF("["..discoverData.name.."]", Path.getCommonFont(), 22)
                else
                     labelName = cc.Label:createWithTTF("【"..discoverData.name.."】", Path.getCommonFont(), 22)
                end
...
-- i18n change lable
function ExploreMapView:_swapImageByI18n()
	
	
	if not Lang.checkLang(Lang.CN) and panelDanmu then
		local UIHelper  = require("yoka.utils.UIHelper")

		local image1 = UIHelper.seekNodeByName(self,"Image_2")
		UIHelper.swapWithLabel(image1,{
			style = "homeland_5",
			text = Lang.getImgText("txt_danmukai01") ,
		})


		local image2 = UIHelper.seekNodeByName(panelDanmu,"Image_2_0")
		UIHelper.swapWithLabel(image2,{
			style = "homeland_3",
			text = Lang.getImgText("txt_danmuguan02") ,
		})
...
-- i18n pos lable
function ExploreMapView:_dealPosByI18n()
    if not Lang.checkLang(Lang.CN) then
        local expProgressName = self._expProgress:getParent():getChildByName("expProgressName")
        local exploreProgressName = self._exploreProgress:getParent():getChildByName("exploreProgressName")

        expProgressName:setAnchorPoint(cc.p(1,0.5))
        exploreProgressName:setAnchorPoint(cc.p(1,0.5))

        self._expProgress:setAnchorPoint(cc.p(0,0.5))
        self._exploreProgress:setAnchorPoint(cc.p(0,0.5))

        self._expProgress:setPositionX(self._expProgress:getPositionX()-6)
        self._exploreProgress:setPositionX(self._exploreProgress:getPositionX()-6)
	
		--self._btnRoll:setScale(0.7)
        self._btnRoll:setPositionX(self._btnRoll:getPositionX()+18)
    end
    if Lang.checkLang(Lang.EN) then
        local width = 250
        local UIHelper = require("yoka.utils.UIHelper")
        local bar1 = UIHelper.seekNodeByName(self,"bar1")
        bar1:setContentSize(cc.size(width,bar1:getContentSize().height))
        bar1:setPositionX(-width)
		local bar2 = UIHelper.seekNodeByName(self,"bar2")
        bar2:setContentSize(cc.size(width,bar2:getContentSize().height))
        bar2:setPositionX(-width)
    end
end

-- i18n pos lable
function ExploreMapView:_updatePosByI18n()
    if not Lang.checkLang(Lang.CN) then
        local expProgressName = self._expProgress:getParent():getChildByName("expProgressName")
        local exploreProgressName = self._exploreProgress:getParent():getChildByName("exploreProgressName")

        expProgressName:setPositionX(self._expProgress:getPositionX()-3)
        exploreProgressName:setPositionX(self._exploreProgress:getPositionX()-3)
