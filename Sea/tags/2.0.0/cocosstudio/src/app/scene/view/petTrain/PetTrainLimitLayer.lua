...
function PetTrainLimitLayer:ctor(parentView)
	...
	PetTrainLimitLayer.super.ctor(self, resource)
end
...
function PetTrainLimitLayer:onEnter()
	self._signalPetLimitUpMaterial =
		G_SignalManager:add(SignalConst.EVENT_PET_LIMITUP_MATERIAL_SUCCESS, handler(self, self._onEventEquipLimitUpPutRes))
end
...
function PetTrainLimitLayer:_initAvatar()
	...
	self._petAvatar = avatar
end

function PetTrainLimitLayer:initInfo()
	self:_updateData()
	-- dump(self._petUnitData)
	self:_updateView()
	self:_updateCost()
end
...
function PetTrainLimitLayer:_updatePet()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		UIHelper.alignCenter(self._imageTitleBg,{self._label_1,self._petName })
		UIHelper.alignCenter(self._imageTitleBg,{self._label_2 })
	end
end

function PetTrainLimitLayer:_updateInfoUI()
	local isCan = PetTrainHelper.canLimit(self._petUnitData)
	self._buttonBreak:setVisible(isCan)
	self._nodeSilver:setVisible(isCan)
end

-- 物品消耗返回
function PetTrainLimitLayer:_onEventEquipLimitUpPutRes(id, costKey)
	self:_updateData()
...
function PetTrainLimitLayer:_playLvUpEffect()
	...
	for key = LimitCostConst.LIMIT_COST_KEY_1, LimitCostConst.LIMIT_COST_KEY_2 do
...
function PetTrainLimitLayer:_putResEffect(costKey)
	...
	if self._materialFakeCostCount and self._materialFakeCostCount > 0 then --如果假球已经飞过了，就不再播球了，直接播剩下的特效和飘字
		...
		local curCount = self._petUnitData:getMaterials()[costKey - 1] or 0
...
function PetTrainLimitLayer:_checkIsMaterialFull(costKey)
	local curCount = self._petUnitData:getMaterials()[costKey - 1]
...
function PetTrainLimitLayer:_doPutRes(costKey, materials)
	...
	G_UserData:getPet():c2sPetPostRankUpMaterial(self._petUnitData:getId(), subItem)
	self._costMaterials = materials
end

function PetTrainLimitLayer:_createEmitter(costKey)
	local names = {
		[LimitCostConst.LIMIT_COST_KEY_1] = "tujiepurple",
		[LimitCostConst.LIMIT_COST_KEY_2] = "tujieorange"
	}
...
function PetTrainLimitLayer:_onClickCostPanelStart(costKey, itemId, count)
	...
	self._materialFakeCurSize = materials[costKey - 1] or 0
end
...
function PetTrainLimitLayer:_updateCost()
	...
	for key = LimitCostConst.LIMIT_COST_KEY_1, LimitCostConst.LIMIT_COST_KEY_2 do
		local curCount = materials[key - 1] or 0
...
function PetTrainLimitLayer:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		label_1:setPositionY(self._imageTitle:getPositionY()+20)
		...
		label_2:setPositionY(self._imageTitle:getPositionY()-20)
