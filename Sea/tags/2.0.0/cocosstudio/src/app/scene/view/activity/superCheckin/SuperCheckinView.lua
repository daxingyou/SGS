...
-- i18n change lable
function SuperCheckinView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        self._alreadyGet = UIHelper.swapSignImage(self._alreadyGet,
		{ 
			 style = "signet_8", 
	...
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
		local title = UIHelper.seekNodeByName(self,"title")
		title:ignoreContentAdaptWithSize(true)
		title:setLocalZOrder(1000)
		if Lang.checkChannel(Lang.CHANNEL_SEA) then
			title:setPositionX(title:getPositionX()+20)
		end
	end
end

-- i18n change lable
function SuperCheckinView:_updatePosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text9 = UIHelper.seekNodeByName(self._btnGet,"Text_9")	
		local split = UIHelper.seekNodeByName(self._btnGet,"split")	
		UIHelper.alignCenter(self._btnGet,{text9,self._curSelect,split,self._curSelect2})
	end
end
