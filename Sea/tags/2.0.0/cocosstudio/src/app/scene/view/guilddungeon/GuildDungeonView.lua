...
function GuildDungeonView:_updatePosByI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		if Lang.checkLang(Lang.EN) then
			...
			image1:setContentSize(cc.size(totalWidth + 15,size.height))
			UIHelper.alignCenter(image1,{self._textCountTitle,self._textRemainCount,self._textTimeTitle,self._textTime})
		end
	end
end
