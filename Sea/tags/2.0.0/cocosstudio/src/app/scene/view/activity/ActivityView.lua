...
function ActivityView:_refreshTitle(activityModuleUI)
	...
	if Lang.checkLang(Lang.TH) then
		local txt = string.gsub( TextHelper.expandTextByLen(actCfg.name, 3), "\n","")
		activityModuleUI:setTitle(txt)
	elseif Lang.checkLang(Lang.EN) then
		local txt = string.gsub( TextHelper.expandTextByLen(actCfg.name, 3), "\x01"," ")
		activityModuleUI:setTitle(txt)
	end
end
