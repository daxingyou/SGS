...
function RankUnitCell:updateUI(data, index)
	...
	if rank >= 1 and rank <= 3 then
		self._imageRank:loadTexture(Path.getComplexRankUI("img_qizhi0"..rank))
	...
	elseif rank == 0 then
		...
		self._imageRank:setVisible(true)
		self._imageRank:loadTexture(Path.getComplexRankUI("img_qizhi04"))
...
function RankUnitCell:_dealPosByI18n()
    if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        
         self._textHurt:setPositionX(
             self._textHurt:getPositionX()
        )
      
		self._imageRank:setPositionX(self._imageRank:getPositionX()-7)
		self._textRank:setPositionX(self._textRank:getPositionX()-7)
		self._textGuildName:setPositionX(self._textGuildName:getPositionX()-7)
	end
end
