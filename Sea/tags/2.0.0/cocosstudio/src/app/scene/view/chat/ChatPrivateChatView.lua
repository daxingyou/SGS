...
function ChatPrivateChatView:onCreate()
	...
	if not Lang.checkLang(Lang.CN) then
		self:_createCommonEmptyListNode()
	else
...
function ChatPrivateChatView:_showChatDetailMsg()
    ...
    local richText =  ccui.RichText:createWithContent(Lang.get("chat_private_chat_title", {name = targetPlayerName, color = targetPlayerNameColor}))
...
-- i18n pos lable
function ChatPrivateChatView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text = UIHelper.seekNodeByName(self._nodeClear,"Text")
        text:setFontSize(
        	text:getFontSize()-4
        )

		local text2 = UIHelper.seekNodeByName(self._nodeReturn,"Text")
		text2:setFontSize(text2:getFontSize()-4)
		text2:setPositionY(text2:getPositionY()-3)

		self._textPrivateMsgTitle:setPositionX(self._textPrivateMsgTitle:getPositionX()-5)
	end
end

-- i18n  ex
function ChatPrivateChatView:_createCommonEmptyListNode()
    local CSHelper = require("yoka.utils.CSHelper")
    local widget = CSHelper.loadResourceNode(Path.getCSB("CommonEmptyListNode", "common"))
    widget:setTipsString(Lang.getImgText("common_empty_tip_chat"))
    self._imageWaterFlow:getParent():addChild(widget)
    local size = self._imageWaterFlow:getParent():getContentSize()
	widget:setPosition(size.width*0.5,size.height*0.5)
	widget:setVisible(true)
	self._imageWaterFlow:setVisible(false)
	self._textEmptyListHint:setVisible(false)
    self._textEmptyListHint = widget
end
