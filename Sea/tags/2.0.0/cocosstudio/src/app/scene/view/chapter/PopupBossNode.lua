...
function PopupBossNode:_onFightClick()
    ...
    G_SceneManager:showScene("stage", self._chapterData:getId(), 0, true)
end
...
function PopupBossNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        ...
        if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
            self._txtChapterName:setAnchorPoint(0.5,1)
            self._txtChapterName:setPositionY(95)
            self._txtChapterName:getVirtualRenderer():setMaxLineWidth(200)
            self._txtChapterName:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
            if Lang.checkLang(Lang.TH) then
                self._txtChapterName:getVirtualRenderer():setLineSpacing(6)
                self._txtChapterName:getVirtualRenderer():setLineBreakWithoutSpace(true)
            end
        end
	end
