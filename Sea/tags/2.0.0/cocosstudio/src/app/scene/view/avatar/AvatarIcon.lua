...
function AvatarIcon:_updateCommon(baseId)
	...
	if Lang.checkLang(Lang.TH) then
		...
		UIHelper.dealVTextWidget(self._textName,param.list_name)
	end
...
-- i18n change lable
function AvatarIcon:_dealI18n()

	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")

		self._imageTop = UIHelper.swapSignImage(self._imageTop,
		{ 
			 style = "avatar_1", 
			 text = Lang.getImgText("avatar_in_equip") ,
			 anchorPoint = cc.p(0.5,0.5),
			 rotation = 46,
			 position = cc.p(34,33),
		},Path.getTextSignet("img_iconsign_zhuangbeizhong"))
		
	end
end

function AvatarIcon:_dealPosOfI18n()
	if Lang.checkLang(Lang.KR) or Lang.checkLang(Lang.TW) or Lang.checkLang(Lang.ZH) then
	elseif not Lang.checkLang(Lang.CN)  then
		local renderLabel = self._textName:getVirtualRenderer()
		--renderLabel:setVerticalAlignment()
		--renderLabel:setMaxLineWidth(400)
		self._textName:setFontSize(self._textName:getFontSize()-2)
		self._textName:setTextAreaSize(cc.size(106, 50))
	end
	
end

function AvatarIcon:_dealSeaByI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		...
		self._imageSelected:setPosition(0,0)
		self._imageTop:setPosition(33,56)
		self._textName:setPosition(-3,-50)
		self._textName:setContentSize(cc.size(90,50))
		self._panelTouch:setPosition(-3,-78)
		self._panelTouch:setContentSize(cc.size(130,145))
	end
end
