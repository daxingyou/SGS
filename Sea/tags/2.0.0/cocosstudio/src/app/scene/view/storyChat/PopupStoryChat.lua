...
-- 	PopupStoryChat.AUTO_SKIP_TIME = 60
-- end
...
function PopupStoryChat:_refreshTalkContent()
	...
	if Lang.checkLang(Lang.EN) then
		if self._chatName:getContentSize().height > 24 then
			self._chatName:setFontSize(20)
		else
			self._chatName:setFontSize(24)
		end
	end
...
function PopupStoryChat:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		...
		if Lang.checkLang(Lang.TW) then
			self._chatContent:setContentSize(cc.size(size.width,size.height+20))
		else
		...
		if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
			self._panelChat:setScale9Enabled(true)
			self._panelChat:setCapInsets(cc.rect(315,55,1,1))
		end
...
-- i18n pos lable
function PopupStoryChat:_updateSizeI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local richSize = self._richTextNode:getChildren()[1]:getContentSize()
		local newSizeHeight = richSize.height + 60
		if newSizeHeight < 150 then
			newSizeHeight = 150
		end

		local size = self._chatContent:getContentSize()

		self._chatContent:setAnchorPoint(cc.p(0.5,1))
		self._chatContent:setPositionY(newSizeHeight - 38)
		if Lang.checkLang(Lang.TH) then
			self._chatContent:setPositionY(newSizeHeight - 44)
		end
		self._chatContent:setContentSize(cc.size(size.width,richSize.height))
		local size1 = self._panelChat:getContentSize()
		self._panelChat:setContentSize(cc.size(size1.width,newSizeHeight))

		
		self._chatName:setPositionY(newSizeHeight - 18)
