...
-- i18n pos lable
function PopupGuildMemberInfo:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		
		
		
		local text0 = ccui.Helper:seekNodeByName(self._panelBg, "Text0")
		local text00 = ccui.Helper:seekNodeByName(self._panelBg, "Text0_0")
		local text01 = ccui.Helper:seekNodeByName(self._panelBg, "Text0_1")
	    local text02 = ccui.Helper:seekNodeByName(self._panelBg, "Text0_2")
        self._textPower:setPositionX(text02:getPositionX()+text02:getContentSize().width)
		self._textLevel:setPositionX(text0:getPositionX()+text0:getContentSize().width)
		self._textDuties:setPositionX(text00:getPositionX()+text00:getContentSize().width)
		self._textContribution:setPositionX(text01:getPositionX()+text01:getContentSize().width)

		text0:setFontSize(text0:getFontSize()-2)
		text00:setFontSize(text00:getFontSize()-2)
		text01:setFontSize(text01:getFontSize()-2)
		text02:setFontSize(text02:getFontSize()-2)

		self._textPower:setFontSize(self._textPower:getFontSize()-2)
		self._textLevel:setFontSize(self._textLevel:getFontSize()-2)
		self._textDuties:setFontSize(self._textDuties:getFontSize()-2)
		self._textContribution:setFontSize(self._textContribution:getFontSize()-2)

		if Lang.checkLang(Lang.ENID) then
			text01:setPositionX(text00:getPositionX())
			text00:setPositionX(290)
			text02:setPositionX(290)
			self._textDuties:setPositionX(text00:getPositionX()+text00:getContentSize().width)
			self._textContribution:setPositionX(text01:getPositionX()+text01:getContentSize().width)
			self._textPower:setPositionX(text02:getPositionX()+text02:getContentSize().width)
		end
		if 	Lang.checkLang(Lang.EN) then
			self._textDuties:getVirtualRenderer():setMaxLineWidth(100)
			self._textDuties:setAnchorPoint(0,1)
			self._textDuties:setPositionY(62)
		end
	end
end
