	...
	for i, value in ipairs(self._shopInfoList) do
		if value.default_create ~= 0 then
			if Lang.checkLang(Lang.EN)  then
				...
				table.insert(tabNameList, UIHelper.dealVText(value.shop_name))
			else
				table.insert(tabNameList, value.shop_name)
			end
		end
	end
...
-- i18n change lable
function ShopView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self,"Node_Mid","Image_2")
...
-- i18n change lable
function ShopView:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text_desc = UIHelper.seekNodeByName(self._tabGroup1,"Text_desc")
