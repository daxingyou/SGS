...
function RecoveryView:onCreate()
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local size = self._scrollViewTab:getContentSize()
		self._scrollViewTab:setContentSize(cc.size(146,size.height))
	end
	...
	self._buttonShop:setPositionX(self._panelDesign:getContentSize().width*87/100)
...
function RecoveryView:_initTab()
	...
	for index = 1, #recoveryData do
		...
		self["_nodeTabIcon"..index]:updateUI(index == #recoveryData, index, recoveryData[index],#recoveryData)
...
-- i18n pos lable
function RecoveryView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		--local node1 = UIHelper.seekNodeByName(self._scrollViewTab,"Node_1")
