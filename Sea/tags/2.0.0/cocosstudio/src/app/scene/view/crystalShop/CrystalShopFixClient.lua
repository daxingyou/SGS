...
-- i18n change lable
function CrystalShopFixClient:_dealPosByI18n()
	if Lang.checkLang(Lang.TH) then 
		self._countdownTime:setPositionX(self._countdownTime:getPositionX() - 15)
	end
	if Lang.checkLang(Lang.EN) then 
		self._countdownTime:setPositionX(self._countdownTime:getPositionX() - 70)
		local UIHelper  = require("yoka.utils.UIHelper")
		local textRefreshTime = UIHelper.seekNodeByName(self,"Text_Refresh_time")
		local textFreeDesc = UIHelper.seekNodeByName(self,"Text_free_desc")
		local posx = textFreeDesc:getPositionX()+textFreeDesc:getContentSize().width + 10
		textRefreshTime:setPositionX(posx)
	end
end
