...
function GuildDungeonEnemyItemNode:update(data,index)
    ...
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
        if index % 2 == 0 then
            self._imageBg:loadTexture(Path.getUICommon("img_com_board_list02b"))
        else
            self._imageBg:loadTexture(Path.getUICommon("img_com_board_list02a"))
        end
    else
...
function GuildDungeonEnemyItemNode:_dealPosByI18n()
