...
function CustomActivityFundsView:onCreate()
    ...
    if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
        self._textActivedDesc:setVisible(false)
    end
end
...
function CustomActivityFundsView:_initScrollTopView()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		local _,currencyStr1 = UIHelper.convertCurrency( cfg.rmb)
		local currencyStr1 = UIHelper.convertDollar(currencyStr1)
        self._btnActive:setString(Lang.get("funds_active", {rmb = currencyStr1}))
	else
...
function CustomActivityFundsView:_initListBottomView()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		local _,currencyStr1 = UIHelper.convertCurrency( cfg.rmb)
		local currencyStr1 = UIHelper.convertDollar(currencyStr1)
        self._btnActiveV2:setString(Lang.get("funds_active", {rmb = currencyStr1}))
	else
...
function CustomActivityFundsView:_update(dt)
    if self._countDownTime > 0 then
        local timeText = G_ServerTime:getTimeStringDHM(self._countDownTime)
        if Lang.checkChannel(Lang.CHANNEL_SEA) then
            timeText = G_ServerTime:_secondToString(self._countDownTime-G_ServerTime:getTime())
        end
        if self._curFundsSigned then
            self._textActivedCountDown:setString(Lang.get("funds_actived_countdown") ..timeText)
            self._textCountDownV2:setString(Lang.get("funds_actived_countdown") ..timeText)
        else
            self._textActivedCountDown:setString(Lang.get("funds_active_countdown") ..timeText)
            self._textCountDownV2:setString(Lang.get("funds_active_countdown") ..timeText)
        end
    end
end

-- i18n change lable
function CustomActivityFundsView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        for index = 1, 5 do 
            self["_imgGot"..index] = UIHelper.swapSignImage( self["_imgGot"..index],
            { 
                 style = "signet_8", 
        ...
        self._imagePurchased = UIHelper.swapSignImage(self._imagePurchased,
		{ 
			 style = "signet_8", 
        ...
        self._imagePurchasedV2 = UIHelper.swapSignImage(self._imagePurchasedV2,
		{ 
			 style = "signet_8", 
			 text = Lang.getImgText("img_yigoumai02") ,
			 anchorPoint = cc.p(0.5,0.5),
...
function CustomActivityFundsView:_dealPosByI18n()
    if not Lang.checkLang(Lang.CN) then
        for index = 1, 5 do 
            self["_labelName"..index]:setFontSize(self["_labelName"..index]:getFontSize()-2)
            local size = self["_labelName"..index]:getContentSize()
            if Lang.checkLang(Lang.TH) then
                local render = self["_labelName"..index]:getVirtualRenderer()
                render:setLineSpacing(6)
                render:setMaxLineWidth(115)
                self["_labelName"..index]:ignoreContentAdaptWithSize(true)
                self["_labelName"..index]:setAnchorPoint(0.5,1)
                self["_labelName"..index]:setPositionY(150)
            elseif Lang.checkLang(Lang.EN) then
                self["_labelName"..index]:setPositionY(self["_labelName"..index]:getPositionY()-6)
                self["_labelName"..index]:setContentSize(cc.size(size.width + 26,size.height+ 20))
            else
                self["_labelName"..index]:setContentSize(cc.size(size.width + 30,size.height ))
            end
        end
    end
    if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
        self._textActivedCountDown:setFontSize(self._textActivedCountDown:getFontSize()-2)
    end
    if  Lang.checkChannel(Lang.CHANNEL_SEA) then
        self._textCountDownV2:setPositionX(300)
        self._textCountDownV2:setAnchorPoint(0.5,0.5)
    end
end
