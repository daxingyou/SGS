...
function PopupHurtRewardCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        ...
        if Lang.checkChannel(Lang.CHANNEL_SEA) then
            textHurt:setPositionY(textHurt:getPositionY()-5)
            self._textHurtNum:setPositionY(self._textHurtNum:getPositionY()-5)
            self._btnGet:setPositionY(self._btnGet:getPositionY()+10)
        end
    end
end
