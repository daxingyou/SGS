...
--i18n
function PopupHorseDetail:_dealByI18n()
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
        local UIHelper  = require("yoka.utils.UIHelper")
        local image = UIHelper.seekNodeByName(self,"Image_194")
		image:setAnchorPoint(0.5,0)
		image:setPositionX(314)
    end
end
function PopupHorseDetail:_updateByI18n()
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
        local UIHelper  = require("yoka.utils.UIHelper")
        local image = UIHelper.seekNodeByName(self,"Image_194")
		local size = image:getContentSize()
		local width = self._textSuit:getContentSize().width+80
		image:setContentSize(cc.size(width,size.height))
    end
end
