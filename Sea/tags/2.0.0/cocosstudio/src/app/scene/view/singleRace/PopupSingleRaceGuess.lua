...
function PopupSingleRaceGuess:_onItemTouch(index, t)
	...
	if answerId == SingleRaceConst.GUESS_TAB_TYPE_1 then
		...
		content = Lang.get("single_race_guess_confirm_desc"..answerId, {name = name, color = color})
	else
...
--更换位置
function PopupSingleRaceGuess:dealPosByI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.ZH) or Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.ENID) then
		...
		self._textCountDownTitle:setPositionX(self._textCountDownTitle:getPositionX()- 10)
	...
	if Lang.checkLang(Lang.ENID) then
		local img1 = ccui.Helper:seekNodeByName(self._imageBar1, "Image_13_0")
		img1:setPositionX(img1:getPositionX()-25)
		local img2 = ccui.Helper:seekNodeByName(self._imageBar1, "Image_13_0_0")
		img2:setPositionX(img2:getPositionX()+25)
	end
end
