...
function CarnivalActivityExchangeCell:_updateRewards(actTaskUnitData)
    ...
    if not Lang.checkLang(Lang.CN) then
        self._commonItemListExchangeNode:setGaps(CarnivalActivityExchangeCell.ITEM_GAP,
            ...
            152,
            CarnivalActivityExchangeCell.ITEM_MID_GAP,512)
	else
        self._commonItemListExchangeNode:setGaps(CarnivalActivityExchangeCell.ITEM_GAP,
            CarnivalActivityExchangeCell.ITEM_ADD_GAP,
            CarnivalActivityExchangeCell.ITEM_OR_GAP,
            CarnivalActivityExchangeCell.ITEM_MID_GAP,400)
	end
...
-- i18n change lable
function CarnivalActivityExchangeCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
...
-- i18n pos lable
function CarnivalActivityExchangeCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        self._nodeProgress:setPositionX(self._nodeProgress:getPositionX()+80)
	end
end
