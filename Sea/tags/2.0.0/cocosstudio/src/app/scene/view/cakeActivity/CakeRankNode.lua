...
function CakeRankNode:updatePlayerRank()
	...
	if rankData then
		...
		self._textMyScore:setString(rankData:getPoint())
		...
		self._textMyScore:setString(Lang.get("cake_activity_no_level"))
...
-- i18n change lable
function CakeRankNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local imgSize = self._imageGuildTip:getContentSize()
		self._imageGuildTip = UIHelper.swapWithLabel(self._imageGuildTip,{
			style = "text_anniversary_1",
			text = Lang.getImgText("txt_anniversary03") ,
		})
		if Lang.checkLang(Lang.EN) or  Lang.checkLang(Lang.TH) then
			self._imageGuildTip:setPositionY(self._imageGuildTip:getPositionY()+20)
			self._imageGuildTip:getVirtualRenderer():setMaxLineWidth(320)
			self._imageGuildTip:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )

			local labelSize = self._imageGuildTip:getVirtualRenderer():getContentSize()
			local addHeight = labelSize.height -(imgSize.height-5)
			addHeight = math.max(addHeight,0)
			local size = self._listViewGuild:getContentSize()
			self._listViewGuild:setContentSize(cc.size(size.width,size.height-addHeight))
			self._listViewGuild:setPositionY(self._listViewGuild:getPositionY()+addHeight)
		end
		
	end
	if Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.ENID) then
		self._imageArrow:ignoreContentAdaptWithSize(true)
	end
end

-- i18n change lable
function CakeRankNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text = UIHelper.seekNodeByName(self._target,"NodePlayer","ImageBgPlayer","Text_21_0_0")
		self._textMyScore:setPositionX(self._textMyScore:getPositionX()+25)
		
		text:setAnchorPoint(cc.p(1,0.5))
		text:setPositionX(self._textMyScore:getPositionX()-3)


		local text = UIHelper.seekNodeByName(self._target,"NodePlayer","ImageBgPlayer","Text_21_1")
		text:setAnchorPoint(cc.p(0,0.5))
		text:setPositionX(text:getPositionX()-49)

	
		self._textMyRank:setAnchorPoint(cc.p(0,0.5))
		self._textMyRank:setPositionX(text:getPositionX()+text:getContentSize().width+3)

		

		local text = UIHelper.seekNodeByName(self._target,"NodeGuild","ImageBgGuild","Text_21_0")
		self._textGuildScore:setPositionX(self._textGuildScore:getPositionX()+25)

		text:setAnchorPoint(cc.p(1,0.5))
		text:setPositionX(self._textGuildScore:getPositionX()-3)


		local text = UIHelper.seekNodeByName(self._target,"NodeGuild","ImageBgGuild","Text_21")
		text:setAnchorPoint(cc.p(0,0.5))
		text:setPositionX(text:getPositionX()-49)


		self._textGuildRank:setAnchorPoint(cc.p(0,0.5))
		self._textGuildRank:setPositionX(text:getPositionX()+text:getContentSize().width+3)
