...
function EquipmentListCell:update(equipId1, equipId2)
	local function updateCell(index, equipId)
		if equipId then
			...
			local heroBaseId = UserDataHelper.getHeroBaseIdWithEquipId(data:getId())
			if heroBaseId then
...
function EquipmentListCell:_dealPosI18n()
	if not Lang.checkLang(Lang.CN) then
		self._textHeroName1:setFontSize(
			self._textHeroName1:getFontSize()-4
		)
		self._textHeroName2:setFontSize(
			self._textHeroName2:getFontSize()-4
		)
		
		self._textHeroName1:setAnchorPoint(cc.p(1,0.5))
		self._textHeroName1:setPositionX(self._textHeroName1:getPositionX()+70)


		self._textHeroName2:setAnchorPoint(cc.p(1,0.5))
