...
function QinTombMonsterAvatar:_didEnterReborn()
	...
	--logWarn("QinTombMonsterAvatar ----------------  _didEnterReborn")
	...
	if not Lang.checkLang(Lang.CN) then
		leftTime = math.max(0.001, time -  curTime)
		--logWarn("QinTombMonsterAvatar ---------------- Reborn callback0 "..tostring(time).." "..tostring(curTime).." "..tostring(leftTime))
		seq = cc.Sequence:create(
			cc.CallFunc:create(playDieAction), 
			cc.FadeOut:create(dieAction),
			cc.CallFunc:create(playDieActionFinish),

			cc.DelayTime:create( math.max(0.001,leftTime-dieAction) ),
			cc.CallFunc:create(callback) 
		)
