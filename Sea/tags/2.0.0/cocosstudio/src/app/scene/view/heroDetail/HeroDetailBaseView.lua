...
function HeroDetailBaseView:ctor(heroId, heroBaseId, rangeType)
	if heroId then
...
function HeroDetailBaseView:_updateInfo()
	...
	self._fileNodeHeroName:setName(heroBaseId, rank, limitLevel)
	self._fileNodeHeroName2:setName(heroBaseId, rank, limitLevel, true)
...
--i18n
function HeroDetailBaseView:_dealByI18n()
	if Lang.checkHorizontal() then
		self._fileNodeFeature:setPosition(self._fileNodeHeroName:getPositionX(),self._fileNodeHeroName:getPositionY()-40)
		self._fileNodeStar:setPositionY(self._fileNodeStar:getPositionY()-30)
	end
end
