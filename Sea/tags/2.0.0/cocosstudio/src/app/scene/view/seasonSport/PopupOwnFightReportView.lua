...
-- i18n pos lable
function PopupOwnFightReportView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local text152 = UIHelper.seekNodeByName(self,"Panel_bottom","Text_15_2")
        self._textSeasonFightNum:setPositionX( text152:getPositionX()+text152:getContentSize().width * 0.5 + 2)
        if Lang.checkLang(Lang.TH) then
            local text151 = UIHelper.seekNodeByName(self,"Panel_center","Text_15_1")
            text151:setPositionX( text151:getPositionX() + 2.5)
        end
        if Lang.checkLang(Lang.ENID) then
            local text1520= UIHelper.seekNodeByName(self,"Panel_bottom","Text_15_2_0")
            local wid = text1520:getVirtualRendererSize().width
            text1520:setAnchorPoint(1,0.5);
            text1520:setPositionX(text1520:getPositionX()+90)
            self._textSeasonFightRate:setPositionX(text1520:getPositionX()+5)
        end
	end
end
