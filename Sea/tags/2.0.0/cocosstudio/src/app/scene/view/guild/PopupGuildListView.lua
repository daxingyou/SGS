...
function PopupGuildListView:_updateInfo(index)
	...
	if Lang.checkLang(Lang.EN) then 
		self._textDeclaration:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_LEFT)
	end
	if Lang.checkLang(Lang.TH) then
		self._textDeclaration:getVirtualRenderer():setLineSpacing(8)
	end
...
function PopupGuildListView:_dealPosByI18n()
    if Lang.checkLang(Lang.EN) then 
        local UIHelper  = require("yoka.utils.UIHelper")
        local text = UIHelper.seekNodeByName(self,"Text_6_2")
        text:setPositionX(text:getPositionX()+20)
	end
	if Lang.checkLang(Lang.TH) then
		self._textDeclaration:setAnchorPoint(0.5,1)
		self._textDeclaration:setPositionY(202)
		self._textDeclaration:setContentSize(cc.size(255,190))
	end
end
