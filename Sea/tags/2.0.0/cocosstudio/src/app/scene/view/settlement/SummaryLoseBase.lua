...
function SummaryLoseBase:_createContinueNode()
    ...
    continueNode:setPosition(cc.p(275 - width*0.5, 25 - height*0.5))
    ...
    elseif Lang.checkLang(Lang.TH) then
        continueNode:setPositionX(continueNode:getPositionX()+120)
    end
...
function SummaryLoseBase:_createLosePic()
    ...
    if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
        ...
        local img = display.newSprite(Path.getBattleFont("txt_com_battle_f01"))
    ...
    -- i18n change text
    if not Lang.checkLang(Lang.CN) then
        return display.newSprite(Path.getBattleFont("txt_com_battle_f01"))
    end
