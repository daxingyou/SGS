...
-- i18n change lable
function CustomActivityWelfareView:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._textTimeTitle:setFontSize(self._textTimeTitle:getFontSize()-4)
		self._textTime:setFontSize(self._textTime:getFontSize()-2)
	
	
		if  Lang.checkChannel(Lang.CHANNEL_SEA) then
			-- self._textTimeTitle:setAnchorPoint(cc.p(1,1))
			-- self._textTimeTitle:setPositionX(self._textTime:getPositionX() - 5)
			self._textTime:setPositionX(self._textTimeTitle:getPositionX()+self._textTimeTitle:getContentSize().width+3)
			self._textTimeTitle:setPositionY(self._textTimeTitle:getPositionY() - 2)
		 end
	end
