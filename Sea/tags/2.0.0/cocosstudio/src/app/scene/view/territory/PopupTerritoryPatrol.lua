...
function PopupTerritoryPatrol:_onClickPatrol(sender)
	...
	if self._cityState == TerritoryConst.STATE_ADD then
		if self._chooseHeroId and self._chooseHeroId > 0 then
			...
			local buyTimesAlert = Lang.get("lang_territory_partol_alert", {itemCost = itemCost, color1 = Colors.colorToNumber(typeItem.icon_color), time = hour})
...
--刷新HeroAvatar
function PopupTerritoryPatrol:_updateHeroAvatar(heroBaseId)
	...
	local function updateCommonHeroAvatar(heroBaseId, talkMsg)
		...
		self._commonAvatar:updateUI(heroBaseId)
	...
	if self._cityState == TerritoryConst.STATE_COUNTDOWN or self._cityState == TerritoryConst.STATE_RIOT then
		...
		self._commonNpcAvatar3:updateUI(heroBaseId)
...
function PopupTerritoryPatrol:_onEventTerritoryClickHero(eventName, heroId)
	...
	if heroId and heroId > 0 then
		...
		self:_updateHeroAvatar(heroUnit:getBase_id())
		...
		local richText = Lang.get("lang_territory_patrol_hero_desc", {
			...
			heroColor = Colors.colorToNumber(Colors.getColor(config.color))})
		...
		if Lang.checkLang(Lang.ENID) then
			...
			widget:setContentSize(cc.size(550,0))
			widget:formatText()
		end
...
function PopupTerritoryPatrol:_stateNone()
	...
	--i18n
	self:_updateDescI18n(self._cityData.cfg.directions)
	self._textStateDes:setVisible(false)
...
function PopupTerritoryPatrol:_updateEventLabel( label, eventCfg, serverData )
	...
	for i,content in ipairs(contents) do
		...
		if text == "hero" then
			color = Colors.colorToNumber(Colors.getColor(heroData.color))
			text = heroData.name
...
function PopupTerritoryPatrol:_onEventRiotHelper(id, message)
	...
	if not Lang.checkLang(Lang.CN) then
		G_Prompt:showTip(Lang.getImgText("territory_ask_help_success"))
	else
...
-- i18n change lable
function PopupTerritoryPatrol:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then

		local UIHelper  = require("yoka.utils.UIHelper")
		local textCountDown = UIHelper.seekNodeByName(self._nodeHeroBottom,"Text_countDown")
		local sizeCd = textCountDown:getContentSize()
		local sizeTitle = self._textHeroBottom:getContentSize()
	

		if textCountDown:isVisible() then	
			textCountDown:setPositionX(240 -(sizeTitle.width + sizeCd.width)  *  0.5 + sizeTitle.width )
			self._textHeroBottom:setPositionX(240 -(sizeTitle.width + sizeCd.width)  *  0.5 + sizeTitle.width*0.5   )
		else
			self._textHeroBottom:setPositionX(240)
		end
...
-- i18n change lable
function PopupTerritoryPatrol:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		--for i=1, 3 do
		--	self["_costRes"..i]:setPositionX(self["_costRes"..i]:getPositionX()+23)
		--end
		self._textPowerDesc:setAnchorPoint(cc.p(1,0.5))
		self._textPowerDesc:setPositionX(self._textPower:getPositionX()-2)

		self._listViewLog:setPositionX(
			self._listViewLog:getPositionX()
		)
		local size = self._listViewLog:getContentSize()
		self._listViewLog:setContentSize(size.width,size.height)
	end
	if Lang.checkLang(Lang.TH) then
		for i=1, 3 do
			self["_costResTxt"..i]:setPositionX(self["_costResTxt"..i]:getPositionX()+40)
			self["_costResNum"..i]:setPositionX(self["_costResNum"..i]:getPositionX()+10)
		end
	end
	if Lang.checkLang(Lang.EN) then
		local textProcess = UIHelper.seekNodeByName(self._nodeRoitTime,"Text_Process")
		local textRoitTime = UIHelper.seekNodeByName(self._nodeRoitTime,"Text_RoitTime")
		UIHelper.alignCenter(self._btnPatrolClick,{textProcess,textRoitTime},{3,0})
	end
...
--i18n
function PopupTerritoryPatrol:_updateDescI18n(strDesc)
...
--i18n
function PopupTerritoryPatrol:_initDescI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		local size = self._textCityDesc:getContentSize()
		...
		local label = cc.Label:createWithTTF("", Path.getCommonFont(), self._textCityDesc:getFontSize())
		label:setColor(self._textCityDesc:getColor())
		label:setMaxLineWidth(size.width)
		label:setAnchorPoint(0,1)
		label:setLineSpacing(10)
		self._textDesTrue = label

		self._textCityDesc:getParent():addChild(sc)
		sc:setAnchorPoint(self._textCityDesc:getAnchorPoint())
		sc:setPosition(cc.p(self._textCityDesc:getPosition()))
		self._textCityDesc:setVisible(false)
		sc:addChild(label)
		self._textDescSc = sc
	end
end
