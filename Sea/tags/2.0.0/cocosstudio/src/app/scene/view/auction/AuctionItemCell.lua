...
function AuctionItemCell:updateUI( index, data )
    ...
    self._commonResInfo1:updateUI(TypeConvertHelper.TYPE_RESOURCE, DataConst.RES_DIAMOND, currPrice)
    self._commonResInfo1:setTextColorToATypeColor()

    local finalPrice = data:getFinal_price()
    self._commonResInfo2:updateUI(TypeConvertHelper.TYPE_RESOURCE, DataConst.RES_DIAMOND, finalPrice)
...
-- i18n pos lable
function AuctionItemCell:_dealPosByI18n()
    if not Lang.checkLang(Lang.CN) then
        self._textTime:setPositionY(self._textTime:getPositionY()+6)
        local x,y = self._textTime:getPosition()
        self._textTimeDesc:setPositionX(x)
		self._textTimeDesc:setAnchorPoint(cc.p(0.5,0.5))
        self._textTimeDesc:getVirtualRenderer():setMaxLineWidth(100)
        self._textTimeDesc:getVirtualRenderer():setLineSpacing(-7)
        

        self._textItemName:getVirtualRenderer():setMaxLineWidth(150)
        self._textItemName:getVirtualRenderer():setLineSpacing(-7)

        self._imageDesc:setPositionY(self._imageDesc:getPositionY()-3)

        if  Lang.checkLang(Lang.EN) or  Lang.checkLang(Lang.TH) then
            self._textItemName:setFontSize(18)
            self._textItemName:getVirtualRenderer():setLineSpacing(-3)
        end
	end
end
