...
function HeroDetailAttrModule:_normalLayout()
	...
	local canLimit = self._heroUnitData:isCanLimitBreak() and not isGold
	local limitIsOpen = LogicCheckHelper.funcIsOpened(FunctionConst.FUNC_HERO_TRAIN_TYPE4)
	...
	if showAwake then
		...
		table.insert(dynamicBtns, {btn = self._buttonAwake, openLevel = funcLevelInfo.level})
	end
	if showLimit then
		showCount = showCount + 1
		...
		table.insert(dynamicBtns, {btn = self._buttonLimit, openLevel = funcLevelInfo.level})
	end
