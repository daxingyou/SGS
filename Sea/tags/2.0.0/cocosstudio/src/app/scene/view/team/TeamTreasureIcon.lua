...
function TeamTreasureIcon:updateIcon(pos, slot)
	...
	if not isOpen then
		...
		if Lang.checkLang(Lang.EN) then
			...
			UIHelper.dealVTextWidget(self._textOpenLevel,self._textOpenLevel:getString())
			self._textOpenLevel:setPositionY(24)
		end
		return
	end 
...
function TeamTreasureIcon:onlyShow(slot, data)
	self:_initUI()
...
function TeamTreasureIcon:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._fileNodeName:setFontSize(24)
	...
	if Lang.checkLang(Lang.EN) then
		local img = self._fileNodeName:getSubNodeByName("Image_1")
...
function TeamTreasureIcon:_updateTextNameI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		...
		label:setTextAreaSize(cc.size(160,80))
		label:ignoreContentAdaptWithSize(false)
		label:setPositionY(12)
		if Lang.checkLang(Lang.TH) then
			label:setTextAreaSize(cc.size(300,80))
		end
	end
end
