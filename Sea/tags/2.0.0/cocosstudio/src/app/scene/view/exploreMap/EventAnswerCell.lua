...
-- i18n pos lable
function EventAnswerCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        self._textAnswer:setFontSize(
            self._textAnswer:getFontSize()-2
        )
    
    end
    if Lang.checkLang(Lang.EN) then
        self._textAnswer:getVirtualRenderer():setMaxLineWidth(125)
		self._textAnswer:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
    end
end
