...
function PopupHeroDetail:ctor(type, value, isPage, limitLevel)
	self._type = type
	self._value = value
	self._isPage = isPage
...
function PopupHeroDetail:_updateHeroInfo(heroBaseId, limitLevel)
	self._heroBaseId = heroBaseId
	self._fileNodeCountry:updateUI(heroBaseId)
	self._fileNodeCountryFlag:updateUI(TypeConvertHelper.TYPE_HERO, heroBaseId)

	self._detailWindow:updateUI(nil, heroBaseId, nil, limitLevel)
	
	local haveHero = G_UserData:getKarma():isHaveHero(heroBaseId)
...
function PopupHeroDetail:_updateHeroItem(sender, widget, index, selectPos)
	...
	if self._avatarPageItems then
		...
		if avatarItem then
			...
			if avatarCount == 0 then
				...
				avatar:updateUI(heroBaseId, nil, nil, limitLevel)
				avatar:setScale(1.4)
	...
	if self._storyPageItems then
		...
		if storyItem then
			...
			if storyCount == 0 then
				...
				story:updateUI(heroBaseId, limitLevel)
				story:setScale(0.8)
	...
	if selectPos == index then
		if self._curSelectedPos ~= selectPos then
			...
			self._curSelectedPos = selectPos
			self:_updateHeroInfo(heroBaseId, limitLevel)
		end
	end
end
