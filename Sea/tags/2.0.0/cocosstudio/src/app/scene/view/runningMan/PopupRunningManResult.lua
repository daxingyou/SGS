...
function PopupRunningManResult:updateUI( ... )
	...
	if runningList and #runningList > #self._runningList then
		local newNum = #runningList
		local oldNum = #self._runningList
		self._runningList = runningList
...
function PopupRunningManResult:_playAnimation(startIndex, endIndex)
	...
	for i = startIndex, endIndex do
		...
		if cellWidget then
			...
			cellWidget:updateUI(self._runningList[i])
			cellWidget:playAnimation()
			self._runEndList[self:getHeroId(runningHero)]  = runningHero
		end
	end
...
function PopupRunningManResult:_createLabelByI18n()
	...
	elseif not Lang.checkLang(Lang.CN)  then
		...
		if Lang.checkLang(Lang.TH) then
			local posX = 400
			lable1:setPositionX(posX)
			self._raceNum:setAnchorPoint(0,0.5)
			self._raceNum:setPositionX(image1:getPositionX()-image1:getContentSize().width/2+posX+lable1:getContentSize().width/2+10)
		end
	end
...
function PopupRunningManResult:_updateTitleByI18n()
	if not Lang.checkLang(Lang.CN) and not Lang.checkLang(Lang.TH) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self,"Image_title")
