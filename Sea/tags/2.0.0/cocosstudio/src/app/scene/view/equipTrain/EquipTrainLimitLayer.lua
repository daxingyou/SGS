...
function EquipTrainLimitLayer:_playFire(isPlay)
    ...
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
        -- self._buttonDetail:setVisible(false)
...
function EquipTrainLimitLayer:_updateItem()
    ...
    --i18n change function show
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
        -- if config.suit_id == LimitCostConst.MAX_SUIT_ID then
        --     equipBaseId = self._unitData:getBase_id()
        -- end
    end
...
function EquipTrainLimitLayer:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        ...
        if Lang.checkLang(Lang.TH) then
            label_2:setPositionY(label_2:getPositionY()-10)
        end
    end
end
