...
function GuildMainView:_createMapView()
	...
	for i = 1,GuildBuildPostion.length(),1 do
		...
		if Lang.checkLang(Lang.TW) and config.id == GuildConst.CITY_CONTRIBUTION_ID then
			local bossCfg = GuildBuildPostion.get(GuildConst.CITY_BOSS_ID)
			if bossCfg then
				local posY = bossCfg.postion_y + 1
				icon:setLocalZOrder(GuildMainView.ZORDER_CITY-posY)
			end
		end
