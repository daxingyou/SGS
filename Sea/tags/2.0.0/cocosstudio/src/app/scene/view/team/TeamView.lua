...
function TeamView:_initView()
	...
	self._buttonPet:updateUI(FunctionConst.FUNC_PET_HOME)
	self._buttonPet:addClickEventListenerEx(handler(self, self._onClickButtonPet))
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local textImage = ccui.Helper:seekNodeByName(self._buttonPet, "TextImage")
		textImage:setPositionY(textImage:getPositionY()+5)
	elseif not Lang.checkLang(Lang.CN) then
		local textImage = ccui.Helper:seekNodeByName(self._buttonPet, "TextImage")
		textImage:setPositionY(textImage:getPositionY()+14)
	end
	self:_initLeftIcons()
	self:_initPageView()
	self:_initPetNode()
...
function TeamView:_updateHeroPageView()
	...
	for i = minIndex, maxIndex do
		...
		self._pageItems[i]:updateUI(info.type, info.value, info.isEquipAvatar, info.limitLevel)
	end
...
function TeamView:setNeedPopupTreasureReplace(showRP)
	local layer = self:getHeroLayer()
	if layer then
		layer:setNeedPopupTreasureReplace(need)
	end
end
