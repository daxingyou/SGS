...
function PointRankView:onCreate()
    ...
    if not Lang.checkLang(Lang.CN) then
        ...
        --self._textOwnName:setPositionX(self._textOwnName:getPositionX()+27)
        --self._textOwnPoint:setPositionX(self._textOwnPoint:getPositionX()+15)
        self._textOwnPoint:setFontSize(self._textOwnPoint:getFontSize()-2)
         --self._textOwnRank:setPositionX(self._textOwnRank:getPositionX() + 25)
         --self._imageRankBack:setPositionX(self._imageRankBack:getPositionX() + 25 )
...
function PointRankView:_initPosition( ... )
    ...
    self._newBtnRankPos = cc.p(64, self._oriBtnRankPosition.y)
end
...
function PointRankView:_updateTab( ... )
    ...
    local index = (leftTime <= 0 and 2 or 1)
...
function PointRankView:_updateOwnInfo(index, point)
    ...
    if not Lang.checkLang(Lang.CN) then
        fontSize = (type(rank) == "number" and 20 or 14)
        if type(rank) == "number" then
            self._imageRankBack:setVisible(true)
            self._textOwnRank:setPositionX(self._imageRankBack:getPositionX() +self._imageRankBack:getContentSize().width * 0.5 - (rank == 1 and 3 or 2 )  )
        else
            self._textOwnRank:setPositionX(self._imageRankBack:getPositionX() +self._imageRankBack:getContentSize().width * 0.5  + 2)
            self._imageRankBack:setVisible(false)
        end
...
-- i18n change lable
function PointRankView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local image1 = UIHelper.seekNodeByName(self, "Image_157")
		UIHelper.swapWithLabel(image1,{
			 style = "countryboss_2",
			 text = Lang.getImgText("txt_boss_jifen01")
		})
        
        self._imageRank = UIHelper.swapWithLabel(self._imageRank,{
            style = "countryboss_2",
            text = Lang.getImgText("txt_boss_jifen02"),
     
       })
       self._imageRank:setAnchorPoint(cc.p(1,0.5))
       self._imageRank:setPositionX(self._newBtnRankPos.x- self._btnRank:getContentSize().width*0.5-2)
