...
-- i18n change lable
function EventHalfPriceCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self,"DiscountBG","ImageDiscount")
		UIHelper.swapWithLabel(image1,{ 
			 style = "text_explore_1", 
			 text = Lang.getImgText("txt_banjiawuzi02") ,
		})
    end
...
-- i18n change pos
function EventHalfPriceCell:_dealPosByI18n()
    if Lang.checkChannel(Lang.CHANNEL_SEA ) then
        self._item:setPositionY(self._item:getPositionY()+7)
    end
end
