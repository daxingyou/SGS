...
function FightUI:ctor()
    ...
    -- i18n
    self:addEnterBackgroundListener()
end
...
function FightUI:onCreate()
    ...
    if not Lang.checkLang(Lang.CN) then
        self._totalNum:setPosition(138+10, 30)
    else
    ...
    if not Lang.checkLang(Lang.CN) then
        self._totalHeal:setPosition(138+10, 30)
    else
...
function FightUI:updateTotalHurt(val, type)
    ...
    local action2 = cc.CallFunc:create(function() self._showLabel:setNumber(val) end)
...
-- i18n 新增参数
function FightUI:playSkillAnim(camp, anim, petId, color,petSkillName)
    ...
    if node then 
        local function effectFunction(effect)
            ...
            elseif not Lang.checkLang(Lang.CN) and effect == "routine_word_shenshou_zi_sb" then 
                local UIHelper = require("yoka.utils.UIHelper")
                local subLabel = UIHelper.createBMFLabel({text=petSkillName,fontName = Path.getImgFont("effect"),fontSize = 26})
                return subLabel
            end
        end
...
function FightUI:playHistoryAnim(hisCamp, hisId, skillShowId)
    ...
    if node then 
        ...
        local function eventFunction(event)
            if event == "skill" then 
                fightSignalManager:dispatchSignal(FightSignalConst.SIGNAL_HISTORY_BUFF)
            elseif event == "finish" then 
                fightSignalManager:dispatchSignal(FightSignalConst.SIGNAL_HISTORY_SHOW_END)
            end
        end
...
function FightUI:pauseFight()
    ...
    if self.enterBackgroudPopupAlert == true then
        return
    end
    local sceneName = G_SceneManager:getRunningSceneName()
    if sceneName == "fight" and not G_TutorialManager:isDoingStep() then
        self.enterBackgroudPopupAlert = true
        local sharedScheduler = cc.Director:getInstance():getScheduler()
        local currentTimeScale = sharedScheduler:getTimeScale()
        local callbackResume = function ()
            self.enterBackgroudPopupAlert = false
            sharedScheduler:setTimeScale(currentTimeScale)
        end
        local callbackQuit = function ()
            self.enterBackgroudPopupAlert = false
            sharedScheduler:setTimeScale(currentTimeScale)
            self:_onJumpTouch()
        end
        sharedScheduler:setTimeScale(0)
        local PopupAlert = require("app.ui.PopupAlert")
        local alert = PopupAlert.new(nil, Lang.getImgText("txt_fight_pause"), callbackResume, callbackQuit)
        alert:setBtnStr(Lang.get("common_msg_push_and_goon"),Lang.get("season_suspend_offline_title"))
        alert:openToOfflineLevel()
        alert:setCloseVisible(false)
        return true
    end
end
-- i18n extend
function FightUI:addEnterBackgroundListener()
    local listener = cc.EventListenerCustom:create("APP_ENTER_BACKGROUND_EVENT", function()
        self:pauseFight()
    end)
