...
function ReplayVSNode:_refreshHeros()
    local upHeros = self._replay:getLeft_heros()
    for i, v in pairs(upHeros) do 
        self:_refreshSingleHero(1, i, v)
    end

    local downHeros = self._replay:getRight_heros()
...
function ReplayVSNode:_refreshSingleHero(camp, pos, heroId)
...
-- i18n change lable
function ReplayVSNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		--local UIHelper  = require("yoka.utils.UIHelper")
		--self._imageCount = UIHelper.swapWithLabel(self._imageCount,{style = "camp_race_2"})
	end
end

-- i18n change lable
function ReplayVSNode:_swapPowerImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
