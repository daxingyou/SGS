    ...
    elseif index == 0 then
		...
		if  not Lang.checkLang(Lang.CN) then
			local function _effectFunction(_effect)
				if _effect == "routine_word_jingjijishi_zhenrongsuoding" then
                    local TypeConst = require("app.i18n.utils.TypeConst")
                    local UIHelper = require("yoka.utils.UIHelper")
                    local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_zhenrongsuoding"),style="effect_text_16",styleType=TypeConst.EFFECT})
                    return subLabel
				end
				return cc.Node:create()
			end	
			local effect = G_EffectGfxMgr:createPlayMovingGfx(self._nodeFight, "moving_effect_jingjijishi_suoding", _effectFunction, nil , true)
        else
...
function CampRacePreMatch:_playWinAnim(isWin)
	...
	if Lang.checkLang(Lang.CN) then
		...
		local function _effectWinFunction(_effect)
			if _effect == "routine_word_win_2" then
				local TypeConst = require("app.i18n.utils.TypeConst")
				local UIHelper = require("yoka.utils.UIHelper")
				local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_shengli"),style="effect_text_27_1",styleType=TypeConst.EFFECT})
				return subLabel
			end
		end	
		local function _effectLoseFunction(_effect)
			if _effect == "routine_word_shibai_9" then
				local TypeConst = require("app.i18n.utils.TypeConst")
				local UIHelper = require("yoka.utils.UIHelper")
				local subLabel = UIHelper.createLabel({text=Lang.getEffectText("effect_shibai"),style="effect_text_27_2",styleType=TypeConst.EFFECT})
				return subLabel
			end
		end	
		
		if isWin then
			G_EffectGfxMgr:createPlayMovingGfx(self._nodeFight, "moving_jingji_shengli", _effectWinFunction, nil , true)
		else
			G_EffectGfxMgr:createPlayMovingGfx(self._nodeFight, "moving_jingji_jinjishibai", _effectLoseFunction, nil , true)
		end	
...
--i18n
function CampRacePreMatch:_dealByI18n()
	if Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.EN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text33 = UIHelper.seekNodeByName(self._panelDesign,"Text_33")
		text33:setPositionX(text33:getPositionX()+20)
	end
end
