...
function PopupTerritoryRiotHelpCell:_updateCell()
	...
	self:updateImageView("Image_official_title", {texture = Path.getOfficialImg(officialInfo.picture)})

	
	if not Lang.checkLang(Lang.CN) then
		self:_updateNamePosByI18n()
	end
end
...
-- i18n change lable
function PopupTerritoryRiotHelpCell:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._fileNodeReward:setPositionX(	self._fileNodeReward:getPositionX() + 80 )

		local imageOfficialTitle = UIHelper.seekNodeByName(self._resourceNode,"Node_mid","Panel_title","Image_official_title")
		imageOfficialTitle:ignoreContentAdaptWithSize(true)
		imageOfficialTitle:setPositionX(imageOfficialTitle:getPositionX()-7)

		self._commonButton:setPositionX(self._commonButton:getPositionX()+12)
	end

	
end

function PopupTerritoryRiotHelpCell:_updateNamePosByI18n()
	if Lang.checkLang(Lang.TH) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local imageOfficialTitle = UIHelper.seekNodeByName(self._resourceNode,"Node_mid","Panel_title","Image_official_title")
		self._textPlayerName:setPositionX(imageOfficialTitle:getPositionX()+imageOfficialTitle:getContentSize().width+3)
	end
end
