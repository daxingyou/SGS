...
function PopupSeasonSilk:_onBtnModifyName(sender)
	self._commonNodeBk:setCloseVisible(false)
	local PopupSilkModifyName = require("app.scene.view.seasonSilk.PopupSilkModifyName").new(self._silkGroupInfo[self._curGroupPos],
	 																							handler(self, self._updateGroupName))
	PopupSilkModifyName:openWithAction()
	PopupSilkModifyName:setCurGroupName(self._curGroupName)
	PopupSilkModifyName:setCloseCallBack(handler(self, self._onEventShowCloseBtn))
end
...
function PopupSeasonSilk:_onGroupItemTouch(index, data)
	if data.state == SeasonSilkConst.SILK_GROUP_SATE_LOCK then
		...
		if success then
			...
			local popup = require("app.ui.PopupAlert").new(title, content, callbackOK, callbackCancel, callbackExit)
			popup:openWithAction()
		else
			popFunc()
		end
...
-- i18n change lable
function PopupSeasonSilk:_dealI18n()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		local textTitle = UIHelper.seekNodeByName(self,"Panel_Left","Text_Title")
		textTitle:setFontSize(textTitle:getFontSize()-2)
	end
end
