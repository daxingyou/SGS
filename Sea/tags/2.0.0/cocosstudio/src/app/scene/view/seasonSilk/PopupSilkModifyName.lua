...
function PopupSilkModifyName:onCreate()
	...
	if not Lang.checkLang(Lang.CN)  then
		...
		local maxLen = UIHelper.getInputLimitedConfig().silk_bag
		...
		self._editBox = InputUtils.createInputView(
		{
				...
				placeholder = Lang.get("season_silk_equip_namelength",{value = 10}),
				maxLength = maxLen,
			}
		)
...
-- i18n change lable
function PopupSilkModifyName:_dealI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textHint:setString(Lang.getImgText("season_sport_skillbag_change_name_hint"))
	end
end
