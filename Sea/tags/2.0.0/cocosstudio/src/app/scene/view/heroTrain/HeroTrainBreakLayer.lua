...
function HeroTrainBreakLayer:_updatePageItem()
	...
	for i = index-1, index+1 do
		if i >= 1 and i <= #allHeroIds then
			...
			self._pageItems[i].avatar1:updateUI(heroBaseId, nil, nil, limitLevel)
			self._pageItems[i].avatar2:updateUI(heroBaseId, nil, nil, limitLevel)
		end
	end
end
...
function HeroTrainBreakLayer:_updateShow()
	...
	self._fileNodeHeroNew:updateUI(heroBaseId, nil, nil, limitLevel)
	...
	if self._isReachLimit then
		...
		self._fileNodeHeroOld:updateUI(heroBaseId, nil, nil, limitLevel)
		...
		if heroRankConfig then
			...
			if Lang.checkLang(Lang.TH) then
				local size = self._imageTalentBg:getContentSize()
				local width = self._textTalentTitle:getContentSize().width+self._textTalentName:getContentSize().width+50
				self._imageTalentBg:setContentSize(cc.size(width,size.height))
				UIHelper.alignCenter(self._imageTalentBg,{self._textTalentTitle,self._textTalentName})
			end
			...
			if Lang.checkLatinLanguage()  then
				...
				label:setContentSize(cc.size(334+60,0))
			elseif Lang.checkLang(Lang.TW) then
				label:setContentSize(cc.size(560,0))
			else
				label:setContentSize(cc.size(334,0))
			end
			if Lang.checkLang(Lang.EN) then
				label:setContentSize(cc.size(554,0))
