...
function PopupMailReward:onCreate()
	...
	self._commonTitle:setTitle(self._title)
...
function PopupMailReward:_updateMailDetailView(mailInfo)
	local refreshRichFunc = function(richNode,richText)
		...
		if Lang.checkChannel(Lang.CHANNEL_SEA) and richNode == self._nodeExpiredTime then
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		...
		expiredTimeRichText = UIHelper.changeRichTextFontSize(expiredTimeRichText,18)
		fromRichText = UIHelper.changeRichTextFontSize(fromRichText,18)
	end
...
-- i18n pos lable
function PopupMailReward:_dealTextI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text = UIHelper.seekNodeByName(self._nodeAttachment,"Text")
		text:setString(Lang.getImgText("mail_attachment") )
	end

end

-- i18n pos lable
function PopupMailReward:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textMailTitle:setFontSize(self._textMailTitle:getFontSize()-2)
	end

	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image_27 = UIHelper.seekNodeByName(self._nodeMailDetail,"Image_27")
		local image_27_1 = UIHelper.seekNodeByName(self._nodeMailDetail,"Image_27_1")
		image_27:setPositionY(image_27:getPositionY()-30)
		image_27_1:setPositionY(image_27_1:getPositionY()-30)

		self._nodeExpiredTime:setPosition(35,448)
		self._listView2:setPositionY(self._listView2:getPositionY()-30)
		local listSize = self._listView2:getContentSize()
		self._listView2:setContentSize(cc.size(listSize.width,listSize.height-30))

		if Lang.checkLang(Lang.TH) then
			self._textMailTitle:setFontSize(28)
		else
			self._textMailTitle:setFontSize(24)
		end
		
		self._textSendTime:setFontSize(18)
	end
end
