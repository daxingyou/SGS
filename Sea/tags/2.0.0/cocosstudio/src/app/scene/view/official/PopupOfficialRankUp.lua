...
-- i18n change lable
	...
	if not Lang.checkLang(Lang.CN) then
		...
		local textTitle = UIHelper.seekNodeByName(self._commonNodeBk,"Text_title")
		self._commonHelp:setAnchorPoint(cc.p(0.5,0.5))
		UIHelper.alignCenter(self._commonNodeBk,{textTitle,self._commonHelp},{5,0},{nil,32})
	end
...
function PopupOfficialRankUp:_updateRes()
	...
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local label1 = UIHelper.seekNodeByName(self._resItem1,"Text_cost_item1")
		local image = UIHelper.seekNodeByName(self._resItem1,"Image")
		local text = UIHelper.seekNodeByName(self._resItem1,"Text")
		UIHelper.alignCenter(self._btnUp,{label1,image,text})
	end
...
-- i18n change lable
function PopupOfficialRankUp:_dealI18n()
	if Lang.checkLang(Lang.KR) or Lang.checkChannel(Lang.CHANNEL_SEA) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local label1 = UIHelper.seekNodeByName(self._resItem1,"Text_cost_item1")
		local label2 = UIHelper.seekNodeByName(self._resItem2,"Text_cost_item2")
		local label3 = UIHelper.seekNodeByName(self._resItem3,"Text_cost_item3")

		label1:setAnchorPoint(cc.p(1,0.5))
		label2:setAnchorPoint(cc.p(1,0.5))
		label3:setAnchorPoint(cc.p(1,0.5))

		label1:setPositionX(-10)
		label2:setPositionX(- 10)
		label3:setPositionX(- 10)


		
		--self._resItem1:setPositionX()
		self._resItem2:setPositionX(20+label2:getContentSize().width + 10)
		self._resItem3:setPositionX(20+label2:getContentSize().width + 10)
	...
	elseif not Lang.checkLang(Lang.CN) then
		...
		label1:setPositionX(- label1:getContentSize().width-2)
		label2:setPositionX(- label2:getContentSize().width-2)
		label3:setPositionX(- label3:getContentSize().width-2)


		self._resItem2:setPositionX(label2:getContentSize().width )
		self._resItem3:setPositionX(label3:getContentSize().width )
	end
	if Lang.checkLang(Lang.ENID) then
		self._resItem2:setPositionX(240)
		self._resItem3:setPositionX(240)
	end
end
