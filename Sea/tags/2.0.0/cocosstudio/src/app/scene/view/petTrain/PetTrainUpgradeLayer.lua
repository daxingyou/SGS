...
function PetTrainUpgradeLayer:_updateData()
	...
	self._limitExp = UserDataHelper.getPetNeedExpWithLevel(G_UserData:getBase():getLevel() , templet)
...
function PetTrainUpgradeLayer:_updateLoadingBar(withAni)
	...
	local petConfig = self._petUnitData:getConfig()
	local templet = self._petUnitData:getLvUpCost()
	local needCurExp = UserDataHelper.getPetLevelUpExp(level, templet)
	local nowExp = self._petUnitData:getExp() - UserDataHelper.getPetNeedExpWithLevel(level,templet)
...
function PetTrainUpgradeLayer:_fakeUpdateView(itemId)
	...
	self._fakeLevel = UserDataHelper.getPetCanReachLevelWithExp(self._fakeCurExp, templet)
	...
	--进度条
	local needCurExp = UserDataHelper.getPetLevelUpExp(self._fakeLevel, templet)

	local nowExp = self._fakeCurExp - UserDataHelper.getPetNeedExpWithLevel(self._fakeLevel ,templet)
	local percent = nowExp / needCurExp * 100
	self._loadingBarExp:setPercent(percent)
	self._textExpPercent1:updateTxtValue(nowExp)
	self._textExpPercent2:setString("/"..needCurExp)
...
function PetTrainUpgradeLayer:_getUpgradeMaterials(level)
	...
	local targetExp = UserDataHelper.getPetNeedExpWithLevel(targetLevel ,templet)
