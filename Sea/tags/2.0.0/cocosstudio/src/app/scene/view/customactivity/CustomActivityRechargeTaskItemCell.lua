	...
	if not notShowProgress then
		if onlyShowMax then
			...
			if not Lang.checkLang(Lang.CN) then
				...
				local _,currency =  UIHelper.convertCurrency(value02)
				richText = Lang.get("customactivity_task_progress_03",{title = progressTitle,max = currency})
			end
...
-- i18n change lable
function CustomActivityRechargeTaskItemCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
