...
function PopupMineSweepTotalCell:_updateReportInfo()
	...
	self._armyBar1:updateUI(myArmy, reportData:getSelf_dec_army(), false)
	self._armyBar2:updateUI(tarArmy, reportData:getTar_dec_army(), true)
end
...
function PopupMineSweepTotalCell:updateTotal(win, lose, selfDie, tarDie, selfArmy, tarArmy, selfRedArmy, tarRedArmy)
...
-- i18n change lable
function PopupMineSweepTotalCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image = ccui.Helper:seekNodeByName(self._resourceNode, "Image_20") -- UIHelper:seekNodeByName(self._resourceNode, "Image_20")
		 UIHelper.swapWithLabel(image,{
			 style = "mine_2",
			 text = Lang.getImgText("img_mine_bt01") ,
		})

		self._textTotal:setFontSize(
			self._textTotal:getFontSize()-2
		)
		self._textTotal:setPositionY(
			self._textTotal:getPositionY()+16
		)
	end
end

-- i18n pos lable
function PopupMineSweepTotalCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textDie1:setFontSize(self._textDie1:getFontSize()-2)
		self._textDie2:setFontSize(self._textDie2:getFontSize()-2)
	end
end
