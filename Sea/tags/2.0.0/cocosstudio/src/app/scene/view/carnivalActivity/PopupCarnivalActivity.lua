...
function PopupCarnivalActivity:_refreshTitleAndOther()
	...
	if not Lang.checkLang(Lang.CN)  then
		self._title:loadTexture(Path.getFullPath(titleConfig.res_id))--标题图片
	else
...
function PopupCarnivalActivity:_updateLeftTabItem(tabItem)
	...
	if stageData then
		if Lang.checkChannel(Lang.CHANNEL_SEA) then
			local name = string.gsub( stageData:getName(), "\\n", "\n")
			tabItem.textWidget:setString(name)
		else
...
function PopupCarnivalActivity:_getActivityModuleUI(actUnitdata)
	...
	if activityModuleUI == nil then
		...
		elseif actType ==  CustomActivityConst.CUSTOM_ACTIVITY_TYPE_SELL then
			...
			activityModuleUI = CarnivalActivityExchangeLayer.new(actType)
...
-- i18n change lable
function PopupCarnivalActivity:_swapImageByI18n()
	--self._imageBg1:setPosition(568,320)
	--self._tabGroup:setPosition(100.2514,159.39)
	if not Lang.checkLang(Lang.CN) then
