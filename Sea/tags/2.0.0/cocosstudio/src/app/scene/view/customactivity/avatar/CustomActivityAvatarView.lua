...
function CustomActivityAvatarView:_initView()
	...
	if itemParams.res_mini then
		self._imageCostIcon1:loadTexture(itemParams.res_mini)
		self._imageCostIcon2:loadTexture(itemParams.res_mini)

		local getCosRes = CustomActivityAvatarHelper.getCosRes(self._activityUnitData:getBatch())
		self._costNum1:setString(getCosRes.size)
	end
...
function CustomActivityAvatarView:_onBtnFive()
	...
	G_UserData:getAvatarActivity():c2sAvatarActivity(2)
end
...
function CustomActivityAvatarView:_onBtnOne()
	...
	self._parentView:pauseUpdateTopBar()
	G_UserData:getAvatarActivity():c2sAvatarActivity(1)
end
...
-- i18n change lable
function CustomActivityAvatarView:_dealI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textTime:setPositionX(self._textTime:getPositionX()+12)

		local image37 = UIHelper.seekNodeByName(self._countDownNode,"Image_37")
		image37:setPositionX(image37:getPositionX()-10)
		self._textTimeTitle:setPositionX(self._textTimeTitle:getPositionX()-10)
	end
end
-- i18n change lable
function CustomActivityAvatarView:_updatePosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image36 = UIHelper.seekNodeByName(self._countDownNode,"Image_36")
		local image37 = UIHelper.seekNodeByName(self._countDownNode,"Image_37")
		UIHelper.alignCenter(image36,{image37,self._textTimeTitle,self._textTime})
		
		local UIHelper  = require("yoka.utils.UIHelper")
		local image11 = UIHelper.seekNodeByName(self._freeNode,"Image_11")
		UIHelper.alignCenter(image11,{self._freeText,self._freeNum})

		local image2 = UIHelper.seekNodeByName(self._oneCostNode,"Image_11")
		local text1 = UIHelper.seekNodeByName(self._oneCostNode,"Text_7")
		local text2 = UIHelper.seekNodeByName(self._oneCostNode,"Text_7_0")
		UIHelper.alignCenter(image2,{text1,self._imageCostIcon1,text2,self._costNum1})
		if Lang.checkLang(Lang.TH) then
			local nodeCost = UIHelper.seekNodeByName(self,"NodeCost")
			local image11 = UIHelper.seekNodeByName(nodeCost,"Image_11")
			local text7 = UIHelper.seekNodeByName(nodeCost,"Text_7")
			local text70 = UIHelper.seekNodeByName(nodeCost,"Text_7_0")
			UIHelper.alignCenter(image11,{text7,self._imageCostIcon2,text70,self._costNum2})
		end
	end
end
