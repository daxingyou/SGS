...
function RedPacketRainView:_initData()
	...
	self._minPosX = G_ResolutionManager:getBangOffset() + 100 --红包飘落起始点X坐标随机范围最小值
	self._maxPosX = G_ResolutionManager:getBangDesignWidth() - 100 --红包飘落起始点X坐标随机范围最大值
...
function RedPacketRainView:_updatePreCountDown()
    ...
    if countDownTime > 0 then
    	self._textCount:setString(countDownTime)
    else
    	self:_stopPreCountDown()
...
function RedPacketRainView:_updateRain()
	...
	local posX = math.random(self._minPosX, self._maxPosX)
	local startPos = cc.p(posX, 590)
	local unitData = self._packetList[self._packetIndex]
	local node = RedPacketNode.new(unitData)
	if node then
		node:setPosition(startPos)
		self:addChild(node)
		local dropTime = DROPTIME[unitData:getRedpacket_type()] or 2.0
...
function RedPacketRainView:_popupSettlement()
	...
	local popup = PopupRedRainSettlement.new(data, handler(self, self._onClickQuit))
...
-- i18n change lable
function RedPacketRainView:_dealPosByI18n()
	if  Lang.checkLang(Lang.VN) then
		self._textCountDown:setAnchorPoint(cc.p(1,0.5))
		self._textCountDown:setPositionX(self._textCountDown:getPositionX()+4)
	end
end
