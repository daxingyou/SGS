...
function CakeActivityShopCell:update(goodId1, goodId2)
...
-- i18n change lable
function CakeActivityShopCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		for i = 1,2 do
			local seasonOffsetY = self["_textDes"..i]:getPositionY()
			self["_textDes"..i]:setFontSize(self["_textDes"..i]:getFontSize()-2)
			self["_textDes"..i]:setPosition(cc.p(118, seasonOffsetY - 20)) 
			self["_textDes"..i]:setAnchorPoint(cc.p(0.0, 0.5))
			if Lang.checkLang(Lang.TH) then
				self["_textName"..i]:setPositionY(self["_textName"..i]:getPositionY()+3)
