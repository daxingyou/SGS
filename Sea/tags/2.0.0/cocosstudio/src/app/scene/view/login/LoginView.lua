...
function LoginView:onCreate()
    ...
    if Lang.checkLang(Lang.VN) or Lang.checkLang(Lang.TW) or Lang.checkChannel(Lang.CHANNEL_SEA) then
        self._textCopyrightInfo1:setString("")
        self._textCopyrightInfo2:setString("")
        self._nodeAgreementContent:setVisible(false)
        local AgreementSetting = require("app.data.AgreementSetting")
    ...
    elseif Lang.checkLang(Lang.KR) then
        self._textCopyrightInfo1:setPositionY(self._textCopyrightInfo1:getPositionY()-8)
        self._textCopyrightInfo1:setFontSize(self._textCopyrightInfo1:getFontSize()+4)
    end

    if Lang.checkLang(Lang.VN) then
        self._labelVersion:setColor(cc.c3b(0x5d,0x38,0x0d) )
    end
end
    ...
    G_ConfigManager.signal:addOnce(
        ...
        if event == "success" then
            ...
            if not Lang.checkLang(Lang.CN) then
                ...
                UIPopupHelper.popupOkDialog(nil,"加载配置失败，请确保您的网络连接畅通！（" .. opId .. "-" .. opgameId .. "）",callback)
            end
        end
    end)
    print("freshConfig")
...
function LoginView:checkAppVersion()
    ...
    if r == Version.LATEST then
        ...
        if appVersionUrl ~= nil and appVersionUrl ~= "" and appVersionType ~= nil and appVersionType ~= "" then
            ...
            if Lang.checkLang(Lang.CN) then
                UIPopupHelper.popupOkDialog(nil,"APP更新出错了",function ()
...
function LoginView:_onClickPrivacyAgreement(sender)
    ...
    elseif Lang.checkLang(Lang.KR) then
        G_NativeAgent:openURL("http://www.gamepub.co.kr/terms/igs.html")
    end
...
function LoginView:_onClickServiceAgreement(sender)
    ...
    elseif Lang.checkLang(Lang.KR) then
        G_NativeAgent:openURL("http://www.gamepub.co.kr/terms/common_term_181017.html")
    end
end
