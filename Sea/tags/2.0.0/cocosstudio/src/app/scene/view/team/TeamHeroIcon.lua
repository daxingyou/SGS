...
function TeamHeroIcon:updateIcon(type, value, funcId, limitLevel)
	self:_hideAllWidget()
...
function TeamHeroIcon:_createCommonIcon(type, value, limitLevel)
	local icon = nil
...
function TeamHeroIcon:onlyShow(type, value, limitLevel)
	self:_hideAllWidget()
	if type > 0 and value > 0 then
		self._isOpen = true
		self:_createCommonIcon(type, value, limitLevel)
