	...
	for i = index-1, index+1 do
		if i >= 1 and i <= #allHeroIds then
			...
			if self._pageItems[i].avatar == nil then
				...
				avatar:setScale(1.4)
				avatar:setPosition(cc.p(self._pageViewSize.width / 2, 192))
			...
			local heroBaseId, isEquipAvatar, avatarLimitLevel = AvatarDataHelper.getShowHeroBaseIdByCheck(unitData)
			local limitLevel = avatarLimitLevel or unitData:getLimit_level()
			self._pageItems[i].avatar:updateUI(heroBaseId, nil, nil, limitLevel)
		end
	end
end
...
function HeroTrainAwakeLayer:_updateShow()
	...
	self._fileNodeHeroName:setName(heroBaseId, rankLevel, limitLevel)
	self._fileNodeHeroName2:setName(heroBaseId, rankLevel, limitLevel)
...
--宝石回调
function HeroTrainAwakeLayer:_onGemstoneCallback(slot)
	local tempSlot = {slot}
...
function HeroTrainAwakeLayer:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self._buttonPreview,"Image_2")
...
function HeroTrainAwakeLayer:_dealPosByI18n()
	...
	if Lang.checkLang(Lang.EN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local textLevel = UIHelper.seekNodeByName(self,"Panel_156","TextLevel")
		textLevel:setFontSize(18)
		self._textOldLevel:setFontSize(18)
		self._textNewLevel:setFontSize(18)
		textLevel:setPositionX(textLevel:getPositionX()-55)
		self._textOldLevel:setPositionX(self._textOldLevel:getPositionX()-55)
		for i = 1, 4 do
			self["_fileNodeAttr"..i]:getSubNodeByName("TextCurValue"):setFontSize(18)
			self["_fileNodeAttr"..i]:getSubNodeByName("TextNextValue"):setFontSize(18)
			self["_fileNodeAttr"..i]:getSubNodeByName("TextAddValue"):setFontSize(18)
			self["_fileNodeAttr"..i]:getSubNodeByName("TextName"):setFontSize(18)
			self["_fileNodeAttr"..i]:setPositionX(self["_fileNodeAttr"..i]:getPositionX()-50)
			self["_fileNodeAttr"..i]:setNextValueGap(53)
			local posX = self["_fileNodeAttr"..i]:getSubNodeByName("TextCurValue"):getPositionX()
			self["_fileNodeAttr"..i]:getSubNodeByName("TextCurValue"):setPositionX(posX+48)
		end
	end
	if Lang.checkLang(Lang.TH) then
		self._textDesc:setFontSize(self._textDesc:getFontSize()-1)
		self._textDesc:getVirtualRenderer():setLineSpacing(9)
		self._textLevel:setPositionY(self._textLevel:getPositionY()+2)
		self._textDesc:setPositionY(self._textDesc:getPositionY()-4)
	end
end
