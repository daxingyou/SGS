...
-- i18n pos lable
function PopupSiegeRank:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local nodeMyRank = UIHelper.seekNodeByName(self,"NodeMyRank")
		local rankTitle = UIHelper.seekNodeByName(nodeMyRank,"RankTitle")
		rankTitle:setAnchorPoint(cc.p(0,0.5))
		rankTitle:setPositionX(rankTitle:getPositionX()-70)
		self._textMyRank:setPositionX(
             rankTitle:getPositionX()+rankTitle:getContentSize().width
        )
        self._myRankIcon:setPositionX(
             rankTitle:getPositionX()+rankTitle:getContentSize().width+49
        )
	end
	if Lang.checkLang(Lang.ENID) then
		self._rewardTitle:setPositionX(self._rewardTitle:getPositionX()-50)
	end
end
