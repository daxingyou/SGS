...
function ChatMiniNode:_onCreate()
	...
	if not G_ConfigManager:isVoiceOpen() then
		self._buttonWorld:setVisible(false)
		self._buttonGuild:setVisible(false)
	end
end
...
function ChatMiniNode:_refreshRedPoint()
    ...
    local showRedPoint = redPointHelper.isModuleReach(FunctionConst.FUNC_CHAT)
...
-- i18n change lable
function ChatMiniNode:_swapImageByI18n()
	
	local UIHelper  = require("yoka.utils.UIHelper")

	if not Lang.checkLang(Lang.CN)  then
		
		local panelDanmu = UIHelper.seekNodeByName(self._target,"PanelDanmu")
		if panelDanmu then

			local image1 = UIHelper.seekNodeByName(panelDanmu,"Image_2")
			...
			image2 = UIHelper.swapWithLabel(image2,{
				style = "system_3",
				text = Lang.getImgText("txt_danmuguan02") ,
			})
			if Lang.checkLang(Lang.ENID) then
				image1:setFontSize(image1:getFontSize()-5)
				image2:setFontSize(image2:getFontSize()-5)
			end
		end
	

		
		local image3 = UIHelper.seekNodeByName(self._target,"ButtonGuild","Image_1")
		image3 = UIHelper.swapWithLabel(image3,{
			style = "system_7",
			text = Lang.getImgText("txt_juntuanyuyin02") ,
			offsetX = 6,
		})

		local image4 = UIHelper.seekNodeByName(self._target,"ButtonWorld","Image_1")
		image4 = UIHelper.swapWithLabel(image4,{
			style = "system_7",
			text = Lang.getImgText("txt_shijieyuyin01") ,
			offsetX = 6,
		})
		if Lang.checkLang(Lang.TW)  then
			image3:setFontSize(22)
			image4:setFontSize(22)
			image3:setPositionX(image3:getPositionX()-6)
			image4:setPositionX(image4:getPositionX()-6)
		elseif Lang.checkLang(Lang.EN) then
			image3:setPositionX(self._buttonGuild:getContentSize().width/2)
			image4:setPositionX(self._buttonWorld:getContentSize().width/2)
			image3:setFontSize(16)
			image4:setFontSize(16)
		end
	end
end
