...
function HorseJudgeView:onCreate()
	...
	if Lang.checkLang(Lang.TH) then 
		local image = self._nodeTip:getParent()
		richText:formatText()
		local richTextSize = richText:getVirtualRendererSize()
		local size = image:getContentSize()
		image:setContentSize(cc.size(richTextSize.width + 40 ,size.height))
		self._nodeTip:setPositionX(richTextSize.width *0.5+20)
	end
