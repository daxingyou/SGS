...
-- i18n change lable
function HomelandGuildListCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")

		self._commonBtnSelf = UIHelper.swapWithLabel(self._commonBtnSelf,{
			style = "homeland_4",
			text = Lang.getImgText("txt_homeland_04b") ,
		})
		self._commonBtnSelf:setTouchEnabled(true)
		self._commonBtnSelf:addClickEventListenerEx(handler(self,self._onClickInvite))
		

		self._commonBtn = UIHelper.swapWithLabel(self._commonBtn,{
			style = "homeland_5",
			text = Lang.getImgText("txt_homeland_04") ,
		})
		self._commonBtn:setTouchEnabled(true)
		self._commonBtn:addClickEventListenerEx(handler(self,self._onClickInvite))
		
		if Lang.checkLang(Lang.ENID) then
			self._commonBtn:setPosition(self._commonBtn:getPositionX()-5,self._commonBtn:getPositionY()+2)
		end
	end
end
