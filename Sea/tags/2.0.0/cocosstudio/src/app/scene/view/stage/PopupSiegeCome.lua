...
function PopupSiegeCome:_createNameNode()
	...
	if not Lang.checkLang(Lang.CN)  then
		local levelPosX = textName:getPositionX()+textName:getContentSize().width*0.5 + 8
		local UIHelper  = require("yoka.utils.UIHelper")
		textLevel:setAnchorPoint(cc.p(0,0.5))
		textLevel:setPositionX(levelPosX)
	end
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		textLevel:setFontSize(textName:getFontSize())
		textLevel:setPositionY(textName:getPositionY())
	end
