...
-- i18n change lable
function AnswerClientEnd:_dealI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text = UIHelper.seekNodeByName(self,"TextTalk")
		text:setString(Lang.getImgText("answer_talk"))
		text:setContentSize(cc.size(200,100))
		--text:setPositionX(text:getPositionX()-14)
	end
	if Lang.checkLang(Lang.ENID) then
		self._textTimeDesc:setPositionX(self._textTimeDesc:getPositionX()-38)
		self._textOverTime:setPositionX(self._textOverTime:getPositionX()+38)
