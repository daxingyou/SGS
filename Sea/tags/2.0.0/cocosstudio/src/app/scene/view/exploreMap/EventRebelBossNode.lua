...
-- i18n change lable
function EventRebelBossNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local image1 = UIHelper.seekNodeByName(self,"Image_4")
    
		UIHelper.swapWithLabel(image1,{
			 style = "explore_1",
			 text = Lang.getImgText("txt_dongzhuozhiluan01") ,
		})

     
		local image2 = UIHelper.seekNodeByName(self,"Reward","Image_15")
...
-- i18n pos lable
function EventRebelBossNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local textActTimeLabel = UIHelper.seekNodeByName(self._countDownNode,"TextActTimeLabel")
        local size = textActTimeLabel:getContentSize()
		self._leftTimeLabel:setPositionX(textActTimeLabel:getPositionX()+
            size.width
        )

        self._talkString:setFontSize(
            self._talkString:getFontSize()-2
        )
         self._talkString:setPositionY(
             self._talkString:getPositionY()+6
         )
        self._talkString:setContentSize(cc.size(190,80))
