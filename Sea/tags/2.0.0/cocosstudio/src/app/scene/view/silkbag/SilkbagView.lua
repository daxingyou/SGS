...
function SilkbagView:_updateData()
	...
	self._silkbagIds = G_UserData:getSilkbag():getListBySort(heroBaseId, heroRank, isInstrumentMaxLevel, curPos, isWeard, heroLimit)
end
...
function SilkbagView:_updateHeroIcons()
	...
	for i, data in ipairs(iconsData) do
		...
		icon:updateIcon(data.type, data.value, data.funcId, data.limitLevel)
	end
end
...
function SilkbagView:_updateBaseInfo()
	local showId, isEquipAvatar, avatarLimitLevel = AvatarDataHelper.getShowHeroBaseIdByCheck(self._heroUnitData)
	...
	local strRankLevel = rankLevel > 0 and "+"..rankLevel or ""
	local heroParam = TypeConvertHelper.convert(TypeConvertHelper.TYPE_HERO, heroBaseId, nil, nil, self._heroUnitData:getLimit_level())
	self._textHeroName:setString(heroParam.cfg.name..strRankLevel)
	self._textHeroName:setColor(heroParam.icon_color)

	self._nodeHeroIcon:updateUI(showId, nil, limitLevel)
...
function SilkbagView:_onItemUpdate(item, index)
	...
	if silkbagId then
		...
		item:update(silkbagId, heroBaseId, heroRank, isInstrumentMaxLevel, curPos, heroLimit)
	end
end
...
-- i18n change lable
function SilkbagView:_dealI18n()

	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text = UIHelper.seekNodeByName(self._buttonDetail,"Text_12")
		UIHelper.dealVTextWidget(text,text:getString())
		if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
			self._buttonDetail:setScale9Enabled(false)
			self._buttonDetail:ignoreContentAdaptWithSize(true)
			self._buttonDetail:loadTextureNormal(Path.getCommonImage("img_btn_ctrl_small01_nml"))
			local size = self._buttonDetail:getContentSize()
			text:setPosition(size.width/2,size.height/2)
			local posX,posY = self._buttonDetail:getPosition()
			self._buttonDetail:setPosition(posX-30,posY-50)
		end
	end
	if Lang.checkLang(Lang.EN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local img = UIHelper.seekNodeByName(self._nodeHero,"Image_119")
		img:setContentSize(cc.size(200,26))
	end
end
