	...
	for i = index - 1, index + 1 do
		...
		if item and item.widget then
			...
			if count == 0 then
				...
				avatar:setPosition(cc.p(self._pageViewSize.width / 2, 222))
				avatar:playAnimationLoopIdle()
				widget:addChild(avatar)
				self._fileNodePetNew = avatar
				self._pageItems[i].avatar = avatar1
			end
		end
	end
...
function PetTrainStarLayer:_updateShow()
	...
	if strDesc then
		...
		self._imageBless:setVisible(true)
	else
		self._imageBless:setVisible(false)
	end
...
function PetTrainStarLayer:_updateCost()
	...
	for i, info in ipairs(self._materialInfo) do
		...
		if not Lang.checkLang(Lang.CN)  then
			--self["_resSubItem1"]:setPositionX(self["_resSubItem1"]:getPositionX() - 16)
...
function PetTrainStarLayer:_doPetBreak()
	...
	local petIds = {}
	local sameCards = G_UserData:getPet():getSameCardCountWithBaseId(petBaseId)
	local count = 0
...
function PetTrainStarLayer:_dealPosI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		textLevel:setPositionX(x + 10)
		textLevel0:setPositionX(x + 10)
		
        self._textOldAdd:setPositionX( textLevel0:getPositionX() + 20)
		self._textNewAdd:setPositionX( self._textOldAdd:getPositionX() + 85)
		self._textOldStar:setPositionX(textLevel:getPositionX() + 20)
    ...
    if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		MATERIAL_POS[2][2] = {265, 56}
	end
	self._nodeResource:setPositionX(30)
	self._nodeNeedLevelPos:setPositionX(self._nodeNeedLevelPos:getPositionX() - 30)
end
