...
-- i18n pos lable
function SeasonPet2Icon:_dealPosByI18n()
	if Lang.checkLang(Lang.EN) then
        for index = 1, 5 do
            local size = self["_textName"..index]:getParent():getContentSize()
            self["_textName"..index]:getParent():setContentSize(cc.size(110,size.height))
            self["_textName"..index]:getVirtualRenderer():setMaxLineWidth(110-4)
        end
	end
end
