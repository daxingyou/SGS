...
function PopupHeroKarma:doActive(id)
    G_UserData:getKarma():c2sHeroActiveDestiny(self._heroUnitData:getBase_id(), id)
end

function PopupHeroKarma:_onItemTouch(index, t)
    local index = index * 3 + t
	local destinyId = self._karmaData[index].id
	G_UserData:getKarma():c2sHeroActiveDestiny(self._heroUnitData:getBase_id(), destinyId)
end
...
--i18n
function PopupHeroKarma:_dealByI18n()
    if Lang.checkLang(Lang.EN) then
        self._panelBg:setTitleFontSize(22)
    end
end
