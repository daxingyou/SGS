...
-- i18n change lable
function PopupSeasonRewardsTip:_dealI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
        local size = self._panelNewSeason:getContentSize()
		self._panelNewSeason:setContentSize(cc.size(size.width+80,size.height))
    end
    if Lang.checkLang(Lang.TH) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local text3 = UIHelper.seekNodeByName(self._panelNewSeason,"Text_3")
        text3:setPositionY(text3:getPositionY()-8)
    end
