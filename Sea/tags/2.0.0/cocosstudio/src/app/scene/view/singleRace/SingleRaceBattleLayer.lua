...
function SingleRaceBattleLayer:_playCountDownEffect(countDownTime)
    ...
    elseif index == 0 then
        ...
        if  not Lang.checkLang(Lang.CN) then
            ...
            effectNode = G_EffectGfxMgr:createPlayMovingGfx(self._nodeCount, "moving_effect_jingjijishi_suoding", _effectFunction, nil , true)
        else
...
function SingleRaceBattleLayer:_updateBetCount()
    ...
    for i = 1, 2 do
        ...
        if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.ZH) or Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.ENID) then
            textTip:formatText()
            local wid =  textTip:getVirtualRendererSize().width + 40
            self["_imageBetBg"..i]:ignoreContentAdaptWithSize(false)
            local height =  self["_imageBetBg"..i]:getContentSize().height
            self["_imageBetBg"..i]:setContentSize(cc.size(wid,height))
        end
    ...
    if userData then
        ...
        for index, id in pairs(formation) do
            ...
            if hero then
                ...
                self._avatars[side][index]:updateUI(baseId, nil, nil, limitLevel)
...
-- i18n change lable
function SingleRaceBattleLayer:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._imageBet1 = UIHelper.swapSignImage(self._imageBet1,
		{ 
			 style = "signet_3", 
			 text = Lang.getImgText("img_camp_yiyazhu") ,
			 anchorPoint = cc.p(0.5,0.5),
			 rotation = -10,
		},Path.getTextSignet("img_common_lv"))


        self._imageBet2 = UIHelper.swapSignImage(self._imageBet2,
		{ 
			 style = "signet_3", 
			 text = Lang.getImgText("img_camp_yiyazhu") ,
			 anchorPoint = cc.p(0.5,0.5),
...
-- i18n  lable
function SingleRaceBattleLayer:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
