...
function SingleRaceGuessServerCell:update(data, index, mySupportId, supportNum, markRes, textButton)
	...
	for i = 1, 4 do
		...
		if user then
			...
			local coverId, limitLevel = user:getCovertIdAndLimitLevel()
			local officialName, officialColor,officialInfo = UserDataHelper.getOfficialInfo(user:getOfficer_level())
			self["_nodeIcon"..i]:updateUI(coverId, nil, limitLevel)
...
-- i18n change lable
function SingleRaceGuessServerCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
