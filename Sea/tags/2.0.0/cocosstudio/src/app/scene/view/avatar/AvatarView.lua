...
function AvatarView:_updateName()
	...
	if not Lang.checkLang(Lang.CN) then
        ...
        UIHelper.dealVTextWidget(self._textName,param.name)
	else
		self._textName:setString(param.name)
	end 
	]]
...
function AvatarView:_buildCombinationModule()
	...
	for i, showId in ipairs(showIds) do
		...
		if Lang.checkChannel(Lang.CHANNEL_SEA) and showConfig.is_work ~= 1 then
		else
...
--i18n
function AvatarView:_dealByI18n()
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		local size = self._listViewIcon:getContentSize()
		self._listViewIcon:setContentSize(size.width+20,size.heigth)
	end
end
