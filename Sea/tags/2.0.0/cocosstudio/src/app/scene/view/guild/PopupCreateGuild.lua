...
function PopupCreateGuild:onCreate()
	...
	if not Lang.checkLang(Lang.CN) then
			...
			local maxLen = UIHelper.getInputLimitedConfig().legion_length
  ...
  else		self._nameText = InputUtils.createInputView(
		    {
				bgPanel = self._imageInput,
				fontSize = 20,
				placeholderFontColor = Colors.INPUT_PLACEHOLDER,
...
-- i18n pos lable
function PopupCreateGuild:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		
		local textCostNameDesc = UIHelper.seekNodeByName(self._resCost,"Text_cost_name_desc")
		textCostNameDesc:setAnchorPoint(cc.p(1,0.5))
		textCostNameDesc:setPositionX( -3 )	
	end
end
