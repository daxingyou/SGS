...
function PopupGuildGiveRedPacket:_checkIsCanAdd()
    local canNotAddIds = { --不能加倍的红包Id，写死，因为没有规则。。。
        3101, 3102, 3103, 3401,3402, 3403,3404, 3405, 7101, 7102, 7103, 7104, 7201, 7202, 7203
    }
...
-- i18n change lable
function PopupGuildGiveRedPacket:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
        local text1 = UIHelper.seekNodeByName(self._resourceNode,"Text01")
        local text2 = UIHelper.seekNodeByName(self._nodeAdd,"Text01_0_1_0")
        text1:setString(Lang.getImgText("guild_give_redpacket_1"))
        text2:setString(Lang.getImgText("guild_give_redpacket_2"))
    end
    if Lang.checkLang(Lang.TH) then
        for i = 1,3 do
            self["_resInfo0"..i]:setPositionX(self["_resInfo0"..i]:getPositionX()+5)
        end
    end
end
