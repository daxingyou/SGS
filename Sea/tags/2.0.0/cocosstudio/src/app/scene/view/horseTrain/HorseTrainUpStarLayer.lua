	...
	for i = index-1, index+1 do
		if i >= 1 and i <= #allHorseIds then
			...
			if self._pageItems[i].avatar == nil then
				...
				avatar:setPosition(cc.p(self._pageViewSize.width / 2, 200))
...
function HorseTrainUpStarLayer:_updateBaseInfo()
	...
	local heroBaseId = HorseDataHelper.getHeroBaseIdWithHorseId(self._unitData:getId())
	...
	if talentInfo then
		...
		-- i18n
		local listViewTab = ccui.ScrollView:create()
		listViewTab:setTouchEnabled(true)
		listViewTab:setBounceEnabled(true) 
		listViewTab:setDirection(ccui.ScrollViewDir.vertical)
		listViewTab:setAnchorPoint(cc.p(0.5, 0.5))
		listViewTab:setPosition(cc.p(0,0))
		listViewTab:setScrollBarWidth(0)
		richText:setAnchorPoint(cc.p(0.5, 1))
		local wid = richText:getContentSize().width
		local height = richText:getContentSize().height
		local panelH = 67
		listViewTab:setInnerContainerSize(cc.size(wid, height))
		listViewTab:setContentSize(cc.size(wid, panelH))
		richText:setPositionY(height)
		richText:setPositionX(wid/2)
		listViewTab:addChild(richText)
		self._nodeTalentDesPos:addChild(listViewTab)
		--self._nodeTalentDesPos:setPositionY(62)
		--end
...
function HorseTrainUpStarLayer:_dealPosByI18n()
	...
	if Lang.checkLang(Lang.EN) then
		local text = self._fileNodeAttr5:getSubNodeByName("TextName")
		text:getVirtualRenderer():setMaxLineWidth(115)
	end
	if Lang.checkLang(Lang.TH) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local panel1 = UIHelper.seekNodeByName(self._panelDesign,"Panel_2_0","Panel_156")
		self._fileNodeDetailTitle:setPositionX(self._fileNodeDetailTitle:getPositionX()-20)
		panel1:setPositionX(panel1:getPositionX()+20)
	end
end
-- i18n pos lable
function HorseTrainUpStarLayer:_updatePosI18n()
	if Lang.checkLang(Lang.EN) then
		local text = self._fileNodeAttr5:getSubNodeByName("TextName")
		local offsetY = 0
		if text:getContentSize().height > 20 then
			offsetY = 10
		end
		for i = 1, 6 do
			self["_fileNodeAttr"..i]:setPositionY(170+offsetY-(i-1)*30)
		end
		local textLevel = ccui.Helper:seekNodeByName(self, "TextLevel")
		textLevel:setPositionY(200+offsetY)
		self._textOldLevel1:setPositionY(200+offsetY)
		self._textOldLevel2:setPositionY(200+offsetY)
		local image = ccui.Helper:seekNodeByName(self, "Image_449")
		image:setPositionY(210+offsetY)
		self._textNewLevel:setPositionY(200+offsetY)
		self._fileNodeAttr5:setPositionY(self._fileNodeAttr5:getPositionY()-text:getContentSize().height+20)
		self._fileNodeAttr6:setPositionY(self._fileNodeAttr5:getPositionY()-28.1)
	end
end
