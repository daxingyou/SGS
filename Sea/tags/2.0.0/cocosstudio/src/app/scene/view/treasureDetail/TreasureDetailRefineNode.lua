...
function TreasureDetailRefineNode:onCreate()
	...
	if self._treasureData:isUserTreasure() then
		...
		if self._treasureData:isCanLimitBreak() and require("app.utils.logic.FunctionCheck").funcIsShow(FunctionConst.FUNC_TREASURE_TRAIN_TYPE3) then
...
function TreasureDetailRefineNode:_createButton2()
	...
	local reach = RedPointHelper.isModuleSubReach(FunctionConst.FUNC_TREASURE_TRAIN_TYPE3, "slotRP", self._treasureData)
	btn:showRedPoint(reach)
