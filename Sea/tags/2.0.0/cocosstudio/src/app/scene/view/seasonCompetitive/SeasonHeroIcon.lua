...
function SeasonHeroIcon:updateUI(tabIndex, cellData)
    ...
    local function updateItem(index, data)
        ...
        if tabIndex == 5 then   --变身卡
            nameStr = data.name
            iconId = SeasonSportHelper.getTransformCardsResId(data.id)
        --[[elseif data.color == SeasonSportConst.HERO_SCOP_LOWERLIMIT and data.limit == SeasonSportConst.HERO_SCOP_LIMIT then
            local redLimit = SeasonSportHelper.getLimitBreak()
            if redLimit == SeasonSportConst.HERO_RED_LINEBREAK then
                iconId = require("app.config.hero_res").get(data.limit_res_id).icon
            end]]
        end
        ...
        if tabIndex == 5 then   --变身卡
            if SeasonSportHelper.checkSeasonRedHero(SeasonSportHelper.getTransformCardsHeroId(data.id)) then
                iconColor = Colors.getColor(SeasonSportConst.HERO_SCOP_REDIMIT)
                local iconBg = Path.getUICommon("frame/img_frame_06")
                self["_fileNodeCommon"..index]:loadColorBg(iconBg)
            end
...
-- i18n change lable
function SeasonHeroIcon:_dealByI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
        --self._textBtnDesc:setPosition(cc.p(118,self._seasonOffsetY - 28))
        --self._textBtnDesc:setAnchorPoint(cc.p(0.0, 0.5))

         for index = 1, 6 do

            local image2 = UIHelper.seekNodeByName(self["_item"..index],"Image_2")
            image2:setVisible(false)
            self["_textHeroName"..index]:retain()
            self["_textHeroName"..index]:removeFromParent()
            self["_item"..index]:addChild(self["_textHeroName"..index])
            self["_textHeroName"..index]:release()

            local size = self["_item"..index]:getContentSize()
            self["_textHeroName"..index]:setPositionX( size.width * 0.5)
            self["_textHeroName"..index]:setPositionY(  self["_textHeroName"..index]:getPositionY() + 10 )
            local render = self["_textHeroName"..index]:getVirtualRenderer()
            render:setMaxLineWidth(120)
            render:setLineSpacing( -3 )
            self["_textHeroName"..index]:setFontSize(18)
            self["_textHeroName"..index]:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )

           self["_textHeroName"..index]:setTextVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_TOP )
