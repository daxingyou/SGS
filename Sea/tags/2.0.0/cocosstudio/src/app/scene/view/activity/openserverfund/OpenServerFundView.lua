...
function OpenServerFundView:_refreshBuyFundView()
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		self._textCurrStage:setVisible(false)
	else
		if Lang.checkLang(Lang.KR) then
			self._imageFanli:setPositionX(group > 1 and 90 or 160)
	...
	if not Lang.checkLang(Lang.CN)  then
		...
		if Lang.checkChannel(Lang.CHANNEL_SEA) then
			textCurr2:setPositionX(self._imageRmb:getPositionX() + self._imageRmb:getContentSize().width+8 )
		else
...
function OpenServerFundView:_dealI18n()
	...
	if not Lang.checkLang(Lang.CN)  then
		...
		if Lang.checkLang(Lang.TH) then
			fundDesTextCurr2:setFontSize(fundDesTextCurr2:getFontSize()+8)
			textCurr:setFontSize(textCurr:getFontSize()+8)
		...
		elseif Lang.checkLang(Lang.EN) then
			...
			self._imageRmb:setFontSize(self._imageRmb:getFontSize()+6)
			self._imageTotalRmb:setFontSize(self._imageRmb:getFontSize()+4)
			textCurr:setPositionY(textCurr:getPositionY()-8)
			self._imageTotalRmb:setPositionY(self._imageTotalRmb:getPositionY()-8)
		...
		if Lang.checkChannel(Lang.CHANNEL_SEA) then
			self._imageTotalRmb:setPositionX(textCurr:getPositionX() +textCurr:getContentSize().width+8)
		else
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		self._nodeVip:setVisible(false)
		self._nodeFundDes:setPositionX(self._nodeFundDes:getPositionX()+60)
		self._imageFanli:setPositionX(160)
		self._imageFanli:ignoreContentAdaptWithSize(true)
	end
end
...
function OpenServerFundView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		for k,v in ipairs(children) do
			...
			elseif not Lang.checkLang(Lang.TW) and not Lang.checkLang(Lang.ZH) then
		...
		if Lang.checkLang(Lang.TW)  then
			self._textCurrStage:setFontSize(26)
		end
		

		if Lang.checkLang(Lang.KR) then
			self._textCurrStage:setPositionX(self._textCurrStage:getPositionX()-22)
		end
