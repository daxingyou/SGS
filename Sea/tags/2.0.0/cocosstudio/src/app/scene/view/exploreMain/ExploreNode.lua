    ...
    if G_UserData:getExplore():isLastPass(self._configData.id, 1) then  --上一张通关，开放
        if self._chapterData:isLastStagePass() then
            ...
            if Lang.checkLang(Lang.TH) or Lang.checkLang(Lang.EN)  then
                self._closeTip = Lang.get("explore_main_close_title", {chapter = chapteConfig.chapter, name = ""})
            else
...
function ExploreNode:_dealI18n()
    ...
    if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
        self._textProgressBg:setPositionY(self._textProgressBg:getPositionY()+26)
        self._textProgress:setPositionY(self._textProgress:getPositionY()+26)
        self._textProgress:getVirtualRenderer():setMaxLineWidth(250)
    end
