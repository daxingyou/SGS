...
local MineRoute = require("app.config.mine_route")
local MineRoad = require("app.config.mine_road")
...
function MineCraftData:_initMineRoad()
	local roads = {}
	local midPoints = {}
	for i = 1, MineRoad.length() do
		local data = MineRoad.indexOf(i)
		roads[i] = {}
		local lastPitId = data.point_1
		for j = 1, 10 do
			local pitId = data["point_" .. j]
			if pitId == 0 then
				break
			end
			table.insert(roads[i], pitId)
			if j > 1 then
				if not midPoints[lastPitId .. pitId] then
					local midIndex = j - 1
					local strMidPoint = data["mid_point_" .. midIndex]
					local strArr = string.split(strMidPoint, "|")
					midPoints[lastPitId .. pitId] = cc.p(tonumber(strArr[1]), tonumber(strArr[2]))
				end
				lastPitId = pitId
			end
		end
...
function MineCraftData:c2sEnterMine(mineId)
	local message = {
		mine_id = mineId
	}
