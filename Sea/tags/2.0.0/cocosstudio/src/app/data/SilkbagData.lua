...
function SilkbagData:getListBySort(heroBaseId, heroRank, isInstrumentMaxLevel, curPos, isWeard, heroLimit)
	...
	for k, data in pairs(self._silkbagList) do
		...
		if isWeard == cloneData:isWeared() then
			if not cloneData:isWearedInPos(curPos) then
				...
				local isEffect = SilkbagDataHelper.isEffectiveSilkBagToHero(baseId, heroBaseId, heroRank, isInstrumentMaxLevel, heroLimit)
...
function SilkbagData:isHaveRedPoint(pos, slot)
	...
	for k, data in pairs(self._silkbagList) do
		if not data:isWeared() and data:isCanWearWithPos(pos) then
			local isEffective = SilkbagDataHelper.isEffectiveSilkBagToHero(data:getBase_id(), heroBaseId, heroRank, isInstrumentMaxLevel, heroLimit)
