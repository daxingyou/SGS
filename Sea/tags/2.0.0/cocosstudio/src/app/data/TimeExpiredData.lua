...
function TimeExpiredData:_calNextExpiredTimeOfDaily(resetTime,lastUpdateTime)
	...
	local resetSeconds = resetTime * 3600
	-- 上一次更新距离今天0点的时间（秒数）
	local lastSeconds = G_ServerTime:getTodaySeconds(lastUpdateTime)--i18n change 
...
function TimeExpiredData:_calNextExpiredTimeOfWeekly(resetWeekDay, resetTime,lastUpdateTime )
	...
	--i18n change 
	local todayPassedSeconds = G_ServerTime:getTodaySeconds(lastUpdateTime)
	local weekDay = G_ServerTime:getDateObject(lastUpdateTime).wday
	weekDay = weekDay -1
	if weekDay == 0 then
		weekDay = 7
	end
	...
	--logWarn(self.class.__cname.." todayPassedSeconds   :"..tostring(todayPassedSeconds))
	--logWarn(self.class.__cname.." weekDay   :"..tostring(weekDay))
	--logWarn(self.class.__cname.." weekResetSeconds   :"..tostring(weekResetSeconds))
	--logWarn(self.class.__cname.." lastUpdateTime:"..tostring(lastUpdateTime))
	--logWarn(self.class.__cname.." weekPassedSeconds:"..tostring(weekPassedSeconds))
	--logWarn(self.class.__cname.." nextUpdateTime:"..tostring(nextUpdateTime))
