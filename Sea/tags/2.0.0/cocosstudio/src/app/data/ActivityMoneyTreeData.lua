...
--摇钱树摇一摇
function ActivityMoneyTreeData:c2sActMoneyTree()
	if self:isExpired() == true then
		self:pullData()
		return
	end
	G_NetworkManager:send(MessageIDConst.ID_C2S_ActMoneyTree, {})
end
...
function ActivityMoneyTreeData:getShakeCost()
...
function ActivityMoneyTreeData:getFreeCount()
	local indexs = act_silver.index()
	local freeCount = 0
	for _, v in pairs(indexs)do
		local config = act_silver.indexOf(v)
		if config and config.cost == 0 then
