...
-- i18n
GuildData.FILE_NAME = "guildEvent"
...
function GuildData:_s2cQueryGuildTrain( id,message )
	...
	elseif  message.ret == MessageErrorConst.RET_GUILD_TRAIN_TRAINING then 
...
function GuildData:_s2cGetGuildBase(id, message)
	...
	-- i18n
	local noGuild = self:_getNoGuild()
	if noGuild and self:isInGuild() then
		self:_setNoGuild(false)
		G_NativeAgent:eventCustom("AT_Join_Guild", '')
	end
end
...
-- i18n join guild event
function GuildData:_setNoGuild(noGuild)
	G_StorageManager:setUserInfo("", G_UserData:getBase():getId())
	G_StorageManager:saveWithUser(GuildData.FILE_NAME,{noGuild=noGuild})
end
function GuildData:_getNoGuild()
	G_StorageManager:setUserInfo("", G_UserData:getBase():getId())
	local data = G_StorageManager:loadUser(GuildData.FILE_NAME) or {}
	return data.noGuild
end
