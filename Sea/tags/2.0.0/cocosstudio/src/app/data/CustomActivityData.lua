...
function CustomActivityData:_s2cGetCustomActivityInfo(id,message)
    ...
    for k,v in ipairs(userQuest) do
     ...
     G_SignalManager:dispatch(SignalConst.EVENT_MAIN_CITY_CHECK_BTNS)--CustomActivityService里处理了，这里还需要么
...
function CustomActivityData:getShowActUnitDataArr()
    ...
    for k,v in pairs( self._actUnitDataList) do
        --i18n 不显示小额充值活动
        if CustomActivityConst.CUSTOM_ACTIVITY_TYPE_SMALL_ACOUNT_RECHANGE  ==  v:getAct_type() then

        elseif CustomActivityConst.CUSTOM_ACTIVITY_TYPE_FUNDS == v:getAct_type() then
...
function CustomActivityData:isQuestCanShow(actId,questId)
	...
	if limitType == CustomActivityConst.QUEST_LIMIT_TYPE_RECHARGE  then
...
function CustomActivityData:hasActivityCanVisible()
     ...
     for k,v in pairs( self._actUnitDataList) do
        -- i18n 忽视小额充值
        if CustomActivityConst.CUSTOM_ACTIVITY_TYPE_SMALL_ACOUNT_RECHANGE ==  v:getAct_type() then
        elseif v:checkActIsVisible() then
           return true
        end
     end
    return false
...
--i18n 小额充值活动
function CustomActivityData:getSmallAmountRechargeActivity()
    local t = {}
    for k,v in pairs( self._actUnitDataList) do
        if v:getAct_type() == CustomActivityConst.CUSTOM_ACTIVITY_TYPE_SMALL_ACOUNT_RECHANGE then
            table.insert(t, v)
        end
    end
    table.sort(t, function(a, b)
...
--i18n 小额充值活动是否可见
function CustomActivityData:isSmallAmountRechargeActivityVisible()
    local acUnitDataList = self:getSmallAmountRechargeActivity()
    return self:hasActivityCanVisibleByUnitList(acUnitDataList)
end

--i18n 判断是否有可见活动
function CustomActivityData:hasActivityCanVisibleByUnitList(actUnitList)
    for k,v in pairs( actUnitList) do
        if v:checkActIsVisible() then
            return true
        end
    end
...
--i18n 小额充值可见活动
function CustomActivityData:getShowSmallAmountRechargeActivity()
    local acUnitDataList = self:getSmallAmountRechargeActivity()
    local dataArr = {}
    for k,v in pairs( acUnitDataList) do
        if v:checkActIsVisible() then
            table.insert( dataArr, v )
        end
    end
