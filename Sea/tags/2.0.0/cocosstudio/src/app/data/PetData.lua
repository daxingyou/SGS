...
function PetData:isPetMapShow( petMapId )
	...
	local enoughOpenDay = UserCheck.enoughOpenDay(config.show_day)
	if (Lang.checkLang(Lang.KR) or Lang.checkChannel(Lang.CHANNEL_SEA))
	and enoughOpenDay == false then
		return false
	end
...
-- }
function PetData:c2sPetPostRankUpMaterial(petId, materials)
	G_NetworkManager:send(MessageIDConst.ID_C2S_PetPostRankUpMaterial, {pet_id = petId, materials = materials})
end
...
function PetData:_s2cPetPostRankUpMaterial(id, message)
	...
	G_SignalManager:dispatch(SignalConst.EVENT_PET_LIMITUP_MATERIAL_SUCCESS, 2)
end
-- #end 神兽界限突破
...
--根据baseId获取同名卡的表
--同名卡：无升级、无突破的卡
function PetData:getSameCardCountWithBaseId(baseId)
	local result = {}
	for k, data in pairs(self._petList) do
		if
