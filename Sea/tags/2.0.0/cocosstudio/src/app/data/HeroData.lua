...
function HeroData:_s2cGetHero(id, message)
	...
	for i, data in ipairs(heroList) do
...
function HeroData:getTransformSrcList()
	...
	for k, unit in pairs(self._heroList) do
		...
		if not isInBattle and not isInReinforcements and color >= 5 and color <= 6 then --橙、红将
...
--武将界限突破放材料
function HeroData:c2sHeroLimitLvPutRes(heroId, pos, subItems)
	G_NetworkManager:send(
...
--武将界限突破
function HeroData:c2sHeroLimitLvUp(heroId)
	G_NetworkManager:send(
