...
function LimitTimeActivityData:_initMainMenuLayerActivityIcons()
	...
	self._mainMenuActivityTimes = {
		...
		},
		{
			...
			getStartTime = function()
				local open = GuildCrossWarHelper.isTodayOpen()
				if not open then
					return 0
				end
	...
	for _, v in ipairs(datas) do
		if curTime < v.endTime then
			if v.funcId == FunctionConst.FUNC_GUILD_WAR then
				local GuildCrossWarHelper = require("app.scene.view.guildCrossWar.GuildCrossWarHelper")
				if not GuildCrossWarHelper.isTodayOpen() or not GuildCrossWarHelper.isGuildCrossWarEntry() then
					curInfo = v
					break
				end
			elseif v.funcId == FunctionConst.FUNC_GUILD_CROSS_WAR then
				local GuildCrossWarHelper = require("app.scene.view.guildCrossWar.GuildCrossWarHelper")
				if GuildCrossWarHelper.isTodayOpen() and GuildCrossWarHelper.isGuildCrossWarEntry() then
					curInfo = v
					break
				end
			else
