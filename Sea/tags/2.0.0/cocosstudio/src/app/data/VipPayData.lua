...
function VipPayData:_initData()
    ...
    for i = 1, VipPayInfo.length() do
        ...
        if rechargeInfo.card_type == RechargeConst.VIP_PAY_TYPE_RECHARGE and matchAppId(rechargeInfo)  
             then
...
function VipPayData:getRechargeList()
    ...
    local function matchVip(rechargeInfo)
        if Lang.checkLang(Lang.KR) then
            ...
            if opgame == "107" or opgame == "109" then
                return true
            end
        end
    ...
    --[[
    logWarn("VipPayData22 ---------------------")
    dump(list)
    logWarn("VipPayData11 ---------------------")
    ]]
	return list
end
