...
RedPointHelper._FUNC_MINE_CRAFT = {
	...
	mainRP = function(params)
		...
		return value or value2 or value3
	end
}
	...
	mainRP = function(pos)
		for i = 1, 2 do
			...
			if treasureId then
				local treasureUnitData = G_UserData:getTreasure():getTreasureDataWithId(treasureId)
				local redValue =
					RedPointHelper.isModuleSubReach(FunctionConst.FUNC_TREASURE_TRAIN_TYPE3, "slotRP", treasureUnitData)
				if redValue then
					return true
				end
			end
...
RedPointHelper._FUNC_HERO_KARMA = {
	mainRP = function(heroUnitData)
		...
		for i, data in ipairs(karmaData) do
			...
			if isCanActivate and not isActivated then
				return true
			end
		end
		return false
...
RedPointHelper._FUNC_ITEM_BAG = {
	mainRP = function(params)
		local isOpen = require("app.utils.logic.FunctionCheck").funcIsOpened(FunctionConst.FUNC_HERO_TRAIN_TYPE3)
		...
		if isOpen then
			...
			hasRed = hasRedItem or hasRedFragment
		else
			hasRed = hasRedItem
		end
