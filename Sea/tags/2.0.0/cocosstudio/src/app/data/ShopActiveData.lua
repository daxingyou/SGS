...
function ShopActiveData:getGoodIdsWithShopAndTabIdBySort(shopId, tabId, curBatch)
	...
	for i, goodId in ipairs(goodIds) do
		...
		if self:_checkIsEnoughBatch(shopId, curBatch, batch) then
...
function ShopActiveData:_getBatchFilterType(shopId)
	...
	if shopId == ShopConst.LOOKSTAR_SHOP or shopId == ShopConst.HORSE_CONQUER_SHOP or shopId == ShopConst.CAKE_ACTIVE_SHOP then
