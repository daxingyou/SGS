...
function CakeActivityData:_initRankMap()
	...
	for i = 1, len do
		...
		local type = info.type
		if self._rankMap[batch] == nil then
			self._rankMap[batch] = {}
		end
		if self._rankMap[batch][type] == nil then
			self._rankMap[batch][type] = {}
		end
		table.insert(self._rankMap[batch][type], info)
	end
end

function CakeActivityData:getRankInfo(batch, type)
	local info = self._rankMap[batch][type]
	assert(info, string.format("cake_rank config can not batch = %d, type = %d", batch, type))
	return info
end
...
function CakeActivityData:isHaveCanGetMaterial()
	...
	for i, data in ipairs(taskList) do
		...
		local info = CakeActivityDataHelper.getCakeTaskConfig(id)
...
function CakeActivityData:isHaveCanGiveMaterial()
	...
	for type = CakeActivityConst.MATERIAL_TYPE_1, CakeActivityConst.MATERIAL_TYPE_3 do
		local itemId = CakeActivityConst["MATERIAL_ITEM_ID_"..type]
...
function CakeActivityData:_s2cGetGuildCakeUpLvReward(id, message)
	...
	G_SignalManager:dispatch(SignalConst.EVENT_CAKE_ACTIVITY_GET_LEVEL_UP_REWARD, awards)
end
