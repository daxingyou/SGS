...
function VoiceAgent:ctor()
    ...
    -- self._voiceOpenId = "0"
    -- --vn-gt
    -- self._voiceId = "1876951177"
    -- self._voiceKey = "1e5367b2e936d3bc7749a54b5fd3622d"
    -- self._voiceOpenId = "0"
	...
	elseif Lang.checkLang(Lang.TW) then
        --tw-eskyfun
		self._voiceId = "1605822430"
		self._voiceKey = "3cdef474a7c24bcfe2d37a6b9be9e469"
		self._voiceOpenId = "0"
	elseif Lang.checkChannel(Lang.CHANNEL_SEA) then
        --tw-eskyfun
		self._voiceId = "1538240997"
		self._voiceKey = "219c5c4ca7bd59829d2f38a1d5eb9afd"
		self._voiceOpenId = "0"
	else
		--yoka
		self._voiceId = "1698864032"
		...
		if ret ~= VoiceConst.GCLOUD_VOICE_SUCC then
			...
			if ret == -1 then
				...
				if not Lang.checkLang(Lang.CN) then
					error_msg = Lang.getImgText("platform_not_support_voice")
				end
			else
			...
			if error_msg ~= nil then
				...
				if Lang.checkChannel(Lang.CHANNEL_SEA) then
					if ret == 12291 then
						local PromptTip = require("app.ui.prompt.PromptTip").new()
        				PromptTip:show(error_msg,nil,1.8)
						return
					end
				end
...
--i18n设置国外服务器地址
function VoiceAgent:setServerInfo(url)
	local ret = voice.setServerInfo(url)
	self:_onVoiceError("VoiceErrno", "setServerInfo", ret)
	return ret
end

--i18n GVoice SDK更新后code有变动
function VoiceAgent:resetVoiceSuccRet()
	self._voiceSuccRet = {
		[VoiceConst.GV_ON_JOINROOM_SUCC] = true,
		[VoiceConst.GV_ON_QUITROOM_SUCC] = true,
		[VoiceConst.GV_ON_MESSAGE_KEY_APPLIED_SUCC] = true,
		...
		[VoiceConst.GV_ON_STT_SUCC] = true,
		[VoiceConst.GV_ON_PLAYFILE_DONE] = true,
		[VoiceConst.GV_ON_RSTT_SUCC] = true,
	}
end
