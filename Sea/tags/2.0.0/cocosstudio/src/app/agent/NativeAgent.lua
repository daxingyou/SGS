...
function NativeAgent:ctor()
	...
	--国家地区编码
	self._countryCode = "unknown"
	--语言编码
	self._languageCode = "any"
	--手机语言编码
...
function NativeAgent:pay(appid, price, productId, productName, productDesc)
	...
	if newProductId then
		...
		if Lang.checkLang(Lang.KR) then 
			local exts = {}
			exts["product_id"] = productId
			exts["op_id"] = G_NativeAgent:getOpId()
			ext = json.encode(exts)
		end
...
--i18n
function NativeAgent:translateWithText(textId,target,sourceText)
	if self:getNativeVersion() < 6 then
		return 
	end

	if target == Lang.ENID then
		target = Lang.OS_LANGUAGE_IOS_ENID
	end

	local p = {}
	table.insert(p, {textId=textId})
	table.insert(p, {source=""})
	table.insert(p, {target=target})
	table.insert(p, {sourceText=sourceText})
	table.insert(p, {untranslatedText=""})
	table.insert(p, {extension=""})

	local paramStr = json.encode(p)

	crashPrint("[GameAgent] translateWithText lua :"..paramStr)
	self:callNativeFunction("translateWithText",p)
	...
	scheduler.performWithDelayGlobal(function()
		print("send"..textId)
		G_SignalManager:dispatch(SignalConst.EVENT_TRANSLATE_SUCCESS,
		{textId = textId,source= "cn",target = "en",sourceText = sourceText,result="sss"})
	end,3)
	]]
