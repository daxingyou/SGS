...
function GameAgent:ctor()
    ...
    self._sdkUserID = nil       --sdk用户id

    self._signalNative = nil
    self._signalLogin = G_NetworkManager:add(MessageIDConst.ID_S2C_Login, handler(self, self._recvLogin))
    self._signalActivate = G_NetworkManager:add(MessageIDConst.ID_S2C_ActivateAccount, handler(self, self._recvActivateAccount))
...
function GameAgent:initSDK()
    ...
    if Lang.checkLang(Lang.CN) then
        ...
        G_VoiceAgent:setAppKey("f3faf617681b352187b72193119f7697")
        G_VoiceAgent:setOpenId("0")
    elseif Lang.checkLang(Lang.VN) then
        --vn-gt
        G_VoiceAgent:setAppId("1876951177")
        G_VoiceAgent:setAppKey("1e5367b2e936d3bc7749a54b5fd3622d")
        G_VoiceAgent:setOpenId("0")
    elseif Lang.checkLang(Lang.TW) then
        --tw-eskyfun
        G_VoiceAgent:setAppId("1605822430")
        G_VoiceAgent:setAppKey("3cdef474a7c24bcfe2d37a6b9be9e469")
        G_VoiceAgent:setOpenId("0")
        G_VoiceAgent:setServerInfo("udp://tw.voice.gcloudcs.com:8700")
    end
...
function GameAgent:_writeGrayTestFile(serverState)
    ...
    G_StorageManager:save("grayFile",
        data
    )
end
...
--i18n
    if uuid then
        data[uuid] = retCode
        G_StorageManager:save("realName",
            data
        )
    end
end
        ...
        local function onCallback()
            if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
                ...
                if ret then --ret and ret.status == 1
                    ...
                    print("GameAgent:getRealNameState retCode  " .. retCode)
                    if retCode == 1 then 
...
function GameAgent:_openExitLogout()
    if self._openExitGamePopupAlert == false then
        ...
        local alert = PopupAlert.new(Lang.get("sdk_title_tishi"), Lang.get("sdk_exit_game"), callbackOk, callbackCancel, callbackCancel)
        alert:openWithAction()
    end
end
--
...
function GameAgent:_onNativeCallback(data)
    ...
    elseif event == NativeConst.SDKLoginResult then
        ...
        if ret == NativeConst.STATUS_FAILED then
            if Lang.checkLang(Lang.KR) then
                G_Prompt:showTip( param )
            end
        end    
    ...
    elseif event == NativeConst.SDKPayResult then
        ...
        elseif ret == NativeConst.STATUS_PROCESS then
            ...
            local platform = G_NativeAgent:getNativeType()
            if platform == "ios" then
                if param == "000917" or param == "000910" or param == "000904"
                    or param == "000906" or param == "000902" or param == "000907"
                    or param == "00909" then
    ...
    elseif event == "VoiceSpeechToText" then
        G_Prompt:showTip(ret .. ": " .. ret  .. ", param: " .. param)
    elseif event == NativeConst.SDKTranslateWithTextResult then--i18n
        if ret == NativeConst.STATUS_SUCCESS then
        ...
        elseif ret == NativeConst.STATUS_FAILED then
            G_Prompt:showTip(Lang.getImgText("chat_translated_fail"))
        end
...
function GameAgent:activateAccount(code)
    G_NetworkManager:send(MessageIDConst.ID_C2S_ActivateAccount, {
        code = code
    })
end
...
function GameAgent:_onNotAllowed()
    ...
    local tip = G_ConfigManager:getServerNotAllowedTipI18n()
...
function GameAgent:_onUUIDNotActivate()
    ...
    local callback = function (name, psw)
        ...
        return true--返回false关闭弹窗，否则保留弹窗
    end
    local cancelCallback = function ()
        G_NetworkManager:reset()
    end
...
function GameAgent:checkContent(strContent, callback)
    ...
    xhr:registerScriptHandler(function() onCallback(callback) end)
    xhr:send()
end
...
function GameAgent:_sendCreateRole(roleName, heroType)
    G_NetworkManager:send(MessageIDConst.ID_C2S_Create, {
        name = roleName, 
        type = heroType
    })
end
...
function GameAgent:getLoginUserName()
    ...
    return nil--Lang.get("sdk_platform_notlogined")
end
...
function GameAgent:isSettingRealName()
    ...
    if Lang.checkLang(Lang.VN) or Lang.checkLang(Lang.KR) then  -- 充值不用防沉迷
        isSetting = true
    end
    return isSetting
end
