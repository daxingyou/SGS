...
local __key_type = {
  id = "int",    --编号-1 
  show_level = "int",    --显示等级-2 
  unlock_level = "int",    --解锁等级-3 
  time_limit = "int",    --时间限制-4 
  good_id = "int",    --商品id-5 
  name = "string",    --物品名称-6 
  type = "int",    --物品类型-7 
  value = "int",    --类型值-8 
...
function act_level_discount.indexOf(index)
    ...
    if Lang.lang ~= "cn" and Lang.lang ~= _lang then 
        _lang = Lang.lang 
        if Lang.isFileExist("app/i18n/".. _lang .."/config/act_level_discount.lua") then _isExist =  true end
    end
    if _lang ~= "cn" and _isExist then
        local table = require( "app.i18n.".. _lang ..".config." .. "act_level_discount" )
        return setmetatable({_raw = act_level_discount._data[index], _lang=table._data[index], _lang_key_map=table.__key_map}, mt)
    end
