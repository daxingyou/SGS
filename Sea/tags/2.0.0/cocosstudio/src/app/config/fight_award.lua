...
function fight_award.indexOf(index)
    ...
    if Lang.lang ~= "cn" and Lang.lang ~= _lang then 
        _lang = Lang.lang 
        if Lang.isFileExist("app/i18n/".. _lang .."/config/fight_award.lua") then _isExist =  true end
    end
    if _lang ~= "cn" and _isExist then
        local table = require( "app.i18n.".. _lang ..".config." .. "fight_award" )
        return setmetatable({_raw = fight_award._data[index], _lang=table._data[index], _lang_key_map=table.__key_map}, mt)
    end
