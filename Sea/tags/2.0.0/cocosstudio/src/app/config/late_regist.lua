...
local mt = { 
    __index = function(t, k) 
        assert(__key_map[k], "cannot find " .. k .. " in late_regist")
...
function late_regist.indexOf(index)
    ...
    if Lang.lang ~= "cn" and Lang.lang ~= _lang then 
        _lang = Lang.lang 
        if Lang.isFileExist("app/i18n/".. _lang .."/config/late_regist.lua") then _isExist =  true end
    end
    if _lang ~= "cn" and _isExist then
        local table = require( "app.i18n.".. _lang ..".config." .. "late_regist" )
        return setmetatable({_raw = late_regist._data[index], _lang=table._data[index], _lang_key_map=table.__key_map}, mt)
    end
    return setmetatable({_raw = late_regist._data[index]}, mt)
end
