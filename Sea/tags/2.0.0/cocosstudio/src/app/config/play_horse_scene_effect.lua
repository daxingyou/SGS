...
function play_horse_scene_effect.indexOf(index)
    ...
    if Lang.lang ~= "cn" and Lang.lang ~= _lang then 
        _lang = Lang.lang 
        if Lang.isFileExist("app/i18n/".. _lang .."/config/play_horse_scene_effect.lua") then _isExist =  true end
    end
    if _lang ~= "cn" and _isExist then
        local table = require( "app.i18n.".. _lang ..".config." .. "play_horse_scene_effect" )
        return setmetatable({_raw = play_horse_scene_effect._data[index], _lang=table._data[index], _lang_key_map=table.__key_map}, mt)
    end
