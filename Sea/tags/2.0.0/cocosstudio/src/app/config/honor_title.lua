...
function honor_title.indexOf(index)
    ...
    if Lang.lang ~= "cn" and Lang.lang ~= _lang then 
        _lang = Lang.lang 
        if Lang.isFileExist("app/i18n/".. _lang .."/config/honor_title.lua") then _isExist =  true end
    end
    if _lang ~= "cn" and _isExist then
        local table = require( "app.i18n.".. _lang ..".config." .. "honor_title" )
        return setmetatable({_raw = honor_title._data[index], _lang=table._data[index], _lang_key_map=table.__key_map}, mt)
    end
