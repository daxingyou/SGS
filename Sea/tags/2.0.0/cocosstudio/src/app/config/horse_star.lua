...
local __key_type = {
  ...
  no_crit = "int",    --免暴-16 
  power = "int",    --战斗力-17 
  skill_type1 = "int",    --技能属性1类型-18 
  skill_size1 = "int",    --技能属性1数值-19 
  skill = "string",    --技能描述-20 
...
local horse_star = {
    _data = {
        ...
        [25] = {9,1,"飞霜千里",5,12,9,1,5,2,3000000,16000,116000,8000,8000,390,390,3600000,0,0,"[飞霜千里]攻击对敌方造成伤害的20%转化为治疗，治疗己方血量最少的目标。",},
        [26] = {9,2,"追风·飞霜千里",5,12,9,1,5,2,6000000,20000,147000,10000,10000,420,420,6000000,0,0,"[追风·飞霜千里]攻击对敌方造成伤害的30%转化为治疗，治疗己方血量最少的目标，并为该目标额外回复10%生命上限的血量。",},
        [27] = {9,3,"逐日·飞霜千里",5,12,9,1,0,0,0,26000,193000,13000,13000,470,470,8400000,0,0,"[逐日·飞霜千里]攻击对敌方造成伤害的30%转化为治疗，治疗己方血量最少的目标，并为该目标额外回复30%生命上限的血量。",},
        [28] = {10,1,"乌云踏雪",5,12,10,1,5,2,3000000,16000,116000,8000,8000,390,390,3600000,19,160,"[乌云踏雪]攻击对敌方造成伤害额外增加16%。",},
        [29] = {10,2,"追风·乌云踏雪",5,12,10,1,5,2,6000000,20000,147000,10000,10000,420,420,6000000,19,250,"[追风·乌云踏雪]攻击对敌方造成伤害额外增加25%，击杀目标后50%概率回复自身1点怒气。",},
        [30] = {10,3,"逐日·乌云踏雪",5,12,10,1,0,0,0,26000,193000,13000,13000,470,470,8400000,19,250,"[逐日·乌云踏雪]攻击对敌方造成伤害额外增加25%，击杀目标后回复自身1点怒气，并且下次回怒时额外回复自身1点怒气",},
...
function horse_star.indexOf(index)
    ...
    if Lang.lang ~= "cn" and Lang.lang ~= _lang then 
        _lang = Lang.lang 
        if Lang.isFileExist("app/i18n/".. _lang .."/config/horse_star.lua") then _isExist =  true end
    end
    if _lang ~= "cn" and _isExist then
        local table = require( "app.i18n.".. _lang ..".config." .. "horse_star" )
        return setmetatable({_raw = horse_star._data[index], _lang=table._data[index], _lang_key_map=table.__key_map}, mt)
    end
...
--
function horse_star.set(id,star, key, value)
