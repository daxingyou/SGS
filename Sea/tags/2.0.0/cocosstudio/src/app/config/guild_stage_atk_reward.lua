...
function guild_stage_atk_reward.indexOf(index)
    ...
    if Lang.lang ~= "cn" and Lang.lang ~= _lang then 
        _lang = Lang.lang 
        if Lang.isFileExist("app/i18n/".. _lang .."/config/guild_stage_atk_reward.lua") then _isExist =  true end
    end
    if _lang ~= "cn" and _isExist then
        local table = require( "app.i18n.".. _lang ..".config." .. "guild_stage_atk_reward" )
        return setmetatable({_raw = guild_stage_atk_reward._data[index], _lang=table._data[index], _lang_key_map=table.__key_map}, mt)
    end
