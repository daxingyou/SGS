...
local formula = {
    _data = {
        ...
        [3] = {3,"((#ATK#*9+#PD#*27+#MD#*27+#HP#*1)*(#HURT#+#HURT_RED#+#PVP_HURT#+#PVP_HURT_RED#+#HIT#+#NO_HIT#+#CRIT#+#NO_CRIT#+#CRIT_HURT#+#CRIT_HURT_RED#))/5+#TALENT_POWER#+#OFFICIAL_POWER#/6+#AVATAR_POWER#/6+#PET_POWER#/6+#SILKBAG_POWER#+#AVATAR_EQUIP_POWER#+#TREE_POWER#/6+#HORSE_POWER#+#JADE_POWER#",},
...
function formula.indexOf(index)
    ...
    if Lang.lang ~= "cn" and Lang.lang ~= _lang then 
        _lang = Lang.lang 
        if Lang.isFileExist("app/i18n/".. _lang .."/config/formula.lua") then _isExist =  true end
    end
    if _lang ~= "cn" and _isExist then
        local table = require( "app.i18n.".. _lang ..".config." .. "formula" )
        return setmetatable({_raw = formula._data[index], _lang=table._data[index], _lang_key_map=table.__key_map}, mt)
    end
