...
function Unit:ctor(data, enterAction, enterCallback)
    ...
    if self.limitLevel and self.limitLevel >= HeroConst.HERO_LIMIT_MAX_LEVEL then
...
function Unit:createBillBoard()
    ...
    self._billBoard:setPosition(cc.p(self._positionIdle[1], self._positionIdle[2] + 175))
...
function Unit:skill(skillInfo, targets, unitPartner)
    ...
    if skillInfo.skill_type == 2 or skillInfo.skill_type == 3 then
		...
		if not Lang.checkLang(Lang.CN) then
			show = StateShow.new(self, skillPlay, skillInfo.skill_type, prePosition,skillInfo)
		else	
			show = StateShow.new(self, skillPlay, skillInfo.skill_type, prePosition)
		end
        self:addState(show)
    end
    ...
    -- self._moveBack = nil
    --攻击后回位
    if prePosition and self.to_alive then
        local cameraTargetPos = nil
        if skillPlay.camera_location_type ~= 0 then
...
function Unit:updateHP(value)
    ...
    if self._billBoard then
        local hpPercent = self.showHp / self.maxHp
        ...
        if hpPercent <= 0.05 and hpPercent ~= 0 then
            hpPercent = 0.05
        end
        self._billBoard:updateHP(hpPercent)
        if value == 0 then
            self._billBoard:updateHpShadow()
        end
    end
...
function Unit:doEndHpEffect(damage, hitInfo)
    ...
    if self:getState() == "StateIdle" and value < 0 then
...
--buff 展示
function Unit:buffPlay(buffId, damage, isAngerBuff, dieIndex, sound)
    if not self.is_alive then
        return
    end
    ...
    if damage and damage.showValue and damage.value then
        ...
        --     if FightConfig.HP_TEST_ID == self.stageID then
...
function Unit:playFeature(skillId, callback)
	...
	-- i18n change skill
	if not Lang.checkLang(Lang.CN) then
		self._hitTipView:popup(skillPath,  _, "feature", cc.p(self._positionFrame[1], self._positionFrame[2]), callback, skillInfo)
	else
...
--展示技能
function Unit:showSkill(imageId,name)
	-- i18n change skill
	if not Lang.checkLang(Lang.CN) then
		self._actor:showSkill(imageId,name)
	else
...
function Unit:doHurt(type, damage, showDamage, hurts)
    local damageType = 0
    if type == 1 then
        damageType = -1
    elseif type == 2 then
