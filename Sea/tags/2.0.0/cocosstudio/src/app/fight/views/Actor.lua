...
function Actor:setAction(name, loop)
	...
	if string.find(name, "skill") == 1 or string.find(name, "win") or string.find(name, "open") or string.find(name, "coming") then
...
function Actor:showSkill(imageId,name)
	...
	local effect = G_EffectGfxMgr:createPlayMovingGfx( self._frontLayer, "moving_tiaozi_jineng", effectFunction, nil , true )
