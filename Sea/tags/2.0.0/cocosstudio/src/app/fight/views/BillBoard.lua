local BillBoard = class("BillBoard", function()
...
function BillBoard:ctor(name, quality, rankLevel, isPlayer, officelLevel, showMark, camp)
    ...
    self._hp = cc.Sprite:create(Path.getBattleRes("img_battle_hp"))
    ...
    if not Lang.checkLang(Lang.CN) then
        labelName = cc.Label:createWithTTF(name, Path.getCommonFont(), 20)
    else
        labelName = cc.Label:createWithTTF(name, Path.getCommonFont(), 22)    
    end
    ...
    self._hpPercent = 1 --目标百分比
    self._angerEffect = nil
    -- self._shadowHpNowPercent = 1  --现在百分比
...
--更新绿色的血条
function BillBoard:updateHP(percent)
    self._hpPercent = percent
    self._hp:setScaleX(percent)
end
...
function BillBoard:updateHpShadow(needScaleMoving) 
    if needScaleMoving then
        ...
        local action2 = cc.CallFunc:create(function() 
                                            self._hpShadow:setScaleX(self._hpPercent)
                                            self._hpShadow:setOpacity(255) 
                                        end)
        local action = cc.Sequence:create(action1, action2)
        self._hpShadow:runAction(action)
    else
        -- self._shadowHpNowPercent = self._hpPercent
        self._hpShadow:setScaleX(self._hpPercent)
        -- self._needUpdate = false
    end
end
...
function BillBoard:update(f)
    ...
    --     if self._shadowHpNowPercent <= self._hpPercent then
    --         self._shadowHpNowPercent = self._hpPercent
    --         self._hpShadow:setScaleX(self._hpPercent)
    --         self._needUpdate = false
    --     else
    --         self._shadowHpNowPercent = self._shadowHpNowPercent - 0.02
    --         print("1112233 set percent = ", self._shadowHpNowPercent)
    --         self._hpShadow:setScaleX(self._shadowHpNowPercent)
    --     end
    -- end
end
