...
function Preload:ctor(report)
	...
	for iWave,vWave in ipairs(waves) do
		...
		local stars = vWave:getBattleStar()
		for iStar, vStar in ipairs(stars) do 
			local starData = HistoricalHero.get(vStar.baseId)
			assert(starData, "wrong history hero id = "..vStar.baseId)
			local heroRes = HeroRes.get(starData.res_id)
			assert(heroRes, "wrong Hero res id = "..starData.res_id)
			if heroRes.story_res_spine ~= 0 then 
				self:_addLoad(Path.getStorySpine(heroRes.story_res_spine))
			end
		end
	end
	...
	for i,v in ipairs(skillIds) do
		...
		assert(skillPlay, "skillPlay wrong skill id "..tostring(skillInfo.id).." wrong skillPlayId "..tostring(skillInfo.skill_show_id))
...
function Preload:start()
	for k,v in pairs(self._loads) do
		local ret = G_SpineManager:addSpineAsync(k, HeroSpineNode.SCALE, function ()
