...
function StateSkill:onDamageEvent(value1, value2)
    ...
    --如果有吸收信息这边处理一下
    local singleValue = math.ceil(damageInfo.showValue * p)
    local realDamage = math.ceil(damageInfo.value * p)
    for i, v in pairs(self._selfHitInfo.hurts) do
        if v.hurtId == 5 then 
    ...
    if damageType ~= 0 then
        self._entity:doHurt(damageType, realDamage, singleValue, self._selfHitInfo.hurts)
