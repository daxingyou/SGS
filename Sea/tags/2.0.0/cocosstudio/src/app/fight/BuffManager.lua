...
--增加历代名将的buff
function BuffManager:addHistoryBuff(data)
    table.insert(self._buffHistory, data)
end
...
function BuffManager:_checkAddBuff(effectTime, attackId, targetId, isNoSkill)
    local function addBuffFunc(i)
        ...
        if unit then
            -- print("1112233 buff unit id =  ", unit.stageID)
...
function BuffManager:_checkBattleEffect(effectTime, attackId, targetId)
    ...
    for i = #self._battleEffect, 1, -1 do
        ...
        if isShowBuff then
            -- if self._battleEffect[i].skillId == 1500099 and self._battleEffect[i].configId == 2385 then 
            --     assert(nil,"1112233 fuck    "..effectTime.."    "..attackId)
            -- end
            doBattleEffectFunc(i)
        end
    end
