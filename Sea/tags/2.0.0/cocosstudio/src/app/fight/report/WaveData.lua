...
local function makeUnit(data)
    ...
    if rawget(data, "show_mark") then
        for i = 1, #data.show_mark do
            table.insert(unit.showMark, data.show_mark[1])
        end
    end
    return unit
end
...
local function makeBattleEffect(data, isPet)
    ...
    battleEffect.damage.value = data.actual_value or data.value
    battleEffect.damage.showValue = data.value
