...
-- i18n change lable
	...
	self._buttonOK:setString("前 往")
	if not Lang.checkLang(Lang.CN) then
		self._buttonOK:setString(Lang.getImgText("item_guide_goto"))
	end
...
function PopupItemGuiderCell:updateUI(index,cellValue)
	...
	self._imageIcon:loadTexture(Path.getCommonIcon("main",cellValue.icon))
	...
	if cellValue.type == 1 then
		local stroyName = getStroyName(cellValue.value)
		local isOpen = G_UserData:getStage():isStageOpen(cellValue.value)

		if isOpen == true then
			local stroyCount, maxCount = G_UserData:getStage():getStageCount(cellValue.value)
			local leftCount = maxCount - stroyCount
			local starCount = G_UserData:getStage():getStarById(cellValue.value)
			...
			if not Lang.checkLang(Lang.CN) then
				...
				local worldPos = self._textNum:convertToWorldSpaceAR(cc.p(0,0))
				local newPos = self._panelCon:convertToNodeSpace(cc.p(worldPos))

				self._starNode:setPositionX(
					newPos.x + self._textNum:getContentSize().width + 5
				)
			end			
		...
		local dirName = Lang.getTxt(cellValue.directions, {name = stroyName})
	...
	elseif(state == ccui.TouchEventType.ended)then
		...
		if(moveOffsetX <= 20 and moveOffsetY <= 20) and isTouchEnabled == true then
			if self._cellValue ~= nil then
				...
				self:dispatchCustomCallback(sender:getTag())
			end
		end
...
-- i18n change lable
function PopupItemGuiderCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
        self._nodeNoOpen = UIHelper.swapSignImage(self._nodeNoOpen,
		{ 
			 style = "signet_1", 
			 text = Lang.getImgText("img_weikaiqi01") ,
...
-- i18n change lable
function PopupItemGuiderCell:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._textDesc:setTextAreaSize(cc.size(314, 60))
        self._textDesc:setPositionY(self._textDesc:getPositionY()-2)
		--self._textDesc:setAnchor()

		
	end

	if Lang.checkLang(Lang.EN) then
		self._textDesc:setFontSize(self._textDesc:getFontSize()-2)
	end
end
