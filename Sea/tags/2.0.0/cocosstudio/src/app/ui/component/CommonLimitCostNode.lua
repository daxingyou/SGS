...
function CommonLimitCostNode:ctor(target, costKey, callback)
    self._target = target
...
function CommonLimitCostNode:_init()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		local label = UIHelper.swapWithLabel(self._imageName,{ 
				style = "limit_1", 
				text = Lang.getImgText(LimitCostConst.RES_NAME[self._costKey].imageName),
		})
		self._imageName = label
	else
		self._imageName:loadTexture(Path.getTextLimit(LimitCostConst.RES_NAME[self._costKey].imageName))
    end
...
function CommonLimitCostNode:getMoving()
    return LimitCostConst.RES_NAME[self._costKey].moving
end

-- 重写
function CommonLimitCostNode:initImageFront()
    self:_initImageFront(
        LimitCostConst.RES_NAME[self._costKey].imageButtom,
        LimitCostConst.RES_NAME[self._costKey].imageFront
    )
end

-- 重写
function CommonLimitCostNode:initRipple()
    self:_initRipple(LimitCostConst.RES_NAME[self._costKey].ripple)
end

-- 重写
function CommonLimitCostNode:initEffectBg()
    self:_initEffectBg(LimitCostConst.RES_NAME[self._costKey].effectBg)
end
...
function CommonLimitCostNode:changeImageName()
    ...
    if Lang.checkLang(Lang.CN) then
        ...
        self._imageName:setString(
            Lang.getImgText(LimitCostConst.RES_NAME[self._costKey].imageName)
        )
    end
end
...
function CommonLimitCostNode:updateUI(limitLevel, curCount)
    self:_updateCommonUI(limitLevel, curCount)
end

function CommonLimitCostNode:_updateCommonUI(limitLevel, curCount)
    ...
    if self._isShowCount then --显示数量
        ...
        local content = Lang.get("hero_limit_cost_count", {curCount = curCount, totalCount = totalCount})
...
-- 计算节点数量比值 需重写
function CommonLimitCostNode:_calPercent(limitLevel, curCount)
    return 0, 1
end

function CommonLimitCostNode:playRippleMoveEffect(limitLevel, curCount)
    ...
    if self._isShowCount then --显示数量
        self._textPercent:setString("")
        self._nodeCount:removeAllChildren()
        local content = Lang.get("limit_cost_count", {curCount = curCount, totalCount = totalCount})
        local richText = ccui.RichText:createWithContent(content)
        richText:setAnchorPoint(cc.p(0, 0.5))
        self._nodeCount:addChild(richText)
    else
...
function CommonLimitCostNode:getEffectReceiveName()
    return LimitCostConst.RES_NAME[self._costKey].effectReceive
end
...
function CommonLimitCostNode:getFullEffectName()
    return LimitCostConst.RES_NAME[self._costKey].effectFull
end

-- 重写 播放突界动画
function CommonLimitCostNode:playSMoving()
    self:_playSmoving(LimitCostConst.RES_NAME[self._costKey].smoving)
end
