...
function CommonShopItemCell:updateUI(shopItem, isShowOutLineColor)
	self._nodeFragment:removeAllChildren()
	-- i18n RichText
	if Lang.checkLang(Lang.CN) then
		self._textFragment = ccui.RichText:create()
	else
		self._textFragment = ccui.RichText:createByI18n()         
	end
	...
	if configType == "fix" then
		self:_updateFixCell(shopItem, isShowOutLineColor)
	...
	if Lang.checkLang(Lang.EN)  then 
		if self.__cname == "CommonShopItemCell2" then
			if Lang.checkLang(Lang.EN) then 
				self._textItemName:setAnchorPoint(0.5,1)
				self._textItemName:getVirtualRenderer():setMaxLineWidth(190)
				self._textItemName:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
			end
		else
			self._nodeFragment:setPosition(118,78)
			self._textFragment:setScale(0.9)
			self._textBtnDesc:setPosition(118,60)
			self._textItemName:setFontSize(18)
			local posY = 90
			if self._nodeFragment:isVisible() then
				posY = 100
			end
			self._textItemName:setPositionY(posY)
			self._imageCost:setPositionY(45)
			self._costRes1:setPositionY(9)
			self._costRes2:setPositionY(9)
		end
	end
end
...
function CommonShopItemCell:_updateFixButton(shopItem)
	local function hardCode()
		...
		if not Lang.checkLang(Lang.CN) then
			...
			self._textBtnDesc:setPosition(cc.p(118,self._seasonOffsetY - 20))
			self._textBtnDesc:setAnchorPoint(cc.p(0.0, 0.5))
		end
	end
...
function CommonShopItemCell:_showFragmentNum(type, value)
	...
	self._nodeFragment:setPosition(txtX + textContent.width + 4, txtY)--中文括号空隙比较大
...
-- i18n change lable
function CommonShopItemCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
        self._imageBuyed = UIHelper.swapSignImage(self._imageBuyed,
		{ 
			 style = "signet_3", 
			 text = Lang.getImgText("img_seal_yigoumai01") ,
...
function CommonShopItemCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		self._textItemName:setPositionY(self._textItemName:getPositionY()+8)
	end
end

function CommonShopItemCell:_setFontSizeByI18n()
	if Lang.checkLang(Lang.TW) then
		self._costRes1:setFontSize(16)
		self._costRes2:setFontSize(16)
	end
end
