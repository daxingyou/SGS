...
function CommonHeroProperty:buildSkillModule()
	...
	local heroRankConfig = UserDataHelper.getHeroRankConfig(heroUnitData:getBase_id(), 0, limitLevel)
	for i = 1, 3 do
...
function CommonHeroProperty:buildTalentModule()
	...
	if heroUnitData:isCanBreak() then
		self._talentModule = HeroDetailTalentModule.new(heroUnitData, nil, self._limitLevel)
...
function CommonHeroProperty:buildKarmaModule()
	...
	self._karmaModule = HeroDetailKarmaModule.new(heroUnitData)
...
function CommonHeroProperty:updateUI(heroId, heroBaseId, rangeType, limitLevel)
	local rank = 0
	if heroId then
	...
	self._heroName2:setName(self._heroUnitData:getBase_id(), rank, limitLevel)
end
