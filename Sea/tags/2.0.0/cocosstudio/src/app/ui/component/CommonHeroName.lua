...
function CommonHeroName:setName(baseId, rankLevel, limitLevel, outLine)
	if baseId == nil then
		self._target:setVisible(false)
		return
	end
	local heroParam = TypeConvertHelper.convert(self._convertType, baseId, nil, nil, limitLevel)

	local heroName = heroParam.name
	if rankLevel and rankLevel > 0 then 
		if heroParam.color == 7 and limitLevel == 0 then  -- 金将
...
function CommonHeroName:setNameInUserDetail(name, officialLevel, rankLevel)
	...
	local iconColorOutline = Colors.getOfficialColorOutline(officialLevel)
	if rankLevel and rankLevel > 0 then
		name = name .. "+" .. rankLevel
	end
	...
	-- self._textName:enableOutline(iconColorOutline, 2)
	self._target:setVisible(true)
end
