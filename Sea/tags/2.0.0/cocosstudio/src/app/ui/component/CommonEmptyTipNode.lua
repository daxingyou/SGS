...
local TYPE2PARAM = {
	...
	[9] = {str = "common_empty_tip_9", funcId = FunctionConst.FUNC_PET_SHOP}, 	--神兵碎片列表空置 i18n 神兽列表空置跳转错误
	...
	[16] = {str = "common_empty_tip_16", funcId = FunctionConst.FUNC_HISTORY_HERO}, 	--武器列表空置
...
function CommonEmptyTipNode:_init()
	...
	if Lang.checkLang(Lang.KR) or Lang.checkLang(Lang.TW) then
		...
		if self._textTip:getParent() == self._image then
			self._textTip:retain()
			self._textTip:removeFromParent()
			self._image:getParent():addChild(self._textTip)
			self._textTip:release()
			self._textTip:setPosition(self._image:getPosition())
		end
	end
	-- i18n  ex
	if G_ConfigManager:isGoogleFeatureOpen() then
