...
function CommonTabGroup:_initTabList()
    if self._containerStyle == 2 then
        ...
        if self._isVertical == 2 then --水平
            ...
            local needWidth = loopCount * (templateNodeSize.width + self._nodeOffset) - self._nodeOffset
...
function CommonTabGroup:setImageTipByTabIndex(tabIndex, show, posPercent, texture)
    ...
    if self._isVertical == 2 then -- 水平
        self:_showImageTip(item.panelWidget, show, posPercent or cc.p(0.92, 0.8), texture)
    else
...
function CommonTabGroup:_updateTextImgTab(tabItem)
    ...
    if textWidget and text then
        if Lang.checkLang(Lang.EN)  then
            local UIHelper  = require("yoka.utils.UIHelper")
            textWidget:setString(UIHelper.dealVText(text))
        else
            if Lang.checkLang(Lang.TH) then
                textWidget:getVirtualRenderer():setLineSpacing(5)
            end
...
-- i18n change lable
function CommonTabGroup:_dealByI18n()
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text_desc = UIHelper.seekNodeByName(self._target,"Text_desc")
        text_desc:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
        local className = self._target:getComponent("ComClassData"):getClassName()
        if Lang.checkLang(Lang.TH) and className == "CommonTabGroupHorizon" then
            text_desc:setPositionY(text_desc:getPositionY()+5)
        end
    end
end
