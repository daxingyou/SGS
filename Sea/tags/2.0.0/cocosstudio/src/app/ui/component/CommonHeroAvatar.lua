...
--获取SpineId
function CommonHeroAvatar:_getSpineData(heroId, animSuffix, limitLevel)
	local TypeConvertHelper = require("app.utils.TypeConvertHelper")
...
function CommonHeroAvatar:_init()
	...
	if Lang.checkLang(Lang.TH) then
		self._textTalk:getVirtualRenderer():setLineSpacing(10)
		self._textTalk2:getVirtualRenderer():setLineSpacing(10)
	end
end

function CommonHeroAvatar:_playAnim(anim, isLoop, isReset)
    print("1112233 play anim ", anim)
...
function CommonHeroAvatar:updateUI(heroId, animSuffix, notPlayIdle, limitLevel, callback, needAnimSp)
    assert(heroId, "CommonHeroAvatar's heroId can't be nil ")
	self:_initSpine(heroId, animSuffix, limitLevel, callback, notPlayIdle)
...
function CommonHeroAvatar:updateAvatar(avatarInfo, animSuffix, notPlayIdle, callback)
	...
	if avatarInfo.isHasAvatar then
		...
		if avatarConfig.limit == 1 then --橙升红标记
			--界限突破参数
			self:updateUI(avatarInfo.covertId, animSuffix, notPlayIdle, HeroConst.HERO_LIMIT_MAX_LEVEL, callback)
		else
...
function CommonHeroAvatar:setAction(name, loop)
...
function CommonHeroAvatar:_initSpine(heroId, animSuffix, limitLevel, callback, notPlayIdle)
	self._heroId = heroId
...
function CommonHeroAvatar:showVName(visible)
	...
	if self._heroId and self._heroId > 0 then
		...
		elseif not Lang.checkLang(Lang.CN) then
			...
			UIHelper.dealVTextWidget(textName,heroData.name)
		else
			textName:setString(heroData.name)
		end
...
function CommonHeroAvatar:updateHeroName(name, color, useVName)
	...
	if useVName then
		...
		elseif not Lang.checkLang(Lang.CN) then
			local UIHelper  = require("yoka.utils.UIHelper")
			UIHelper.dealVTextWidget(textName,name)
		else
			textName:setString(name)
		end
...
-- i18n
function CommonHeroAvatar:_swapImageHorizontal()
	if Lang.checkHorizontal() then
		self._panelVName:setBackGroundImageScale9Enabled(true)
		self._panelVName:setBackGroundImageCapInsets(cc.rect(10,10,1,1))
		self._panelVName:setBackGroundImage(Path.getCommonImage("img_mingzidi_h"))
		self._panelVName:setPosition(0,220)
		self._panelVName:setAnchorPoint(0.5,0.5)
		local textName = self._panelVName:getSubNodeByName("Text_v_name")
		textName:setAnchorPoint(0.5,0.5)
		textName:setPositionY(15)
		textName:ignoreContentAdaptWithSize(true)
	end
end
-- i18n
function CommonHeroAvatar:_dealHorizontal()
	if Lang.checkHorizontal() then
		local textName = self._panelVName:getSubNodeByName("Text_v_name")
		local width = textName:getContentSize().width + 30
		self._panelVName:setContentSize(cc.size(width,30))
		textName:setPositionX(width/2)
		self._imageCountry:setPosition(-width/2-13,self._panelVName:getPositionY())
	end
end
