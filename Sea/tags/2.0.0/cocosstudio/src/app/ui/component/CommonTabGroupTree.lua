...
function CommonTabGroupTree:_init(param)
    ...
    self:setCustomColor({
		{cc.c3b(0xb8, 0xc9, 0xee)},
		{cc.c3b(0x9f, 0x4d, 0x1b)},
...
function CommonTabGroupTree:_updateTabItem(tabItem)
    ...
    if Lang.checkLang(Lang.TH) then
        tabItem.textWidget:getVirtualRenderer():setLineSpacing(8)
        local image = ccui.Helper:seekNodeByName(tabItem.panelWidget, "Image_1")
        if image then
            image:setPosition(100,10)
        end
    end
    --高亮按钮
