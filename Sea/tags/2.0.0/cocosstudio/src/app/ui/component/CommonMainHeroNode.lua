...
function CommonMainHeroNode:updateUI(heroId, isEquipAvatar, limitLevel)

	self._avatar:updateUI(heroId, nil, nil, limitLevel)
	...
	if not Lang.checkLang(Lang.CN) then
		self._heroLevel:setPositionY(height-10)
	end
...
function CommonMainHeroNode:updateHeroName(heroId,heroRank, heroLevel, limitLevel)
	local heroParams = TypeConvertHelper.convert(TypeConvertHelper.TYPE_HERO, heroId, nil, heroRank, limitLevel)
	if heroParams then
...
-- i18n pos lable
function CommonMainHeroNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        self._heroName:setFontSize(
             self._heroName:getFontSize()-2
        )

		self._textOfficial:setFontSize(
             self._textOfficial:getFontSize()-2
        )
	end
end
