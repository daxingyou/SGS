	...
	if funcName == "shopNumBanType" then
		...
		if type == ShopConst.NORMAL_SHOP_SUB_MONEY then --商店充值商品特殊处理
			...
			if not Lang.checkLang(Lang.CN) then
				...
				local _,currencyStr1 = UIHelper.convertCurrency(money)
				currencyStr1 = UIHelper.convertDollar(currencyStr1)
				self._buttonOK:setString(Lang.get("common_rmb",{num = currencyStr1}))
			else
...
function CommonShopItemCell2:_updateFixCell(shopItem)
	local fixData = shopItem:getConfig()
...
-- i18n change lable
function CommonShopItemCell2:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
...
-- i18n change lable
function CommonShopItemCell2:_dealI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		--local textItemName = UIHelper.seekNodeByName(self,"Text_item_name")
		self._textItemName:setFontSize(18)
	end
	if Lang.checkLang(Lang.ZH) then
		self._textItemName:setFontSize(20)
		self._textItemName:setPositionY(112)
	end
end
