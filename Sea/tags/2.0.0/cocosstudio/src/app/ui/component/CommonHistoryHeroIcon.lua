		...
		if moveOffsetX < 20 and moveOffsetY < 20 then
			if self._callback then
				...
				local PopupHeroDetail = require("app.scene.view.historyhero.PopupHistoryHeroDetail").new(
				        self._type, self._itemParams.uniqueId, self._itemParams.cfg.id, nil, 0)
				--local PopupHeroDetail = require("app.scene.view.heroDetail.PopupHeroDetail").new(TypeConvertHelper.TYPE_HISTORY_HERO, self._itemParams.cfg.id, nil, 0)
