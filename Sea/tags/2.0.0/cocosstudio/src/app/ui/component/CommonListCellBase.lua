...
function CommonListCellBase:updateUI(type,value,size)
	self._commonIcon:unInitUI()
	self._commonIcon:initUI(type,value,size)

	local params = self._commonIcon:getItemParams()
...
function CommonListCellBase:setName(name, color)
	if name == nil then
		name = ""
	end
...
function CommonListCellBase:setCountText(richText)
	...
	if richText then
		...
		if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
			self._nodeCount:setPosition(cc.p(posX, posY-38))
		end
	end
end
...
-- i18n pos lable
function CommonListCellBase:_dealPosI18n()
	if not Lang.checkLang(Lang.CN) and not Lang.checkLang(Lang.TW) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textName:setPositionX(self._textName:getPositionX()-10)
	end
end
