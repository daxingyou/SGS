...
--根据传入参数，创建并，更新UI
--由于加入界限突破参数，重写此接口
function CommonHeroIcon:updateUI(value, size, limitLevel)
	if value == 0 then
		self:refreshToEmpty()
		return
	end
...
function CommonHeroIcon:updateIcon(avatarInfo, size, frameId, scale)
	...
	if avatarInfo.isHasAvatar then
		...
		if avatarConfig.limit == 1 then   --橙升红标记
			self:updateUI(avatarInfo.covertId, size, HeroConst.HERO_LIMIT_MAX_LEVEL)--界限突破参数
		else
	...
	if(state == ccui.TouchEventType.ended)then
		...
		if moveOffsetX < 20 and moveOffsetY < 20 then
			if self._callback then
				self._callback(self._target, self._itemParams, self._limitLevel)
			else
