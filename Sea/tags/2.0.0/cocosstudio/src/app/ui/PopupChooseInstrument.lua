...
function PopupChooseInstrument:updateUI(fromType, callBack, instrumentDatas, showRP, curEquipUnitData)
...
function PopupChooseInstrument:_onItemUpdate(item, index)
	...
	if self._instrumentDatas[index + 1] then
		...
		data1 = PopupChooseInstrumentHelper.addInstrumentDataDesc(instrumentdata, self._fromType)
	end

	if self._instrumentDatas[index + 2] then
		local instrumentdata = self._instrumentDatas[index + 2]
		data2 = PopupChooseInstrumentHelper.addInstrumentDataDesc(instrumentdata, self._fromType)
	end
