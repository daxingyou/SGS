...
local UserDetailJadeNode = require("app.scene.view.team.UserDetailJadeNode")
...
function PopupUserDetailInfo:_updateLeftIcons()
	...
	for i, data in ipairs(iconData) do
		...
		if icon then
			icon:onlyShow(data.type, data.value, data.limitLevel)
		end
	end
end
...
function PopupUserDetailInfo:_updateHeroPageView()
	...
	for i = minPos, maxPos do
		...
		self._pageItems[i]:updateUI(info.type, info.value, nil, info.limitLevel)
...
function PopupUserDetailInfo:switchPanelView(index)
	...
	if layer == nil then
		...
		elseif self._curSelectedPanelIndex == SWItCH_JADE_NODE then
			layer = UserDetailJadeNode.new(self)
		end
		if layer then
...
function PopupUserDetailInfo:_updateView()
	...
	if layer then
		...
		elseif self._curSelectedPanelIndex == SWItCH_YOKE_NODE then
			...
			layer:updatePanel()
		elseif self._curSelectedPanelIndex == SWItCH_JADE_NODE then
			layer:updateView(self._detailData, self._pos)
		end
	end
end
...
function PopupUserDetailInfo:_onLeftIconClicked(pos)
	...
	if self._curSelectedPanelIndex == SWItCH_JADE_NODE then
		...
		self:_updateHeroIconsSelectedState()
		self:_updateHeroPageView()
		self:switchPanelView(SWItCH_JADE_NODE)
		return
	end
	if info.type == TypeConvertHelper.TYPE_HERO then
...
-- 切换到查看玉石
function PopupUserDetailInfo:switchToJadeNode()
	self:switchListLineUp(LIST_JADE)
	self:switchPanelView(SWItCH_JADE_NODE)
end
