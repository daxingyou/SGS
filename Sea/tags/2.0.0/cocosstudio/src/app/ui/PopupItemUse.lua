...
function PopupItemUse:checkSlectNum(isClose)
    ...
    if isClose and self._selectNumNode:getMaxLimit() == 0 then
        G_Prompt:showTip(Lang.get("common_tip_enough", {name = self._itemName}))
        return false
    end
    if self._itemName and self._selectNumNode:getMaxLimit() == 0 then
        G_Prompt:showTip(Lang.get("common_tip_enough", {name = self._itemName}))
        return false
    end

	if rawequal(self._useNum, self._selectNumNode:getMaxLimit()) then
         -- 1. Func活动商店/ 2.Shop商品ID 同步2.7代码修复提示异常
...
function PopupItemUse:updateUI(itemType,itemId,itemSize)
    ...
    self._itemName = itemParams.name
end
...
-- i18n pos lable
function PopupItemUse:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		
		self._itemOwnerCount:setAnchorPoint(cc.p(0,0.5))
		self._itemOwnerCount:setPositionX(self._itemOwnerDesc:getPositionX() + 
			self._itemOwnerDesc:getContentSize().width + 2
		 )	


		 if self._textTips then
		 	self._textTips:setAnchorPoint(cc.p(0.5,0.5))
			local parent = self._textTips:getParent()
			local size = parent:getContentSize()
			self._textTips:setPositionX(size.width*0.5)

			self._textTips:setTextHorizontalAlignment(1 )
			self._textTips:getVirtualRenderer():setLineSpacing(tonumber(-3) )
			self._textTips:getVirtualRenderer():setMaxLineWidth(500)
		 end
		if self._costResInfo1 then
			self._costResInfo1:setPositionX(self._costResInfo1:getPositionX() + 40 )	
		end
		if self._costResInfo2 then
			self._costResInfo2:setPositionX(self._costResInfo2:getPositionX() + 10 )
		end
	
	end
	if Lang.checkLang(Lang.EN) then
		self._itemName:setAnchorPoint(0,1)
		self._itemName:setPositionY(43)
		self._itemName:getVirtualRenderer():setMaxLineWidth(320)
		self._textTips:getVirtualRenderer():setLineSpacing(0)
	end
end
