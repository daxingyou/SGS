...
function PromptTip:_updateText(node, text)
    ...
    if Lang.checkChannel(Lang.CHANNEL_SEA) then
        label:getVirtualRenderer():setMaxLineWidth(420)
        label:ignoreContentAdaptWithSize(true)
        local contentSize = label:getVirtualRendererSize()
        local background = node:getSubNodeByName("Image_tip_background")
        local backContentSize = background:getContentSize()
        background:setContentSize(cc.size(backContentSize.width, contentSize.height + 35))
        return
    end
    -- 宽度超过一定范围就换行
...
function PromptTip:_updateRichTextType2(node, params)
	if params and params.str then
        ...
        if Lang.checkLang(Lang.TH) then
            richText = UIHelper.createMultiAutoCenterRichText(params.str, params.defaultColor or Colors.OBVIOUS_YELLOW, params.fontSize or 22, 10)
        end
		local label = node:getSubNodeByName("Text_tip_content")
	    richText:setPosition(label:getPosition())
	    label:setVisible(false)
