...
function PopupBoxReward:setChapterDesc(num)
	...
	if Lang.checkLang(Lang.EN) or Lang.checkLang(Lang.TH) then
		self._nodeChapter:setPositionX(0)
		local detail4 = node:getChildByName("detail4")
		local width = detail4:getPositionX()+detail4:getContentSize().width
...
function PopupBoxReward:_createCellEx(award)
	...
	if Lang.checkChannel(Lang.CHANNEL_SEA) then
		uiNode:setPositionY(uiNode:getPositionY()+10)
		local name = ccui.Helper:seekNodeByName(uiNode,"Icon_name")
		name:setFontSize(18)
		local width = 130
		name:setContentSize(cc.size(width, width))
		name:getVirtualRenderer():setMaxLineWidth(width)
		if Lang.checkLang(Lang.TH) then
			name:getVirtualRenderer():setLineSpacing(6)
		end
		name:setAnchorPoint(0.5,1)
		name:setPositionY(43)
		local nameBg = ccui.Helper:seekNodeByName(uiNode,"ImageNameBg")
		local size = name:getVirtualRendererSize()
		width = width + 8
		nameBg:setContentSize(cc.size(width, size.height + 5))
		local panel = ccui.Helper:seekNodeByName(uiNode,"Panel_root")
		panel:setContentSize(cc.size(width, panel:getContentSize().height))
		local icon = ccui.Helper:seekNodeByName(uiNode,"Icon_template")
		icon:setPositionX(width/2)
		nameBg:setPositionX(width/2)
		name:setPositionX(width/2)
	end
