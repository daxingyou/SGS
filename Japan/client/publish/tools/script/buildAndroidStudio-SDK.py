...
def main(cfgFile=None):
    ...
    studioPath          = os.path.join(dirProjectFramework, "runtime-src","proj.android-studio")
    ...
    if androidTarget == "android-26" or apversion > 26:
        ...
        utils.copyDir(os.path.join(dirProjectFramework, "runtime-src/proj.android26/src"), os.path.join(studioPath, "app/src"))
        shutil.copy(os.path.join(dirProjectFramework, "runtime-src/proj.android26/AndroidManifest.xml"), os.path.join(studioPath, "app/AndroidManifest.xml"))
    
    # android
    utils.printSplit("修改游戏名")
    filename = os.path.join(studioPath, "app/res/values/strings.xml")
    ...
    if cfgI18n.has_key("top_server"):
        ...
        nativeAgentFileName = os.path.join(studioPath, "app/src/org/cocos2dx/sdk/NativeAgent.java")
    ...
    backupFileList = list()
    manifestFileName = os.path.join(studioPath, "app/AndroidManifest.xml")
    utils.backUpFile(backupFileList, manifestFileName)
    buildcfgFileName = os.path.join(studioPath, "build-cfg.json")
    utils.backUpFile(backupFileList, buildcfgFileName)

    files = {}
    for dirpath, dirnames, filenames in os.walk(studioPath):
    ...
    filename = os.path.join(studioPath, "build-cfg.json")
    ...
    utils.copyDir2(os.path.join(dirSdkTemp, "libs"), os.path.join(studioPath, "app", "libs"))
    utils.copyDir2(os.path.join(dirSdkTemp, "res"), os.path.join(studioPath, "app", "res"))
    

    utils.printSplit("处理gradle.properties")

    gradlePropertiesName=os.path.join(studioPath, "gradle.properties")
    utils.modifyFile1(gradlePropertiesName, lambda f, line:
    f.write(re.sub("PROP_APPLICATION_ID=.*",
            "PROP_APPLICATION_ID=%s" % cfgAppPackage, line))
    )

    if cfgNewVersion.has_key("build"):
        utils.modifyFile1(gradlePropertiesName, lambda f, line:
        f.write(re.sub("PROP_VERSION_CODE=.*",
                "PROP_VERSION_CODE=%s" % cfgNewVersion["build"], line))
        )
    else:
        utils.modifyFile1(gradlePropertiesName, lambda f, line:
        f.write(re.sub("PROP_VERSION_CODE=.*",
                "PROP_VERSION_CODE=%s" % cfgNewVersion["svn"], line))
        )
    
    utils.modifyFile1(gradlePropertiesName, lambda f, line:
        f.write(re.sub("PROP_VERSION_NAME=.*",
                "PROP_VERSION_NAME=%s" % cfgNewVersion["name"], line))
    )

    utils.modifyFile1(gradlePropertiesName, lambda f, line:
        f.write(re.sub("PROP_COMPILE_SDK_VERSION=.*",
                "PROP_COMPILE_SDK_VERSION=%s" % apversion, line))
    )

    if apversion > 19:
        utils.modifyFile1(gradlePropertiesName, lambda f, line:
            f.write(re.sub("PROP_TARGET_SDK_VERSION=.*",
                    "PROP_TARGET_SDK_VERSION=%s" % apversion, line))
        )
    ...
    if args.compatible != None and args.compatible.lower() != "false" and apversion > 19:
        bCompatible = True
    bAndroidStudio = True
    utils.printSplit("编译AndroidStudio母包")
    print(cfgBuildMode)
    print(androidTarget)
    print(app_abi)
