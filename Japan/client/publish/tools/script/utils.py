...
def getSVNInfo():
    ...
    info["n"] = linecache.getline(file, 10).strip()
    info["p"] = linecache.getline(file, 11).strip()
    return info
...
def getVersionConfigHD(DIR, cfgDir):
    cfgPath = os.path.join(DIR, cfgDir, "version_hd.json")
    cfg = json.load(open(cfgPath, 'r'))
    cfg = byteify(cfg)
    return cfg
...
def modifyConfigLua(backUp, filename, version, server, gateway, config, rolelist,clientserv):
    ...
    modifyFile(backUp, filename, lambda f, line:
        f.write(re.sub("[\s\t]*KEY_VALUE_URL[\s\t]*=.*", "KEY_VALUE_URL = \"%s\"" % clientserv, line))
    )

    printFile(filename, os.path.basename(filename))


def modifyDevelopLua(backUp, filename, guide, jumpBattle, fakeReport, speedBattle, recharge, token, version, server, gateway, config, gameop, op, rolelist, clientserv):
    modifyFile(backUp, filename, lambda f, line:
