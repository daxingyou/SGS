...
-- i18n ja 获取story_res_spine
function HeroUnitData:getStoryResSpine()
	return self:getHeroResConfig().story_res_spine
end

function HeroUnitData:getHeroResConfig()
	-- local heroBaseId, isEquipAvatar, avatarLimitLevel, arLimitLevel = AvatarDataHelper.getShowHeroBaseIdByCheck(self)
	-- local limitLevel = avatarLimitLevel or self:getLimit_level()
	-- local limitRedLevel = arLimitLevel or self:getLimit_rtg()

    local heroBaseId = self:getBase_id()
    local limitLevel = self:getLimit_level()
    local limitRedLevel = self:getLimit_rtg()

	local info = require("app.config.hero").get(heroBaseId)
