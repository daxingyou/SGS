...
function RechargeData:_s2cRechargeNotice(id,message)
	...
	if Lang.checkUI("ui4") then
		local vipPayId = nil
		local productId = rawget(message, "product_id")
		local VipPayInfo = require("app.config.vip_pay")
        for i = 1, VipPayInfo.length() do
            local rechargeInfo = VipPayInfo.indexOf(i)
            if rawequal(rechargeInfo.product_id, productId) then
                vipPayId = rechargeInfo.id
            end
        end
		if vipPayId then
			local PosterGirlVoiceConst = require("app.const.PosterGirlVoiceConst")
			G_SignalManager:dispatch(SignalConst.EVENT_POSTER_GIRL_VOICE_UPDATE,PosterGirlVoiceConst.TRIGGER_POS_PAY_SUCCESS,
				{vip_pay_id = vipPayId}
			)
		end
