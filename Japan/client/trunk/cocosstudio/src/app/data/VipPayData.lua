...
function VipPayData:getRechargeList()
    ...
    local function checkLargeAmount(rechargeInfo)
        ...
        -- i18n change pay 
        return rechargeInfo.large_amount == 0 or 
            (rechargeInfo.large_amount == 1 and G_ConfigManager:isLargeCashReCharge() and G_NativeAgent:getNativeType() ~= "ios")
    end
...
--是否是新商品
--i1n8 ja 
function VipPayData:isHasNewGoods(goodsList)
    local hasNew = false
    local goodsMap = {}
    local newGoodsList = {}
    ...
    for k,v in ipairs(self._jadeRechargeList) do
        if not goodsMap[v.cfg.id] then
            hasNew = true
        end 
        table.insert(newGoodsList,v.cfg.id)
    end
    return hasNew,newGoodsList
end

--i1n8 ja 
function VipPayData:getGoodsList()
    local newGoodsList = {}
    for k,v in ipairs(self._jadeRechargeList) do
        table.insert(newGoodsList,v.cfg.id)
    end
