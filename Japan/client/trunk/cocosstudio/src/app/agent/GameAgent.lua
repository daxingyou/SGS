...
-- i18n
GameAgent.FILE_NAME = "loginEvent"
...
function GameAgent:ctor()
	...
	-- i18n change proto
    self._deviceToken = nil     --远程推送token  ios & android
    self._isLastLogin = false   --是否存在上次登录
    ...
    -- i18n ja 绑定
    self._signalReportEvent = G_NetworkManager:add(MessageIDConst.ID_S2C_ReportEvent, handler(self, self._recvReportEvent))
...
function GameAgent:clear()
    ...
    if self._signalReportEvent then 
        self._signalReportEvent:remove()
        self._signalReportEvent = nil
    end
end
...
function GameAgent:initSDK()
    ...
    elseif Lang.checkLang(Lang.JA) then
        ...
        G_VoiceAgent:setAppKey("f3faf617681b352187b72193119f7697")
        G_VoiceAgent:setOpenId("0")
        G_VoiceAgent:setServerInfo("udp://jp.voice.gcloudcs.com:8700")
    end
    G_VoiceAgent:init()
end
...
function GameAgent:getRealNameState(callback)
    ...
    if Lang.checkLang(Lang.VN) then
        if data[uuid] == 0 then data[uuid] = nil end
    end
    if not data[uuid] then
        -- local url = "http://10.235.102.205:12167/getauthinfo?uuid="
        local url = "https://authservmjz.sanguosha.com/getauthinfo?uuid="
        if Lang.checkLang(Lang.VN) then
            -- url = "http://61.28.254.66:12167/getauthinfo?uuid="
            url = "https://authservmjz.dt3q.zing.vn/getauthinfo?uuid="
            -- url = "http://61.28.254.92:12167/getauthinfo?uuid="
        end
        local urlContent = ""
...
-- i18n change func vn
--手机验证码
function GameAgent:setPhoneAuthCode(param,callback)
    if param then 
        local sid = param.sid or self._loginServer
        local uid = param.uid or G_UserData:getBase():getId()
        local uuid = self:getTopUserId()
        local plat_uid = self._sdkUserID
        local name = param.name or G_UserData:getBase():getName()
        local phone = param.phone or ""

        -- http://127.0.0.1:8000/mobile_verify_req?zone=vn&sid=123&uid=234&uuid=xxxx&name=xxx&phone=123456778
        -- local url = "http://10.235.102.205:12167/getauthinfo?uuid="
        local url = "http://61.28.254.66:12167/mobile_verify_req?"
        if Lang.checkLang(Lang.VN) then
            url = "https://authservmjz.dt3q.zing.vn/mobile_verify_req?"
            -- url = "http://61.28.254.92:12167/mobile_verify_req?"
        end
        local urlContent = ""
        urlContent = urlContent .. "zone=" .. Lang.lang .. "&"
        urlContent = urlContent .. "sid=" .. sid .. "&"
        urlContent = urlContent .. "uid=" .. uid .. "&"
        urlContent = urlContent .. "uuid=" .. uuid .. "&"
        urlContent = urlContent .. "plat_uid=" .. plat_uid .. "&"
        urlContent = urlContent .. "name=" .. name .. "&"
        urlContent = urlContent .. "phone=" .. phone .. ""
        url = url..urlContent
        print("GameAgent:setPhoneAuthCode： url  " .. url)
        local xhr = cc.XMLHttpRequest:new()
        xhr.responseType = cc.XMLHTTPREQUEST_RESPONSE_STRING
        xhr:open("GET", url)
        local function onCallback()
            ...
            if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
                ...
                if ret then --ret and ret.status == 1
                    local retCode = tonumber(ret.ret)
                    if callback then 
                        print("GameAgent:setPhoneAuthCode： retCode  " .. retCode)
                        callback(retCode,param)
                    end
                end
            else
                -- G_Prompt:showTip("network error cod:"..xhr.status)
            end
            xhr:unregisterScriptHandler()
        end
...
-- i18n change func vn
--实名制手机绑定
function GameAgent:setRealPhone(param,callback)
    if param then 
        local sid = param.sid or self._loginServer
        local uid = param.uid or G_UserData:getBase():getId()
        local uuid = self:getTopUserId()
        local code = param.code or ""

        -- http://127.0.0.1:8000/mobile_verify_code?zone=vn&sid=123&uid=234&uuid=xxx&code=123456
        -- local url = "http://10.235.102.205:12167/getauthinfo?uuid="
        -- local url = "https://authservmjz.sanguosha.com/getauthinfo?uuid="
        local url = "http://61.28.254.66:12167/mobile_verify_code?"
        if Lang.checkLang(Lang.VN) then
            url = "https://authservmjz.dt3q.zing.vn/mobile_verify_code?"
            -- url = "http://61.28.254.92:12167/mobile_verify_code?"
        end
        local urlContent = ""
        urlContent = urlContent .. "zone=" .. Lang.lang .. "&"
        urlContent = urlContent .. "sid=" .. sid .. "&"
        ...
        xhr.responseType = cc.XMLHTTPREQUEST_RESPONSE_STRING
        xhr:open("GET", url)
        print("GameAgent:setRealPhone url  " .. url)
        local function onCallback()
            if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
                local ret = json.decode(xhr.response)
                if ret then --ret and ret.status == 1
                    ...
                    if callback then 
                        print("GameAgent:setRealPhone retCode  " .. retCode)
                        callback(retCode,param)
                    end
                end
            else
...
--
function GameAgent:onKeyboardGoBack()
    crashPrint("GameAgent:onKeyboardGoBack")
    self:exitGame()
end
        ...
        listener:registerScriptHandler(
            function(keyCode, event)
                if keyCode == cc.KeyCode.KEY_BACK then
	                G_NativeAgent:eventCustom("AT_Key_Back", '')
	                --开启返回上一层
	                if G_ConfigManager:isGoBack() then
	                    G_SceneManager:popToLastScene()
	                else --直接退出
	                    self:exitGame()
	                end
...
function GameAgent:enterGame()
    ...
    if self._isLogin then
        ...
        self._wantAutoEnterGame = true --去掉自动进游戏的机制
        self:_loginPlatform()
    end
end
...
function GameAgent:exitGame()
    ...
    -- i18n change vn
    if t == NativeConst.EXIT_TYPE_HAS_DIALOG  and Lang.checkLang(Lang.CN) then
        G_NativeAgent:exit()
    else
...
function GameAgent:_onNativeCallback(data)
	...
	-- i18n change proto
    elseif event == NativeConst.DeviceToken then
        if ret == NativeConst.STATUS_SUCCESS then
            local platform = G_NativeAgent:getNativeType()
            if platform == "android" or platform == "ios" then
                self._deviceToken = param
            end
        end
    elseif event == NativeConst.SDKBindGuestResult then
        local platform = G_NativeAgent:getNativeType()
        dump(data,"[GameAgent] SDKBindGuestResult")
        if platform == "android" or platform == "ios" then
            G_SignalManager:dispatch(SignalConst.EVENT_SDK_BINDGUEST,data)
            if ret == NativeConst.STATUS_SUCCESS then
                if Lang.checkLang(Lang.JA) then
                    local params = json.decode(param)
                    local bindType = params["type"]
                    local channel = params["channel"]
                    crashPrint("[GameAgent] SDKBindGuestResult<bindType=" .. bindType .. ", channel=" .. channel .. ">")
                    if bindType == NativeConst.SDK_SYSTEM_BIND and channel ~= NativeConst.SDK_CHANNEL_GUEST then
                        self:_onBindResult(params)
                    elseif bindType == NativeConst.SDK_SYSTEM_SWITCH then 
                        if channel ==  NativeConst.SDK_CHANNEL_CITATIONCODE then
    ...
    elseif event == NativeConst.SDKPayResult then
        ...
        elseif ret == NativeConst.STATUS_PROCESS then
			...
			if Lang.checkUI("ui4") then
				...
				G_SignalManager:dispatch(SignalConst.EVENT_POSTER_GIRL_VOICE_UPDATE,PosterGirlVoiceConst.TRIGGER_POS_PAY_FAILD)
            end
    ...
    elseif event == NativeConst.SDKShareResult then
        if ret == NativeConst.STATUS_SUCCESS then
            if Lang.checkLang(Lang.CN) then
                ...
                G_Prompt:showTip( Lang.getImgText("share_success") )
            end
        elseif ret == NativeConst.STATUS_CANCEL then
            if Lang.checkLang(Lang.CN) then
                G_Prompt:showTip("分享取消")
            else
                G_Prompt:showTip( Lang.getImgText("share_cancel") )
            end
        else
            if Lang.checkLang(Lang.CN) then
                G_Prompt:showTip("分享失败")
            else
                G_Prompt:showTip( Lang.getImgText("share_fail") )
            end
        end
    ...
    elseif event == "VoiceSpeechToText" then
        G_Prompt:showTip(ret .. ": " .. ret .. ", param: " .. param)
    elseif event == NativeConst.SDKGenCitationCodeResult then --i18n 获取引继码结果
        local codeInfo = json.decode(param)
        dump(codeInfo,"SDKGenCitationCodeResult codeInfo")
        if ret == NativeConst.STATUS_SUCCESS then
            local citationCode = codeInfo["citationCode"]
            dump(citationCode,"SDKGenCitationCodeResult citationCode")
            local PopupCitationCode = require("app.ui.PopupCitationCode").new(citationCode)
            PopupCitationCode:openWithAction()
            --上报引继码奖励
            local funcId = G_GameAgent:getSdkBindChannelRewardFuncId(NativeConst.SDK_CHANNEL_CITATIONCODE)
            G_GameAgent:c2sReportEvent({type="2",param=funcId})
        else    
            if codeInfo["error"] then
                G_Prompt:showTip( codeInfo["error"])
            elseif codeInfo["message"] then
                G_Prompt:showTip( codeInfo["message"])
            end
        end
    elseif event == NativeConst.SDKLoginWithCitationCodeResult then --i18n  使用引继码结果
        local codeInfo = json.decode(param)
        if ret == NativeConst.STATUS_SUCCESS then
            self._ticket = nil
            self._topUserID = nil
    ...
    elseif event == NativeConst.SDKTranslateWithTextResult then --i18n 翻译文本结果
        if ret == NativeConst.STATUS_SUCCESS then
            dump(param, "translate result:")
            local translateResult = json.decode(param)
            G_SignalManager:dispatch(SignalConst.EVENT_TRANSLATE_SUCCESS,translateResult)
        elseif ret == NativeConst.STATUS_FAILED then
            G_Prompt:showTip(ret .. ": " .. ret  .. ", param: " .. param)
        end
    elseif event == NativeConst.SDKSavePhotoResult then --i18n 保存到相册
        local imagePath =  cc.FileUtils:getInstance():getWritablePath()..NativeConst.SDK_TEMP_CITATIONCODE_IMG
        local aTrue = cc.FileUtils:getInstance():isFileExist(imagePath)
        if aTrue then 
            cc.FileUtils:getInstance():removeFile(imagePath)
        end
...
function GameAgent:_clearLocalLoginData()
    self._ticket = nil
    self._topUserID = nil
    self._sdkUserID = nil
    self._sdkUserName = nil
...
-- 登陆引继码
function GameAgent:_openLoginCitation()
    local PopupCitationLogin = require("app.ui.PopupCitationLogin")
    local popup = PopupCitationLogin.new(function(code,password)
        crashPrint("PopupPlayerDetail:_onClickTempCitation() code = " .. code)
        crashPrint("PopupPlayerDetail:_onClickTempCitation() password = " .. password)
...
function GameAgent:_sendLoginGame(token, sid, channel_id, device_id)
    ...
    local language = Lang.channel .. "_" .. Lang.lang .. "_" .. G_NativeAgent:getCountryCode()
...
function GameAgent:_recvLogin(id, msg)
    ...
    elseif msg.ret == MessageErrorConst.RET_OK then
        ...
        local serverLanguage = rawget(msg, "language") or ""
        Lang.server_language = serverLanguage
        release_print("Lang.server_language: " .. Lang.server_language)
...
function GameAgent:_onNotAllowed()
    ...
    local tip = G_ConfigManager:getServerNotAllowedTipI18n()--i18n
...
function GameAgent:_recvFlush(id, message)
    if message.ret == MessageErrorConst.RET_OK then
        if Lang.checkUI("ui4") then
            G_UserData:getBase():setOnlineTime(message.today_online_time,message.this_online_time) --i18n ui4 
        end
...
function GameAgent:_onLoginedGame()
    ...
    if Lang.checkUI("ui4") then
        G_PosterGirlManager:start()
        G_NewLevelPkgManager:start()
    end
    ...
    --设置sdk数据
    self:_updateGameData()
    --i18n change 多语言上报用户数据
    self:_reportUserInfo()
    -- 推送登陆成功事件
...
function GameAgent:_onEventLevelUp()
    ...
    StoreCommentHelper.setShowFlag()
end
...
function GameAgent:getRecommendServer()
    ...
    if server == nil then
        crashPrint("[GameAgent] getRecommendServer getLastServer not found!")
        server = G_ServerListManager:getFirstServer()
        crashPrint("[GameAgent] getRecommendServer getFirstServer = " .. #G_ServerListManager:getList())
    end
...
function GameAgent:isSettingRealName()
    local isSetting = false
    if Lang.checkLang(Lang.VN) then  -- 充值不用防沉迷
        isSetting = true
    end
    return isSetting
end
...
function GameAgent:pay(appid, price, productId, productName, productDesc)
    if G_ConfigManager:isRecharge() == false then
        if Lang.checkLang(Lang.CN) then
            ...
            G_Prompt:showTip(Lang.getImgText("recharge_not_open"))           
        end
        return 1
    end
    ...
    -- i18n change pay
    local isSetting = false
    if Lang.checkLang(Lang.VN) then  -- 越南充值不用防沉迷
        isSetting = true  
    end
    if G_ConfigManager:isRealName() and not self._isRealName and not isSetting then    --未实名认证
...
-- i18n change proto
function GameAgent:_reportUserInfo()
    -- 
    local pushToken = self._deviceToken or ""
    local platform = G_NativeAgent:getNativeType()
    local appId = "DT3Q"
    -- local pushToken = "9f22c9cdeb6726057161773f5fcbac75520c4b6dab6ec8780f2adce56652f7eb"
    -- local platform = "ios"
    -- local appId = "447"
    local nativeType = 2 -- 客户端类型 1 安卓 2 ios  默认为ios
    if platform == "android" or platform == "ios" then
        if platform == "android" then nativeType = 1 end
        if platform == "ios" then nativeType = 2 end
        G_NetworkManager:send(MessageIDConst.ID_C2S_ReportUserInfo, {
            push_token = pushToken,
            client_type = nativeType,
            app_id = appId
        })
    end
    print("reportUserInfo platform = " .. platform)
    print("reportUserInfo push_token = " .. pushToken)
    print("reportUserInfo client_type = " .. nativeType)
    print("reportUserInfo app_id = " .. appId)
end

-- i18n custom event
function GameAgent:_customEvent()
    local createTime = G_UserData:getBase():getCreate_time()
    local date = G_ServerTime:getDateObject(createTime)
    local secondDay = createTime - date.hour*3600 - date.min*60 - date.sec + 24*3600
    local thirdDay = secondDay + 24*3600
    local now = G_ServerTime:getTime()
    --创角次日
    if now >= secondDay and now < thirdDay then
        local rechargeTotal = G_UserData:getBase():getRecharge_total()
        local rechargeToday = G_UserData:getBase():getRecharge_today()
        local rechargeYesterday = rechargeTotal - rechargeToday
        --首日付费
        if rechargeYesterday > 0 and not self:_getLoginEvent() then
            G_NativeAgent:eventCustom("AT_1_Pay_2_Login", '')
            self:_setLoginEvent(true)
        end
    end
end

function GameAgent:_setLoginEvent(hasReported)
	G_StorageManager:setUserInfo("", G_UserData:getBase():getId())
	G_StorageManager:saveWithUser(GameAgent.FILE_NAME,{hasReported=hasReported})
end

function GameAgent:_getLoginEvent()
	G_StorageManager:setUserInfo("", G_UserData:getBase():getId())
	local data = G_StorageManager:loadUser(GameAgent.FILE_NAME) or {}
	return data.hasReported
end

-- i18n ex 
function GameAgent:updateLastLoginStatus()
	local info = G_StorageManager:load(G_NativeAgent.__cname)
	if info and info.userName ~= "" then
		self._isLastLogin = true
	else
		self._isLastLogin = false
    end
end

function GameAgent:isLastLogin()
	return self._isLastLogin
end

function GameAgent:getSdkBindChannelRewardFuncId(channelId)
    assert(channelId, "[GameAgent] getSdkBindChannelRewardFuncId channelId not found")
    -- if not channelId then return nil end
    local funcId = 0
    channelId = tostring(channelId)
    crashPrint("[GameAgent] getSdkBindChannelRewardFuncId<channelId=" .. channelId .. ">")
    if channelId == NativeConst.SDK_CHANNEL_FACEBOOK then
        funcId = 1001
    elseif channelId == NativeConst.SDK_CHANNEL_LINE  then
        funcId = 1002
    elseif channelId == NativeConst.SDK_CHANNEL_APPLE  then
        funcId = 1003
    elseif channelId == NativeConst.SDK_CHANNEL_TWITTER  then
        funcId = 1004
    elseif channelId == NativeConst.SDK_CHANNEL_GOOGLE  then
        funcId = 1005
    elseif channelId == NativeConst.SDK_CHANNEL_GAMECENTER  then
        funcId = 1006
    elseif channelId == NativeConst.SDK_CHANNEL_CITATIONCODE  then
        funcId = 1007
    end
    crashPrint("[GameAgent] getSdkBindChannelRewardFuncId return <funcId=" .. funcId .. ">")
    return funcId
end

-- i18n ex ja 绑定定账户
function GameAgent:_onBindResult(params)
    local channel = params["channel"]
    local type = "1"
    local funcId = G_GameAgent:getSdkBindChannelRewardFuncId(channel)
    crashPrint("[GameAgent] _onBindResult<channel=" .. channel .. ">")
    crashPrint("[GameAgent] _onBindResult<type=" .. type .. ">")
    crashPrint("[GameAgent] _onBindResult<funcId=" .. funcId .. ">")

    local data = {type=type,param=funcId}
    dump(data,"GameAgent onBindResult")
    G_GameAgent:c2sReportEvent(data)
end
-- i18n ex ja 客户端上报事件
function GameAgent:c2sReportEvent(data)
	local type = tonumber(data["type"]) 
	local param1 = tonumber(data["param"])
	G_NetworkManager:send(MessageIDConst.ID_C2S_ReportEvent,{
		type = type,
		param1 = param1 
	})
end

-- i18n ex ja 客户端上报事件回调
function GameAgent:_recvReportEvent(id, message)
    if message.ret == MessageErrorConst.RET_OK then
        self._KeyValueUrlRequest:doRequestSetKeyValue("isBind","binded")
        G_UserData:getBase():setIsBinded(true)
        G_SignalManager:dispatch(SignalConst.EVENT_BIND_SUCCESS)
    end
end
...
function GameAgent:isShowUsingConvente() 
    ...
    local info = require("app.config.parameter").get(50002) 
    if info and info.content ~= "1" then
        return false
    end
    return true  
end
