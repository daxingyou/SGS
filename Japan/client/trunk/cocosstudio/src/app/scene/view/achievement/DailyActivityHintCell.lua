...
-- 特殊处理标题字体大小
function DailyActivityHintCell:_dealFontSize()
	if Lang.checkLang(Lang.JA) then
		self._title:setFontSize(self._title:getFontSize() - 1.5)
	end
end
...
-- i18n pos lable
function DailyActivityHintCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        self._openTime1:getVirtualRenderer():setMaxLineWidth(150)
        --self._openTime1:getVirtualRenderer():setLineSpacing(-3)
		self._openTime1:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )

		self._openTime1:setPositionY(self._openTime1:getPositionY()+3)
		self._openTime2:setPositionY(self._openTime2:getPositionY()-3)
	end
	if Lang.checkUI("ui4") then  -- i18n ja 标题换行
		self._openTime1:setFontSize(self._openTime1:getFontSize() - 2)		
        self._title:setFontSize(self._title:getFontSize() - 3)		
		self._title:ignoreContentAdaptWithSize(true)
		self._title:setTextAreaSize(cc.size(self._title:getFontSize()*7, 0)) 
		self._title:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
	end
end
