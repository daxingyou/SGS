...
function JoyAwardsCell:updateUI(data)
    ...
    local function createItem(index, itemData)
        ...
        if Lang.checkLang(Lang.CN) then
            ...
            UIHelper.dealVTextWidget( self["_textGroup" ..index],
                Lang.get("gacha_goldenjoy_itemgroup", {num = itemData.cfg.group})
            )
        end
...
-- i18n change lable
function JoyAwardsCell:_swapImageByI18n()
    if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        for i=1,7 do
            self["_imageOpened"..i] = UIHelper.swapSignImage(self["_imageOpened"..i],
            { 
                style = "signet_8", 
                text = Lang.getImgText("img_seal_yikaijiang01") ,
