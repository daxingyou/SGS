...
function TeamHeroBustIcon:ctor(pos, onClick)
	...
	if Lang.checkUI("ui4") then
		resource.file = Path.getCSB("TeamHeroBustIcon2", "team")
	end	
...
function TeamHeroBustIcon:_initView()
	...
	-- i18n ja change property
	if Lang.checkUI("ui4") then
		self._imageSelected:ignoreContentAdaptWithSize(false)
		self._imageSelected:setContentSize(cc.size(121, 92))
		self._imageSelected:setPositionX(-44)  

		-- 装备 宝物 神兵
		local sceneName = G_SceneManager:getTopScene():getName()    
		if sceneName == "equipmentDetail" or sceneName == "treasureDetail"  or 
		   sceneName == "instrumentDetail" or sceneName == "horseDetail"  then   
			self._imageSelected:loadTexture(Path.getUICommon("frame/img_touxiang_01"))
			self._imageSelected:ignoreContentAdaptWithSize(false)
			self._imageSelected:setContentSize(cc.size(102, 96))
			self._imageSelected:setPosition(cc.p(-47, 0))
			self._panelTouch:setScale(1)
			self._panelTouch:setContentSize(cc.size(102, 102))
		end  
	end
end
...
function TeamHeroBustIcon:loadIcon(res)
	...
	-- i18n ja change res path
	if Lang.checkUI("ui4") then
		res = string.gsub(res, "team_hero", "hero") 
		self._imageIcon:loadTexture(res)
		self._imageIcon:ignoreContentAdaptWithSize(false)
		self._imageIcon:setContentSize(cc.size(78, 78))
		return
	end
	self._imageIcon:loadTexture(res)
end
...
function TeamHeroBustIcon:loadColorBg(res,opacity)
	...
	-- i18n ja change res path
	if Lang.checkUI("ui4") then
		res = string.gsub(res, "img_drawing_frame_", "img_frame_")  
		self._imageBg:loadTexture(res)
		self._imageBg:ignoreContentAdaptWithSize(false)
		self._imageBg:setContentSize(cc.size(93, 93))
		return
	end
	self._imageBg:loadTexture(res)
end
