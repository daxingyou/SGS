...
function CityNode:updateUI(picture)
    ...
    if  Lang.checkLang(Lang.CN) then
        ...
        local cfg = GuildCrossWarHelper.getWarCfg(self._pointId)
        local UIHelper  = require("yoka.utils.UIHelper")
        UIHelper.setLabelStyle( self._cityName,{
            style = self._pointId == 25 and "guild_war_3" or "guild_war_2",
        })
        UIHelper.dealVTextWidget(self._cityName,cfg.point_name)
    end
...
-- i18n change lable
function CityNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
        self._cityName = UIHelper.swapWithLabel(self._cityName,{
            style = "guild_war_2",
        })     
	end
end
