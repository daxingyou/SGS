...
-- i18n pos lable
function StageRewardBoxNode:_dealPosByI18n()
    if not Lang.checkLang(Lang.CN) then
		self._boxName:setPositionY(self._richName:getPositionY()-10)
	end
end

function StageRewardBoxNode:_dealByI18n()
    if not Lang.checkLang(Lang.CN) then
		self._boxName:setColor(Colors.DARK_BG_THREE)
		-- 改boxName的颜色 位置
		if Lang.checkUI("ui4") then
			self._boxName:setFontSize(18)
			self._boxName:setColor( cc.c3b(0xff, 0xb8, 0x0c))
		end
	end
end
