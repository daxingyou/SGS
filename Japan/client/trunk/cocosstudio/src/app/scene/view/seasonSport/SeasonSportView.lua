...
function SeasonSportView:_initNewerView()
	...
	if Lang.checkLang(Lang.CN) then
	    ...
	    UIHelper.setSignStyle(self._imageDivision,{
			style = SeasonSportConst.SEASON_STAGE_TIP[curStage],
			text = Lang.getImgText(
				SeasonSportConst.SEASON_STAGE_TIP[curStage]
			) ,
	   },Path.getTextSignet(SeasonSportConst.SEASON_STAGE_TIP[curStage]))
end
...
function SeasonSportView:_update(dt)
	...
	if SeasonSportConst.SEASON_COUNTDOWN > G_ServerTime:getLeftSeconds(self._seasonEndTime) then
		...
		if not Lang.checkLang(Lang.CN) then
			dateStr = (Lang.get("season_lasttime", {num = curSeason}) ..G_ServerTime:getLeftSecondsString(self._seasonEndTime, "00:00:00"))
		else
			dateStr = (Lang.get("season_lasttime", {num = curSeason}) ..G_ServerTime:getLeftSecondsString(self._seasonEndTime, "00：00：00"))
		end
		self._textSeasonTime:setString(dateStr)
...
-- i18n change lable
function SeasonSportView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._imageWaiting = UIHelper.swapWithLabel(self._imageWaiting,{
			 style = "season_sport_1",
			 text = Lang.getImgText("txt_fight_bg04") ,
		})
--[[
		local image1 = UIHelper.seekNodeByName(self._imageNewer,"Image_36")
		UIHelper.swapWithLabel(image1,{
			 style = "season_sport_1",
			 text = Lang.getImgText("txt_fight_bg06") ,
		})
]]
		local offsetY2 = -30
		local offsetY3 = -35
		...
		local image2 = UIHelper.seekNodeByName(self._btnReport,"Image_33")
		local labelReport = UIHelper.swapWithLabel(image2,{
			 style = "icon_txt_3",
			 text = Lang.getImgText("txt_main_enter6_report") ,
			 offsetY =  offsetY2,
		})
		labelReport:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        labelReport:getVirtualRenderer():setLineSpacing(-7)


		local image3 = UIHelper.seekNodeByName(self._btnRank,"Image_28")
	    local labelRank = UIHelper.swapWithLabel(image3,{
			 style = "icon_txt_3",
			 text = Lang.getImgText("txt_main_enter6_rankbig") ,
			 offsetY = offsetY3,
			 offsetX = 6,
		})


		local image4 = UIHelper.seekNodeByName(self._btnSeasonRewards,"Image_23")
	    local label2 = UIHelper.swapWithLabel(image4,{
			 style = "icon_txt_3",
			 text = Lang.getImgText("txt_main_enter6_prize") ,
			 offsetY = -37,
		})
		label2:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        label2:getVirtualRenderer():setLineSpacing(-7)

		local image5 = UIHelper.seekNodeByName(self._btnSeasonShop,"Image_23")
	    local label = UIHelper.swapWithLabel(image5,{
			 style = "icon_txt_3",
			 text = Lang.getImgText("txt_main_enter6_fightshop") ,
			 offsetY = -37,
		})
		...
		local image6 = UIHelper.seekNodeByName(self._commonHelp,"Image_31")
	    local label4 = UIHelper.swapWithLabel(image6,{
			 style = "icon_txt_3",
		...
		local image7 = UIHelper.seekNodeByName(self._btnSilkConfig,"Image_24")
	    local label3 = UIHelper.swapWithLabel(image7,{
			 style = "icon_txt_3",
			 text = Lang.getImgText("txt_main_enter6_configuration") ,
			 offsetY = -37,
		})
		...
		if  Lang.checkUI("ui4") then
			labelRank:setPositionY(labelRank:getPositionY()-10)
			labelReport:setPositionY(labelReport:getPositionY()-10)
			label2:setPositionY(label2:getPositionY()-10)
			label:setPositionY(label:getPositionY()-10)
			label4:setPositionY(label4:getPositionY()-10)
			label3:setPositionY(label3:getPositionY()-10)

			local image = cc.Sprite:create(Path.getTextSignet("img_fight_01"))
			local size = image:getContentSize()
			 
			self._imageDivision = UIHelper.swapSignImage(self._imageDivision,{
				style = "signet_12",
				text = Lang.getImgText("img_fight_01") ,
				anchorPoint = cc.p(0.5,0.5),
				position = cc.p(size.width * 0.5-11 ,size.height * 0.5)
		   },Path.getTextSignet("img_fight_01") )
		   self._imageDivision:setPositionX(self._imageDivision:getPositionX())
		   self._commonTip:setPositionX(self._commonTip:getPositionX() - 11)  		 
		   --image:release()  
		else  
			self._imageDivision = UIHelper.swapSignImage(self._imageDivision,{
				style = "signet_12",
		   ...
		   self._imageDivision:setPositionX(self._imageDivision:getPositionX()-6)
		end

		self._commonTip:setLocalZOrder(1)
	end
end


-- i18n change lable
function SeasonSportView:_createLabelByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local size = self._btnMatch:getContentSize()
		local label = UIHelper.createLabel({
			style = "season_sport_3",
			text = Lang.getImgText("img_fight_05") ,
			position = cc.p(size.width * 0.5,50),
		})
		self._btnMatch:addChild(label)
		
		self._btnRank:setPositionY(self._btnRank:getPositionY() - 6)
		self._btnRank:ignoreContentAdaptWithSize(true)
		self._btnRank:loadTextureNormal(Path.getCommonIcon("main","btn_main_enter4_rank"))

		if Lang.checkLang(Lang.JA) then
			label:setFontSize(36)
			self._btnRank:ignoreContentAdaptWithSize(true)
			self._btnRank:setContentSize(cc.size(80,84))
			local image3 = UIHelper.seekNodeByName(self._btnRank,"Image_28")
			image3:setPositionX(image3:getPositionX()-5)
		else
			self._btnRank:setContentSize(cc.size(100,84))
		end
	end
end

-- i18n 
function SeasonSportView:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textWaiting:setPositionY(self._textWaiting:getPositionY() - 6)
...
-- i18n change lable
function SeasonSportView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
	end
	if Lang.checkLang(Lang.ZH) then
		self._commonTip:setPositionX(self._commonTip:getPositionX()-5)
	end
	if Lang.checkLang(Lang.JA) then
		self._commonTip:setPositionX(self._commonTip:getPositionX()+10)
	end
end
