...
function CustomActivityTaskView:ctor(actView,actType)
	...
	self._textNode = nil
	local resource = {
		file = Path.getCSB("CustomActivityTaskView2", "customactivity"),
...
function CustomActivityTaskView:_getTemplate(data)
	local CustomActivityTaskItemCell = require("app.scene.view.customactivity.CustomActivityTaskItemCell2")
...
function CustomActivityTaskView:_createProgressRichText(msg)
	...
	local richMsg =  json.encode(CustomActivityUIHelper.getRichMsgListForHashText(
				msg,Colors.CUSTOM_ACT_DES_HILIGHT,nil,Colors.CUSTOM_ACT_DES,nil, 20))
	...
	widget:setVerticalSpace(5)
	
	if not Lang.checkLang(Lang.CN) then
		widget:setContentSize(cc.size(546,0))--高度0则高度自适应
	else
...
-- i18n change lable
function CustomActivityTaskView:_dealByI18n()
	-- if not Lang.checkLang(Lang.CN) then
	-- 	local UIHelper  = require("yoka.utils.UIHelper")	
	-- 	self._textTimeTitle:setFontSize(self._textTimeTitle:getFontSize()-2)
	-- 	self._textTime:setFontSize(self._textTime:getFontSize()-2)
	-- end
end

-- i18n update pos lable
function CustomActivityTaskView:_updatePosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._textTime:setPositionX(self._textTimeTitle:getPositionX()+self._textTimeTitle:getContentSize().width+3)
	end
end
