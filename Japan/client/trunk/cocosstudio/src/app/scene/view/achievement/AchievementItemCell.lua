...
function AchievementItemCell:_updateAchievementNode(achInfo)
	...
	if Lang.checkUI("ui4") then
		...
		if Lang.checkUI("ui4") then
			if self._achInfo.cfgData['tab'] == AchievementData.FIRST_MEET_TYPE then
				width = width + 10
			else
				width = width - 30
			end
...
function AchievementItemCell:_updateBtnState(achInfo)
	...
	if cfg.if_display ~= 2 then
		...
		if not canGetAward and not isReceive then
			...
			if Lang.checkUI("ui4") then
				richText = Lang.get("achievement_condition",
				{
					curr = TextHelper.getAmountText1(tonumber(achInfo.serverData.now_value)),
					currColor = currColor,
					total = TextHelper.getAmountText1(tonumber(achInfo.serverData.max_value)),
					totalColor = totalColor,
				})
			end
...
function AchievementItemCell:_dealPosByI18n()
	...
	if Lang.checkUI("ui4") then
		...
		self._commonResInfo1:setPositionX(textResName:getPositionX() + textResName:getContentSize().width + 20)
