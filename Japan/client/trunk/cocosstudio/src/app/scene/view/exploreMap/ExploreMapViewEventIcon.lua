...
function ExploreMapViewEventIcon:onCreate()
	...
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textImage:setString(  Lang.getImgText("txt_"..discoverData.res_id2.."_1") )
	else
...
-- i18n change lable
function ExploreMapViewEventIcon:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textImage = UIHelper.swapWithLabel(self._textImage,{
			 style = "explore_2",
			 text = Lang.getImgText("txt_bjwz") ,
			 offsetY = -40,
		})

		self._textImage:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        self._textImage:getVirtualRenderer():setLineSpacing(-7)
	end
end

-- i18n ja pos lable
function ExploreMapViewEventIcon:_dealPosByI18n()
	if Lang.checkUI("ui4") then   
		local UIHelper  = require("yoka.utils.UIHelper")

		self._leftTimeLabel:setFontSize(18)
        -- self._leftTimeLabel:setPositionX(
		-- 	self._leftTimeLabel:getPositionX() - 10
        -- )
	end
