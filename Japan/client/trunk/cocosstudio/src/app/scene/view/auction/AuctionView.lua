...
function AuctionView:_dealPosByI18n()
    if not Lang.checkLang(Lang.CN) then
		...
		local size = self._textAuctionShare:getVirtualRenderer():getContentSize()
        self._commonRes1:setPositionX(self._textAuctionShare:getPositionX()+10+size.width);
        self._commonRes2:setPositionX(self._commonRes1:getPositionX()+108);

		
		if Lang.checkUI("ui4") then
			image1:setPositionX(image1:getPositionX()+15)
		end
	end
	if Lang.checkLang(Lang.JA) then
		self._commonBtnAuction:setFontSize(18)
	end
end
