...
function PopUpPlayerFrameItemCell:onCreate()
    ...
    if Lang.checkUI("ui4") then
        local panel = self:getSubNodeByName("Panel_root")
        panel:setContentSize(cc.size(432,125))
        for index=1,3 do
            self["_heroNode"..index]:setPositionX(77+(index-1)*140)
            self["_select"..index]:setPositionX(77+(index-1)*140)
        end
    end
end
