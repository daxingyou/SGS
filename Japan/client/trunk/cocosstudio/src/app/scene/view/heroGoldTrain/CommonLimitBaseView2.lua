...
local CommonLimitBaseView = class("CommonLimitBaseView", ListViewCellBase)
...
function CommonLimitBaseView:ctor(parentView, resource)
...
function CommonLimitBaseView:_openPopupPanel(costKey, level)
    ...
    self._parentView._nodeGoldPopup:addChild(self._popupPanel)
...
function CommonLimitBaseView:_playEmitterEffect(emitter, startNode, endNode, costKey, curCount)
    ...
    local startPos = UIHelper.convertSpaceFromNodeToNode(startNode, self._parentView) -- self) 
    emitter:setPosition(startPos)
    self._parentView:addChild(emitter) 	  			-- self:addChild(emitter)  -- 父节点 
    local endPos = UIHelper.convertSpaceFromNodeToNode(endNode, self._parentView) -- self) 
...
function CommonLimitBaseView:_playLvUpEffect()
    ...
    self:_updateMaterialMaxSize()
    G_EffectGfxMgr:createPlayMovingGfx(self._parentView._nodeGoldHetiMoving, "moving_tujieheti", effectFunction, eventFunction, true)
