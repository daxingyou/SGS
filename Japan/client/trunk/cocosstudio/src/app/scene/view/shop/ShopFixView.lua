...
function ShopFixView:onCreate()
	...
	if Lang.checkUI("ui4") then
		if self._shopId == 10 then
			local offset = 50
			self._crystalBg:setVisible(true)
			self._listViewTab:setPositionY(self._listViewTab:getPositionY()+offset)
			local size = self._listViewTab:getContentSize()
			size.height = size.height - offset
			self._listViewTab:setContentSize(size)
...
-- i18n ja 水晶商店
function ShopFixView:_startCountDown()
	local t = G_ServerTime:secondsFromZero()
