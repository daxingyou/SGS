...
-- i18n change lable
function GuildDungeonView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self._nodeReward,"Image_144")
		UIHelper.swapWithLabel(image1,{ 
			 style = "countryboss_2", 
			 text = Lang.getImgText("txt_worldboss_jiangli01") ,
			 offsetY = 0,
		})
...
function GuildDungeonView:_updatePosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self,"Node_MiddleTop","Image_123")
		local totalWidth = UIHelper.alignCenter(image1,{self._textCountTitle,self._textRemainCount,self._textTimeTitle,self._textTime})
		if Lang.checkUI("ui4") then
			UIHelper.alignCenter(image1,{self._textCountTitle,self._textRemainCount,self._textTimeTitle,self._textTime},{10,10,10,0})
		end
	end
end
...
function GuildDungeonView:_dealUIByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image2 = UIHelper.seekNodeByName(self._nodeReward,"Image_147")
		image2:setVisible(false)

		local image3 = UIHelper.seekNodeByName(self._nodeReward,"Image_145")
		image3:ignoreContentAdaptWithSize(true)
		--image3:setPositionX(image3:getContentSize().width*0.5+6)

		local text1 = UIHelper.seekNodeByName(self._nodeReward,"Text_45")
		text1:setLocalZOrder(1)
	end
end

function GuildDungeonView:_awardPosByI18n()
	if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
		...
		panel:setBackGroundImage(Path.getUICommon("img_com_dark_board_bg01"))
    	panel:setBackGroundImageCapInsets(cc.rect(3,3,1,1))
		local size = cc.size(336,80 + text45:getContentSize().height)
		...
		image145:setPosition(panel:getPositionX() + 22,panel:getPositionY() + 40)
		local image3 = UIHelper.seekNodeByName(self._nodeReward,"Image_144")
		image3:setString(Lang.getImgText("txt_boss_jiangli01"))
		image3:getVirtualRenderer():setWidth(20)
		image3:setFontSize(20)
		image3:setColor(cc.c3b(0xfe,0xe1,0x02))
		image3:setPosition(panel:getPositionX() + 22,panel:getPositionY() + 40)
