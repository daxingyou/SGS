...
local OpenServerFundItemCell = class("OpenServerFundItemCell", ListViewCellBase)
  ...
  -- i18n ja change CSB pResource
function OpenServerFundItemCell:ctor(pResource)
    ...
    -- i18n ja change CSB
	if Lang.checkUI("ui4") and pResource then
	    resource = pResource
    end
...
-- i18n change lable
function OpenServerFundItemCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        self._imageReceive = UIHelper.swapSignImage(self._imageReceive,
		{ 
			 style = "signet_8", 
			 text = Lang.getImgText("txt_yilingqu01") ,
...
-- i18n change lable
function OpenServerFundItemCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")	
        self._imageConditionBg:setPositionX(self._imageConditionBg:getPositionX()-7)
		self._nodeCondition:setPositionX(self._nodeCondition:getPositionX()-7)
    end
end
