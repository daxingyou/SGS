...
-- i18n pos lable
function PopupArenaRankCell:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
        local image1 = UIHelper.seekNodeByName(self._resourceNode,"Image_1")
        image1:setPositionX(image1:getPositionX() - 16)
		self._textPower:setPositionX(self._textPower:getPositionX() - 16)
	end
	if Lang.checkLang(Lang.JA) then
		self._textPlayerName:setPositionX(self._textPlayerName:getPositionX()+20)
	end
end

-- i18n pos lable
function PopupArenaRankCell:_updateResPosI18n()
	if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
		local textRankAward = UIHelper.seekNodeByName(self._resourceNode,"root","Text_rank_award")
		local startX = textRankAward:getPositionX() + textRankAward:getContentSize().width 
		startX = startX + 4  -- 添加冒号 需增大间距
		self._resInfo1:setPositionX(startX)
		UIHelper.seekNodeByName(self._resInfo1,"Image"):setPositionX(21)
		self._resInfo2:setPositionX(startX + self._resInfo1:getWidth() + 10)
	end
end
