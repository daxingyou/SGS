...
function GuildWarEnemyListNode:_dealByI18n()
    if Lang.checkLang(Lang.JA) then
        self._textTime:setAnchorPoint(0,0.5)
        local width = self._textCdTitle:getContentSize().width/2
        local posX = self._textCdTitle:getPositionX() + width + 5
        self._textTime:setPositionX(posX)
    end
end
