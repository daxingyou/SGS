...
function PopupStageDetail:ctor(stageId)
    ...
    local filePath =  Path.getCSB("PopupStageDetail", "stage")
    --ui4 特殊处理
    if  Lang.checkUI("ui4") then  
        filePath =  Path.getCSB("PopupStageDetail1", "stage")
    end
	local resource = {
		file = filePath,
		binding = {
...
function PopupStageDetail:_refreshExpMoney()
    ...
    if  Lang.checkUI("ui4") then
        -- body
        self._costStamina:setTextColor(Colors.BRIGHT_BG_ONE)
        local UIHelper  = require("yoka.utils.UIHelper")
        local Text = UIHelper.seekNodeByName(self._costStamina,"Text")
        Text:setPositionX(42)
        -- self._drop1:setTextColor(Colors.BRIGHT_BG_ONE)
        -- self._drop2:setTextColor(Colors.BRIGHT_BG_ONE)
    end
end
...
function PopupStageDetail:_updateDropList()
    ...
    if  Lang.checkUI("ui4") then
        self._listDrop:updateUI(awards, 0.8,false,true, doubleTimes > 0)
        self._listDrop:setItemsMargin(15)
    else
...
-- i18n change lable
function PopupStageDetail:_dealPosByI18n()
    if Lang.checkUI("ui4") then   -- i18n ja change pos
        ...
        local textCount = UIHelper.seekNodeByName(self._panelBase,"TextCount")
        local textFirstKill = UIHelper.seekNodeByName(self._panelBase,"TextFirstKill")

        local image_talk = UIHelper.seekNodeByName(self._ImageHero,"Image_talk_bg2")
        image_talk:setPositionY(120)

        -- imageSword:setPositionX(imageSword:getPositionX())
         text:setPositionX(text:getPositionX()+16)
    
        -- self._costStamina:setPositionX(self._costStamina:getPositionX()+36)
        -- self._drop1:setPositionX(self._drop1:getPositionX()+36)
        -- self._drop2:setPositionX(self._drop2:getPositionX()+36)
        
        -- textCount:setAnchorPoint(cc.p(0,0.5))
        -- textFirstKill:setAnchorPoint(cc.p(0,0.5))
        -- self._textCountNum:setAnchorPoint(cc.p(0,0.5))
        -- self._textFirstKillName:setAnchorPoint(cc.p(0,0.5))
        -- textCount:setPositionX(185)
        -- textFirstKill:setPositionX(185)
        local size1 = textCount:getContentSize()
        local size2 = textFirstKill:getContentSize()
        self._textCountNum:setPositionX(textCount:getPositionX()+size1.width + 6)
        self._textFirstKillName:setPositionX(textFirstKill:getPositionX()+size2.width + 7)  
        -- local textDrop = UIHelper.seekNodeByName(self._panelBase,"TextDrop")      
        -- textDrop:setPositionX(textDrop:getPositionX() - 47)
	end
end

function PopupStageDetail:_adjustPosByI18n()
    if not Lang.checkLang(Lang.CN) then
        local textCount = UIHelper.seekNodeByName(self._panelBase,"TextCount")
        local textFirstKill = UIHelper.seekNodeByName(self._panelBase,"TextFirstKill")
        local size1 = textCount:getContentSize()
        local size2 = textFirstKill:getContentSize()
        self._textCountNum:setPositionX(textCount:getPositionX()+size1.width)
        self._textFirstKillName:setPositionX(textFirstKill:getPositionX()+size2.width)
    end
end

-- i18n change lable
function PopupStageDetail:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")

        local image3 = UIHelper.seekNodeByName(self,"Image_2")
        if image3 then
