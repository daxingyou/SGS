...
function YokeDesNode:ctor(isHeroDetailYokeModule2)
	self.isHeroDetailYokeModule2 = isHeroDetailYokeModule2;
end

--i18n add title
function YokeDesNode:updateView(info, width, dis, title)
	local richText = nil
	-- i18n richtext 
...
function YokeDesNode:_createUnit(text, isActivated)
	...
	unit.msg = text
	unit.fontSize = 20
	if self.isHeroDetailYokeModule2 then  -- i18n ja change font size
		unit.fontSize = 18
	end 
...
function YokeDesNode:_createTemplate2(info)
	...
	self:changeTabDes() -- i18n ja change
...
-- i18n ja change UI
function YokeDesNode:changeTabDes( )
	if not Lang.checkUI("ui4") then
		return
	end

	local str =  self._content[2].msg
	str = string.gsub(str, " ", "") 
	str = "[" .. str .. "] "
	self._content[2].msg = str

	local temp = self._content[1]
	self._content[1] = self._content[2]
	self._content[2] = temp
end
