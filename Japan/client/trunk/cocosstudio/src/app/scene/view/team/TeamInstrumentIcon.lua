...
function TeamInstrumentIcon:_initUnlock( ... )
    ...
    if not isOpen then
		...
		if Lang.checkUI("ui4") then  -- i18n ja change line
			local str = self._textTip:getString()
			local num =  string.match(str, "%d+")
			num = "Lv" .. num
			str = string.gsub(str, num, "")
			str = num .. "\n" .. str
...
function TeamInstrumentIcon:_onPanelTouch()
	...
	if self._instrumentId then
		if Lang.checkUI("ui4") then   -- i18n ja 已存在装备
			...
			if scene:getName() == "instrumentDetail" then   
				local view = scene:getSceneView()
				view:_onBtnInstrumentClick()
			else  
...
-- i18n change lable
function TeamInstrumentIcon:_swapImageByI18n()

	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")

		local image1 = UIHelper.seekNodeByName(self._target,"Image_179")
...
-- i18n pos lable
function TeamInstrumentIcon:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local textName = ccui.Helper:seekNodeByName(self._fileNodeName, "TextName")
		textName:getVirtualRenderer():setMaxLineWidth(100)
		textName:setFontSize(18)
		textName:setTextHorizontalAlignment(
			cc.TEXT_ALIGNMENT_CENTER
		) 
		textName:setAnchorPoint(cc.p(0.5,1))
		textName:setPositionY(textName:getPositionY()+13)
	end
end
