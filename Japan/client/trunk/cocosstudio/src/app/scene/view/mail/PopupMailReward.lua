...
function PopupMailReward:_onEventProcessAllMail(id, message)
	...
	local function takeMailList( ... )
		...
		if #idList > 0 then
			...
			if Lang.checkUI("ui4") then
				local awards,skinAwards = G_UserData:getMails():processAwardsByI18n(awardList)
				self:_showAwardsByI18n(function()
					if #awards > 0 and not (#awards == 1 and TypeConvertHelper.getTypeClass(awards[1].type) == nil) then 
...
function PopupMailReward:_onEventProcessMail(id, message,mailInfo)
	...
	if mailInfo then
		...
		if #mailInfo.awards > 0 then
			if Lang.checkUI("ui4") then
				local awards,skinAwards = G_UserData:getMails():processAwardsByI18n(mailInfo.awards)
				self:_showAwardsByI18n(function()
...
function PopupMailReward:_dealPosByI18n()
	...
	if Lang.checkLang(Lang.JA) then
		self._textMailTitle:setString("")
		self._textMailTitle:setFontSize(24)
		self._textMailTitle:getVirtualRenderer():setWidth(270)
		self._textMailTitle:setPosition(25,478)
	end
			
end

-- i18n ja
function PopupMailReward:_showAwardsByI18n(callback,skinAwards)
	local UIPopupHelper = require("app.utils.UIPopupHelper")
	UIPopupHelper.popupAwardsBackgroundSkin(callback,skinAwards,true)
end
