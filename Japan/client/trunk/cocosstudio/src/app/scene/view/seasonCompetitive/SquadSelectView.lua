...
-- i18n change lable
function SquadSelectView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self,"Panel_Pet","Image_6")
		UIHelper.swapWithLabel(image1,{
			 style = "season_sport_2",
			 text = Lang.getImgText("txt_fight_creatures01") ,
		})
	end
end
-- i18n pos lable
function SquadSelectView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		self._textSquadCountTip:setFontSize(
			self._textSquadCountTip:getFontSize()-2
		)
	end
	if Lang.checkLang(Lang.JA) then
		for index = 1, SeasonSportConst.HERO_SQUAD_USEABLECOUNT do
			self["_textSilkName"..index]:setFontSize(18)
		end
    end
end
