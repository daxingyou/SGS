...
function CakeActivityView:_initView()
	...
	if  Lang.checkLang(Lang.CN) then  
		...
		local iconTxtUIInfo = {
			name = config.gain_icon_word,
			script = config.gain_icon_word_script,
			alignment = config.gain_icon_word_alignment,
			space = config.gain_icon_word_space,
			place = config.gain_icon_word_place
		}
		self._buttonGetMaterial:updateUI(FunctionConst.FUNC_CAKE_ACTIVITY_GET_MATERIAL, customIcon, customIconTxt,iconTxtUIInfo)
