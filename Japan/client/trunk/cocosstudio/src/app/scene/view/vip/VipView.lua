...
VipView.TAB_INDEX_PRIVILEGE = 3 --特权
VipView.AGE_VERIFICATE = 50001 -- i18n ja 年龄确认开关id
...
function VipView:_initTab( ... )
    ...
    self._imageRope3:setVisible(self._isOpenJade)
    self._nodeTabIcon3:setNodeVisible(self._isOpenJade)

    self._tabNum = self._isOpenJade and 3 or 2
...
function VipView:_onClickTabIcon(index)
    ...
    elseif index == VipView.TAB_INDEX_PRIVILEGE then
...
function VipView:_initView()
    ...
    self._nodeView:setVisible(self._selectTabIndex == VipView.TAB_INDEX_RECHARGE)
	...
	if self._selectTabIndex == VipView.TAB_INDEX_RECHARGE then
		self:switch2Recharge()
	elseif self._selectTabIndex == VipView.TAB_INDEX_PRIVILEGE then
        self:_switch2Privilege()
...
-- i18n ja 年龄弹框开关
function VipView:getParameterCfg()
	local ParameterIDConst = require("app.const.ParameterIDConst")
	local Parameter = require("app.config.parameter")
	
    local info = Parameter.get(VipView.AGE_VERIFICATE)    
...
-- i18n ja 年龄弹框
function VipView:showAgeVerificateBox()
...
-- i18n ja 年龄弹框
function VipView:_onEventShowAgeVerificate(e, data) 
	if e == "fail" then
		return 
	end
  ...
  -- i18n ja 
function VipView:sendSelectAge( )
