...
function PopupSilkRecommandCell:onCreate()
    ...
    if Lang.checkLang(Lang.JA) then
        self._textDesc:removeFromParent()

        self._textDesc = ccui.Text:create("", Path.getCommonFont(), 20)
        self._textDesc:setColor(Colors.TacticsDescriptionColor) 
        self._textDesc:setAnchorPoint(0, 0)
        self._textDesc:setPosition(0, 0)

        local sc = ccui.ScrollView:create()
        sc:setPosition(22, 82)
		sc:setBounceEnabled(true)
		sc:setDirection(ccui.ScrollViewDir.horizontal)
		sc:setTouchEnabled(true)
		sc:setSwallowTouches(false)
		sc:setScrollBarEnabled(false)
        sc:setContentSize(cc.size(630, 50))
        self._resource:addChild(sc, 10)
        sc:addChild(self._textDesc, 10)
    end
end
