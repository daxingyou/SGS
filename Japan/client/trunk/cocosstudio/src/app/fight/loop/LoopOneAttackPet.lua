...
function LoopOneAttackPet:_startSkill()
	...
	if self._skillInfo and self._skillInfo.skill_type == 2 then
		...
		if not Lang.checkLang(Lang.CN) then
			logWarn("xxx  xx "..self._skillInfo.name)
			FightSignalManager.getFightSignalManager():dispatchSignal(FightSignalConst.SIGNAL_PLAY_PET_SKILL, self._unit.camp, self._unit:getSkillAnim(), petId, color, self._skillInfo.name)
		else
