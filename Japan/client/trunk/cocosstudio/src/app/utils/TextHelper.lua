...
function TextHelper.isNameLegal(txt, min, max)
    ...
    if not Lang.checkLang(Lang.CN) and max then
        -- max = max * 2
        max = 15
	end
...
function TextHelper.expandTextByLen(txt, len)
    ...
    if Lang.checkLang(Lang.JA) and tempLen == 1 then
        return txt
    end
...
-- i18n ja 判断字符串里是否有除数字和字母之外字符
function TextHelper.checkNotNumOrLetter(txt)
	local function isNum(ascciValue)

		if ascciValue < tonumber(string.format("%#x",48)) or ascciValue > tonumber(string.format("%#x",57)) then   
			return false
		end
		return true
	end 

	local function isLetter(ascciValue)
 
		local isBig = (ascciValue >= tonumber(string.format("%#x",65)) and ascciValue <= tonumber(string.format("%#x",90))) 
		local isSmall = (ascciValue >= tonumber(string.format("%#x",97)) and ascciValue <= tonumber(string.format("%#x",122)))  
		if not (isBig or isSmall) then
			return false
		end
		return true
	end
	...
	for i = 1, len do
		...
		if isNum(ascciValue) == false and isLetter(ascciValue) == false then
			return true
		end	
    end
...
-- i18n ja  判断字符串是否为数字和英文字母组合
function TextHelper.checkNumLetterCombine(str)
    local isHaveNum = false
	local isHaveLetter = false

	local len = string.len(str) 
	for i = 1, len do
		local ascciValue = string.byte(str, i)

		local isBig = (ascciValue >= tonumber(string.format("%#x",65)) and ascciValue <= tonumber(string.format("%#x",90))) 
		local isSmall = (ascciValue >= tonumber(string.format("%#x",97)) and ascciValue <= tonumber(string.format("%#x",122))) 
		if isBig or isSmall then
			isHaveLetter = true
		elseif ascciValue >= tonumber(string.format("%#x",48)) and ascciValue <= tonumber(string.format("%#x",57)) then
			isHaveNum = true		
		end	
	end
	return (isHaveNum and isHaveLetter)
end

-- i18n ja 获取战力百万数据
function TextHelper.getAmountTextUI4(amount)
    assert(type(amount) == "number", "Invalid amount: "..tostring(amount))
    if amount >= 1000000 then
        local num = 10000
        return math.floor(amount / num),true
    else
        return tostring(amount)
    end
end
