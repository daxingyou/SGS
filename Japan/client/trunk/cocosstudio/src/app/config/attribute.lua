...
local __key_type = {
  id = "int",    --资源id-1 
  type = "int",    --数值类型-2 
  cn_name = "string",    --中文名称-3 
  en_name = "string",    --英文名称-4 
  order = "int",    --排序-5 
  order2 = "int",    --统计排序-6 
...
local attribute = {
    ...
    _data = {
        ...
        [71] = {80,1,"全体攻击","ATTR_ATK_FREE",65,0,},
        [72] = {81,1,"全体物防"," ATTR_PD_FREE",66,0,},
        [73] = {82,1,"全体法防","ATTR_MD_FREE",67,0,},
        [74] = {83,1,"全体HP","ATTR_HP_FREE",68,0,},
    }
}
...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in attribute")
...
--
function attribute.isVersionValid(v)
    if attribute.version then
        if v then
            return attribute.version <= v
        else
            return false
        end
    else
...
function attribute.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/attribute.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/attribute.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/attribute.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "attribute" )
                _isDataExist = attribute.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "attribute" )
                _isBaseExist = attribute.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "attribute" )
                _isExist = attribute.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = attribute._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "attribute" )
...
--
function attribute.set(id, key, value)
