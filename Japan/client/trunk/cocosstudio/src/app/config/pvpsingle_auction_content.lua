...
local __key_type = {
  ...
  order = "int",    --排序-18 
  price_id = "int",    --竞拍货币-19 
  start_price = "int",    --起拍价-20 
  fare = "int",    --加价-21 
  net = "int",    --一口价-22 
...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in pvpsingle_auction_content")
...
--
function pvpsingle_auction_content.isVersionValid(v)
    if pvpsingle_auction_content.version then
        if v then
            return pvpsingle_auction_content.version <= v
        else
            return false
        end
    else
...
function pvpsingle_auction_content.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/pvpsingle_auction_content.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/pvpsingle_auction_content.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/pvpsingle_auction_content.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "pvpsingle_auction_content" )
                _isDataExist = pvpsingle_auction_content.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "pvpsingle_auction_content" )
                _isBaseExist = pvpsingle_auction_content.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "pvpsingle_auction_content" )
                _isExist = pvpsingle_auction_content.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = pvpsingle_auction_content._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "pvpsingle_auction_content" )
