...
function CommonTabGroup:_initTabList()
    if self._containerStyle == 2 then
        ...
        if self._isVertical == 2 then --水平
            ...
            if not Lang.checkLang(Lang.CN)then
                needWidth = loopCount * (templateNodeSize.width + self._nodeOffset) - self._nodeOffset
            end
            ...
            if not Lang.checkLang(Lang.CN) then
                ...
                self._target:setPositionX(templateNodeSize.width)
            else
...
function CommonTabGroup:_updateTextImgTab(tabItem)
    ...
    if textWidget and text then
        if Lang.checkLang(Lang.JA)  then    -- i18n ja change line 
            ...
            if text == Lang.get("achievement_tab_title2") then  -- 限时活动换行
                local str = Lang.get("achievement_tab_title2")
                str = string.gsub(str, " ", "\n")
                textWidget:setString(str)
            end

            if text == Lang.get("groups_title_online_friend") then -- 在线好友字体大小微调
                textWidget:setFontSize(textWidget:getFontSize() - 2)
            end

            if  text == Lang.get("lang_activity_Invite_tab_names")[1] or text == Lang.get("lang_activity_Invite_tab_names")[2] then   -- 好友邀请字号缩小
                textWidget:setFontSize(textWidget:getFontSize() - 2)
            end 
        else
            textWidget:setString(text)
