...
function PopupHorseEquipInfo:updateUI( itemType ,itemId )
	...
	if not Lang.checkLang(Lang.CN) then
		self._itemEffect2:setPositionX(self._itemEffect1:getPositionX()+self._itemEffect1:getContentSize().width)
	end

	-- i18n ja 騰蛇描述向下移位
	if Lang.checkLang(Lang.JA) and itemParams.value == 203 then
		self._itemEffect1:setPositionY(self._itemEffect1:getPositionY() - 18)
		self._itemEffect2:setPositionY(self._itemEffect2:getPositionY() - 18)
	end
end
...
-- i18n pos 
function PopupHorseEquipInfo:_dealByI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
		local size = self._itemDesc:getContentSize()
        self._itemDesc:setContentSize(cc.size(size.width + 100,size.height))
	end
end
