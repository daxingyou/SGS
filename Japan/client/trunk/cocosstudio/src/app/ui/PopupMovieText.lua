...
function PopupMovieText:_playMingJiangling()
	local nodeMovie = self._nodeChapterEnd
    local function eventFunction(event)
        if event == "finish" and not self._isSkipped then--in8
...
function PopupMovieText:onClickPanel(sender)
	if self._commonContinueNode:isVisible()== true then
		self:onCloseEvent()
	elseif self._movieType == MovieConst.TYPE_CHAPTER_END then--in8
...
function PopupMovieText:_dealByI18n()
	if not Lang.checkLang(Lang.CN)  then
		...
		local lable1 = UIHelper.swapWithLabel(image1,{
			style = Lang.checkUI("ui4") and "chatper_game_name_1_ja" or "chatper_game_name_1",   	-- i18n ja change
	...
	if Lang.checkLang(Lang.JA) then  -- 18n ja change
		local textContent = UIHelper.seekNodeByName(self._nodeChapterEnd,"Text_content")
		...
		-- i18n ja change text size
		textMovie:setFontSize(28)
		textContent:getVirtualRenderer():setLineSpacing(4)

		textStart:setPositionY(textStart:getPositionY() - 15)
		nodeMingjiangling:setPositionY(-135)
	end
end
