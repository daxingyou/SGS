...
-- i18n ja add func  用于神兽信息页签
function PetDetailViewHelper.createTalentDes2(petUnitData,star,width, posOffset)
    local UserDataHelper = require("app.utils.UserDataHelper")
    local function isActiveWithStar( star)
        -- body
        local starLevel = petUnitData:getStar()
    ...
    width = 326-20 --width or 360 
    posOffset = posOffset or 10 -- 与左边界距离
	local label = ccui.RichText:createWithContent(content)
	label:setAnchorPoint(cc.p(0, 1))
	label:ignoreContentAdaptWithSize(false)
	label:setContentSize(cc.size(width,0))
