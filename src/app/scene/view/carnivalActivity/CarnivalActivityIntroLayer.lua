...
function CarnivalActivityIntroLayer:_updateText()
	...
	assert(resConfig ~= nil, "festival_res can not find res id "..resId)
...
function CarnivalActivityIntroLayer:_updateTime()
	...
	if not Lang.checkLang(Lang.CN) then
		local UIHelper = require("yoka.utils.UIHelper")
		local image = UIHelper.seekNodeByName(self._otherClient,"Image_12")
		if self._countdownText:isVisible() then
			UIHelper.alignCenter(image,{self._countdownText,self._countdownTime})
		else
			UIHelper.alignCenter(image,{self._countdownTime})
		end
