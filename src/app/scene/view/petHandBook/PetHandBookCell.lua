	...
	for i, value in ipairs(petIdList) do
		...
		if petName and cc.isRegister("CommonPetVName") then
			...
			if Lang.checkLang(Lang.JA) then
				local petNick = ccui.Helper:seekNodeByName(petName, "Text_Name") 
				--petNick:getVirtualRenderer():setMaxLineWidth(petNick:getFontSize()) 
				petNick:getVirtualRenderer():setLineSpacing(-2)  
				
				local UTF8 = require("app.utils.UTF8")
				local strContent = ""     
				local name = petNick:getString()
		...
		if Lang.checkHorizontal() then
			local posX,posY = petName:getPosition()
			local offsetY = 80
			if i%2 ~= 0 then
				offsetY = 110
			end
			petName:setPosition(posX+110,posY+offsetY)
		end
...
-- i18n change lable
function PetHandBookCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		self._imageActive = UIHelper.swapSignImage(self._imageActive,
		{ 
			 style = "sign_1", 
			 text = Lang.getImgText("img_activatedsign") ,
...
-- i18n pos lable
function PetHandBookCell:_dealPosByI18n()
	if Lang.checkLang(Lang.JA) then -- i18n ja
		self._nodeAttr1:setPosition(70,124)
		self._nodeAttr2:setPosition(70,94)
		self._nodeAttr3:setPosition(70,64)
		self._nodeAttr4:setPosition(70,34)
		self._imageActive:setPosition(340,85)
		self._commonBtn:setPosition(340,85)
	end
end
