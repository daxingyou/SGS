...
function PopupHonorTitleItemCell:ctor()
    ...
    if Lang.checkUI("ui4") then
        resource.file = Path.getCSB("PopupPlayerHonorTitleCell2", "playerDetail")
    end
...
function PopupHonorTitleItemCell:updateUI(index, cellValue)
    if Lang.checkUI("ui4") then
        local img = Path.getPlayerDetail("img_framebg")
        if index % 2 == 0 then
            img = Path.getPlayerDetail("img_bgline")
        end
        self._resourceNode:setBackGroundImage(img)
    end
...
-- i18n pos lable
function PopupHonorTitleItemCell:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
        self._titleDescTime:setPositionX(self._btnClick:getPositionX())
