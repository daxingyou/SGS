...
function PopupHurtReward:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		image21:setPositionX(textReset:getPositionX() - size.width*0.5 - 10)
		if Lang.checkLang(Lang.JA) then
			textReset:setPosition(0,-160)
			textReset:setFontSize(18)
			local size = textReset:getContentSize()
			image21:setPositionX(textReset:getPositionX() - size.width*0.5 - 10)
			image21:setPositionY(-160)

			-- i18n ja 位置微调
			self._textHurtNum:setPosition(self._textHurtNum:getPositionX() + 3, textHurt:getPositionY() - 1)
			self._panelCon:setPositionY(self._panelCon:getPositionY() - 12)
		end
	end
end
