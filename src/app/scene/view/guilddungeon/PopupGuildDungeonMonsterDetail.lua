...
function PopupGuildDungeonMonsterDetail:ctor(data)
    ...
    local filePath =  Path.getCSB("PopupGuildDungeonMonsterDetail", "guildDungeon")
    --ui4 特殊处理
    if  Lang.checkUI("ui4") then  
        filePath =  Path.getCSB("PopupGuildDungeonMonsterDetail1", "guildDungeon")
    end
    local resource = {
        file = filePath,
		binding = {
...
function PopupGuildDungeonMonsterDetail:onCreate()
    ...
    --ui4 特殊处理
    if  Lang.checkUI("ui4") then  
        self._btnFight:setTextStyle({
            fontName = Path.getFontW8(),
            color = cc.c3b(0x8c,0x31,0x00),
        })
    end
...
function PopupGuildDungeonMonsterDetail:_refreshMonsterBaseInfo()
    ...
    if  Lang.checkUI("ui4") then  
        cc.bind(self._fileNodePower, "CommonHeroPower")
