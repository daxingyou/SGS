...
function GuildTrainView:onCreate()
	...
	if Lang.checkUI("ui4") then
		self._btnRule:removeSelf()
		self._topbarBase:addChild(self._btnRule)
		self._btnRule:setPositionY(-28)
	end
	if Lang.checkLang(Lang.JA) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text1 = UIHelper.seekNodeByName(self._progressLabel,"text1")
		local text2 = UIHelper.seekNodeByName(self._progressLabel,"text2")
		text1:setFontSize(18)
		text2:setFontSize(18)

		self._progressLabel:setScale9Enabled(true)
		self._progressLabel:setCapInsets(cc.rect(54,12,1,1))
		local size = self._progressLabel:getContentSize()
		self._progressLabel:setContentSize(cc.size(size.width+40,size.height))
	end

	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image = UIHelper.seekNodeByName(self._topbarBase,"ImageTitle")	
		local button = UIHelper.seekNodeByName(self._btnRule,"Button")	
		UIHelper.alignRight({image,button})
		if Lang.checkUI("ui4") then -- i18n ja change
			self._btnRule:setPositionX(self._btnRule:getPositionX() + 50)
		end 
    end
...
function GuildTrainView:_createEndEffect()
    local function effectFunction(effect)
		if effect == "gongke_txt" or 
			(not Lang.checkLang(Lang.CN) and effect == "routine_word_kuang_9") then
...
function GuildTrainView:_updatePosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text1 = UIHelper.seekNodeByName(self._progressLabel,"text1")
		local text2 = UIHelper.seekNodeByName(self._progressLabel,"text2")
		UIHelper.alignCenter( self._progressLabel,{text1,text2})
	end
end
