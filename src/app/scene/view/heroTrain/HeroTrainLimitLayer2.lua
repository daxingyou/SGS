...
local HeroTrainLimitLayer = class("HeroTrainLimitLayer", ListViewCellBase)
local HeroLimitCostNode = require("app.scene.view.heroTrain.HeroLimitCostNode2")
...
function HeroTrainLimitLayer:ctor(parentView)
	...
	local resource = {
		file = Path.getCSB("HeroTrainLimitLayer2", "hero"),
	...
	self:setName("HeroTrainLimitLayer")
...
function HeroTrainLimitLayer:_initOrgPos()
	local list = {self._nodeHero, self._nodeGogok, self._textName, self._parentView._nodeBgMoving, self._parentView._nodePopup, self._imageTitle,
		self._buttonBreak, self._nodeSilver, self._parentView._nodeHetiMoving, self._parentView._nodeFire, self._nodeGoldLock}
...
function HeroTrainLimitLayer:_initView()
	...
	self._parentView._buttonShow:updateUI(FunctionConst.FUNC_HERO_TRAIN_TYPE4)
	self._buttonBreak:setString(Lang.get("hero_limit_break_btn"))
	self._nodeHero:setScale(0.95)
	...
	self._nodeSilver:getChildByName("Image"):setPositionY(18 + 3)
	self._nodeSilver:getChildByName("Image"):setScale(0.8)
	-- G_EffectGfxMgr:createPlayMovingGfx(self._parentView._nodeBgMoving, "moving_tujie_huohua", nil, nil, false) 移除火星特效
end

function HeroTrainLimitLayer:_adjustScaleAndPos()
	-- 父界面展示
	...
	if self._limitUIType==LIMIT_UI_TYPE_4 then
		self._nodeDes:setPosition(cc.p(0, -22))

		-- 调整+号 文字位置
		for key = HeroConst.HERO_LIMIT_COST_KEY_1, HeroConst.HERO_LIMIT_COST_KEY_4 do
			self["_cost"..key]._buttonAdd:setScale(0.7)  -- +号缩放
			...
			self["_cost"..key]._nodeFull:getParent():setScale(0.9) 	-- 这个node缩放90%
		end
	elseif self._limitUIType==LIMIT_UI_TYPE_6 then
		self._nodeDes:setPosition(cc.p(0,0))

		-- 调整+号 文字位置
		for key = HeroConst.HERO_LIMIT_COST_KEY_1, HeroConst.HERO_LIMIT_COST_KEY_6 do
			self["_cost2_"..key]._buttonAdd:setScale(0.6)  -- +号缩放
			self["_cost2_"..key]._imageName:setAnchorPoint(cc.p(1, 0.5))
			self["_cost2_"..key]._imageName:setPosition(cc.p(0, -53))
			self["_cost2_"..key]._textPercent:setAnchorPoint(cc.p(0, 0.5))
			self["_cost2_"..key]._textPercent:setPosition(cc.p(2, -53))

			self["_cost2_"..key]._textPercent:setFontSize(18)
			self["_cost2_"..key]._textPercent:setColor(Colors.BRIGHT_BG_ONE)
			self["_cost2_"..key]._textPercent:disableEffect(cc.LabelEffect.OUTLINE)
			if not Lang.checkLang(Lang.CN) then
				self["_cost2_"..key]._imageName:setFontSize(18)
				self["_cost2_"..key]._imageName:setColor(Colors.NORMAL_BG_ONE) 
				self["_cost2_"..key]._imageName:disableEffect(cc.LabelEffect.OUTLINE)
			end

			self["_cost2_"..key]._target:setScale(0.9)
		end
	end

	self._textName:setFontSize(18)
	self._texDesc:setString(Lang.get("hero_transform_result_title_limit"))
	if not Lang.checkLang(Lang.CN) then   
...
function HeroTrainLimitLayer:_updateData()
	...
	if self._limitDataType==HeroConst.HERO_LIMIT_TYPE_RED then
		...
		self._parentView._buttonShow:updateUI(FunctionConst.FUNC_HERO_TRAIN_TYPE4_RED)
	end
...
function HeroTrainLimitLayer:_doLayout()
...
function HeroTrainLimitLayer:_updateBaseInfo()
	...
	if self._limitDataType==HeroConst.HERO_LIMIT_TYPE_RED then
		...
		if Lang.checkLang(Lang.CN) then
			...
			local UIHelper  = require("yoka.utils.UIHelper")
			self._imgLockTip:setString(Lang.getImgText("txt_limit_05"))
		end
		isTop = (limitLevel>=HeroConst.HERO_LIMIT_RED_MAX_LEVEL)
		lv = limitLevel
		self:_adjustNodeGogok2(3, lv)
	else
		self._nodeGogok:resetSize(4)
		self._nodeGogok:setScale(0.45)
		if Lang.checkLang(Lang.CN) then
			self._imageTitle:loadTexture(Path.getTextLimit("txt_limit_06i"))
		else
			local UIHelper  = require("yoka.utils.UIHelper")
			self._imageTitle:setString(Lang.getImgText("txt_limit_06i"))
		end
		...
		if Lang.checkLang(Lang.CN) then
			...
			local UIHelper  = require("yoka.utils.UIHelper")
			self._imgLockTip:setString(Lang.getImgText("txt_limit_05b"))
		end
		isTop = (limitRedLevel>=HeroConst.HERO_LIMIT_GOLD_MAX_LEVEL)
		lv = limitRedLevel
		self:_adjustNodeGogok2(4, lv)
	end
	...
	-- 策划：放界限突破前的形象
	 -- self._nodeHero:updateUI(baseId, nil, nil, limitLevel, nil, nil, limitRedLevel)

	if isTop then
		self._imageTitle:setVisible(false)
		self._nodeGoldLock:setVisible(false)
		-- 显示文字
		self._nodeGogok2:setVisible(false)
		local textMaxDes = UIHelper.createLabel({
			text = Lang.get("hero_break_txt_reach_limit"),
			position = cc.p(70, self._nodeGogok2:getPositionY()),
			color = cc.c3b(0x2f, 0x9f, 0x07),
			fontSize = 18,
			anchorPoint = cc.p(0, 0.5)
		})
		self._nodeDes:addChild(textMaxDes)
	else
...
function HeroTrainLimitLayer:_updateBtnAndSilverState()
	...
	if (self._limitDataType==HeroConst.HERO_LIMIT_TYPE_RED and self._heroUnitData:getLimit_level()>=HeroConst.HERO_LIMIT_RED_MAX_LEVEL)
		...
		self:_adjustBtnVisible(false, true)
		return
	end
	...
	self:_adjustBtnVisible(isAllFull)
end

-- _nodeFire特效 “我预问鼎天下 试问谁与争锋”     _nodeHetiMoving成功后特效  _nodeBgMoving背景后火星特效  _nodePopup黑圈圈父节点
        ...
        elseif event == "finish" then
			...
			local sequence = cc.Sequence:create(delay, cc.CallFunc:create(function()
				if self and self._parentView then
					self:_playPrompt()
					self._parentView:refreshBtnAndUI() -- 刷新神兽形象
				end
		    end))
...
function HeroTrainLimitLayer:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		self._imgLockTip = UIHelper.swapWithLabel(self._imgLockTip,{ 
			style = "text_limit_1", 
			text = Lang.getImgText("txt_limit_05b") ,
	   })
...
function HeroTrainLimitLayer:_adjustPosI18n()
	if not Lang.checkLang(Lang.CN) then
		...
		local size2 = self._textName:getContentSize()  --getBoundingBox() 
		self._imageTitle:setPositionX(0 - size2.width* 0.5)
		self._textName:setPositionX( size1.width* 0.5-size2.width*0.5 + 2)
...
function HeroTrainLimitLayer:_adjustNodeGogok2(num, limitLevel)
	if num == 3 then
		self._nodeGogok2:setScale(0.6)
	elseif num == 4 then
		self._nodeGogok2:setScale(0.45)
	end
	self._nodeGogok2:resetSize(num)
 
	limitLevel = limitLevel + 1 
	self._nodeGogok2:setCount( (limitLevel >= num and {num} or {limitLevel})[1] )
end         

function HeroTrainLimitLayer:_adjustBtnVisible(bShow, bFullLevel)
	if bFullLevel then --策划： 满级不显示  其余情况一直显示
