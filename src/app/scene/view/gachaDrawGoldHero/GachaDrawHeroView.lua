...
function GachaDrawHeroView:onCreate()
    if Lang.checkLang(Lang.VN) or Lang.checkLang(Lang.JA) then
        self._imageTenIcon:setPositionX( self._imageTenIcon:getPositionX()-13)
        self._imageFreeIcon:setPositionX( self._imageFreeIcon:getPositionX()-20)
    end
...
function GachaDrawHeroView:_palyShowZhaomu(isDraw, awards)
    ...
    local function eventFunction(event)
        ...
        elseif event == "gongxihuode" then  
            -- i18n ja bug: 见龙在田 无法连续抽奖，抽奖后页面上抽奖按钮消失，需要退出界面重新选择
            local callFunc = function () 
                local tab = G_UserData:getGachaGoldenHero():getGachaDrawHeroViewData()
                if tab then
                    G_SceneManager:popScene()
                    G_SceneManager:popScene()
                   G_SceneManager:showScene("gachaDrawGoldHero", tab[1], tab[2])
                end
            end

            local PopupGetRewards = require("app.ui.PopupGetRewards").new(callFunc)
...
function GachaDrawHeroView:_updateDrawFreeDesc()    
    ...
    richText:setAnchorPoint(cc.p(0, 0.5))
     ...
     if(Lang.checkLang(Lang.VN)) then
        richText:setPositionX(richText:getPositionX()+25)
    end
...
function GachaDrawHeroView:_updateDrawTenDesc()
    ...
    --越南版本有重叠
    if(Lang.checkLang(Lang.VN)) then
        richText:setPositionX(richText:getPositionX()+20)
    end
