...
function InstrumentLimitDetailAttrNode:_update(level, index)
	...
	if Lang.checkUI("ui4") then -- i18n ja change
        ccui.Helper:seekNodeByName(self["_nodeTitle"..index], "ImageBase"):loadTexture(Path.getTextTeam("img_zr_di03"))
        ccui.Helper:seekNodeByName(self["_nodeTitle"..index], "ImageBase"):setContentSize(cc.size(402, 37))  
	end
