...
-- i18n pos lable
function PopupGroupsAgreementView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		for i = 1, GroupsConst.MAX_PLAYER_SIZE do
			local icon = self["_fileNodeIcon"..i]
			local textName = UIHelper.seekNodeByName(icon,"TextName")
		...
		if Lang.checkLang(Lang.JA) then -- i18n ja change
			self._txtNum:setAnchorPoint(cc.p(1, 0.5))
			self._txtNum:setPositionX(225-55)
		end
	end
end
