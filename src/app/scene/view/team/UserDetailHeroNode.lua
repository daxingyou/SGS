...
function UserDetailHeroNode:_updateView()
	...
	if Lang.checkUI("ui4") and self._heroTitle then  -- i18n ja  刷新默认武将信息
		self._heroTitle:updateUI(self._curHeroData)  
		if self._pos == 1 then
			ccui.Helper:seekNodeByName(self._heroTitle, "TextName"):setString(ccui.Helper:seekNodeByName(self._fileNodeHeroName, "TextName"):getString())
			ccui.Helper:seekNodeByName(self._heroTitle, "TextName"):setColor(ccui.Helper:seekNodeByName(self._fileNodeHeroName, "TextName"):getColor())			
		end
	end
end
...
function UserDetailHeroNode:_updateOneYoke(index)
	...
	if allYokeData and allYokeData.yokeInfo and allYokeData.yokeInfo[index] then
		...
		text:setColor(color)
		if not Lang.checkLang(Lang.CN) and not Lang.checkLang(Lang.JA) then -- i18n ja change bg
...
function UserDetailHeroNode:_dealPosByI18n()
	...
	-- 调整属性字号
	self._nodeLevel:setOpacity(0) 
	self._nodePotential:setOpacity(0) 
	local strPos = {{41, 405}, {41, 375},  {185, 405}, {185, 375} }
	for i = 1, 4 do
		self["_fileNodeAttr"..i]:setPosition(cc.p(strPos[i][1], strPos[i][2]))
	end

	-- 调整背景图
	local titleBg = {self._nodeDetailTitleBasic, self._nodeDetailTitleTactics, self._nodeDetailTitleKarma, self._nodeDetailTitleYoke}
	for n = 1, #titleBg do
		ccui.Helper:seekNodeByName(titleBg[n], "ImageBase"):loadTexture(Path.getTextTeam("img_zr_di03"))
		ccui.Helper:seekNodeByName(titleBg[n], "ImageBase"):setContentSize(cc.size(342, 37))
		if n == 1 or n == 2 then
			titleBg[n]:setPositionX(149)
		elseif n == 3 then  
			titleBg[n]:setPositionX(145)
		elseif n == 4 then  
			titleBg[n]:setPositionX(132)
		end
	end
	
	-- 更换标题
	self._nodeDetailTitleBasic:setOpacity(0) 
	self._heroTitle = CSHelper.loadResourceNode(Path.getCSB("CommonHeroDetailAttrTitle", "common"))
	...
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local nodeKama = UIHelper.seekNodeByName(self,"PanelAttr","_panelKarma")
		for i = 1, 8 do
			local panel = UIHelper.seekNodeByName(nodeKama,"Panel_"..i)
			if panel then
				panel:setPositionX(panel:getPositionX()-1)
			end
		end

		for i = 1, 8 do
			self["_textYuanFenDes"..i]:setFontSize(self["_textYuanFenDes"..i]:getFontSize()-2)
			self["_textYuanFenDes"..i]:setPositionX(self["_textYuanFenDes"..i]:getPositionX()-1)
		end

		
		for i = 1, 6 do
			
			local panel = UIHelper.seekNodeByName(self._nodePanelYoke,"Panel_6_"..i)
			panel:setPositionX(panel:getPositionX()-11)
