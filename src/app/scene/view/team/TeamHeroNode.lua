...
function TeamHeroNode:ctor(parentView)
	...
	if Lang.checkUI("ui4") then
		resource.file = Path.getCSB("TeamHeroNode2", "team") 
		resource.binding["_buttonPet"] = nil
	end
...
function TeamHeroNode:_initView()
	...
	if Lang.checkLang(Lang.ZH) then
		...
		self._nodePotential:setFontSize(18)
	else
	...
	if Lang.checkLang(Lang.JA) then
		local posY = self._instrument._fileNodeName:getPositionY() - 5
		self._instrument._fileNodeName:setPositionY(posY)
	end
...
-- i18n ja add func
function TeamHeroNode:_initInfor()
	if not Lang.checkUI("ui4") then   
		return 	 
	end	
	
	self._nodeLevel:setVisible(false)
	self._nodePotential:setVisible(false) 
	self._fileNodeFeature:setVisible(false) 
	...
	self._nodeDetailTitleBasic:setTitle("")
	self._nodeDetailTitleBasic:setImageBaseSize(cc.size(323, 44))
	self._nodeDetailTitleKarma:setImageBaseSize(cc.size(323, 44))
	self._nodeDetailTitleYoke:setImageBaseSize(cc.size(323, 44))
	self._heroTitle:updateUI(self._curHeroData)  
	ccui.Helper:seekNodeByName(self._heroTitle, "imgBg"):setContentSize(cc.size(315, 40))
	ccui.Helper:seekNodeByName(self._heroTitle, "imgBg"):setPositionX(315*0.5 - 3)
	-- 调整穿戴描述字 背景图位置大小
	for index = 1, 4 do
		local FileNodeName  =  self["_fileNodeEquip" .. index]:getChildByName("FileNodeName")
		FileNodeName:getChildByName("TextName"):setFontSize(18)
		FileNodeName:getChildByName("Image_1"):loadTexture(Path.getTextTeam("img_com_cvd"))
		FileNodeName:setPositionY(FileNodeName:getPositionY() + 6)  
		FileNodeName:getChildByName("TextName"):setPositionY(FileNodeName:getChildByName("Image_1"):getPositionY()) 
	end

	for index = 1, 2 do
		local FileNodeName  =  self["_fileNodeTreasure" .. index]:getChildByName("FileNodeName")   
		FileNodeName:getChildByName("TextName"):setFontSize(18)
		FileNodeName:getChildByName("Image_1"):loadTexture(Path.getTextTeam("img_com_cvd"))
		FileNodeName:setPositionY(FileNodeName:getPositionY() + 6) 
		FileNodeName:getChildByName("TextName"):setPositionY(FileNodeName:getChildByName("Image_1"):getPositionY()) 
	...
	self._fileNodeInstrument:getChildByName("FileNodeName"):getChildByName("TextName"):setFontSize(18)
	self._fileNodeInstrument:getChildByName("FileNodeName"):getChildByName("ImageBg"):loadTexture(Path.getTextTeam("img_com_cvd"))
	self._fileNodeInstrument:getChildByName("FileNodeName"):setPositionY(self._fileNodeInstrument:getChildByName("FileNodeName"):getPositionY() + 6)
 
	self._fileNodeHorse:getChildByName("TextName"):setFontSize(18)
	self._fileNodeHorse:getChildByName("ImageNameBg"):loadTexture(Path.getTextTeam("img_com_cvd"))
	self._fileNodeHorse:getChildByName("ImageNameBg"):setPositionY(self._fileNodeHorse:getChildByName("ImageNameBg"):getPositionY() + 6)	
	self._fileNodeHorse:getChildByName("TextName"):setAnchorPoint(cc.p(0.5, 0.5))
	self._fileNodeHorse:getChildByName("TextName"):setPositionY(self._fileNodeHorse:getChildByName("ImageNameBg"):getPositionY()) 

	self._fileNodeHistoryHero:getChildByName("TextName"):setFontSize(18)
	self._fileNodeHistoryHero:getChildByName("ImageNameBg"):loadTexture(Path.getTextTeam("img_com_cvd")) 
	self._fileNodeHistoryHero:getChildByName("ImageNameBg"):setPositionY(self._fileNodeHistoryHero:getChildByName("ImageNameBg"):getPositionY() + 6)
	self._fileNodeHistoryHero:getChildByName("TextName"):setAnchorPoint(cc.p(0.5, 0.5)) 
	self._fileNodeHistoryHero:getChildByName("TextName"):setPositionY(self._fileNodeHistoryHero:getChildByName("ImageNameBg"):getPositionY()) 
end
...
function TeamHeroNode:_updateBaseInfo()
	...
	if self._fileNodeCountry then   -- i18n ja change
	...
	if self._fileNodeHeroName then  -- i18n ja change
...
function TeamHeroNode:_playCurHeroVoice()
	...
	if Lang.checkUI("ui4") then
		local spine,item = self._parentView:getCurHeroSpine()
		local heroBaseId = AvatarDataHelper.getShowHeroBaseIdByCheck(self._curHeroData)
		G_HeroVoiceManager:playVoiceWithHeroId(heroBaseId,nil,item:getAvatar())
	else
...
--处理按钮位置  
function TeamHeroNode:_formatButtons()  -- i18n ja 新版日本的无神兽按钮
	local buttons = {self._buttonMaster, self._buttonOneKey, self._buttonChange, self._buttonAvatar, self._buttonSilkbag, self._buttonHistoryHero}
	local btnCount2Pos = {
		[1] = {cc.p(306, 67)},
		[2] = {cc.p(248, 67), cc.p(363, 67)},
		[3] = {cc.p(190, 67), cc.p(306, 67), cc.p(420, 67)},
		[4] = {cc.p(135, 67), cc.p(248, 67), cc.p(361, 67), cc.p(475, 67)},
		[5] = {cc.p(125, 67), cc.p(223, 67), cc.p(321, 67), cc.p(419, 67), cc.p(517, 67)}
	}
	...
	local showButtons = {true, oneKeyOpen, not isRole, isRole and avatarOpen, silkbagOpen, false} --各个按钮是否显示
...
function TeamHeroNode:_updateTacticsPos()
	...
	if not isTacticsPos3Show then
		...
		local posX = {61, 160, 260} -- i18n ja pos
		for i=1,3 do
			self["_nodeTactics"..i]:setPositionX(posX[i])
			self["_fileNodeTactics"..i]:updateUI(pos,i)
		end
...
function TeamHeroNode:_updateAttrScrollView()
	...
	if Lang.checkUI("ui4") then 	-- i18n ja  大佬需求：重新排版
		if not isTacticsOpen then
			...
			self._panelBasic:setPositionY(452-offset + 10)
			self._panelKarma:setPositionY(295-offset + 25)
			self._nodePanelYoke:setPositionY(self._karamOffset-30 + 3)
		else
			local offset = self._karamOffset
			local height = 158+566-offset 
			local contentSize = self._scAttr:getInnerContainerSize()
			local size = cc.size(contentSize.width, height + 35)
			self._scAttr:setInnerContainerSize(size)
			self._scAttr:setTouchEnabled(true)
	
			self._panelBasic:setPositionY(452-offset + 35)
			self._panelTactics:setPositionY(313-offset + 35)
			self._panelKarma:setPositionY(153-offset +35)
			self._nodePanelYoke:setPositionY(-37 + 35)
		end
		return
	end
...
function TeamHeroNode:_updateOneYoke(index)
	...
	if allYokeData and allYokeData.yokeInfo and allYokeData.yokeInfo[index] then
		...
		if Lang.checkUI("ui4") then
			...
			-- renderLabel:setMaxLineWidth(100)   会自动换行
			local UTF8 = require("app.utils.UTF8")
			if UTF8.utf8len(info.name) <=6 then
				text:setString(info.name)
			else
				local str = UTF8.utf8sub(info.name, 1, 6)
				str = str .. "..."
				text:setString(str)
			end 
		else 
...
function TeamHeroNode:_updateInstrument()
    if not self._isInstrumentShow then
		...
		self._instrument:setVisible(not Lang.checkUI("ui4"))  -- i18n ja change 隐藏神兵, 如蓝将无神兵
		return
	end
	local curPos = G_UserData:getTeam():getCurPos()
...
function TeamHeroNode:_onChooseTreasure(treasureId)
	local curPos = self.curPos   -- i18n ja change
	local curTreasureId = G_UserData:getTreasure():getCurTreasureId()
	local curTreasureUnitData = G_UserData:getTreasure():getTreasureDataWithId(curTreasureId)
	local curTreasureSlot = curTreasureUnitData:getSlot()
...
-- i18n change lable
function TeamHeroNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self._buttonAvatar,"Image_4")
		local label1 = UIHelper.swapWithLabel(image1,{ 
			style = (Lang.checkUI("ui4") and {"limit_1_ja"} or {"team_3"})[1],   					-- i18n ja label style
			text = Lang.getImgText("img_btn_fashiontxt01") ,
			fontSize = (Lang.checkUI("ui4") and {Colors.getStyle("limit_1_ja").size} or {24})[1],  -- i18n ja fontsize
		})
		label1:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        label1:getVirtualRenderer():setLineSpacing(-7)

		local image2 = UIHelper.seekNodeByName(self._buttonMaster,"Image_6")
		local label2 = UIHelper.swapWithLabel(image2,{ 
			style = (Lang.checkUI("ui4") and {"limit_1_ja"} or {"team_3"})[1],   					-- i18n ja label style
			text = Lang.getImgText("img_btn_mastertxt01") ,
			fontSize = (Lang.checkUI("ui4") and {Colors.getStyle("limit_1_ja").size} or {24})[1],  -- i18n ja fontsize
		})
		label2:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        label2:getVirtualRenderer():setLineSpacing(-7)

		local image4 = UIHelper.seekNodeByName(self._buttonSilkbag,"Image_4")
		local label4 = UIHelper.swapWithLabel(image4,{ 
			style = (Lang.checkUI("ui4") and {"limit_1_ja"} or {"team_3"})[1],   					-- i18n ja label style
			text = Lang.getImgText("img_btn_silkbagtxt") ,
			fontSize = (Lang.checkUI("ui4") and {Colors.getStyle("limit_1_ja").size} or {24})[1],  -- i18n ja fontsize
		})
		label4:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        label4:getVirtualRenderer():setLineSpacing(-7)
		...
		--替换神兽的文本
		-- local image6 = UIHelper.seekNodeByName(self._buttonPet,"Image_4")
		-- local label6 = UIHelper.swapWithLabel(image6,{ 
		-- 	 style = "team_3", 
		-- 	 text = Lang.getImgText("txt_main_enter_beast02") ,
		-- 	 fontSize =24,
		-- })
		-- label6:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        -- label6:getVirtualRenderer():setLineSpacing(-7)
	end
	
	if Lang.checkSquareLanguage() then
		local image3 = UIHelper.seekNodeByName(self._buttonChange,"Image_4")
...
-- i18n pos lable
function TeamHeroNode:_dealPosByI18n()
	if Lang.checkLang(Lang.JA) then
		for i = 1, 4 do
			self["_fileNodeAttr"..i]:setFontSize(18)
		end
		for i = 1, 6 do
			self["_textYuanFenDes"..i]:setFontSize(18)
		end
		for i = 1, 6 do
			self["_textJiBanDes"..i]:setFontSize(18)
		end
	end
	...
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		for i = 1, 8 do
			local panel = UIHelper.seekNodeByName(self._panelKarma,"Panel_"..i)
			panel:setPositionX(panel:getPositionX()-2)
		end
		for i = 1, 8 do
			self["_textYuanFenDes"..i]:setFontSize(self["_textYuanFenDes"..i]:getFontSize()-2)
			self["_textYuanFenDes"..i]:setPositionX(self["_textYuanFenDes"..i]:getPositionX()-3)
		end
		for i = 1, 6 do
			offset = (Lang.checkUI("ui4") and {-10} or {-11})[1]   	   -- i18n ja pos
			local panel = UIHelper.seekNodeByName(self._panelYoke,"Panel_6_"..i)
			panel:setPositionX(panel:getPositionX() + offset)

			self["_imageJiBanMark"..i]:setScale(0.1)
		...
		for i = 1, 6 do
			...
			if i % 2 ~= 0 then
				offset = (Lang.checkUI("ui4") and {-9} or {-18})[1]    -- i18n ja pos
				self["_textJiBanDes"..i]:setPositionX(self["_textJiBanDes"..i]:getPositionX() + offset)
			else
				offset = (Lang.checkUI("ui4") and {-3} or {-6})[1]     -- i18n ja pos
				self["_textJiBanDes"..i]:setPositionX(self["_textJiBanDes"..i]:getPositionX() + offset)
			end
		end
		...
		self._nodeLevel:setFontSize(18)
		self._fileNodeFeature:setPositionX(self._fileNodeFeature:getPositionX()+12)
		
	end
	if Lang.checkHorizontal() then
		self._fileNodeFeature:setPosition(self._fileNodeHeroName:getPositionX(),self._fileNodeHeroName:getPositionY()-35)
	end
end



function TeamHeroNode:_adjustPosByI18n()
	-- i18n ja change
	if  Lang.checkUI("ui4") then
		return
	end
	...
	if not Lang.checkLang(Lang.CN) then
		...
		self._fileNodeCountry:setPositionX(	self._fileNodeHeroName:getPositionX()-textName:getContentSize().width *0.5-28 )
	end
end	
