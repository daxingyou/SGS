...
function ChatMainView:onCreate()
    ...
    if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
        self._chatTextLength = UIHelper.getInputLimitedConfig().chat_length
	end
	...
	if not G_ConfigManager:isVoiceOpen() then
		self._buttonVoice:setVisible(false)
	end
end
...
function ChatMainView:_checkTabImageText(tabItem, bright)
    ...
    if channelId == ChatConst.CHANNEL_CROSS_SERVER then --跨服频道
        ...
        if  Lang.checkLang(Lang.CN) then
            ...
            local color_type = bright and "text_voice_1" or "text_voice_2"
            local UIHelper  = require("yoka.utils.UIHelper")
            UIHelper.setLabelStyle(imageWidget,{
                style = color_type,
                text = Lang.getImgText("voice_kuafu"),
            })     
        end
...
-- i18n pos lable
function ChatMainView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        self._nodeHint6:setVisible(false)

        local textDesc = UIHelper.seekNodeByName(self._nodeTab,"Text_desc")
		textDesc:setFontSize(textDesc:getFontSize()-7) --i18n ja change
	end
end


-- i18n change lable
function ChatMainView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
        local image_desc = UIHelper.seekNodeByName(self._nodeTab,"Image_desc")
        UIHelper.swapWithLabel(image_desc,{
            style = "text_voice_1",
            text = Lang.getImgText("voice_kuafu"),
        })     
	end
