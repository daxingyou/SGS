...
-- i18n change pos
function DailySigninView:_dealPosI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self,"Image_40")
		image1:setPositionX(image1:getPositionX()-87)

	
		local text = UIHelper.seekNodeByName(self,"Text_9")
		text:setString(Lang.getImgText("daily_signin_total_day"))
	end
end


-- i18n change lable
function DailySigninView:_updatePosI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text = UIHelper.seekNodeByName(self,"Text_9")
		local image1 = UIHelper.seekNodeByName(self,"Image_40")
		UIHelper.alignCenter(image1,{text,self._bitmapFontLabel,self._textTian})
		if Lang.checkLang(Lang.JA) then
			image1:setPositionX(940)
			UIHelper.alignCenter(image1,{text,self._bitmapFontLabel,self._textTian},{5,10,0})
		end
	end
end
