...
-- i18n pos lable
function HeroTransformCommonAwake:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local panel1 = UIHelper.seekNodeByName(self,"Panel_1")
		local size = panel1:getContentSize()


		local width = self._nodeDesValue:getWidth()
		self._nodeDesValue:setPositionX(
			size.width*0.5-width*0.5
		)
	end
end
