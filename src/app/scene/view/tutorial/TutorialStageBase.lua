...
function TutorialStageBase:ctor(stageId)
	...
	if Lang.checkUI("ui4") then
		table.sort(self._stepCfgList,function (a,b)
			return a.id<b.id
		end)
		TutorialStageScript = require("app.scene.view.tutorial.TutorialStageScript2")
	end
...
-- i18n ja 重建引导步骤，动态引导步骤
function TutorialStageBase:rebuildStepList()
	if self._hasRebuilded then
		return
	end
	local rebuildFunc = TutorialStageScript["rebuildStage"..self:getId()]
	if rebuildFunc then
		self._stepDataList = {}
		rebuildFunc(self)
		self._hasRebuilded = true
	end
end
