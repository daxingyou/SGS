...
function TutorialStepHelper.STEP_TYEP_CLICK(stepData,tipLayer,params)
	...
	if Lang.checkUI("ui4") then
		TutorialStepExtend = require("app.scene.view.tutorial.TutorialStepExtend2")
	end
...
function TutorialStepHelper.commonClickFunc(stepTable,stepData,tipLayer)
	...
	if boxRect then
		...
		boxRect.y = nodePos.y
		-- i18n ja 弱引导
		if stepTable.isWeak then
			TutorialStepHelper.showGuideWeak(stepData, tipLayer, boxRect, simulate, callBack)
		else
...
function TutorialStepHelper.simulateGuide(tipLayer, rect, callback, stepData, sound)
	...
	local function onTouchEvent(sender)
		...
		if cc.rectContainsPoint(rect,nodePos) then
			...
			if Lang.checkUI("ui4") and sound then
				G_AudioManager:stopSound(sound)
			end
		end
	end
...
function TutorialStepHelper.showGuideTip(info, tipLayer)
	...
	if string.len(info.comment) > 0 then
		-- 创建对话框
		local csb = "TutorialTipContentNode"
		if Lang.checkUI("ui4") then
			csb = "TutorialTipContentNode2"
		end
		local dialogue = cc.CSLoader:createNode(Path.getCSB(csb, "tutorial"))
		...
		if not Lang.checkLang(Lang.CN) then
			wordLabel:setPositionX(wordLabel:getPositionX()-10)
		end
		...
		if Lang.checkUI("ui4") then
			...
			local bg = nodeRoot:getSubNodeByName("Image_bk")
			bg:setContentSize(cc.size(bg:getContentSize().width,richText:getContentSize().height+46))
			local posY = bg:getPositionY()+bg:getContentSize().height/2
			richText:setPosition(wordLabel:getPositionX()+10,posY)
		end
	end
end
...
function TutorialStepHelper.showGuideFinger(stepData, tipLayer, boxRect, offset, simulate, callback)
	...
	local sound
	if info.voice ~= "" then
		sound = G_AudioManager:playSound(Path.getTutorialVoice( info.voice ) )
	end
	if simulate then
		TutorialStepHelper.simulateGuide(tipLayer, rect, callback,stepData, sound)
	end
...
function TutorialStepHelper._parseTutorialTip(tip, fontSize, fontColor, contentSize)
	...
	for i=1, #contents do
		...
		table.insert(richTextContents, {
			type = "text",
			msg = content.content,
			color = Colors.colorToNumber(content.isKeyWord and color or fontColor),
			fontSize = fontSize,
			opacity = 255
		})
	...
	if contentSize then
		...
		-- i18n pos lable
		if Lang.checkUI("ui4") then
			richText:setContentSize(cc.size(contentSize.width,0))
		...
		elseif not Lang.checkLang(Lang.CN) then
			...
			richText:setContentSize(cc.size(contentSize.width+30,0))
		else
			richText:setContentSize(contentSize)
		end
...
function TutorialStepHelper.buildEquipUpTable(useBindFinc)
	local tempTable = {
		...
		findfunc = function()
			if Lang.checkUI("ui4") then
				local _listView =  display.getRunningScene():getSubNodeByName("EquipDetailBaseView")  
...
function TutorialStepHelper.buildTreasureUpTable(useBindFinc)
	local tempTable = {
		...
		findfunc = function()
			if Lang.checkUI("ui4") then
				local _listView =  display.getRunningScene():getSubNodeByName("TreasureDetailBaseView")  
				_listView = _listView:getSubNodeByName("_listView")
				_listView:requestDoLayout()
				_listView:doLayout()
			end
...
-- i18n ja
function TutorialStepHelper.buildMainIconGoTable2(btnName, wayFuncId)
	local tempTable = {
		simulate = true,
		findfunc = function(nodeName)
			local root = display.getRunningScene():getSubNodeByName(btnName)
...
-- i18n ja
function TutorialStepHelper.buildTeamViewPosTable(pos)
	local tempTable = {
		simulate = true,
		findfunc = function()
			local listView = display.getRunningScene():getSubNodeByName("_listViewLineup")
			if pos == 6 then
				listView:jumpToBottom()
			end
			local itemList = listView:getChildren()
			local cellWidget = itemList[pos]
			local clickWidget = cellWidget:getSubNodeByName("_imageIcon")
			return clickWidget:getCascadeBoundingBox()
		end,
		clickfunc = function(sender, stepData)
			-- logNewT("TutorialStepExtend.step307")
			local teamView = display.getRunningScene():getSubNodeByName("TeamView")
			teamView:_onLeftIconClicked(pos)
			stepData.doNextStep()
		end
	}
	return tempTable
...
function TutorialStepHelper.showGuideWeak(stepData, tipLayer, boxRect, simulate, callback)
	local rect = boxRect or cc.rect(0,0,0,0)
	local info = stepData.cfg
	...
	--显示蒙层高亮
	tipLayer:showHighLightWeak(boxRect,info)
	
	-- 手指
	local effect = "effect_chuping_guangxiao"
	local effectNode = EffectGfxNode.new(effect)
	tipLayer:addChild(effectNode)
	effectNode:play()
	...
	if simulate then
		TutorialStepHelper.simulateGuide(tipLayer, nil, callback,stepData, sound)
	end

end

-- i18n ja 合计将礼包引导步骤列表
function TutorialStepHelper.createComboHeroStepList2()
	local tableList = {}
	local stepData1 = TutorialStepHelper.createFuncStep(function(stepData, tipLayer)
	...
	local stepData3 = TutorialStepHelper.createFuncStep(function(stepData, tipLayer)
		...
		TutorialStepHelper.showSimulateGuide(stepData, nil, tipLayer, onClickFunc )
	end)
	table.insert(tableList,stepData1)
	table.insert(tableList,stepData2)
	table.insert(tableList,stepData3)
