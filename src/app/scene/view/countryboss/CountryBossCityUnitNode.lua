...
function CountryBossCityUnitNode:onCreate()
	...
	if not Lang.checkLang(Lang.CN) then
		...
		if self._cfg.type == 1 then
			styleName = "countryboss_5"
		elseif 	self._cfg.type == 2 then
			styleName = "countryboss_6"
		end
...
-- i18n change lable
function CountryBossCityUnitNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._imageName = UIHelper.swapWithLabel(self._imageName,{
			 style = "countryboss_5",
			 text = Lang.getImgText("txt_guild_cangting")
		})
	end
	if Lang.checkHorizontal() then
		self._nameBg:ignoreContentAdaptWithSize(true)
		self._nameBg:loadTexture(Path.getGuildRes("img_juntuan_txtbg01_h"))
	end
end
