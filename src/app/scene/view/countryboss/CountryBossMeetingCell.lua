...
-- i18n change lable
function CountryBossMeetingCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then

        local UIHelper  = require("yoka.utils.UIHelper")	
        self._alreadyVote = UIHelper.swapSignImage(self._alreadyVote,
		{ 
			 style = "signet_8", 
			 text = Lang.getImgText("img_seal_yitoupiao01") ,
...
-- i18n pos lable
function CountryBossMeetingCell:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._heroName:getVirtualRenderer():setMaxLineWidth(100)
		self._heroName:setTextHorizontalAlignment( cc.TEXT_ALIGNMENT_CENTER  )
		self._heroName:getVirtualRenderer():setLineSpacing(0)


		self._canNotVoteLable:setPositionX(
			self._canNotVoteLable:getPositionX()+10
		)
		self._canNotVoteLable:setFontSize(
			self._canNotVoteLable:getFontSize()-2	
		)
	end
end
