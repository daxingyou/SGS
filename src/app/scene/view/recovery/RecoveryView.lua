...
function RecoveryView:_initTab()
	...
	for index = 1, #recoveryData do
		...
		if Lang.checkLang(Lang.JA) then
			local text = ccui.Helper:seekNodeByName(self["_nodeTabIcon"..index], "Text")
			local str = text:getString()
			text:ignoreContentAdaptWithSize(true)
			text:setTextAreaSize(cc.size(text:getFontSize()*2, 0)) 
			text:setString(str)
			-- local UTF8 = require("app.utils.UTF8")
			-- local strEle = UTF8.utf8sub(str, 1, 2) .. "\n"
			-- str = string.gsub(str, UTF8.utf8sub(str, 1, 2), strEle)
			-- text:setString(str)
		end
	end
end
