...
function CustomActivityEquipmentView:_initView()
	...
	local moneyStr = self._configInfo.money
	if not Lang.checkLang(Lang.CN) then
		-- local _,money = UIHelper.convertDollar(self._configInfo.money)
		moneyStr = moneyStr
	end
...
function CustomActivityEquipmentView:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		self._textTimeTitle:setPositionX(self._textTimeTitle:getPositionX()-62)
	end
	if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
		local text45 = UIHelper.seekNodeByName(self._imageCostBg2,"Text_45")
		UIHelper.alignCenter(self._imageCostBg2,{text45,self._textCost2,self._imageCost2},{10,10,0})

		local UIHelper  = require("yoka.utils.UIHelper")
		local text45 = UIHelper.seekNodeByName(self._imageCostBg1,"Text_45")
		UIHelper.alignCenter(self._imageCostBg1,{text45,self._textCost1,self._imageCost1},{10,10,0})
	end
	if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
		local img87 = UIHelper.seekNodeByName(self,"Image_87")
		img87:ignoreContentAdaptWithSize(true)
	end
end

-- i18n pos lable
function CustomActivityEquipmentView:_updatePosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self,"Image_90_2")
		UIHelper.alignCenter(image1,{self._textTimeTitle,self._textTime})
		if Lang.checkLang(Lang.JA) then
			UIHelper.alignCenter(image1,{self._textTimeTitle,self._textTime},{10,0})
		end
	end

	if Lang.checkUI("ui4") then		-- i18n ja 
		self._btnShop:setPositionX(420)  
		--self._btnShop:getTextImage():setFontSize(14)  
	end
end
