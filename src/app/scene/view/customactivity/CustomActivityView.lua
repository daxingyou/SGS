...
function CustomActivityView:ctor(selectActivityId)
	-- i18n ja 四大活动和其他活动分开显示
	self._isFourActivity = selectActivityId and true or false
    --数据
	...
	-- i18n ja
	self:setName("CustomActivityView")
...
function CustomActivityView:_refreshData()
	...
	if Lang.checkUI("ui4") then
		self._mainTabGroupData =  CustomActivityUIHelper.getTabDatas2(self._isFourActivity)
	else
...
-- i18n change lable
function CustomActivityView:_dealByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
	end
	if Lang.checkHorizontal() then
		local UIHelper  = require("yoka.utils.UIHelper")
		local img = UIHelper.seekNodeByName(self,"Image_12")
		...
		local bg = UIHelper.seekNodeByName(self,"Image_1")
		bg:addChild(titleBg)
		local size = bg:getContentSize()
		titleBg:setPosition(size.width/2,size.height)
		local img4 = UIHelper.seekNodeByName(self,"Image_4")
		img4:ignoreContentAdaptWithSize(true)
		img4:setPosition(-size.width/2,size.height)
	end
	if  Lang.checkLang(Lang.JA) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local textDesc = UIHelper.seekNodeByName(self._tabGroup,"Text_desc")
		textDesc:setFontSize(textDesc:getFontSize()-4)
	end
end

-- i18n change lable
function CustomActivityView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
	    self._consumptionTips = UIHelper.swapWithLabel(self._consumptionTips,{
			 style = "text_1",
			 text = Lang.getImgText("txt_com_shop_tips01") ,
		})
		self._consumptionTips:setAnchorPoint(cc.p(1,0.5))
		self._consumptionTips:setPositionX(self._consumptionTips:getPositionX()+60)
	end
end
