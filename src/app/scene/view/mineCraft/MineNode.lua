...
function MineNode:onCreate()
    ...
    if not Lang.checkLang(Lang.CN) then
        local styleName = nil
        if self._configData.pit_color == 3 then
            styleName = "mine_7" 
        elseif self._configData.pit_color == 4 then 
            styleName = "mine_6" 
        elseif self._configData.pit_color == 5 then
             styleName = "mine_5" 
        end
        local UIHelper  = require("yoka.utils.UIHelper")
        UIHelper.setLabelStyle(self._imageNodeName,{
            style = styleName,
			text = self._configData.pit_name
        })
    else
...
-- i18n change lable
function MineNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")

		self._imageNodeName = UIHelper.swapWithLabel(self._imageNodeName,{
			 style = "mine_5",
			 text = Lang.getImgText("txt_mine01_100")
		})
	end
end
