...
function MineCraftView:onCreate()
    ...
    if not Lang.checkLang(Lang.CN) then
        local isShow = require("app.utils.logic.FunctionCheck").funcIsShow(FunctionConst.FUNC_GRAIN_CAR)
        if isShow then
...
function MineCraftView:onEnter()
    ...
    if not Lang.checkLang(Lang.CN) then
        local isShow = require("app.utils.logic.FunctionCheck").funcIsShow(FunctionConst.FUNC_GRAIN_CAR)
        self._btnGrainCar:setVisible(isShow)
        if isShow then
            self:_grainCarProc() 
        else
            if not self._scheduleGrainCarHandler then
                self._scheduleGrainCarHandler = SchedulerHelper.newSchedule(function() 
...
function MineCraftView:_setCountDownType(type)
    if not Lang.checkLang(Lang.CN) then
        ...
        image8:setContentSize(cc.size(size.width,type == 0 and 56 or 28))
    end
    ...
    if GrainCarConfigHelper.isInActivityTime() then
        if GrainCarConfigHelper.isInLaunchTime() then
            ...
            if not Lang.checkLang(Lang.CN) then
                local UIHelper  = require("yoka.utils.UIHelper")
                local image8 = UIHelper.seekNodeByName(self,"Image_8")
                UIHelper.alignCenter(image8,{self._textGrainCar,self._textTimeGrainCar})
...
function MineCraftView:_dealI18n()
	...
	if not Lang.checkLang(Lang.CN) then
        ...
        self._textRichMine:setPositionY(self._textRichMine:getPositionY()+25)
        self._textTime:setPositionY(self._textTime:getPositionY()+25)
        self._textPeaceMine:setPositionY(self._textPeaceMine:getPositionY())
        self._textTimePeace:setPositionY(self._textTimePeace:getPositionY())
        
        local image8 = UIHelper.seekNodeByName(self,"Image_8")
        image8:setPositionY(image8:getPositionY()-10)


        self._nodeInfame:setPositionX(label:getPositionX()+label:getContentSize().width*0.5+17)
       
	end
 
    if Lang.checkLang(Lang.JA) then
        local UIHelper  = require("yoka.utils.UIHelper")
        local textInfo = UIHelper.seekNodeByName(self,"Text_Info")
        textInfo:setAnchorPoint(0,0.5)
        textInfo:setPositionX(40)
    end
end


-- i18n pos lable
function MineCraftView:_adjustPosByI18n()
    if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
        local image8 = UIHelper.seekNodeByName(self,"Image_8")
        ...
        UIHelper.alignCenter(image8,{self._textRichMine,self._textTime})
        UIHelper.alignCenter(image8,{self._textPeaceMine,self._textTimePeace})
