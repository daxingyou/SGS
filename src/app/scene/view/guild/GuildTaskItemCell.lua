...
-- i18n change lable
function GuildTaskItemCell:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
        self._imageComplete = UIHelper.swapSignImage(self._imageComplete,
		{ 
			 style = "signet_5", 
			 text = Lang.getImgText("img_woyiwancheng01") ,
...
-- i18n change lable
function GuildTaskItemCell:_adjustI18n()
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")

        local size = self._textTaskDesc2:getContentSize()
		self._textReputationValue:setPositionX(self._textTaskDesc2:getPositionX()+size.width+15)

        local size2 = self._textTaskName:getContentSize()
        self._nodeProgress:setPositionX(self._textTaskName:getPositionX()+size2.width+15)
