...
-- i18n pos lable
function PopupRecentReportView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")	
		local text17 = UIHelper.seekNodeByName(self,"Panel_center","Panel_own","Text_17")
        text17:setAnchorPoint(cc.p(0,0.5))
        text17:setPositionX(text17:getPositionX()-40)

        self._textFightCount:setPositionX(text17:getContentSize().width+3)
           --日文特殊处理
         if Lang.checkLang(Lang.JA) then
            local text17 = UIHelper.seekNodeByName(self,"Panel_center","Panel_own","Text_17")
            self._textFightCount:setPositionY(text17:getFontSize()/2)
            self._textWinPercent:setPositionX(self._textWinPercent:getPositionX()+30)
        end
	end
end
