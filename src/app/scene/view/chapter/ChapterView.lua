...
function ChapterView:onCreate()
    ...
    if Lang.checkLang(Lang.JA) then
        local node = ccui.Helper:seekNodeByName(self, "Node_Type")
        local image = ccui.Helper:seekNodeByName(node, "Image_23")    
...
function ChapterView:_playEliteBoss()
	...
	if not Lang.checkLang(Lang.CN) then
        local function effectFunction(effect)
            if effect == "routine_word_taofa_icon_zi" then
                ...
                if Lang.checkLang(Lang.JA) then
                    ...
                    subLabel:setPositionY(subLabel:getPositionY() + 4)
        ...
        local textEffect = G_EffectGfxMgr:createPlayMovingGfx( self._nodeEliteEffect, "moving_taofa_zi", effectFunction )
    end
end
...
-- i18n pos lable
function ChapterView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
    ...
    if Lang.checkLatinLanguage() then
        for type = 1, ChapterView.TOTAL_TYPE do
            self["_textType"..type]:setFontSize(self["_textType"..type]:getFontSize()-4)
        end
    end
end
