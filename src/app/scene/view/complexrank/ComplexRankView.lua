...
function ComplexRankView:onCreate()
	...
	if Lang.checkLang(Lang.JA) then  
		tabNameList[1] = string.gsub(tabNameList[1] , "レベル ランキング", "    レベル\nランキング")
		tabNameList[2] = string.gsub(tabNameList[2] , "戦力 ランキング", "     戦力\nランキング")
	end
...
-- i18n pos lable
function ComplexRankView:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local node1 = UIHelper.seekNodeByName(self._listViewPanel,"Node_1")
		local textMyRank = UIHelper.seekNodeByName(node1,"Text_my_rank")
		local textMyRankNum = UIHelper.seekNodeByName(node1,"Text_my_rank_num")
		textMyRankNum:setPositionX(
			textMyRank:getPositionX()+textMyRank:getContentSize().width
		)


		local node2 = UIHelper.seekNodeByName(self._listViewPanel,"Node_2")
		local textMyDes = UIHelper.seekNodeByName(node2,"Text_my_des")
		local textMyDesNum = UIHelper.seekNodeByName(node2,"Text_my_des_num")
		textMyDesNum:setPositionX(85)
		textMyDes:setPositionX(
			textMyDesNum:getPositionX()-textMyDes:getContentSize().width
		)
