...
function HorseView:_swapImageByI18n()
	...
	if Lang.checkUI("ui4") then
		local UIHelper  = require("yoka.utils.UIHelper")
		local list = { 
			{name = "_btnRace",pos = cc.p(140,12),red = "_imageHorsePlayRP",text = "txt_horse_play"},
			{name = "_buttonHorseList",pos = cc.p(70,30),red = "_imageHorseListRP",text = "txt_horse_mine"},
			{name = "_buttonHorseJudge",pos = cc.p(0,58),red = "_imageHorseJudgeRP",text = "txt_horse_choice"},
			{name = "_buttonHorseShop",pos = cc.p(255,10),red = "_imageHorseShopRP",text = "txt_horse_shop"},
		}
		for i, v in ipairs(list) do
			-- local img = UIHelper.seekNodeByName(self[v.name],"ImageNameBG")
			-- img:setScale9Enabled(true)
			-- img:setCapInsets(cc.rect(18,10,1,1))
			-- local label = UIHelper.seekNodeByName(img,"Image_153")
			-- label:getVirtualRenderer():setMaxLineWidth(22)
			-- label:getVirtualRenderer():setLineSpacing(-2)   
			-- label:setAnchorPoint(0.5,0.5)
			-- local width = img:getContentSize().width
			-- local height = label:getVirtualRendererSize().height+40
			-- img:setContentSize(cc.size(width,height))
			-- label:setPosition(width/2,height/2)
			-- -- label:ignoreContentAdaptWithSize(true)
			-- self[v.red]:setPosition(width,height)
				local  text  = v.text;
				local _imageNameBG = UIHelper.seekNodeByName(self[v.name],"ImageNameBG")
				_imageNameBG:setScale(1)
				_imageNameBG:setScale9Enabled(false)
				local _imageName = UIHelper.seekNodeByName(_imageNameBG,"Image_153")
				UIHelper.setLabelStyle(_imageName,{
					style = "challenge_2_ui4" ,
					text = Lang.getImgText(text  ),
				})  
				_imageName:setAnchorPoint(cc.p(0.5, 1))  
				_imageName:getVirtualRenderer():setMaxLineWidth(24)  
				UIHelper.loadCommonBgImageByI18n( _imageNameBG,Lang.getImgText(text))
				_imageNameBG:ignoreContentAdaptWithSize(true)
				local imageNameBgSize = _imageNameBG:getContentSize()
				local imageNameBgVirtualSize = _imageNameBG:getVirtualRendererSize()
		
				_imageName:setPositionY(imageNameBgVirtualSize.height-36)
				_imageName:setPositionX(imageNameBgVirtualSize.width/2)
				_imageNameBG:setPosition(v.pos)
				_imageNameBG:setPositionY(_imageNameBG:getPositionY()+imageNameBgSize.height/2)

		
				self[v.red]:setPositionY(imageNameBgVirtualSize.height-10)
