...
function PopupFirstPayView:ctor()
	...
	if Lang.checkUI("ui4") then
		resource.file = Path.getCSB("PopupFirstPayView2", "firstpay")
	end
...
function PopupFirstPayView:_onItemUpdate(item, index)
	if self._listData[index + 1] then
		item:updateUI(self._listData[index + 1] ,index + 1)-- i18n change
	end
end
...
function PopupFirstPayView:_onItemTouch(lineIndex,index)
    ...
    elseif firstPayData:hasReceive(itemData.id) then
        ...
        if Lang.checkUI("ui4") then
            ...
            WayFuncDataHelper.gotoModuleByFuncId(FunctionConst.FUNC_RECHARGE2)
        else
            local WayFuncDataHelper = require("app.utils.data.WayFuncDataHelper")
...
-- i18n pos lable
function PopupFirstPayView:_dealPosByI18n()
