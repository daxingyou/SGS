...
function HistoryHeroTrainView:onCreate()
	if Lang.checkLang(Lang.JA) then
		self._buttonReplace:setPositionY(self._buttonReplace:getPositionY()-5)
	end
	self:_initBaseView()
	self:_initBtn()
...
function HistoryHeroTrainView:_initStarListView()
	...
	if Lang.checkLang(Lang.JA) then
		local height = 160
		self._listViewLineup:setPositionY(self._listViewLineup:getPositionY() - height)
		self._listViewLineup:setContentSize(cc.size(self._listViewLineup:getContentSize().width, self._listViewLineup:getContentSize().height + height))
	end
...
function HistoryHeroTrainView:_updateAttr(bAnimate)
	...
	if not Lang.checkLang(Lang.CN) then
		self._attrBg:setContentSize(cc.size(610, 32))
		self:_updatePosI18n()
	end
	...
	if self._skillDesc == nil then
		...
		self._skillDesc = cc.Label:createWithTTF("", Path.getCommonFont(), fontSize)
...
-- i18n change lable
function HistoryHeroTrainView:_updatePosI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		
        local label_11 = UIHelper.seekNodeByName(self._node1,"titleAttack")
		local label_12 = UIHelper.seekNodeByName(self._node1,"plus")
		
		local label_21 = UIHelper.seekNodeByName(self._node2,"titlelLife")
		local label_22 = UIHelper.seekNodeByName(self._node2,"plus")
		

		local label_31 = UIHelper.seekNodeByName(self._node3,"titlePDef")
		local label_32 = UIHelper.seekNodeByName(self._node3,"plus")
		
		local label_41 = UIHelper.seekNodeByName(self._node4,"titleMDef")
		local label_42 = UIHelper.seekNodeByName(self._node4,"plus")
		
       
        local UIHelper  = require("yoka.utils.UIHelper")
		UIHelper.alignCenter(self._attrBg,
			{label_11,label_12,self._labelAttr1,label_21,label_22,self._labelAttr2,label_31,label_32,self._labelAttr3,label_41,label_42,self._labelAttr4},
			{5,2,10,5,2,10,5,2,10,5,2,0}
		)
	end
end
