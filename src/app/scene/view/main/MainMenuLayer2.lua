...
local AudioPatchIcon = require("app.scene.view.audiopatch.AudioPatchIcon")
local SchedulerHelper = require("app.utils.SchedulerHelper")
local Queue = require("app.utils.Queue")
local BannerPageView = require("app.scene.view.main.BannerPageView")

MainMenuLayer.ICON_INTERVAL = 45+10
MainMenuLayer.ICON_SIZE = 80

MainMenuLayer.RIGHT_TOP_ICON_INTERVAL = 82 -- 67

MainMenuLayer.TIP_FONT_SIZE = 16
MainMenuLayer.TIP_POSITION = cc.p(0, -54)
MainMenuLayer.TIP_COLOR = cc.c3b(255,255,255)--cc.c3b(0x39, 0xe7, 0x32)
MainMenuLayer.TIP_OUTLINE_COLOR = cc.c3b(0,0,0)--cc.c3b(0x0f,0x4c,0x07)
--左下按钮
...
MainMenuLayer.RIGHT_TOP_ICON_A2 = {
    ...
    {funcId = FunctionConst.FUNC_EQUIP_ACTIVITY}, --割须弃袍
    {funcId = FunctionConst.FUNC_PET_ACTIVITY}, --卧龙观星
    {funcId = FunctionConst.FUNC_HORSE_CONQUER_ACTIVITY}, --训马
    {funcId = FunctionConst.FUNC_AVATAR_ACTIVITY}, --变身卡
...
MainMenuLayer.OTHER_ICON = {
    ...
    -- {funcId = FunctionConst.FUNC_FIRST_RECHARGE},   --首充礼包
    {funcId = FunctionConst.FUNC_MAIN_STRONGER},    --我要变强
    ---------------
    --右下
...
--右侧固定按钮
MainMenuLayer.RIGHT_PANEL_ICON = {
    {funcId = FunctionConst.FUNC_MINE_CRAFT}, --矿战
    {funcId = FunctionConst.FUNC_ARMY_GROUP}, --军团
    {funcId = FunctionConst.FUNC_TRAVEL},     --游历
    {funcId = FunctionConst.FUNC_ADVENTURE},  --冒险
}

--左中按钮
MainMenuLayer.LEFT_MID_ICON = {
    {funcId = FunctionConst.FUNC_SHOP_SCENE},   --商店
    {funcId = FunctionConst.FUNC_AUCTION},      --拍卖
    -- 充值，replaceFuncIds替换功能为更换图标的充值
    {funcId = FunctionConst.FUNC_RECHARGE, replaceFuncIds = {{funcId = FunctionConst.FUNC_JADE2,param = {1,1}}}},
    {funcId = FunctionConst.FUNC_WELFARE},
    {funcId = FunctionConst.FUNC_DAILY_MISSION}, --任务
}

--左上按钮
MainMenuLayer.LEFT_TOP_ICON = {
    {funcId = FunctionConst.FUNC_NOTICE},           --通知公告
    {funcId = FunctionConst.FUNC_QUESTIONNAIRE},    --谷歌问卷
    {funcId = FunctionConst.FUNC_LOBI},             --lobi
    -- {funcId = FunctionConst.FUNC_FIRST_RECHARGE},   --首充礼包
    -- {funcId = FunctionConst.FUNC_MAIN_STRONGER},    --我要变强
}

--左上第二排按钮
MainMenuLayer.LEFT_TOP_ICON_2 = {
    {funcId = FunctionConst.FUNC_FIRST_RECHARGE},   --首充礼包
    {funcId = FunctionConst.FUNC_LEVEL_GIFTPKG}  --等级礼包
}
...
function MainMenuLayer:ctor(callback)
    ...
    local resource = {
        file = Path.getCSB("MainMenuLayer2", "main"),
...
function MainMenuLayer:onCreate()
    ...
    -- self._morePanel:setPositionY(345)
    ...
    self._AudioPatch = AudioPatchIcon.new(self._btnDownload)
    ...
    self:_initRightPanelBtn()
    self._bannerPageView = BannerPageView.new(self)
    local size = self._panelRight:getContentSize()
    self._bannerPageView:setPosition(size.width/2,size.height)
    self._panelRight:addChild(self._bannerPageView)
...
function MainMenuLayer:_resetButtons()
    ...
    -- i18n bug
    if self._cacheReset:size() > 0 then
        if self._scheduleResetHandler then
            SchedulerHelper.cancelSchedule(self._scheduleResetHandler)
            self._scheduleResetHandler = nil
        end
...
function MainMenuLayer:_getCakeActivityVisibleAndCustomData()
    ...
    if isVisible then
        ...
        if not Lang.checkLang(Lang.CN) then  
            local CakeResouce1 = require("app.config.cake_resouce_1")
            local config = CakeResouce1.get(info.type)
            assert(config, "Invalid cake_resouce_1 can not find type "..info.type)
            customData.iconTxtUIInfo = {
                name = config.icon_word,
                script = config.icon_word_script,
                alignment = config.icon_word_alignment,
                space = config.icon_word_space,
                place = config.icon_word_place
            }
        end
        return true, customData
    end
...
function MainMenuLayer:_getNewLevelPkgVisibleAndCustomData()
    local FunctionCheck = require("app.utils.logic.FunctionCheck")
    --local isOpen = FunctionCheck.funcIsOpened(FunctionConst.FUNC_LEVEL_GIFTPKG)
   -- if not isOpen then
    --    return false
   -- end
    ...
    if visible then
        ...
        G_ServiceManager:registerOneAlarmClock(
            ...
            function()
                G_SignalManager:dispatch(SignalConst.EVENT_MAIN_CITY_CHECK_BTNS, FunctionConst.FUNC_LEVEL_GIFTPKG)
            end
        )
...
function MainMenuLayer:_getFunctionBtnVisibleAndCustomData(functionId)
    ...
    elseif functionId == FunctionConst.FUNC_EQUIP_ACTIVITY then
        visible = G_UserData:getCustomActivity():isEquipActivityVisible()
        -- customData = {callFunc = function(menuBtn, menuData)
        --     menuBtn:showRunningImage(true)
        -- end}
    elseif functionId == FunctionConst.FUNC_PET_ACTIVITY then
        visible = G_UserData:getCustomActivity():isPetActivityVisible()
        -- customData = {callFunc = function(menuBtn, menuData)
        --     menuBtn:showRunningImage(true)
        -- end}
    elseif functionId == FunctionConst.FUNC_HORSE_CONQUER_ACTIVITY then
        visible = G_UserData:getCustomActivity():isHorseConquerActivityVisible()
        -- customData = {callFunc = function(menuBtn, menuData)
        --     menuBtn:showRunningImage(true)
        -- end}
    elseif functionId == FunctionConst.FUNC_AVATAR_ACTIVITY then
        visible = G_UserData:getCustomActivity():isAvatarActivityVisible()
        -- customData = {callFunc = function(menuBtn, menuData)
        --     menuBtn:showRunningImage(true)
        -- end}
    elseif functionId == FunctionConst.FUNC_LEVEL_GIFTPKG then --i18n ja
        visible, customData =  self:_getNewLevelPkgVisibleAndCustomData()
    end
    return visible, customData
end
...
function MainMenuLayer:_setOtherBtnCallback ()
    ...
    -- table.insert(otherList, self._btnFirstCharge)
...
function MainMenuLayer:_resetOtherBtn()
    ...
    -- self._btnFirstCharge:setBgScale(0.7)
end
...
function MainMenuLayer:_resetRightTop2()
    local posX = G_ResolutionManager:getBangDesignWidth() - 490
    local funBtnList = {}
...
function MainMenuLayer:_resetBagMergeBtns(...)
    ...
    if self._bagMergePosX + panelW/2 > G_ResolutionManager:getBangDesignWidth() then
        self._bagMergePosX = G_ResolutionManager:getBangDesignWidth() - panelW/2
    end
...
function MainMenuLayer:_createMenu(menuData, rootPanel)
    ...
    if iconDataFunc ~= nil and iconDataFunc.node ~= nil then
        ...
        --i18n ja BUG 
        --self:_setBtnMapData(iconDataFunc.node:getButton(), iconDataFunc, iconDataFunc.node:getSubNodeByName("RedPoint"))
    ...
    local node = CSHelper.loadResourceNode(Path.getCSB("CommonMainMenu2", "common"))
    ...
    if  Lang.checkLang(Lang.CN) or not menuData.customData then
        ...
        node:updateUI(iconShowFunctionId or menuData.functionId, customIcon, 
            customIconTxt,menuData.customData.iconTxtUIInfo)
    end
...
function MainMenuLayer:_onButtonClick(sender)
    ...
    if (menuData ~= nil) then
        ...
        if menuData.functionId == FunctionConst.FUNC_LEVEL_GIFTPKG then
            local UserDataHelper = require("app.utils.UserDataHelper")
            local iconData = UserDataHelper.getNewLevelPkgMainIconData()
            local listNum =  #iconData.list --#(menuData.customData.iconData.list)
            if listNum  > 1 then
               local popup = require("app.scene.view.newLevelPkg.PopupLevelPkgMenuTips").new(menuData.node)
	           popup:open()
            else
                G_SceneManager:showDialog("app.scene.view.newLevelPkg.PopupLevelPkg", nil, nil,
                menuData.customData.iconData.main.condition,menuData.customData.iconData.main.conditionValue)
            end
        else
    ...
    local isFuncShow = require("app.utils.logic.FunctionCheck").funcIsShow(FunctionConst.FUNC_MORE)
    if isFuncShow then
...
function MainMenuLayer:_onClickPanelInfo()
    local PopupPlayerDetail = require("app.scene.view.playerDetail.PopupPlayerDetail2").new()
...
function MainMenuLayer:_updateRoleInfo()
    ...
    if not self._label then
        local width = 13
        local height = 26
        local label = cc.LabelAtlas:_create(0, Path.getMain2("img_zhanli"), width, height,  string.byte("0"))
        if cc.isRegister("CommonRollNumber") then
            cc.bind(label, "CommonRollNumber")
        end
        label:setRollListener(self)
        self._nodePower:removeAllChildren()
        self._nodePower:addChild(label)
        self._label = label
        self._label:setAnchorPoint(0,0.5)
        local labelWan = ccui.ImageView:create(Path.getMain2("img_zhanliwan"))
        labelWan:setAnchorPoint(0,0.5)
        labelWan:setPositionY(height/2)
        self._label:addChild(labelWan)
...
function MainMenuLayer:setNumberValue(value)
    ...
    local str,isWan = TextHelper.getAmountTextUI4(value)
...
function MainMenuLayer:onEnter()
    ...
    self._signalNoticeRedPoint =
        G_SignalManager:add(SignalConst.EVENT_NOTICE_RED_POINT, handler(self, self._onEventNoticeRedPoint))
...
function MainMenuLayer:_initFuncMap()
    ...
    self._funcList = {MainMenuLayer.FUNC_TERRITORY, MainMenuLayer.OTHER_ICON, MainMenuLayer.MORE_ICON,
        ...
        MainMenuLayer.LEFT_BOTTOM_ICON,MainMenuLayer.LEFT_MID_ICON,MainMenuLayer.LEFT_TOP_ICON,MainMenuLayer.LEFT_TOP_ICON_2}
    local functionList = {self._resetTerritory, self._resetOtherBtn, self._resetMoreBtns,
        ...
        self._resetLeftBottom,self._resetLeftMid,self._resetLeftTop,self._resetLeftTop2}
...
function MainMenuLayer:onExit()
    ...
    self._signalNoticeRedPoint:remove()
    self._signalNoticeRedPoint = nil

    G_ServiceManager:DeleteOneAlarmClock("SeasonMainBtn")

    G_ServiceManager:DeleteOneAlarmClock("NewLevelPkgCountDown")--i18n ja
...
function MainMenuLayer:_openMore(boolValue)
    ...
    -- self._btnMore:setVisible(not boolValue)
...
function MainMenuLayer:_updateRedPointByFuncId(funcId, param)
    if funcId == FunctionConst.FUNC_NOTICE then
        return
    end
...
function MainMenuLayer:_onEventRedPointUpdate(id, funcId, param)
    ...
    local function inTheRechargeList(funcId)
        local list = {FunctionConst.FUNC_SHOP_SCENE,FunctionConst.FUNC_VIP_GIFT,FunctionConst.FUNC_WELFARE}
    ...
    if funcId and type(funcId) == "number" then
        ...
        if inTheRechargeList(funcId) then
            self:_updateRedPointByFuncId(FunctionConst.FUNC_JADE2, param)
        end
...
function MainMenuLayer:_initRightPanelBtn ()
    local rightList = {}
    table.insert(rightList, self._btnMine)
    table.insert(rightList, self._btnGroup)
    table.insert(rightList, self._btnExplore)
    table.insert(rightList, self._btnAdventure)

    for i, value in ipairs(MainMenuLayer.RIGHT_PANEL_ICON) do
        local btn = rightList[i]
        local menuData = self:_createMenuData(value)
        ...
        self._btnMap[btn] = self:_createMenuData(menuData.funcData)
        local funcId = menuData.functionId
        self:_setRedDotData(btn:getSubNodeByName("RedPoint"), funcId, btn)
        btn:addClickEventListenerEx(handler(self, self._onButtonClick))
    end
end

--左中，排序
function MainMenuLayer:_resetLeftMid()
    local posX = 75
    local funBtnList = {}
    for k, v in ipairs(MainMenuLayer.LEFT_MID_ICON) do
        table.insert(funBtnList, v)
    end
    local menuList = self:_createMenuList(funBtnList)
    for i, value in ipairs(menuList) do
        if value.functionId == FunctionConst.FUNC_SHOP_SCENE then
            self._btnShop = value.node
        end
        local node = value.node
        if node then
            local posY = 475 - (i-1) * 80
            node:setPositionX(posX)
            node:setPositionY(posY)
            -- node:moveLetterToRight()
            node:setIconScale(0.8)
...
--左上，排序
function MainMenuLayer:_resetLeftTop()
    self._homelandBuff:updateUI()
    local posY = 535
    local funBtnList = {}
    for k, v in ipairs(MainMenuLayer.LEFT_TOP_ICON) do
        table.insert(funBtnList, v)
    end
    local menuList = self:_createMenuList(funBtnList)
    if self._btnBuff:isVisible() then
        menuList[#menuList+1] = {node = self._btnBuff}
    end
    menuList[#menuList+1] = {node = self._btnDownload}
    for i, value in ipairs(menuList) do
        local node = value.node
        if node then
            local posX = 140 + (i-1) * 43
            node:setPositionX(posX)
            node:setPositionY(posY)
...
--左上2，排序
function MainMenuLayer:_resetLeftTop2()
    self._homelandBuff:updateUI()
    local posY = 485
    local funBtnList = {}
    for k, v in ipairs(MainMenuLayer.LEFT_TOP_ICON_2) do
        table.insert(funBtnList, v)
    end
    local menuList = self:_createMenuList(funBtnList)
    for i, value in ipairs(menuList) do
        local node = value.node
        if node then
            local posX = 160 + (i-1) * 80
            node:setPositionX(posX)
            node:setPositionY(posY)
            -- node:setScale(0.9)
            node:setBgScale(0.65)
            if rawget(value, "customData") ~= nil then
                local customData = value.customData
                if customData then
                    if customData.callFunc then
...
function MainMenuLayer:_onEventShowAudioPatch(_, param)
    self._AudioPatch:updateUI(param)
end

function MainMenuLayer:_updateAudioPatch()
    local AudioPatchHelper = require("app.scene.view.audiopatch.AudioPatchHelper")
    local popup = AudioPatchHelper.getPopupAudioPatch()
    if popup and popup:isDownloading() then
        self._AudioPatch:updateUI(true)
    else
        self._AudioPatch:updateUI(false)
    end
end

function MainMenuLayer:_onChangeBtn()
    self._callback(false)
end

-- 满级后若经验溢出, 显示红点
function MainMenuLayer:_isExpOver( ... )
	--玩家经验
...
function MainMenuLayer:_updateNoticeRedPoint()
    local url = G_ConfigManager:getPopupRedUrl()
    local noticeUrl = G_ConfigManager:getPopupUrlI18n()
    if url == "" or noticeUrl == "" then
        return
    end
    ...
    urlContent = urlContent .. passId
    url = url .. urlContent
    print("NoticeRedPointURL = ", url)
    local xhr = cc.XMLHttpRequest:new()
    xhr.responseType = cc.XMLHTTPREQUEST_RESPONSE_STRING
    xhr:setRequestHeader("uid",G_UserData:getBase():getId())
    xhr:open("GET", url)
    local function onCallback()
        if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
            local ret = json.decode(xhr.response)
            dump(ret,"_updateNoticeRedPoint")
            if ret then
                if ret.data then
                    local dot = ret.data.dot or 0
                    if self._redDotMap then
                        local redDot = self._redDotMap[FunctionConst.FUNC_NOTICE] or {}
                        for i, value in ipairs(redDot) do
                            value.imgRed:setVisible(dot>0)
                            value.imgRed:setPosition(10,10)
                            -- value.imgRed:setScale(0.64)
                        end
                    end
                end
            end
