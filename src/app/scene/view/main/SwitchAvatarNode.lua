...
local colorList = {
	...
	[3] = cc.c3b(0xb4 , 0xec  , 0xf6),
	[4] = cc.c3b(0xfa , 0xce  , 0xff),
...
function SwitchAvatarNode:updateIcon(data,selectId,onClick)
	...
	local resCfg = require("app.config.hero_res").get(data.resId)
	self._imageIcon:loadTexture(Path.getHeroBustIcon(resCfg.icon))
	self._nameLabel:setString(data.config.name)
	...
	self:setSelected(selectId == data.id)
end
