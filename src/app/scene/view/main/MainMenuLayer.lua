...
function MainMenuLayer:_getVisibleAndCountDownCallbackForGrainCar(labelStr)
    ...
    if actStage == GrainCarConst.ACT_STATGE_GENERATED  then
      ...
      if not Lang.checkLang(Lang.CN) then    
        if actStage == GrainCarConst.ACT_STATGE_GENERATED and 
            (GrainCarConfigHelper.getGrainCarOpenTimeStamp()-300) > G_ServerTime:getTime() then
            G_ServiceManager:registerOneAlarmClock(
                "GrainCarMainBtn2",
                GrainCarConfigHelper.getGrainCarOpenTimeStamp()-300,
                function()
    ...
    local callBack = function(menuBtn, menuData)
        ...
        if actStage == GrainCarConst.ACT_STATGE_GENERATED  then
            ...
            menuBtn:showRunningImage(false)
			if not Lang.checkLang(Lang.CN) then
	            local leftTime = GrainCarConfigHelper.getGrainCarOpenTimeStamp()-G_ServerTime:getTime()
	            local isNeedShowEffect = leftTime <= 300
	            if isNeedShowEffect then
	                menuBtn:playBtnMoving()
	                menuBtn:playFuncGfx()
	            else
...
function MainMenuLayer:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        ...
        --self._btnBuff:setPositionX(self._btnBuff:getPositionX()+210)
        --self._btnBuff:setPositionY(self._btnBuff:getPositionY()+60)
	end
end
