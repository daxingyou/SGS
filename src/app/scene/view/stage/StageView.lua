...
function StageView:onCreate()
	...
	if self._topBar then
        ...
        if Lang.checkLang(Lang.JA) then
            local imageIcon = self._topBar:getTopBarItemList():getResNode(2):getImageResIcon()
            local buttonAdd = self._topBar:getTopBarItemList():getResNode(2):getButtonAdd()
            imageIcon:setPositionX(imageIcon:getPositionX() + 4)
            buttonAdd:setPositionX(buttonAdd:getPositionX() + 4)
        end
    ...
    --越南版本添加阵容文本
	if  Lang.checkLang(Lang.VN) or Lang.checkUI("ui4") then
...
function StageView:_checkFamousUI()
    ...
    if not Lang.checkLang(Lang.CN) then
        self:_updateScrollViewSizeByI18n()
    end
...
function StageView:_onEventExecuteStage(eventName, message, isFirstPass, stageId)
    ...
    local function enterFightView()
        ...
        if isFirstPass then 
            ...
            -- i18n ja 通过游戏第x章
            if stageInfo.type == 3 then
                G_NativeAgent:eventCustom("AT_Pass_Chapter", tostring(stageInfo.chapter_id))
            end
...
function StageView:_doLayoutTopButton()
	...
	local startIndex = 0
	local gap = 90
    if Lang.checkLang(Lang.JA) then -- i18n ja change offset
        gap = 110
    end
...
-- i18n change lable
function StageView:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		
		local image = UIHelper.seekNodeByName(self._btnPassBox,"Image_1")
	    local labe3 = UIHelper.swapWithLabel(image,{
			 style = "icon_txt_3",
			 text = Lang.getImgText("txt_main_enter6_passreward") ,
			 offsetY = -37,
             offsetX = -5,
		})
        labe3:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER )
        labe3:getVirtualRenderer():setLineSpacing(-7)


        local image1 = UIHelper.seekNodeByName(self._imageFamousStory,"Image_112")
	    local label1 = UIHelper.swapWithLabel(image1,{
			 style = "chapter_1",
			 text = Lang.getImgText("txt_mingjiangzhuanji01") ,
             offsetX = -18,
		})

        local image2 = UIHelper.seekNodeByName(self._imageFamousStoryOpen,"Image_115")
	    local label2 = UIHelper.swapWithLabel(image2,{
			 style = "chapter_1",
			 text = Lang.getImgText("txt_shouhui01") ,
             offsetX = -8,
		})
        label2:setScaleX(-1)
	end
...
-- i18n pos lable
function StageView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        local imageCountBG = UIHelper.seekNodeByName(self._nodeFamous,"ImageCountBG") 
        local text1 = UIHelper.seekNodeByName(self._nodeFamous,"Text1") 
        UIHelper.alignCenter(imageCountBG,{text1,self._textFamousCount},{10,0})
        
        local text57 = UIHelper.seekNodeByName(self._imageFamousStoryOpen,"Text_57")  
        text57:setVisible(false)
        self._textFamousStory:setPositionX(self._textFamousStory:getPositionX()-12)
        self._textFamousStory:setFontSize(18)
        self._textFamousStory:setTextAreaSize(cc.size(310 + 34, 76)) 

        -- i18n ja pos
        if Lang.checkLang(Lang.JA) then
            self._textFamousCount:setPositionY(self._textFamousCount:getPositionY() - 2)
        end
    end
    
    if Lang.checkUI("ui4") then
        self._textFamousStory:setFontSize(16)
        self._textFamousStory:ignoreContentAdaptWithSize(true)
        self._textFamousStory:setTextAreaSize(cc.size(310 + 20, 0))
        local renderLabel = self._textFamousStory:getVirtualRenderer()
       --renderLabel:setMaxLineWidth(310+34)
        local labelSize = renderLabel:getContentSize()
        labelSize.height = math.max(70,labelSize.height)
        local scrollView = ccui.ScrollView:create()
        logWarn("--xxxxxx---------"..labelSize.height)
        scrollView:setDirection(ccui.ScrollViewDir.vertical)
        scrollView:setScrollBarEnabled(false)
        scrollView:setPosition(cc.p(0,0))
        scrollView:setInnerContainerSize(labelSize)
        local scrollSize = cc.size(310 + 20, 70)
        scrollView:setContentSize(scrollSize)
        local parent = self._textFamousStory:getParent()
        local x,y = self._textFamousStory:getPosition()
        self._textFamousStory:retain()
        self._textFamousStory:removeFromParent()
        scrollView:addChild(self._textFamousStory)
        self._textFamousStory:release()
        scrollView:setAnchorPoint(cc.p(0.5,0.5))
        scrollView:setPosition(x,y)
        self._textFamousStory:setAnchorPoint(cc.p(0,1))
        self._textFamousStory:setPosition(0,labelSize.height)
        parent:addChild(scrollView)
        self._scrollViewI8n = scrollView
    end

    if Lang.checkUI("ui4") then
        -- self._btnPassBox:ignoreContentAdaptWithSize(true)
        self._btnPassBox:setContentSize(cc.size(100,100))
        local image = UIHelper.seekNodeByName(self._btnPassBox,"Image_1")
        image:setPosition(50,0)
        self._btnPassBox:setPositionY(-135)
        local scale = 0.8
        self._btnPassBox:setScale(scale)
        image:setScale(1/scale)
    end
end

function StageView:_updateScrollViewSizeByI18n()
    if Lang.checkUI("ui4") then
        local renderLabel = self._textFamousStory:getVirtualRenderer()
        local labelSize = renderLabel:getContentSize()
        labelSize.height = math.max(70,labelSize.height)
        logWarn("--xxxxxx---------"..labelSize.height)
        self._scrollViewI8n:setInnerContainerSize(labelSize)
        self._textFamousStory:setPosition(0,labelSize.height)
    end
