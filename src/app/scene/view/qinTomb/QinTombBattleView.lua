...
function QinTombBattleView:onEnter()
	...
	self._commonNodeTime:setAddTouchCallback(function ()
		if Lang.checkUI("ui4") then
			local shopFixConfig = UserDataHelper.getFixShopConfig(70005)
			local itemId = shopFixConfig.value
			local itemType = shopFixConfig.type
			local shopItemData = G_UserData:getShops():getFixShopGoodsByResId(ShopConst.VIP_EXCHANGE_SHOP, itemType, itemId)
...
-- i18n pos lable
function QinTombBattleView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
