	...
	elseif level == 5 then
		if self._bIsRightType then
			...
			if Lang.checkLang(Lang.JA) then
				self._bgTrain:setPositionX(-105)
			end
		else
...
-- i18n change lable
function GrainCarInfoNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
        self._title = UIHelper.swapWithLabel(self._title,{
            style = "txt_escort01_muniu",
            text = Lang.getImgText("txt_escort01_muniu"),
        })     
	end
end

-- i18n change lable
function GrainCarInfoNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
		self._labelHp:setPositionX(
			self._titleHp:getPositionX()+ self._titleHp:getContentSize().width+5
		)
		
		
		local titleAbility = UIHelper.seekNodeByName(self._nodeAbility,"titleAbility")
		self._labelAbility:setPositionX(
			titleAbility:getPositionX()+ titleAbility:getContentSize().width+5
		)
	end
end
