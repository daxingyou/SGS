...
-- i18n change lable
function PopupGrainCarDonate:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
        local UIHelper  = require("yoka.utils.UIHelper")
        local image3 = UIHelper.seekNodeByName(self._nodeMaxLevel,
            "Image_139")
        UIHelper.swapWithLabel(image3,{
            style = "text_escort_1",
            text = Lang.getImgText("txt_escort_dengji02"),
        })    
    ...
    if Lang.checkLang(Lang.JA) then
        local offset = 15
        local UIHelper  = require("yoka.utils.UIHelper")
        local nodeTitle = UIHelper.seekNodeByName( self, "nodeTitle")
        nodeTitle:setPositionY(-offset)
        self._nodeProgress:setPositionY(self._nodeProgress:getPositionY()+offset)
        self._checkBoxAdmin:setPositionY(self._checkBoxAdmin:getPositionY()+offset)
        self._checkBoxAll:setPositionY(self._checkBoxAll:getPositionY()+offset)
    end
end
...
-- i18n pos lable
function PopupGrainCarDonate:_updateTimePosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        UIHelper.alignCenter(self._titleBg,{self._titleOpenTime,self._textOpenTime},{5,0})
    end
end


-- i18n pos lable
function PopupGrainCarDonate:_updateDonateCountPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
        UIHelper.alignCenter(self._nodeRun,{self._donateCount,self._labelDonateCount},{5,0})
    end
end
