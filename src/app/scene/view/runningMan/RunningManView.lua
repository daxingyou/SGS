...
-- i18n pos lable
function RunningManView:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._betIcon:setPositionY(self._betIcon:getPositionY() + 4)
	end
end


-- i18n pos lable
function RunningManView:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
	

		local size = self._panelTop:getContentSize()
		local size1 = self._topDesc:getContentSize()
		local size2 = self._topTime:getContentSize()
		
		self._topDesc:setPositionX(size.width*0.5-size2.width*0.5+size1.width*0.5)
		self._topTime:setPositionX(size.width*0.5+size1.width*0.5-size2.width*0.5)
		if Lang.checkUI("ui4") then
			self._topDesc:setPositionX(self._topDesc:getPositionX()-5)
			self._topTime:setPositionX(self._topTime:getPositionX()+5)
		end
	end
end
