...
function PetMerge:ctor(petId, count)
    ...
    if Lang.checkUI("ui4") then
        resource.file = Path.getCSB("PetMerge2", "petMerge")
        PetMerge.super.ctor(self, resource, 107)
    else
...
function PetMerge:onCreate()
    if Lang.checkUI("ui4") then
        G_EffectGfxMgr:createPlayGfx(self._imageGetBG, "effect_biaoti_zhaomutx", function() end, false)
    end
...
function PetMerge:_playHeroOpen()
    ...
    if self._petData.color >= 4 then
        if Lang.checkUI("ui4") then
            local PetShow = require("app.scene.view.petMerge.PetShow2")
            local pop = PetShow.create(self._petData.id, function() self:_playAvatarOpen() end)
