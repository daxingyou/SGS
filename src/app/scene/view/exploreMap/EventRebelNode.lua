...
-- i18n change lable
function EventRebelNode:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")

		local image1 = UIHelper.seekNodeByName(self,"Reward","Image_15")
		UIHelper.swapWithLabel(image1,{ 
			 style = "text_challenge_1", 
			 text = Lang.getImgText("txt_jiangli") ,
		})
    ...
    if not Lang.checkLang(Lang.CN) then
        ...
        local label = UIHelper.createLabel({ 
            style = "explore_4", 
            text = Lang.getImgText("img_luoyangzhiluan01") ,
       })
       local size = self._rebelBG:getContentSize()
       label:setAnchorPoint(cc.p(0.5,0.5))
       label:setPosition(size.width*0.5,size.height-20)
       self._rebelBG:addChild(label)
...
-- i18n pos lable
function EventRebelNode:_dealPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
    ...
    if not Lang.checkLang(Lang.CN) then
        self._textRebelContent:setPositionY(
            self._textRebelContent:getPositionY()+36
        )
        local size = self._textRebelContent:getContentSize()
        self._textRebelContent:setContentSize(cc.size(size.width,size.height+36))
    end

    if  Lang.checkLang(Lang.VN) then
        self._textRebelContent:getVirtualRenderer():setLineSpacing(0)
    end

    if  Lang.checkLang(Lang.JA) then
        ccui.Helper:seekNodeByName(self._resourceNode, "TextDrop"):setPositionX(-161)
    end
