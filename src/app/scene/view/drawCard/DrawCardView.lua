...
local DrawTenEffect2 = require("app.scene.view.drawCard.DrawTenEffect2")-- i18n ja
local DrawOneEffect2 = require("app.scene.view.drawCard.DrawOneEffect2")-- i18n ja
local DrawNormalEffect2 = require("app.scene.view.drawCard.DrawNormalEffect2")-- i18n ja
...
function DrawCardView:ctor()
	...
	if Lang.checkUI("ui4") then
		resource.file = Path.getCSB("DrawCardView2", "drawCard")
		self:setName("DrawCardView")
		DrawCardView.super.ctor(self, resource,nil)
	else
		self:setName("DrawCardView")
...
function DrawCardView:onCreate()
	...
	local param = 
	{
		...
		imageAn = Path.getCommonImage("box_bg_an"), --i18n ui4
		imageLiang = Path.getCommonImage("box_bg_liang"),--i18n ui4
	}
...
function DrawCardView:onEnter()
	...
	if not Lang.checkLang(Lang.CN) then
		self._signalItemOpUpdate = G_SignalManager:add(SignalConst.EVENT_ITEM_OP_UPDATE, handler(self, self._onEventRecruitInfo))
		self._signalItemOpInsert = G_SignalManager:add(SignalConst.EVENT_ITEM_OP_INSERT, handler(self, self._onEventRecruitInfo))
...
function DrawCardView:onExit()
	...
	if not Lang.checkLang(Lang.CN) then
		self._signalItemOpUpdate:remove()
		self._signalItemOpUpdate = nil
		self._signalItemOpInsert:remove()
		self._signalItemOpInsert = nil
		self._signalItemOpDelete:remove()
...
function DrawCardView:_refreshBoxState()
	...
	local totalPoint = 0 --i18n ui4
	local boxInfoList  = {}--i18n ui4
	for i = 1, DrawCardView.BOX_NUM do
		...
		if Lang.checkUI("ui4") then
			totalPoint = totalPoint + box:getBoxPoint()
			table.insert(boxInfoList, {
				startPos = box:getPositionX()-34,
				endPos = box:getPositionX() + 41,
				point = box:getBoxPoint()
			})
		end
	end
	if Lang.checkUI("ui4") then
		local loadBarStartX = self._loadbar:getParent():getPositionX()
		local dstX = loadBarStartX
		for k = #boxInfoList,1,-1 do
			local v = boxInfoList[k]
			if v.point < myPoint then
				local nextBoxInfo = boxInfoList[k+1]
				if nextBoxInfo then
		...
		local width = self._loadbar:getContentSize().width
		local percent = math.min(100, (dstX-loadBarStartX) * 100/width)
		self._loadbar:setPercent(percent)
		logWarn(myPoint.."  DrawCardViewxxx "..totalPoint)
	end
end
...
function DrawCardView:_onInfoClick()
	...
	if Lang.checkUI("ui4") then
		local popupInfo2 = require("app.scene.view.drawCard.PopupInfo2").new(self._btnInfo)
		popupInfo2:open()
	else
