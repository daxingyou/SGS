...
function SummaryBase:_createContinueNode()
    ...
    if Lang.checkUI("ui4") then -- i18n ja change color
        ccui.Helper:seekNodeByName(continueNode, "Text_continue_desc"):setColor(Colors.BRIGHT_BG_ONE) 
    end
    ...
    if open then
        ...
        if not Lang.checkLang(Lang.CN) then
            local label = UIHelper.createLabel({
                style = "fight_1",
                text = Lang.getImgText("txt_tongji01"),
                position = cc.p(95, 20),
                anchorPoint = cc.p(0.5,0.5),
            })
    ...
    if open then
        ...
        if not Lang.checkLang(Lang.CN) then
            local label = UIHelper.createLabel({
                style = "fight_1",
                text = Lang.getImgText("txt_huifang02"),
                position = cc.p(40, 20),
                anchorPoint = cc.p(0.5,0.5),
            })
...
function SummaryBase:_playWinText(effect)
    ...
    -- if not Lang.checkLang(Lang.CN) then
    --     if effect == "mid" then
    --         return display.newSprite(Path.getBattleFont("txt_com_battle_v05"))
    --     else
    --         return nil
    --     end
    -- end
    ...
    -- if effect == "left" then
        if Lang.checkUI("ui4") then
            ...
            local sp = display.newSprite(SummaryBase.PicLeft[self._winType])
            node:addChild(sp)
            sp:setPositionX(-10)
            return node
        end
    ...
    elseif effect == "right" then
        if Lang.checkUI("ui4") then
            local node = cc.Node:create()
            local sp = display.newSprite(SummaryBase.PicRight[self._winType])
            node:addChild(sp)
            sp:setPositionX(-10)
            return node
        end
