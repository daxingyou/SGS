...
function PopupTowerChoose:ctor(layerConfig)
    ...
    local filePath = Path.getCSB("PopupTowerChoose", "tower")
    if Lang.checkUI("ui4") then
        filePath = Path.getCSB("PopupTowerChoose1", "tower")
    end
	local resource = {
		file = filePath,
		binding = {
...
-- i18n change lable
function PopupTowerChoose:_dealI18n()
    if Lang.checkUI("ui4") then
        return
    end
	if not Lang.checkLang(Lang.CN)  then
		local UIHelper  = require("yoka.utils.UIHelper")
        local textPass = UIHelper.seekNodeByName(self,"TextPass")
		textPass:setAnchorPoint(cc.p(0,0.5))
        textPass:setPositionX(220)
        local size = textPass:getContentSize()
        self._textCondition:setPositionX(220+size.width)
	end
end

    -- i18n change lable
function PopupTowerChoose:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")

        local image3 = UIHelper.seekNodeByName(self,"Image_1")
        if image3 then
