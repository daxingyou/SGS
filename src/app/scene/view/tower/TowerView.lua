...
function TowerView:_refreshCount()
    ...
    if not Lang.checkLang(Lang.CN)  then
        timeString = Lang.get("challenge_tower_remain_min", {count = min}).." "..Lang.get("challenge_tower_remain_second", {count = second})
    else
        timeString = Lang.get("challenge_tower_remain_min", {count = min})..Lang.get("challenge_tower_remain_second", {count = second})
    end
    if min == 0 then
    ...
    if maxCount == towerCount then
        ...
        if not Lang.checkLang(Lang.CN)  then
            self._textRecoverTime:setAnchorPoint(cc.p(0,0.5))
            self._textRecoverTime:setPositionX(
                self._textRecoverTitle:getContentSize().width+6
            )
    end
    end
...
-- i18n pos lable
function TowerView:_dealPosI18n()
	if not Lang.checkLang(Lang.CN)  then
        ...
        local text6 = UIHelper.seekNodeByName(self._imageInfoBG,"Text6")
        text6:setPositionX(self._textStar:getPositionX()-text6:getContentSize().width)

        local textChallengeCount = UIHelper.seekNodeByName(self._imageInfoBG,"TextChallengeCount")
        
        self._textChallengeCount:setPositionX(textChallengeCount:getPositionX() + textChallengeCount:getContentSize().width +2)
        ...
        -- 排行榜按钮下移
        self._btnRank:setPositionY(self._btnRank:getPositionY()-10)
        if Lang.checkUI("ui4") then
            self._btnRank:setPositionY(self._btnShop:getPositionY())
        end
...
-- i18n pos lable
function TowerView:_adjustPosI18n()
	if not Lang.checkLang(Lang.CN)  then
        -- i18n ja
        if Lang.checkLang(Lang.JA) then
