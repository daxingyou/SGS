...
function PopupInspireView:_updateBtn( ... )
    ...
    if not Lang.checkLang(Lang.CN) then
        self._btnInspire1:setVisible(self._selfData.atk_insp_level < 3)
        self._btnInspire2:setVisible(self._selfData.def_insp_level < 3)
    end
    if not Lang.checkLang(Lang.CN) then
        if Lang.checkLang(Lang.VN) then
            local UIHelper  = require("yoka.utils.UIHelper")
            UIHelper.setLabelStyle(self._btnInspire1,{
                style = "guild_cross_war_4",
                text = Lang.getImgText("txt_guild_cross_war_inspire01"),
            })     
            UIHelper.setLabelStyle(self._btnInspire2,{
                style = "guild_cross_war_4",
                text = Lang.getImgText("txt_guild_cross_war_inspire02"),
            })    
            return
        end
        local UIHelper  = require("yoka.utils.UIHelper")
        UIHelper.setLabelStyle(self._btnInspire1,{
            style = "guild_cross_war_3",
            text = Lang.getImgText("txt_guild_cross_war_inspire01"),
        })     
        UIHelper.setLabelStyle(self._btnInspire2,{
            style = "guild_cross_war_3",
...
-- i18n change lable
function PopupInspireView:_swapImageByI18n()
    if not Lang.checkLang(Lang.CN) then
        if Lang.checkLang(Lang.VN) then
            local UIHelper  = require("yoka.utils.UIHelper")
            self._btnInspire1 = UIHelper.swapWithLabel(self._btnInspire1,{
            style = "guild_cross_war_4",
            text = Lang.getImgText("txt_guild_cross_war_inspire01"),
            })     
            self._btnInspire2 = UIHelper.swapWithLabel(self._btnInspire2,{
            style = "guild_cross_war_4",
            text = Lang.getImgText("txt_guild_cross_war_inspire02"),
            })     
            self._btnInspire1:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
            self._btnInspire2:setTextHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
            self._btnInspire1:setTouchEnabled(true)
            self._btnInspire2:setTouchEnabled(true)
            return
        end
        local UIHelper  = require("yoka.utils.UIHelper")
        self._btnInspire1 = UIHelper.swapWithLabel(self._btnInspire1,{
