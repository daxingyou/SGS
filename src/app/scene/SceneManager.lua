...
function SceneManager:popToLastScene()
    ...
    if Lang.checkUI("ui4") then
        ...
        if AudioPatchHelper.checkPopupAudioPatch() then return end
        -- i18n ja 主界面切换看板娘
        if G_SceneManager:checkSwitchAvatar() then return end
    end
...
-- i18n ja 主界面切换看板娘
function SceneManager:checkSwitchAvatar()
    local mainView = ccui.Helper:seekNodeByName(display.getRunningScene(), "MainView")
    if mainView then
        local switchAvatarLayer =  mainView:getChildByName("SwitchAvatarLayer")
        if switchAvatarLayer then
            switchAvatarLayer:onBack()
            return true
        end
    end
    return false
