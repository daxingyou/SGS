...
--18岁+玩家警告
schema["yearsOldWarn"]              = {"boolean", true}        -- 已接入
--android 返回上一层
schema["goBack"]                    = {"boolean", true}        -- 已接入
-- 屏蔽字开关
schema["blackListOpen"]             = {"boolean", true}        -- 已接入
-- 开启语音
schema["voiceOpen"]                 = {"boolean", true}       -- 未接入
-- 跳过每章关卡文字
schema["skipChapterText"]           = {"boolean", false}       -- 未接入

-- Google推荐功能开关，战斗暂停、剧情对话
schema["googleFeatureOpen"]         = {"boolean", false}       -- 未接入
schema["openSdkRealName"]  = {"boolean", false}  --是否开启sdk实名 --i18n默认关闭
...
-- Appstore推荐功能开关
schema["appstoreFeatureOpen"]       = {"boolean", false}       -- 未接入

-- 商店评论开关
schema["storeCommentOpen"]          = {"boolean", true}        -- 未接入
-- 商店评论url
schema["storeCommentUrl"]           = {"string", ""}           -- 未接入
-- 客户中心、官方论坛url
schema["centerForumUrl"]            = {"string", ""}           -- 未接入
-- vip客服照片url
schema["vipPhotoUrl"]               = {"string", ""}           -- 未接入
-- 攻略区开关
schema["strategyOpen"]              = {"boolean", false}        -- 未接入

-- i18n 聊天翻译开关
schema["chatTranslate"]              = {"boolean", false}   -- 未接入

-- i18n ja 语音分包下载
schema["audioVersion"]				    = {"string", "0.0.0"}
schema["audioVersionUrl"]				= {"string", ""}
-- i18n ja 公告红点url
schema["popupRedUrl"]					= {"string", ""}             -- 已接入
...
--i18n 公告多语言
function ConfigManager:getPopupUrlI18n()
    local popupUrl = self:getPopupUrl()
    popupUrl = string.gsub(popupUrl,"#uid#",tostring(G_UserData:getBase():getId()))
    return Lang.getLangTxtFromChannel(popupUrl) 
end

--i18n 未开服提示多语言
function ConfigManager:getServerNotAllowedTipI18n()
    local tip = self:getServerNotAllowedTip()
    return Lang.getLangTxtFromChannel(tip) 
end

--i18n number类型接收保护
function ConfigManager:protectNumber(data)
    for field, schema in pairs(self.class.schema) do
        local typ, def, id = schema[1], schema[2] , schema[3]
        local val = nil
        if rawget(data, field) then
            val = data[field]
        end
        if val ~= nil then
            if typ == "number" then
