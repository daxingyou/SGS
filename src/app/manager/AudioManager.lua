...
function AudioManager:ctor()
	...
	self._vcEnabled = true	-- i18n ja 配音开启

	self._musicPath = ""
	self._musicId = cc.AUDIO_INVAILD_ID
	self._musicVolume = 1
	self._soundVolume = 1
	self._vcVolume = 1	-- i18n ja 配音音量
...
function AudioManager:playSound(path, pitch)
	...
	if Lang.checkUI("ui4") then
		if string.find(path,"/voice/") or string.find(path,"/vip_voice/") then
			if self._vcEnabled then
				local convertStr = self:getConvertPath(path)
				local soundId = AudioHelper.playSound(convertStr, false, self._vcVolume, pitch or 1)
				self._soundList[soundId] = {isRun = true,startPlayTime = G_ServerTime:getTime(),isVC = true}
...
-- i18n ja 获取配音音量
function AudioManager:getVcVolume()
	return self._vcVolume
end

-- i18n ja 设置配音开启关闭
function AudioManager:setVcEnabled(enable)
	if enable ~= self._vcEnabled then
		self._vcEnabled = enable
	end
end

-- i18n ja 获取配音是否开启
function AudioManager:isVcEnable()
	return self._vcEnabled
end
