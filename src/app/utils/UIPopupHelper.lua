...
function UIPopupHelper.getBoxItemList(boxId,itemId)
	--i18n config change
	if BoxConfig == nil then BoxConfig = require("app.config.box") end
...
function UIPopupHelper.popupResetTimesNoEnough(funcType,vipLevel)
    ...
    local function popFunc()
       ...
       -- local content = vipCfg.hint1.."\n"..vipCfg.hint2
		local richList = {}
		local fontSize = Lang.checkUI("ui4") and 18 or 22   -- i18n ja font size
		table.insert( richList,  json.encode(RichTextHelper.getRichMsgListForHashText(
					vipCfg.hint1,Colors.BRIGHT_BG_GREEN,Colors.COLOR_POPUP_DESC_NORMAL,fontSize)))
		table.insert( richList,  json.encode(RichTextHelper.getRichMsgListForHashText(
					vipCfg.hint2,Colors.BRIGHT_BG_GREEN,Colors.COLOR_POPUP_DESC_NORMAL,fontSize)))
...
function UIPopupHelper.popupCostYubiTip(params, callback, ...)
	local MODULE_INFO = {
		["COST_YUBI_MODULE_NAME_1"] = {itemName = {"蟠螭龙纹璧"}, todo = "招募"}, --见龙在田
		["COST_YUBI_MODULE_NAME_2"] = {itemName = {"红羽穿心箭"}, todo = "追击曹操"}, --割须弃袍
		["COST_YUBI_MODULE_NAME_3"] = {itemName = {"卧龙占星盘"}, todo = "观星"}, --卧龙观星
		["COST_YUBI_MODULE_NAME_4"] = {itemName = {"玄天八卦"}, todo = "转动转盘"}, --身外化身
		["COST_YUBI_MODULE_NAME_5"] = {itemName = {"金丝套马索"}, todo = "驯马"}, --关公驯马
		["COST_YUBI_MODULE_NAME_6"] = {itemName = {"灵宝葫芦","紫金葫芦"}, todo = "祈灵"}, --祈灵
	}
	--非中文版本特殊处理
	if not Lang.checkLang(Lang.CN) then
		MODULE_INFO = {
			["COST_YUBI_MODULE_NAME_1"] = {itemName = {Lang.getImgText("equipment_activity_name1")}, todo =	Lang.getImgText("equipment_activity_todo1")}, --见龙在田
			["COST_YUBI_MODULE_NAME_2"] = {itemName = {Lang.getImgText("equipment_activity_name2")}, todo =	Lang.getImgText("equipment_activity_todo2")}, --割须弃袍
			["COST_YUBI_MODULE_NAME_3"] = {itemName = {Lang.getImgText("equipment_activity_name3")}, todo =	Lang.getImgText("equipment_activity_todo3")}, --卧龙观星
			["COST_YUBI_MODULE_NAME_4"] = {itemName = {Lang.getImgText("equipment_activity_name4")}, todo = Lang.getImgText("equipment_activity_todo4")}, --身外化身
			["COST_YUBI_MODULE_NAME_5"] = {itemName = {Lang.getImgText("equipment_activity_name5")}, todo =	Lang.getImgText("equipment_activity_todo5")}, --关公驯马
			["COST_YUBI_MODULE_NAME_6"] = {itemName = {Lang.getImgText("equipment_activity_name6"),Lang.getImgText("equipment_activity_name7")}, todo = Lang.getImgText("equipment_activity_todo6")}, --祈灵
		}
	end
	...
	local content = Lang.get("common_cost_yubi_tip", {yubiCount = yubiCount, 
														itemCount = itemCount, 
...
-- i18n ja add
function UIPopupHelper.popupVersionUpdate(tip,okCallback,okStr,cancelStr)	
	local PopupUpdateAlert = require("app.scene.view.login.PopupUpdateAlert")
	local popup = PopupUpdateAlert.new(okCallback,function()
		G_NativeAgent:exit()
	end)
	popup:setBtnStr(okStr,cancelStr)
	popup:setTip(tip)
...
-- i18n ja show backgroung and skin awards
function UIPopupHelper.popupAwardsBackgroundSkin(callback,skinAwards,bShowScene)
	local skinIdx = 1
	local newSceneList = {}
	if bShowScene then
		newSceneList = G_UserData:getMails():getNewMainSceneList()
	end
	local sceneIdx = 1
	local function showAwards()
		local skinAward = skinAwards[skinIdx]
		local sceneId = newSceneList[sceneIdx]
		if skinAward then
			skinIdx = skinIdx + 1
			local PopupGetVipSkinShow = require("app.scene.view.vip.PopupGetVipSkinShow")
			local popupGetVipSkinShow = PopupGetVipSkinShow.new(skinAward.value,showAwards)
			popupGetVipSkinShow:open()
		elseif sceneId then
			sceneIdx = sceneIdx + 1
			local PopupGetSceneShow = require("app.scene.view.main.PopupGetSceneShow")
			local popupGetSceneShow = PopupGetSceneShow.new(sceneId,showAwards)
