...
-- i18n ja change
function HeroDataHelper.findNextAwakeLevel2(level, awakenCost, maxLevel)
	local nextLevel = nil
	local attrInfo = {}
	local des = ""
	local des2 = ""
	...
	for i = level + 1, maxLevel do
		...
		if isHave then
			nextLevel = i
			des = info["talent_description"]   
			des2 = info["detail_description"]  -- 修改返回描述
			for j = 1, 4 do
				local attrType = info["talent_attribute_type" .. j]
				if attrType > 0 then
					local attrValue = info["talent_attribute_value" .. j]
	...
	return nextLevel, attrInfo, des, des2
end
...
-- i18n ja get spine action  获取对应立绘动作    
function HeroDataHelper.getHeroAction(show_action, bClickAvatar)   
	show_action = show_action .. "," .. show_action .. "|" .. show_action    --先写死
 
	local actionList1 = string.split(show_action, ",")
	local actionList2 = {}
	if #actionList1 > 1 then  
	   actionList2 = string.split(actionList1[2], "|")
	end
	table.insert(actionList2, 1, actionList1[1])

	if bClickAvatar == nil or #actionList2 == 1 then
		return actionList2[1]     -- 默认idle/秀将动作
	else 
		local index = math.random(2, #actionList2)
		return actionList2[index] -- 点击武将动作
	end
end

-- i18n ja 判断是否提示界限突破可能，全部条件达成才提示
function HeroDataHelper.isPromptHeroLimitEx(heroUnitData)
	local isAllFull = true
	for key = HeroConst.HERO_LIMIT_COST_KEY_1, HeroConst.HERO_LIMIT_COST_KEY_6 do
		local isOk, isFull = HeroDataHelper.isPromptHeroLimitWithCostKey(heroUnitData, key)
		isAllFull = isAllFull and (isFull or isOk)
	end
	if isAllFull then
		local limitDataType = HeroDataHelper.getLimitDataType(heroUnitData)
		local lv
