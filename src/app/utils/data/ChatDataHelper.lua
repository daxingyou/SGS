...
-- local Chat = require("app.config.chat")--in8
...
--是否已经对玩家静默 --i18nja
function  ChatDataHelper.isSlienceToPlayer(userId)
	local curTime = G_ServerTime:getTime()
	local silenceUserDataList = G_UserData:getChat():getSilenceUserDataList()
	for k,v in ipairs(silenceUserDataList) do
		if v:getUser_id() ==  userId and v:getEnd_time() > curTime then
			return true
		end
	end
	return false
...
--是否已经对玩家举报 --i18nja
function ChatDataHelper.isHasTipOffChatMsg(chatMsg)
	return chatMsg:getIs_report() == 1
end
