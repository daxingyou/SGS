...
function WayFuncConvert._FUNC_RECHARGE()
	if Lang.checkUI("ui4") then
		...
		local returnFunc = function(isRecharge,subTabType)
...
function WayFuncConvert._FUNC_JADE2()
	if Lang.checkUI("ui4") then
		local returnFunc = function(isRecharge,subTabType,threeTabType)
			local VipConst = require("app.const.VipConst")
			isRecharge = isRecharge == nil and true or isRecharge 
			subTabType = subTabType == nil and VipConst.TAB_TYPE.JADE_BUY or subTabType
			if subTabType == VipConst.TAB_TYPE.DISCOUNT_GIFT  then
				threeTabType = threeTabType == nil and VipConst.SUB_TAB_TYPE.EXCHANGE or threeTabType
			end
			
			local sceneName = G_SceneManager:getRunningSceneName()
			if sceneName == "vip" then
				--local vipViewNew = G_SceneManager:getRunningScene():getSceneView()
				--vipViewNew:gotoView(isRecharge and 1 or 2,subTabType,threeTabType)
				G_SignalManager:dispatch(SignalConst.EVENT_VIP_GO_TO_TAB,isRecharge and 1 or 2,subTabType,threeTabType)
			else
				G_SceneManager:showScene("vip",isRecharge and 1 or 2,subTabType,threeTabType)
				--G_SceneManager:showScene("vip")
				--G_SignalManager:dispatch(SignalConst.EVENT_VIP_GO_TO_TAB,isRecharge and 1 or 2,subTabType,threeTabType)
			end
		end
...
function WayFuncConvert._FUNC_ACHIEVEMENT()
	local returnFunc = function(...)
		if Lang.checkUI("ui4") then
			G_SceneManager:showScene("achievement",3)
		else
...
function WayFuncConvert._FUNC_ACTIVITY()
	local returnFunc = function(...)
		if Lang.checkUI("ui4") then
			local popupView = G_SceneManager:getRunningScene():getPopupByName("CustomActivityView")
			if popupView then
				popupView:close()
			end
		end
...
function WayFuncConvert._FUNC_CRYSTAL_SHOP_TAB_RECHARGE()
	local returnFunc = function()
		if Lang.checkUI("ui4") then
			G_SceneManager:showScene("shop", ShopConst.CRYSTAL_SHOP)
			return			
		end
...
-- i18n ja
-- 游戏公告
function WayFuncConvert:_FUNC_NOTICE()
	local returnFunc = function()
		print("FUNC_NOTICE")
...
-- 谷歌问卷
function WayFuncConvert:_FUNC_QUESTIONNAIRE()
	local returnFunc = function()
		print("FUNC_QUESTIONNAIRE")
...
-- lobi
function WayFuncConvert:_FUNC_LOBI()
	local returnFunc = function()
		print("FUNC_LOBI")
		if G_NativeAgent:hasForum() then
			G_NativeAgent:openForum()
...
-- i18n ja 跳转任务军团活跃度页签
function WayFuncConvert._FUNC_GUILD_ACTIVITY()
	local returnFunc = function(...)
		G_SceneManager:showScene("achievement",2)
	end
	return returnFunc
...
-- i18n ja 跳转充值2-2页签
function WayFuncConvert._FUNC_RECHARGE_SHOP()
	local returnFunc = function()
		local VipConst = require("app.const.VipConst")
		local sceneName = G_SceneManager:getRunningSceneName()
		if sceneName == "vip" then
			G_SignalManager:dispatch(SignalConst.EVENT_VIP_GO_TO_TAB,1,VipConst.TAB_TYPE.DISCOUNT_GIFT,VipConst.SUB_TAB_TYPE.LIMIT_SHOP)
		else
