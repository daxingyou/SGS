...
TypeConvertHelper.TYPE_MAIN_SCENE = 50 -- 主场景
TypeConvertHelper.TYPE_POSTER_GIRL_SKIN = 80 -- 皮肤  i18n ja
...
TypeConvertHelper.CLASS_NAME[TypeConvertHelper.TYPE_MAIN_SCENE] = "CommonSceneIcon"
TypeConvertHelper.CLASS_NAME[TypeConvertHelper.TYPE_POSTER_GIRL_SKIN] = "CommonPosterGirlSkinIcon" -- i18n ja
...
function TypeConvertHelper._convert_TYPE_HERO(params, info)
    ...
    if country2ImageRes[info.country] == nil then
        ...
        --i18n ui4
        retTemp.country_text_2_normal = Path.getMain2(countryTexture[info.country].normal)
        retTemp.country_text_2_highlight = Path.getMain2(countryTexture[info.country].highlight)
    end
...
--返回主场景类型
function TypeConvertHelper._convert_TYPE_MAIN_SCENE(params, info)
    local retTemp = {}
    retTemp.item_type = params.type
    retTemp.item_control = TypeConvertHelper.getTypeClass(params.type)
	local BattleScene = require("app.config.battle_scene")
	local sceneData = BattleScene.get(info.scene_day)
    retTemp.icon = Path.getMainSceneIcon(sceneData.icon)
    retTemp.name = info.name
    retTemp.description = info.description
    retTemp.icon_color = Colors.getColor(info.color)
    retTemp.icon_color_outline = Colors.getColorOutline(info.color)
