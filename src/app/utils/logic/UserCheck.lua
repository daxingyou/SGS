...
function UserCheck.enoughMoney(needMoney,popupDlg)
    ...
    local function popFunc()
        if not Lang.checkLang(Lang.CN) then
            G_Prompt:showTip( Lang.getImgText("prompt_silver_not_enough") )
        else
...
function UserCheck.enoughSoul(needSoul,popupDlg)
    ...
    local function popFunc()
        if Lang.checkLang(Lang.CN) then
            ...
            G_Prompt:showTip(Lang.getImgText("hero_soul_not_enough"))
        end
    ...
    local function popFunc()
        local function callBackOk()
            if Lang.checkUI("ui4") then
                ...
                local WayFuncDataHelper = require("app.utils.data.WayFuncDataHelper")
                WayFuncDataHelper.gotoModuleByFuncId(FunctionConst.FUNC_RECHARGE,{true,VipConst.TAB_TYPE.DISCOUNT_GIFT ,VipConst.SUB_TAB_TYPE.EXCHANG})
            else
                local WayFuncDataHelper = require("app.utils.data.WayFuncDataHelper")
                WayFuncDataHelper.gotoModuleByFuncId(FunctionConst.FUNC_RECHARGE)
            end
...
function UserCheck.isHeroFull()
    ...
    -- i18n ja 防止武将背包满导致开合击将宝箱失败
    if Lang.checkUI("ui4") then
        if isFull and (G_TutorialManager:isDoingStep(17) or G_TutorialManager:isDoingStep(33)) then
            return false,1
        end
    end
    ...
    local function checkBattleInfo()
        ...
        local function prompt()
            if not Lang.checkLang(Lang.CN) then
                ...
                G_Prompt:showTip(Lang.getImgText("power_not_enough") )
            else
...
function UserCheck.vipFuncIsOpened(funcType, callback)
    ...
    if not Lang.checkLang(Lang.CN) then
        errMsg = vipData and vipData.hint1 or Lang.getImgText("vip_level_not_enough")
    end
