...
function HeroData:getReplaceDataBySort(filterId)
	...
	local sortFun = function(a, b)
		if comboSort then
			local comboA = comboList[a:getId()] or 999
			local comboB = comboList[b:getId()] or 999
	...
	for k, unit in pairs(self._heroList) do
		...
		if not isInBattle and not isInReinforcements and not same and heroType ~= 3 then
			...
			if comboSort then
				if awards[unit:getBase_id()] and
				unit:getLevel() == 1 and
				unit:getExp() == 0 and
				unit:getRank_lv() == 0 then
					comboList[unit:getId()] = awards[unit:getBase_id()]
					awards[unit:getBase_id()] = nil
				end
			end
		end
	end
...
-- i18n ja 获取筛选掉相同story_res_spine后的武将列表信息
function HeroData:getHeroListByFiltSameRes()
	local sortFun = function(a, b)
		if a:getConfig().type ~= b:getConfig().type then
			if a:getConfig().type == 1 then
				return true
			end
			if b:getConfig().type == 1 then
				return false
			end
		end
		if a:getConfig().color ~= b:getConfig().color then
	...
	local temp = {}
	for k, unit in pairs(self._heroList) do
		local tempId = unit:getStoryResSpine()
		if temp[tempId] ~= true then
			table.insert(result, unit)
		end
		if temp[tempId] == nil then
			temp[tempId] = true
		end
	end
	table.sort(result, sortFun)
