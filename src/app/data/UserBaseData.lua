...
local KeyValueUrlRequest = require("app.manager.KeyValueUrlRequest") -- i18n ja 利用规约
...
schema["store_comment_flag"]		= {"number", 0}	--是否弹出了商店评论 0 未弹出 其他 已弹出
schema["today_init_vip"]			= {"number", 0}	--i18n ja 今天上线时vip等级
schema["kan_ban_niang"]				= {"number", 0}	-- i18n ja 看板娘id
schema["last_week_share_time"]		= {"number", 0}	-- i18n 周分享时间
...
function UserBaseData:ctor(properties)
	...
	self._bindCode = "" --绑定码
	self._showCommentFlag = false --i18n comment 本地商店评论标志
	...
	--i18n 每日礼包
	self._s2cDailyGiftInfoListener = G_NetworkManager:add(MessageIDConst.ID_S2C_GetDailyGiftInfo, handler(self, self._s2cDailyGiftInfo))
	self._signalZeroTime = G_SignalManager:add(SignalConst.EVENT_ZERO_CLOCK, handler(self, self._onEventZeroTime))
	-- i18n ja 利用规约
	self._selectGetLaw = self._keyValueUrlRequest.signal:registerListener(handler(self, self._onEventShowUsingConvente))    
	self._s2cGetEcpOverAwardListener = G_NetworkManager:add(MessageIDConst.ID_S2C_GetExpOverAward, handler(self,self._s2cGetExpOverAward)) -- i18n ja 经验溢出
...
function UserBaseData:clear()
	...
	--i18n 每日礼包
	self._s2cDailyGiftInfoListener:remove()
	self._s2cDailyGiftInfoListener = nil
	self._signalZeroTime:remove()
	self._signalZeroTime = nil
	-- i18n ja  利用规约
	self._selectGetLaw:remove()
	self._selectGetLaw = nil
	-- i18n ja  经验溢出
	self._s2cGetEcpOverAwardListener:remove()
	self._s2cGetEcpOverAwardListener = nil
	-- i18n ja 周分享
...
--i18n ui4 新增参数 thisOnlineTime
function UserBaseData:setOnlineTime(onlineTime,thisOnlineTime)
	self:setToday_online_time(onlineTime)
	if Lang.checkUI("ui4") then
		self:setOnline_time_update_time(thisOnlineTime)
	else
...
function UserBaseData:getOnlineTime()
	if Lang.checkUI("ui4") then
        ...
        local zeroTime = TimeConst.RESET_TIME_24 * 60 *60
		local time = self:getToday_online_time()
		local currTime = G_ServerTime:getTime()
		local updateTime = self:getOnline_time_update_time()
		local zero1 = G_ServerTime:getTimeOfZero(currTime,zeroTime)
		local zero2 = G_ServerTime:getTimeOfZero(updateTime,zeroTime)
		if zero1 ~= zero2 then
			return currTime - zero1
		else
			local elapsed =  currTime -  updateTime
			return time + elapsed
		end
	else
		local time = self:getToday_online_time()
...
function UserBaseData:_s2cGetTotalOnlineTime(id, message)
	...
	if Lang.checkUI("ui4") then
		self:setToday_online_time(message.total_online_time)
		self:setOnline_time_update_time(message.this_online_time)
	else
...
-- i18n comment
function UserBaseData:getShowCommentFlag( )
	return self._showCommentFlag
end
-- i18n comment
function UserBaseData:setShowCommentFlag(flag)
	self._showCommentFlag = flag
end

--i18n 每日礼包
function UserBaseData:_s2cDailyGiftInfo(id, message)
	self._dailyGiftInfo = rawget(message, "acquired_gifts") or {}
	G_SignalManager:dispatch(SignalConst.EVENT_MAIN_CITY_CHECK_BTNS)
end
function UserBaseData:getDailyGiftInfo( )
	return self._dailyGiftInfo
end
function UserBaseData:_onEventZeroTime()
	self._dailyGiftInfo = {}
	G_SignalManager:dispatch(SignalConst.EVENT_MAIN_CITY_CHECK_BTNS)
end
...
-- i18n ja 设置看板娘
function UserBaseData:c2sKanbanNiang(id)
	G_NetworkManager:send(MessageIDConst.ID_C2S_KanbanNiang,{
		kanid = id
	})
end

-- i18n ja 利用规约
function UserBaseData:sendUsingConventeRequest( )
	self._keyValueUrlRequest:doRequestGetKeyValue("law", true) 
end

-- i18n ja 利用规约
function UserBaseData:_onEventShowUsingConvente(e, data)
	if e == "fail" then
		return 
	end

	if data and data.field and data.field == "" then
		local code = require("app.scene.view.login.PopupUsingConventions").new(true)
		code:openWithAction()
		return
	end	
...
-- i18n ja 经验溢出
function UserBaseData:_s2cGetExpOverAward(id, message)
	if message.ret ~= 1 then
		return
	end
	G_SignalManager:dispatch(SignalConst.EVENT_GET_EXP_OVER_AWARD, message)
end

-- i18n ja 是否绑定过
function UserBaseData:getIsBinded()
    return self._isBinded
end

-- i18n ja 设置是否绑定过
function UserBaseData:setIsBinded(isBinded)
    self._isBinded = isBinded
end


-- i18n ja 周分享
function UserBaseData:c2sWeekShare(id)
	G_NetworkManager:send(MessageIDConst.ID_C2S_WeekShare,{
	})
end


-- i18n ja 周分享
function UserBaseData:_s2cWeekShare(id, message)
	if message.ret ~= 1 then
		return
	end
	G_SignalManager:dispatch(SignalConst.EVENT_WEEK_SHARE_REWARD, message.awards)
end

function UserBaseData:hasDoWeekShare()
	local function getWeekStartTime(time)
		local todayPassedSeconds = G_ServerTime:getTodaySeconds(time)
		local weekDay = G_ServerTime:getDateObject(time).wday
		weekDay = weekDay -1
		if weekDay == 0 then
			weekDay = 7
		end
		local weekPassedSeconds = todayPassedSeconds + (weekDay-1) * 24 * 3600

		local TimeConst  = require("app.const.TimeConst")
		local zeroTime = TimeConst.RESET_TIME_24 * 60 *60

		return  time - weekPassedSeconds + zeroTime
	end


	local lastWeekShareTime = self:getLast_week_share_time()
	if lastWeekShareTime == 0 then
		return false
	end
	local currTime = G_ServerTime:getTime()
	local weekStartTime1 = getWeekStartTime(lastWeekShareTime)
	local weekStartTime2 = getWeekStartTime(currTime)
	if weekStartTime1 ~= weekStartTime2 then
		return false
	else
		return true
	end
