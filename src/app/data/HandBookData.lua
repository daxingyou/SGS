...
function HandBookData:_s2cGetResPhoto(id, message)
	...
	--i18n
	if self._firstSetFlag == true then
		self._firstSetFlag = false
		self._loginHandbookList = clone(self._handbookList)
	end
...
--i18n ui4
function HandBookData:isNewPet(baseId,key)
	return self:isNew(HandBookData.PET_TYPE ,baseId,key)
end

--i18n ui4
function HandBookData:isNewHero(baseId,key)
	return self:isNew(HandBookData.HERO_TYPE ,baseId,key)
end

--i18n ui4
function HandBookData:isNew(type,baseId,key)
	local typeMap = self._loginHandbookList["k"..type] or {}
	local data = typeMap["k"..baseId]
	local new = data == nil
	if not new then
		return false
	end
	if not self._hasList[key] then
		self._hasList[key] = {}
		self._hasList[key][type]= {}
		self._hasList[key][type]['k_'..baseId] = baseId
	elseif not self._hasList[key][type] then
		self._hasList[key][type] =  {}
		self._hasList[key][type]['k_'..baseId] = baseId
	elseif self._hasList[key][type]['k_'..baseId] == nil then
		self._hasList[key][type]['k_'..baseId] = baseId
	else
		new = false
	end
