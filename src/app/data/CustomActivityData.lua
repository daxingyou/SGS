...
function CustomActivityData:_s2cGetCustomActivityInfo(id,message)
    ...
    --dump(message)
     ...
     if Lang.checkUI("ui4") then
        self:dispatchFourActivityRedPoint()
     end
end
...
function CustomActivityData:getShowActUnitDataArr()
    ...
    for k,v in pairs( self._actUnitDataList) do
        ...
        elseif not isReturnServer and CustomActivityConst.CUSTOM_ACTIVITY_TYPE_RETURN_SERVER_GIFT == v:getAct_type() then
            ...
            -- i18n ja 军团活跃从活动移到任务
            if Lang.checkUI("ui4") then
                if v:getAct_id() ~= CustomActivityConst.CUSTOM_ACTIVITY_GUILD_ID then
                    if v:checkActIsVisible() then
                        table.insert( dataArr, v )
                    end
                end
...
-- i18n ja 是否四大活动
function CustomActivityData:_isFourActivity(data)
    if data:getAct_type() == CustomActivityConst.CUSTOM_ACTIVITY_TYPE_AVATAR
        or data:getAct_type() == CustomActivityConst.CUSTOM_ACTIVITY_TYPE_EQUIP
        or data:getAct_type() == CustomActivityConst.CUSTOM_ACTIVITY_TYPE_PET
        or data:getAct_type() == CustomActivityConst.CUSTOM_ACTIVITY_TYPE_HORSE_CONQUER
        or self:_isBaguaActivity(data:getAct_id()) then
        return true
    end
    return false
end
-- i18n ja 是否登录送八卦活动
function CustomActivityData:_isBaguaActivity(actId)
    local id = math.floor(actId/100000)
    if id == 21 then
        return true
    end
    return false
end
-- i18n ja 变身卡红点 包含登录送八卦活动
function CustomActivityData:isAvatarHasRedPoint()
    for k,v in pairs(self._actUnitDataList) do
        if self:_isBaguaActivity(v:getAct_id()) then
            if v:checkActIsVisible() then
                local red = self:hasRedPointByActId(v:getAct_id())
                if red then
                    return true
    ...
    return self:_avatarActivityHasRedPoint()
end
-- i18n ja 刷新四大活动红点
function CustomActivityData:dispatchFourActivityRedPoint()
    G_SignalManager:dispatch(SignalConst.EVENT_RED_POINT_UPDATE,FunctionConst.FUNC_EQUIP_ACTIVITY)
    G_SignalManager:dispatch(SignalConst.EVENT_RED_POINT_UPDATE,FunctionConst.FUNC_PET_ACTIVITY)
    G_SignalManager:dispatch(SignalConst.EVENT_RED_POINT_UPDATE,FunctionConst.FUNC_HORSE_CONQUER_ACTIVITY)
    G_SignalManager:dispatch(SignalConst.EVENT_RED_POINT_UPDATE,FunctionConst.FUNC_AVATAR_ACTIVITY)
end
