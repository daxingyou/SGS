...
RedPointHelper._FUNC_DAILY_MISSION = {
	...
	mainRP = function(params)
		...
		if Lang.checkUI("ui4") then
			...
			local redValueGuild = G_UserData:getCustomActivity():_hasActCanReceived(CustomActivityConst.CUSTOM_ACTIVITY_GUILD_ID)
			return redValue1 or redValue2 or redValue3 or redValueGuild
		end
		return redValue1 or redValue2 or redValue3
	end,
...
RedPointHelper._FUNC_ACHIEVEMENT = {
	...
	guildRP = function()
		local CustomActivityConst = require("app.const.CustomActivityConst")
		local redValue = G_UserData:getCustomActivity():_hasActCanReceived(CustomActivityConst.CUSTOM_ACTIVITY_GUILD_ID)
		return redValue
	end
}
...
RedPointHelper._FUNC_RECHARGE = {
	mainRP = function()
		if Lang.checkUI("ui4") then
			--logWarn("_FUNC_RECHARGE sssssssssssssss")
			local redPointShow = RedPointHelper.isModuleReach(FunctionConst.FUNC_VIP_GIFT)
			if not redPointShow then
			...
			if not redPointShow then
				...
				redPointShow = G_UserData:getActivity():hasRedPointByPosition(ActivityConst.SHOW_POSITION_VIP)
			end
				
			if not redPointShow then
				redPointShow = G_UserData:getPosterGirl():hasRedPoint()
			end
	
			return redPointShow
		else
			local redPointShow = G_UserData:getVip():hasVipGiftCanBuy()
			return redPointShow
		end
	end
}
...
RedPointHelper._FUNC_TEAM = {
	mainRP = function()
		...
		if Lang.checkUI("ui4") and redValue == false then		-- i18n ja change redPoint
			local redValue4 = require( "app.i18n.utils.TeamVeiwMainRP" ).getRedPoint()
			redValue = redValue4
		end
		return redValue
	end,
...
-- i18n ja 割须弃袍
RedPointHelper._FUNC_EQUIP_ACTIVITY = {
	mainRP = function()
		return G_UserData:getCustomActivity():_equipActivityHasRedPoint()
	end,
}

-- i18n ja 变身卡
RedPointHelper._FUNC_AVATAR_ACTIVITY = {
	mainRP = function()
		return G_UserData:getCustomActivity():isAvatarHasRedPoint()
	end,
}

-- i18n ja 卧龙观星
RedPointHelper._FUNC_PET_ACTIVITY = {
	mainRP = function()
		return G_UserData:getCustomActivity():_petActivityHasRedPoint()
	end,
}
