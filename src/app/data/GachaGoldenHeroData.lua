...
schema["gachaDrawHeroViewData"]	= {"table"}	-- i18n ja bug: 见龙在田 无法连续抽奖，抽奖后页面上抽奖按钮消失，需要退出界面重新选择
...
function GachaGoldenHeroData:_initGroupHeroId()
    ...
    for i = 1, paramConfig.length() do
        local data = paramConfig.indexOf(i)
        if Lang.checkLang(Lang.CN) or (not Lang.checkLang(Lang.CN) and data.is_open == 1) then
