...
function ActivityData:getActivityDataById(activityId)
	...
	--i18n late regist
	elseif activityId == ActivityConst.ACT_ID_LATE_REGIST then --i18n后加入玩家补偿
		return G_UserData:getActivityLateRegist()
	elseif  activityId == ActivityConst.ACT_ID_FRIEND_INVITE then --i18n ja 好友邀请
		return  G_UserData:getActivityFriendInvite() 	
...
--i18n ja 新增参数 position
--返回开启活动的配置列表
function ActivityData:_createOpenActCfgListFromConfig(position)
	if Lang.checkUI("ui4") then
		position = position or ActivityConst.SHOW_POSITION_WELFARE
	end
	...
	--i18n ja 显示位置判断 
	local showPositionCheckFunc = function(cfg)
	...
	for i = 1,length,1 do
		...
		if actOpenCheckFunc(cfg) and showPositionCheckFunc(cfg) then ----i18n ja
			...
			--i18n late regist
			elseif ActivityConst.ACT_ID_LATE_REGIST == cfg.id then
				local isOpen = G_UserData:getActivityLateRegist():isOpen()
				if isOpen then
					table.insert(cfgList,cfg)
				end
			-- i18n ja开服时间超过param配置时间,不显示
			elseif ActivityConst.ACT_ID_FRIEND_INVITE == cfg.id then  
				local isOpen = G_UserData:getActivityFriendInvite():isOpen() 
				if isOpen then
					table.insert(cfgList,cfg)
				end
			else
...
--i18n ja
function ActivityData:getActivityListByShowPosition(position)
	return self:_createOpenActCfgListFromConfig(position)
end
...
function ActivityData:hasRedPointForSubAct(actId)
	...
	--i18n late regist
	if actId > ActivityConst.ACT_ID_OPEN_SERVER_FUND and actId ~= ActivityConst.ACT_ID_LATE_REGIST then	 --基金1,2,3,4,5,6
...
--i18n ja
function ActivityData:hasRedPointByPosition(position)
	local actCfg = self:getActivityListByShowPosition(position)
	for k,v in ipairs(actCfg) do
		local actData = self:getActivityDataById(v.id)
		local red =  actData:hasRedPoint()
		if red then
