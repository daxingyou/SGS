...
local __key_type = {
  level = "int",    --角色等级-1 
  exp = "int",    --升级经验-2 
  lvup_power = "int",    --升级恢复体力-3 
  lvup_energy = "int",    --升级恢复精力-4 
  hero_limit = "int",    --武将上限-5 
  equipment_limit = "int",    --装备上限-6 
  treasure_limit = "int",    --宝物上限-7 
  instrument_limit = "int",    --神兵上限-8 
  pet_limit = "int",    --神兽等级上限-9 
  horse_limit = "int",    --战马上限-10 
  historical_hero_limit = "int",    --历代名将上限-11 
  arena_card = "int",    --竞技场翻盘-12 
  silver_para = "int",    --摇钱树参数-13 
  recommend_hero_lv = "int",    --推荐武将等级-14 
  recommend_equipment_lv = "int",    --推荐装备强化等级-15 
  recommend_equipment_refine_lv = "int",    --推荐装备精炼等级-16 
  recommend_treasure_lv = "int",    --推荐宝物强化等级-17 
  recommend_treasure_refine_lv = "int",    --推荐宝物精炼等级-18 
  recommend_instrument_refine_lv = "int",    --推荐神兵进阶等级-19 
  recommend_pet_lv = "int",    --推荐神兽等级-20 
...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in role")
...
--
function role.isVersionValid(v)
    if role.version then
        if v then
            return role.version <= v
        else
            return false
        end
    else
...
function role.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/role.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/role.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/role.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "role" )
                _isDataExist = role.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "role" )
                _isBaseExist = role.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "role" )
                _isExist = role.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = role._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "role" )
...
--
function role.set(level, key, value)
