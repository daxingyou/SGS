...
local __key_type = {
  ...
  type = "int",    --类型-3 
  cost = "int",    --消耗体力-4 
  monster_team = "int",    --怪物组-5 
  kill_type = "int",    --击杀奖励类型-6 
  kill_resource = "int",    --击杀奖励ID-7 
  kill_size = "int",    --击杀奖励数量-8 
  kill2_type = "int",    --击杀奖励类型2-9 
  kill2_resource = "int",    --击杀奖励ID2-10 
  kill2_size = "int",    --击杀奖励数量2-11 
  name = "string",    --反贼名称-12 
  color = "int",    --反贼品质-13 
  in_res = "int",    --战斗背景-14 
...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in explore_rebel")
...
--
function explore_rebel.isVersionValid(v)
    if explore_rebel.version then
        if v then
            return explore_rebel.version <= v
        else
            return false
        end
    else
...
function explore_rebel.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/explore_rebel.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/explore_rebel.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/explore_rebel.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "explore_rebel" )
                _isDataExist = explore_rebel.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "explore_rebel" )
                _isBaseExist = explore_rebel.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "explore_rebel" )
                _isExist = explore_rebel.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = explore_rebel._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "explore_rebel" )
