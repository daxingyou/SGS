...
local mt = { 
    __index = function(t, k) 
        ...
        assert(key_map[k], "cannot find " .. k .. " in guild_stage_rank_reward")
...
--
function guild_stage_rank_reward.isVersionValid(v)
    if guild_stage_rank_reward.version then
        if v then
            return guild_stage_rank_reward.version <= v
        else
            return false
        end
    else
...
function guild_stage_rank_reward.indexOf(index)
    ...
    if Lang.lang ~= _lang then 
        ...
        if Lang.lang ~= "cn" then
            _isDataExist = Lang.isFileExist("app/i18n/".. _lang .."/data/guild_stage_rank_reward.lua")
            _isExist = Lang.isFileExist("app/i18n/".. _lang .."/config/guild_stage_rank_reward.lua")
            _isBaseExist =  Lang.isFileExist("app/i18n/".. _lang .."/base/guild_stage_rank_reward.lua")

            if _isDataExist then
                local table = require( "app.i18n.".. _lang ..".data." .. "guild_stage_rank_reward" )
                _isDataExist = guild_stage_rank_reward.isVersionValid(table.version)
            end
            if _isBaseExist then
                local table = require( "app.i18n.".. _lang ..".base." .. "guild_stage_rank_reward" )
                _isBaseExist = guild_stage_rank_reward.isVersionValid(table.version)
            end
            if _isExist then
                local table = require( "app.i18n.".. _lang ..".config." .. "guild_stage_rank_reward" )
                _isExist = guild_stage_rank_reward.isVersionValid(table.version)
            end
        end
    end
    ...
    config._raw = guild_stage_rank_reward._data[index]
    if _lang ~= "cn" and _isBaseExist then
        local table_base = require( "app.i18n.".. _lang ..".base." .. "guild_stage_rank_reward" )
