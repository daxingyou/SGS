...
-- i18n ja change text 
function CommonTopbarBase:onLangHandle()
	local UIHelper = require("yoka.utils.UIHelper")
	local TypeConst = require("app.i18n.utils.TypeConst")

	self._imageTitle = UIHelper.swap(self._imageTitle,{text="", offsetX=0, offsetY= -10, style="big_tab",styleType=TypeConst.TEXT,anchorPoint=cc.p(0,1),h_align=cc.TEXT_ALIGNMENT_LEFT})
    self._imageTitle:setVisible(false)
    if Lang.checkLang(Lang.JA) then
        self._imageTitle:setFontSize(32)
        self._imageTitle:setPosition(141, -7)
    end
end	
...
function CommonTopbarBase:setImageTitle(imgName)
	...
	if not Lang.checkLang(Lang.CN) then 
		self._imageTitle:setString(Lang.getImgText(imgName) or "")
	else
...
-- i18n pos lable
function CommonTopbarBase:_dealPosI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		self._textTitle:setPositionY(self._textTitle:getPositionY()-6)
		self._textTitle:setPositionX(self._textTitle:getPositionX()-14)
	end
end

-- i18n
function CommonTopbarBase:getTitleWidth()
    return self._imageTitle:getContentSize().width
end
