...
-- i18n change lable
function CommonGroupsMiniList:_swapImageByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image15 = UIHelper.seekNodeByName(self._imageBg ,"ButtonGroup","Image_15")	
         UIHelper.swapWithLabel(image15,
		{ 
			 style = "icon_txt_2", 
			 text = Lang.getImgText("txt_qintomb_03") ,
			 anchorPoint = cc.p(0.5,0.5),
		})

		if Lang.checkLang(Lang.CN) then -- i18n ja change font size
			image15:setFontSize(image15:getFontSize() + 5) 
		end
	end
end

-- i18n pos lable
function CommonGroupsMiniList:_adjustPosByI18n()
	if not Lang.checkLang(Lang.CN) then
		local UIHelper  = require("yoka.utils.UIHelper")
		local image1 = UIHelper.seekNodeByName(self._imageBg ,"Image_1")	
        local imageArrow = UIHelper.seekNodeByName(image1 ,"ImageArrow")	
		local textAwardTips = UIHelper.seekNodeByName(image1 ,"TextAwardTips")	
		UIHelper.alignNodeByI18n(image1,textAwardTips,imageArrow)
	end
	-- i18n ja font size
