...
function CommonEquipIcon:setRlevel(rLevel)
    if self._textRlevel == nil then
        local params = {
            ...
            text = "+" .. "0",
            fontSize = 20,
            color = Lang.checkUI("ui4") and Colors.CUSTOM_ACT_DES_HILIGHT or Colors.COLOR_QUALITY[2], -- i18n ja change
            outlineColor = Lang.checkUI("ui4") and Colors.getStyle("text_Rlevel_ja").outlineColor or Colors.COLOR_QUALITY_OUTLINE[2] -- i18n ja change
        }
    ...
    for i, v in pairs(jadeDatas) do
        if jadeDatas[i] > 0 then
            ...
            if jconfig then
                ...
                local EquipConst = require("app.const.EquipConst")
                if Lang.checkUI("ui4") then  -- i18n ja change slot
                    slot:setPosition(EquipConst.EQUIPMENT_JADE_SLOT_POS2[config.suit_id][i])
                else  
