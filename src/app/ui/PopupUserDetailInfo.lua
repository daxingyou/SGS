...
function PopupUserDetailInfo:_initView()
	...
	-- i18n ja change face list 
  	self._listViewLineup:setPosition(6, 33)
	self._listViewLineup:setContentSize(cc.size(95+7, 508))
	self._listViewLineup:setScale(0.9)
	--武将头像滑动条
...
function PopupUserDetailInfo:_updateHeroPageView()
	...
	for i = minPos, maxPos do
		...
		if info.type == TypeConvertHelper.TYPE_HERO then
			...
			-- i18n ja change size  调整尺寸
			if Lang.checkUI("ui4") then   
				self._pageItems[i]:setAvatarScale(0.75)
			end
		else
...
function PopupUserDetailInfo:changeLocalZorder( )
	...
	self._imageAwakeBg:setLocalZOrder( self._nodeContent:getLocalZOrder() + 1)  
