...
function PopupComboHeroGift:onCreate()
    ...
    if Lang.checkUI("ui4") then
        self._commonBtn:setPositionY(200)
    end
end
...
function PopupComboHeroGift:play()
    ...
    local function effectFunction(effect)
		...
		elseif not Lang.checkLang(Lang.CN) and effect == "routine_word_hejilibao_dazi" then
			print("i18n change effect  font node： routine_word_hejilibao_dazi" )
			local UIHelper = require("yoka.utils.UIHelper")
			local subLabel = UIHelper.createBMFLabel({text=Lang.getEffectText("effect_fnt_mingjianghejilibao"),fontName = Path.getImgFont("effect")})
			return subLabel
        else
