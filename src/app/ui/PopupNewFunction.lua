...
function PopupNewFunction:play()
    ...
    local function effectFunction(effect)
		...
		elseif not Lang.checkLang(Lang.CN) and effect == "routine_word_xingongneng_dazi" then
			local UIHelper = require("yoka.utils.UIHelper")
			local subLabel = UIHelper.createBMFLabel({text=Lang.getEffectText("effect_fnt_xingongnengkaiqi"),fontName = Path.getImgFont("effect")})
			return subLabel
        else
