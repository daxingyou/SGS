...
function PopupGetRewards:ctor(callback)
    ...
    if Lang.checkUI("ui4") then
        self._titleImagePath  = Path.getTextCommon("txt_system_gongxihuode")
    else
...
--in8
function PopupGetRewards:_updateAwardsNew(rewardParentNodes, awards,itemTextLineWidth,isDouble)
    LINE_ITEM_BLACK =  55
    LINE_ITEM_BLACK_V = 3
    local itemScale = 1 ---缩放80%
    local itemWidgets = {}
    ...
    for i = 1, #awards, 1 do
        local award = awards[i]
        local awardSize = isDouble == true and award.size * 2 or award.size
        local itemNode = require("app.ui.component.ComponentIconHelper").createIcon(award.type, award.value, awardSize)
        local itemPanel = itemNode:getSubNodeByName("_panelItemContent")
        -- i18n
        local defaultWidth = 120
      
        itemNode:showName(true, itemTextLineWidth or defaultWidth)
        if Lang.checkUI("ui4") then  -- i18n ja change
            itemNode:setNameFontSize(20)
        end
		local itemSize =  itemNode:getSize()
        local iconSize = itemNode:getPanelSize()
        itemSize.width = 98
        ...
        local x = (currCol - 1) * itemSize.width * itemScale + (currCol - 1) * LINE_ITEM_BLACK
        itemNode:setPositionX(x + itemSize.width * itemScale * 0.5)
        itemNode:setPositionY(y - iconSize.height * itemScale * 0.5)
        itemWidgets[i] = itemNode
        ...
        maxLineHeight = math.max(itemSize.height* itemScale,maxLineHeight)
        if currLineNum ~= lineNum and LINE_ITEM_COUNT ==  currCol then
            totalH = totalH + maxLineHeight + LINE_ITEM_BLACK_V
        end

        if LINE_ITEM_COUNT ==  currCol then
            y = y -  maxLineHeight -  LINE_ITEM_BLACK_V 
    ...
    local totalW = maxCol * commonTemSize.width * itemScale + math.max(maxCol - 1,0) * LINE_ITEM_BLACK
    if lineNum > 0 then
          totalH = totalH + maxLineHeight
    end
  

    node:setPosition(0,totalH)
    rewardParentNodes:addChild(node)
    rewardParentNodes:setContentSize(cc.size(totalW,totalH))
    return itemWidgets
end
...
function PopupGetRewards:show(awards,fromText,tipsText,finishCallback, titleImagePath, isDouble, mustShow)
    ...
    local itemWidgets =  nil

    if not Lang.checkLang(Lang.CN)  then
        itemWidgets = self:_updateAwardsNew(rewardsNode, awards,nil,isDouble)
    else
        itemWidgets = self:_updateAwards(rewardsNode, awards, isDouble)  
    end
    ...
    if #awards > LINE_ITEM_COUNT then
        ...
        if Lang.checkUI("ui4") then
            self._scrollView:setPositionY(self._scrollView:getPositionY()-60)
        end
    else
        ...
        if Lang.checkUI("ui4") then
            rewardsNode:setPositionY(rewardsNode:getPositionY()-50)
        end
    ...
    local function callStepAction( ... )
        ...
        if Lang.checkUI("ui4") and G_TutorialManager:isDoingStep(33) then
...
function PopupGetRewards:showDrawCard(awards,fromText,tipsText,finishCallback, titleImagePath)
    if Lang.checkUI("ui4") then
        self:show(awards,fromText,tipsText,finishCallback, titleImagePath)
        return;
    end
    G_AudioManager:playSoundWithId(AudioConst.SOUND_POPUP_REARD)
    ...
    if not Lang.checkLang(Lang.CN)  then
        local size = bgSp:getContentSize()
        bgSp:setContentSize(cc.size(size.width,size.height+50))
    end
    ...
    local itemWidgets = nil
    if not Lang.checkLang(Lang.CN)  then
        itemWidgets = self:_updateAwardsNew(rewardsNode, awards, 145)
    else
        itemWidgets = self:_updateAwards(rewardsNode, awards)    
    end
...
function PopupGetRewards:_createActionNode(effect)
    ...
    if effect == "txt" then
        ...
        if not Lang.checkLang(Lang.CN) then
            ...
            local subLabel = UIHelper.createLabel({text=Lang.getImgText("txt_system_gongxihuode"),style="system_1",styleType=TypeConst.TEXT})
            if self._titleImagePath == Path.getSystemImage("txt_sys_tongguanbaoxiang") then
                subLabel = UIHelper.createLabel({text=Lang.getImgText("txt_system_tongguanjiangli"),style="system_1",styleType=TypeConst.TEXT})
            elseif self._titleImagePath == Path.getSystemImage("txt_sys_shoutongbaoxiang") then
                subLabel = UIHelper.createLabel({text=Lang.getImgText("txt_system_shoutongjiangli"),style="system_1",styleType=TypeConst.TEXT})
            end                
            return subLabel
        else
...
function PopupGetRewards:showRewardsTutorial(awards,finishCallback)
    ...
    label:setColor(cc.c3b(0xe8, 0xb8, 0x80))
